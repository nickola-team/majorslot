...
namespace VanguardLTE\Http\Controllers\Web\Backend\Argon
{
    class ReportController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function report_game(\Illuminate\Http\Request $request)
        {
            ...
            $dateQuery = 'SELECT w_category_summary.id as id, totalbet, totalwin, totaldealbet, totaldealwin,total_deal,total_mileage, total_ggr, total_ggr_mileage, category_id, date, if (w_categories.parent>0, w_categories.parent, w_categories.id) AS parent, w_categories.title AS title FROM w_category_summary JOIN w_categories ON w_categories.id=w_category_summary.category_id WHERE ';
            ...
            if (!auth()->user()->hasRole('admin'))
            {
                $totalQuery = "SELECT SUM(a.totalbet) AS totalbet, SUM(a.totalwin) AS totalwin, SUM(a.totaldealbet) AS totaldealbet, SUM(a.totaldealwin) AS totaldealwin,SUM(a.total_deal) as total_deal,SUM(a.total_mileage) as total_mileage, SUM(a.total_ggr) as total_ggr, SUM(a.total_ggr_mileage) as total_ggr_mileage, a.parent AS category_id, b.title, b.type  FROM ($totalQuery) a JOIN w_categories as b on b.id=a.parent GROUP BY a.parent ORDER BY totalbet desc";
        ...
        public function report_user(\Illuminate\Http\Request $request)
        {
            ...
            $totalstatics = \VanguardLTE\UserDailySummary::where('date', '>=', $start_date)->whereIn('id', $users)->where('date', '<=', $end_date)->selectRaw('user_id, SUM(totalbet) as totalbet, SUM(totalwin) as totalwin')->groupBy('user_id')->orderBy(\DB::raw($fieldtype), $fieldsort)->paginate(50);
            $tablestatics = \VanguardLTE\UserDailySummary::where('date', '>=', $start_date)->whereIn('id', $users)->where('date', '<=', $end_date)->where('gametype', 'table')->selectRaw('user_id, SUM(totalbet) as totalbet, SUM(totalwin) as totalwin')->groupBy('user_id')->orderBy(\DB::raw($fieldtype), $fieldsort)->paginate(50);
            $slotstatics = \VanguardLTE\UserDailySummary::where('date', '>=', $start_date)->whereIn('id', $users)->where('date', '<=', $end_date)->where('gametype', 'slot')->selectRaw('user_id, SUM(totalbet) as totalbet, SUM(totalwin) as totalwin')->groupBy('user_id')->orderBy(\DB::raw($fieldtype), $fieldsort)->paginate(50);
            $pballstatics = \VanguardLTE\UserDailySummary::where('date', '>=', $start_date)->whereIn('id', $users)->where('date', '<=', $end_date)->where('gametype', 'pball')->selectRaw('user_id, SUM(totalbet) as totalbet, SUM(totalwin) as totalwin')->groupBy('user_id')->orderBy(\DB::raw($fieldtype), $fieldsort)->paginate(50);
