...
namespace VanguardLTE\Http\Controllers\Web\GameProviders {
    ...
    class NEXUSController extends \VanguardLTE\Http\Controllers\Controller
    {
                    ...
                    while (true) {
                        try {
                            ...
                            if ($spinType == 's') {
                                ...
                                } elseif ($gamecode == 'vs243queenie') {
                                    if ($bw != 1) {
                                        $data['ind'] = isset($result['wi']) ? ($result['wi'] + 1) : 1;
                                    }
                                } elseif (in_array($gamecode, $arr_b_gamble_games)) {
                                    $data['ind'] = 1;
                                } elseif (in_array($gamecode, $arr_b_no_ind_games)) {
                                    $data['ind'] = 0;
                                } elseif (in_array($gamecode, $arr_b_ind_games)) {
                                    if ($bgt == 9 || $bgt == 23) {
                                        $data['ind'] = 0;
                                    } elseif ($bgt == 11 || $bgt == 31) {
        ...
        public function balance(\Illuminate\Http\Request $request)
        {
            ...
            $username = $data['params']['siteUsername'];
            $userid = preg_replace('/' . self::NEXUS_BLUEPREFIX . '(\d+)/', '$1', $username);
            $userid = intval(preg_replace('/' . self::NEXUS_PPVERIFY_PROVIDER . '(\d+)/', '$1', $userid));
        ...
        public static function addGameRound($callbackType, $data)
        {
            ...
            $result = \VanguardLTE\StatGame::create([
                ...
                'tablekey' => $gameObj['gamecode'] == 'evolution' ? $round['gameKey'] : '',
            ]);
            \DB::commit();
        ...
        public static function transactionDetail()
        {
            ...
            \VanguardLTE\StatGame::where([
                ...
                ->chunk(50, function ($data) {
                    $transactionKeys = $data->pluck('transactionid')->implode(',');

                    $response = NEXUSController::sendRequest('/transaction/detail', [
