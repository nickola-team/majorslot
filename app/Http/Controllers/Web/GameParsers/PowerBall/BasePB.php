...
namespace VanguardLTE\Http\Controllers\Web\GameParsers\PowerBall
{
    ...
    class BasePB
    {
        ...
        public function generateGameRounds($date)
        {
            ...
            $to_sl = $date . ' 23:59:59';
            \VanguardLTE\PowerBallModel\PBGameRound::where('s_time','>=', $from_sl)->where('s_time','<', $to_sl)->where('game_id',$this->game)->delete();
            ...
            { 
                ...
                \VanguardLTE\PowerBallModel\PBGameRound::create(
                    [
                        'game_id' => $this->game,
                        'ground_no' => $dno,
        ...
        public function processRound()
        {
            ...
            $pendingRounds = \VanguardLTE\PowerBallModel\PBGameRound::where('status',0)->where('game_id',$this->game)->where('e_time', '<', $currTime)->orderby('s_time')->get();
                    ...
                    foreach ($pendingRounds as $trend)
                    {
                        foreach ($data as $ga)
                        {
                            ...
                            if ($trend->ground_no == $ground_no)
                            {
                                ...
                                $totalBets = \VanguardLTE\PowerBallModel\PBGameBet::where([
