...
class MonitorBetWin extends Command
{
    ...
    const MSG_TITLE = '고배당 당첨알림';
    const MSG_CONTENT = '<a href="/replace_with_backend/player/gamehistory?player=%s">%s</a> [%s 매장]님이 %s에 %s게임에서 %.2f배 당첨되었습니다';
        ...
        foreach($stat_games as $stat_game) {
            if ($stat_game->odd >= self::MAX_ODD)
            {
                ...
                if ($user)
                {
                    ...
                    //     'content' => sprintf(self::MSG_CONTENT, $user->id,$msgStartTime, $msgEndTime, $user->username, $user->shop->name, $game_round->category->title ,$stat_game->odd)
                    // ]);
                    ...
                    \VanguardLTE\Message::create([
                        'user_id' => $partner->id,
                        'writer_id' => 0,
                        'title' => self::MSG_TITLE . ' - [' . $user->username .'] 유저 - '.  number_format($stat_game->odd,2) .'배 당첨' ,
                        'content' => sprintf(self::MSG_CONTENT, $user->username,$user->username, $user->shop->name, $game_round->date_time, $game_round->category->title ,$stat_game->odd)
                    ]);
                }
            }
            else
