...
class ThePlusLaunch extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'theplus:launch';
    ...
    public function handle()
    {
        ...
        $launchRequests = \VanguardLTE\GameLaunch::where(['finished'=> 0, 'provider' => TPController::TP_PROVIDER])->orderby('created_at', 'asc')->get();
        $processed_users = [];
        foreach ($launchRequests as $request)
        {
            ...
            $url = TPController::makelink($request->gamecode, $request->user_id);
            if ($url != null)
            {
                $request->update([
                    'launchUrl' => $url,
