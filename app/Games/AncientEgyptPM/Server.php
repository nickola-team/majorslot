    ...
    class Server
    {
        ...
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                $slotSettings->SetGameData($slotSettings->slotId . 'Bgt', 0);
                $response = 'tw='. $slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') .'&bgid=0&balance='.$Balance.'&wins='.implode(',', $newWins).'&coef='.($betline * $lines).$strOtherResponse.'&level=1&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na='.$spinType.'&status='. implode(',', $status) .'&rw='. $totalWin .'&stime=' . floor(microtime(true) * 1000) .'&bgt=9&lifes=0&wins_mask='. implode(',', $newWinsMask) .'&wp='.$wp.'&end=1&sver=5&counter='. ((int)$slotEvent['counter'] + 1);
                ...
                $response = 'fsmul=1&balance='.$Balance.'&fsmax=' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') . '&ms='.$slotSettings->GetGameData($slotSettings->slotId . 'BonusSymbol') . '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&n_reel_set=7&balance_bonus=0.00&na=s&fswin=0.00&stime=' . floor(microtime(true) * 1000) .'&fs=1&fsres=0.00&sver=5&counter='. ((int)$slotEvent['counter'] + 1);
