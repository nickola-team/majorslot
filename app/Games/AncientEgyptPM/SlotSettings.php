...
namespace VanguardLTE\Games\AncientEgyptPM
{
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
            ...
            $this->Paytable[5] = [0,0,2,15,100,500];
            $this->Paytable[6] = [0,0,2,15,100,500];
        ...
        public function GetReelStrips($winType,$spintype, $bet)
        {
            if($winType == 'bonus' || $spintype == 1){
                $winAvaliableMoney = $this->GetBank('bonus');
            }else if($winType == 'win'){
            ...
            while(true){
                $stacks = \VanguardLTE\PPGameStackModel\PPGameAncientEgyptStack::where('spin_type', $spintype)->whereNotIn('id', $existIds);
                if($winType == 'win'){
