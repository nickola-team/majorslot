...
namespace VanguardLTE\Games\MuayThaiCQ9
{
    class SlotSettings
    {
        ...
        {
            // if($winType == 'bonus'){
                    // $stack = \VanguardLTE\CQ9GameStackModel\CQ9GameMuayThaiStack::where('id', 92416)->first(); 
            ...
            while(true){
                if($selId > -1){
                    $stacks = \VanguardLTE\CQ9GameStackModel\CQ9GameMuayThaiStack::where(['spin_type' => 1, 'free_count' => $selId])->whereNotIn('id', $existIds);
                }else if($winType == 'bonus'){
                    $stacks = \VanguardLTE\CQ9GameStackModel\CQ9GameMuayThaiStack::where(['spin_type' => 2,'pur_level' => $pur])->whereNotIn('id', $existIds);
                }else{
                    $stacks = \VanguardLTE\CQ9GameStackModel\CQ9GameMuayThaiStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
                ...
                if($isLowBank == true){
                    if($selId > -1){
                        $stacks = $stacks->where('odd', '<=', 15)->where('re_spin', -1);    
                    }
