    ...
    class Server
    {
        public function get($request, $game)
        {
            ...
            if( $_obf_0D221D1040101E0C18152D38350A220B2431190A3E1822['slotEvent'] == 'gameInit' ) 
            {
                ...
                $response = '{"status":"success","microtime":0.23813390731812,"dateTime":"2020-02-18 10:17:41","error":"","content":{"cmd":"gameInit","balance":' . $_obf_0D1A3E15343531081F13061E15332D2C3B403D0F100901 . ',"session":"55492646_b3d33c4034356bc47136d98eae43f081","betInfo":{"denomination":0.01,"bet":' . $_obf_0D34145C302B1D0101103437210F3D3C2D1C3836051D11[0] . ',"lines":25},"betSettings":{"denomination":[0.01],"bets":[' . implode(',', $_obf_0D34145C302B1D0101103437210F3D3C2D1C3836051D11) . '],"lines":[1,5,10,15,20,25]},"symbols":' . $_obf_0D282A3E130518100418230D042B0A2B400930193F3232 . ',"reels":{"base":{"1":["' . implode('","', $slotSettings->reelStrip1) . '"],"2":["' . implode('","', $slotSettings->reelStrip2) . '"],"3":["' . implode('","', $slotSettings->reelStrip3) . '"],"4":["' . implode('","', $slotSettings->reelStrip4) . '"],"5":["' . implode('","', $slotSettings->reelStrip5) . '"]},"feature":{"1":["' . implode('","', $slotSettings->reelStripBonus1) . '"],"2":["' . implode('","', $slotSettings->reelStripBonus2) . '"],"3":["' . implode('","', $slotSettings->reelStripBonus3) . '"],"4":["' . implode('","', $slotSettings->reelStripBonus4) . '"],"5":["' . implode('","', $slotSettings->reelStripBonus5) . '"]}},"exitUrl":"\/","pingInterval":60000,"restore":' . $restore . ',"freeSpin":{' . $_obf_0D5C3831071A3610163227012704053C131A110B2E3122 . '},"hash":"af0bc9acbee8a5a9b559dfe95e6527c4"}}';
            }
            ...
            else if( $_obf_0D221D1040101E0C18152D38350A220B2431190A3E1822['slotEvent'] == 'bet' || $_obf_0D221D1040101E0C18152D38350A220B2431190A3E1822['slotEvent'] == 'freespin' ) 
            {
                ...
                $_obf_0D2D350C1338352E3F0236115C1407341C0926053F2211 = '{"responseEvent":"spin","responseType":"' . $_obf_0D221D1040101E0C18152D38350A220B2431190A3E1822['slotEvent'] . '","serverResponse":{"slotLines":' . $_obf_0D221D1040101E0C18152D38350A220B2431190A3E1822['slotLines'] . ',"slotBet":' . $_obf_0D221D1040101E0C18152D38350A220B2431190A3E1822['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('LuckyCountATFreeGames') . ',"Mpl":' . $slotSettings->GetGameData('LuckyCountATMpl') . ',"currentFreeGames":' . $slotSettings->GetGameData('LuckyCountATCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $_obf_0D33120B1B18292D30293B191C3D383E3D2D0C195B2101 . '],"bonusInfo":{},"Jackpots":' . $_obf_0D1B370B073F123C3210300C0336351F3E072217172A22 . ',"reelsSymbols":' . $_obf_0D140A1C122D065B2A1629031B280E272815082A0D2122 . '}}';
                $_obf_0D1D385C3B2E29221F233F3E3D09295B0F1B0D063B3222 = [];
                for( $i = 1; $i <= 5; $i++ ) 
                {
