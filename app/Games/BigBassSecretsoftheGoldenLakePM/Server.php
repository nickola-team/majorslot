<?php 
namespace VanguardLTE\Games\BigBassSecretsoftheGoldenLakePM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->setGameData($slotSettings->slotId . 'LastReel', [1,12,3,3,10,5,11,12,5,11,3,7,11,6,8]);
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                if($stack['g'] != ''){
                    $arr_g = $stack['g'];
                    $arr_status = explode(',', $arr_g['bg_0']['status']);
                    $arr_winsmask = explode(',', $arr_g['bg_0']['wins_mask']);

                    $sub_status = $arr_status[0];
                    $sub_winsmask = $arr_winsmask[0];

                    $arr_status[0] = $arr_status[$ind];
                    $arr_status[$ind] = $sub_status;

                    $arr_winsmask[0] = $arr_winsmask[$ind];
                    $arr_winsmask[$ind] = $sub_winsmask;

                    $arr_g['bg_0']['status'] = implode(',', $arr_status);
                    $arr_g['bg_0']['wins_mask'] = implode(',', $arr_winsmask);
                }
                $fsmax = $stack['fsmax'];
                $str_trail = $stack['trail'];
                $strOtherResponse = '';
                ...
                $response = 'fsmul=1&balance='.$Balance.'&fsmax='. $fsmax . $strOtherResponse .'&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na=s&fswin=0.00&rid='. $slotSettings->GetGameData($slotSettings->slotId . 'RoundID') .'&bl='.$slotSettings->GetGameData($slotSettings->slotId . 'Bl') .'&stime=' . floor(microtime(true) * 1000) .'&fs=1&fsres=0.00&sver=5&counter='. ((int)$slotEvent['counter'] + 1);
