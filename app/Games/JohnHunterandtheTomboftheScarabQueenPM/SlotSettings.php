...
namespace VanguardLTE\Games\JohnHunterandtheTomboftheScarabQueenPM
{
    class SlotSettings
    {
        ...
        public $reelStripFree1 = null;
        public $reelStripFree2 = null;
        public $reelStripFree3 = null;
        public $reelStripFree4 = null;
        public $reelStripFree5 = null;
        public $reelStripFree6 = null;
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
           /* if( config('LicenseDK.APL_INCLUDE_KEY_CONFIG') != 'wi9qydosuimsnls5zoe5q298evkhim0ughx1w16qybs2fhlcpn' ) 
            ...
            $this->money_respin = [
                ...
                [25,50,75,125,200,250,300,375,450,500,625,750,875]
            ];
            $this->money_symbol = [
                [800, 20, 30, 50, 100],
                [17, 16, 15, 14, 13]
            ...
            {
                if( count($reel->reelsStripFree[$reelStrip]) ) 
                {
                    $this->$reelStrip = $reel->reelsStripFree[$reelStrip];
                }
            }
            foreach( [
                'reelStripBonus1', 
        ...
        public function GetMoneyMulti(){
            $multis = [2, 2, 2, 3, 3];            
            return $multis[rand(0, 4)];
        }
        public function GetMoneySymbol(){
            $percent = rand(0, 1000);
            $sum = 0;
            for($i = 0; $i < count($this->money_symbol[0]); $i++){
                $sum = $sum + $this->money_symbol[0][$i];
                if($sum >= $percent){
                    return $this->money_symbol[1][$i];
                }
            }
            return 17;
        }
        ...
        public function GetReelStrips($winType, $reelset)
        {
            $isScatter = false;
            if($reelset == 1){
                if( $winType != 'bonus' ) 
                ...
                else
                {
                    ...
                    for( $i = 0; $i < count($_obf_reelStripNumber); $i++ ) 
                    {
                        if( $i == 1 || $i == 2 || $i == 3 ) 
                        {
                            $_obf_reelStripCounts[$_obf_reelStripNumber[$i]] = $this->GetRandomScatterPos($this->{'reelStripFree' . $_obf_reelStripNumber[$i]});
                            $isScatter = true;
                        }
                        else
                        {
                            $_obf_reelStripCounts[$_obf_reelStripNumber[$i]] = rand(0, count($this->{'reelStripFree' . $_obf_reelStripNumber[$i]}) - 3);
                        }
                    }
                }
            }else if($reelset == 2){
