...
namespace VanguardLTE\Games\BigBassBonanza3ReelerPM
{
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
            ...
            $this->jpgs = [];
            $this->Line = [1];
            $this->Bet = explode(',', $game->bet); //[15.00,30.00,45.00,60.00,75.00,150.00,225.00,300.00,375.00,625.00,875.00,1250.00,2000.00,4000.00,6250.00,9000.00]; 
        ...
        {
            // if($winType == 'bonus'){
            //     $stack = \VanguardLTE\PPGameStackModel\PPGameBigBassBonanzaReelActionStack::where('id', 16842)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    if($pur >= 0){
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameBigBassBonanza3ReelerStack::where('spin_type', 1)->where('pur_level', $pur)->whereNotIn('id', $existIds);
                    }else{
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameBigBassBonanza3ReelerStack::where('spin_type', 1)->where('pur_level','<=', 0)->whereNotIn('id', $existIds);
                    }
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameBigBassBonanza3ReelerStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
