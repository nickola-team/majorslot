...
namespace VanguardLTE\Games\_5LionsGoldPM
{
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
           /* if( config('LicenseDK.APL_INCLUDE_KEY_CONFIG') != 'wi9qydosuimsnls5zoe5q298evkhim0ughx1w16qybs2fhlcpn' ) 
            ...
            $this->money_respin = [
                ...
                [25,50,75,100,125,150,175,200,250,350,400,450,500,600,750,2500]
            ];
            ...
            $this->Paytable[3] = [0,0,2,15,45,126];
            $this->Paytable[4] = [0,0,0,12,30,81];
            $this->Paytable[5] = [0,0,0,12,30,81];
            $this->Paytable[6] = [0,0,0,9,24,63];
            $this->Paytable[7] = [0,0,0,9,24,63];
            $this->Paytable[8] = [0,0,0,4,12,36];
            $this->Paytable[9] = [0,0,0,4,12,36];
            $this->Paytable[10] = [0,0,0,3,9,27];
            $this->Paytable[11] = [0,0,0,3,9,27];
            $this->Paytable[12] = [0,0,0,2,6,18];
            $this->Paytable[13] = [0,0,0,2,6,18];
            $reel = new GameReel();
            foreach( [
                'reelStrip1_1', 
                'reelStrip1_2', 
        ...
        public function GetFreeMul($muls){
            $perencts = [
                [0, 1, 2],
                [70, 20, 10]                
            ];
            $percent = mt_rand(0, 90);
            $sum = 0;
            for($i = 0; $i < 3; $i++){
                $sum = $sum + $perencts[1][$i];
                if($sum >= $percent){
                    return $muls[$perencts[0][$i]];
                }
            }
            return $muls[$perencts[0][0]];
        }
        public function GetFreeData($spinType){
            $muls = [
                [25, [2, 3, 5]],
                [20, [3, 5, 8]],
                [15, [5, 8, 10]],
            ...
            if($spinType == 6){
                $freeIndex = mt_rand(0, 5);
                $mulIndex = mt_rand(0, 5);
                return[$muls[0][$freeIndex], [$muls[1][$mulIndex]]];
            }else{
                return $muls[$spinType];
            }
        }
        public function GetJackpotWins($bet){
            $bonusBank = $this->GetBank('bonus');
            $jackpotType = 1;
            if(mt_rand(0, 100) > 97 && $bet * 150 < $bonusBank){
                $jackpotType = 2;
            }
            $winTypes = [];
            for($i = 1; $i <= 3; $i++){
                $max_count = mt_rand(0, 2);
                if($jackpotType == $i){
                    $max_count = 2;
                }
            ...
            shuffle($winTypes);
            $winTypes[] = $jackpotType;
            return $winTypes;
        }
        public function GetGri($winType){
            $count = 0;
            if($winType == 'win'){
                $arr_gri =[
                    [3, 4, 5],
            ...
            if($count == 0){
                ...
                return $this->GetRandomNumber(0, 4, $count);
            }
        }
        ...
        public function GetReelStrips($winType, $slotEvent, $slotReelId, $scattercount, $isJackpot)
        {
            ...
            if( $winType != 'bonus' ) 
            {
                ...
                ] as $index => $reelStrip ) 
                {
                    ...
                    {
                        $_obf_reelStripCounts[$index + 1] = mt_rand(0, count($this->$reelStrip) - 4);
                    }
                }
            }
            else
            {
                ...
                for( $i = 0; $i < count($_obf_reelStripNumber); $i++ ) 
                {
                    ...
                    if($issame == true && $isJackpot == false){
                        $_obf_reelStripCounts[$_obf_reelStripNumber[$i]] = $this->GetRandomScatterPos($this->{'reelStrip'.$slotReelId .'_'. $_obf_reelStripNumber[$i]});
                        $isScatter = true;
                    }else{
                        $_obf_reelStripCounts[$_obf_reelStripNumber[$i]] = rand(0, count($this->{'reelStrip'.$slotReelId .'_'. $_obf_reelStripNumber[$i]}) - 4);
                    }
                }
            }
