...
namespace VanguardLTE\Games\TundrasFortunePM
{
    class SlotSettings
    {
        ...
        {
            // if($fsmax > 0){
                // $stack = \VanguardLTE\PPGameStackModel\PPGameTundrasFortuneStack::where('id', 41209)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameTundrasFortuneStack::where('spin_type', 1)->whereNotIn('id', $existIds);
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameTundrasFortuneStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
                if($pur > -1){
