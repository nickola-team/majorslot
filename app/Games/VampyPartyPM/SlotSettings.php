...
namespace VanguardLTE\Games\VampyPartyPM
{
    class SlotSettings
    {
        ...
        public function BonusWinChance($currentIndex, $bet)
        {
            ...
            $fsChance = [63, 69, 73, 30, 20, 5]; // [8,16,32,64,128,256]
            $percent = mt_rand(0, 100);
            if($currentIndex < 2 && $limitOdd < 20){
                return false;
            }else if($currentIndex == 2 && $limitOdd < 50){
                return false;
            }else if($currentIndex >= 3 && $limitOdd < 200){
                return false;
        ...
        public function GetReelStrips($winType, $bet, $pur, $ind = -1)
        {
            // if($ind > 0){
            //     $id = 45137;
            //     if($ind == 6){
            ...
            //         $id = 13692;
            //     }
                // $stack = \VanguardLTE\PPGameStackModel\PPGameVampyPartyStack::where([
                //     'id' => 49968
                // ])->first();
            ...
            if($ind > 0){
                $winAvaliableMoney = $this->GetBank('bonus');
            }else if($winType == 'win' || ($winType == 'bonus' && $ind == -1)){
            ...
            while(true){
                if($ind >= 0){
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameVampyPartyStack::where([
                        'fsmax' => $ind,
                        'spin_type' => 1
                    ]);
                }else if($winType == 'bonus'){
                    if($pur >= 0){
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameVampyPartyStack::where('spin_type', 2)->where('pur_level', $pur)->whereNotIn('id', $existIds);
                    }else{
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameVampyPartyStack::where('spin_type', 2)->where('pur_level', '<=', 0)->whereNotIn('id', $existIds);
                    }
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameVampyPartyStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
