...
namespace VanguardLTE\Games\TinyToadsPM
{
    class SlotSettings
    {
        ...
        {
            // if($winType == 'bonus'){
                // $stack = \VanguardLTE\PPGameStackModel\PPGameTinyToadsStack::where('id', 9)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    if($pur >= 0){
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameTinyToadsStack::where(['spin_type'=>1, 'pur_level'=>$pur])->whereNotIn('id', $existIds);
                    }else{                        
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameTinyToadsStack::where('spin_type', 1)->where('pur_level','<', 0)->whereNotIn('id', $existIds);
                    }
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameTinyToadsStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
