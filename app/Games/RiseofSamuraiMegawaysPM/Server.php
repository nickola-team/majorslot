<?php 
namespace VanguardLTE\Games\RiseofSamuraiMegawaysPM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                if(isset($stack)){
                    ...
                    if($str_iaw != ''){
                        $strOtherResponse = $strOtherResponse . '&iaw=' . $str_iaw;
                    }
                    if($arr_g != null){
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                if($bl == 1){
                    $allBet = $betline * 25;
                }else if($pur >= 0 && $slotEvent['slotEvent'] != 'freespin'){
                    $allBet = $allBet * 100;
                }
                $tumbAndFreeStacks = []; 
                ...
                $response = 'tw='.$slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') . $strOtherResponse .'&balance='.$Balance. '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na='.$spinType .'&bl='. $bl .'&stime=' . floor(microtime(true) * 1000).'&sh=8&st=rect&c='.$betline.'&sw=6&sver=5&counter='. ((int)$slotEvent['counter'] + 1) .'&l=20&w='.$totalWin.'&s=';
                ...
                if($bl == 1){
                    $allBet = $betline * 25;
                }else if($slotEvent['slotEvent'] == 'freespin' && $isState == true && $slotSettings->GetGameData($slotSettings->slotId . 'BuyFreeSpin') >= 0){
                    $allBet = $allBet * 100;
                }
