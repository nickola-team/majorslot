...
namespace VanguardLTE\Games\DowntheRailsPM
{
    class SlotSettings
    {
        ...
        {
            // if($winType == 'bonus'){
                // $stack = \VanguardLTE\PPGameStackModel\PPGameDowntheRailsStack::where('id', 451)->first();
            ...
            while(true){
                $stacks = \VanguardLTE\PPGameStackModel\PPGameDowntheRailsStack::where('spin_type', $spintype)->whereNotIn('id', $existIds);
                $index = mt_rand(0, 39000);
                if($winType == 'win'){
                    $stacks = $stacks->where('odd', '>', 0);
                }
                if($freeIndex >= 0 && $spintype == 1){
                    $stacks->where('pur_level', $freeIndex);
                }
                if($isLowBank == true){
