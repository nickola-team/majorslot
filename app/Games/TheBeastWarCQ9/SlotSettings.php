...
namespace VanguardLTE\Games\TheBeastWarCQ9
{
    class SlotSettings
    {
        ...
        public function GetReelStrips($winType, $bet, $selId = -1,$pur)
        {
            ...
            while(true){
                if($selId > -1){
                    $stacks = \VanguardLTE\CQ9GameStackModel\CQ9GameTheBeastWarStack::where(['spin_type' => 1, 'free_count' => $selId])->whereNotIn('id', $existIds);
                }else if($winType == 'bonus'){
                    $stacks = \VanguardLTE\CQ9GameStackModel\CQ9GameTheBeastWarStack::where(['spin_type' => 2,'pur_level' => $pur])->whereNotIn('id', $existIds);
                }else{
