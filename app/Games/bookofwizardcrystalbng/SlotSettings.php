...
namespace VanguardLTE\Games\bookofwizardcrystalbng
{
    class SlotSettings
    {
        ...
        public function GetAchievements($scatterCount){
            $number = $scatterCount;
            if($scatterCount > 120){
                $scatterCount = 120;
            }
            $totalPercent = number_format($scatterCount / 120, 3);
            $level = floor($scatterCount / 12);
            if($level == 1){
                $levelPercent = 1.0;    
            }else if(($scatterCount % 12) == 0){
                $levelPercent = 0;
            }else{
                $levelPercent = number_format(($scatterCount % 12) / 12, 3);
            }
        ...
        {
            // if($winType == 'bonus'){
                // $stack = \VanguardLTE\BNGGameStackModel\BNGGameBookofWizardCrystalStack::where('id', 137109)->first();
            ...
            while(true){
                if($ind > 0){
                    $stacks = \VanguardLTE\BNGGameStackModel\BNGGameBookofWizardCrystalStack::where(['spin_type' => 1, 'pur_level' => $ind]);
                }else if($winType == 'bonus'){
                    $stacks = \VanguardLTE\BNGGameStackModel\BNGGameBookofWizardCrystalStack::where('spin_type',2);
                }else{
                    $stacks = \VanguardLTE\BNGGameStackModel\BNGGameBookofWizardCrystalStack::where('spin_type', 0);
                }
