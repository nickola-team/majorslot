<?php 
namespace VanguardLTE\Games\BigBassMissionFishinPM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->setGameData($slotSettings->slotId . 'LastReel', [3,11,5,3,11,6,8,12,7,9,5,10,3,4,10]);
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                if($slotEvent['slotEvent'] == 'freespin' || $slotSettings->GetGameData($slotSettings->slotId . 'CurrentRespin') >= 0){
                    ...
                    $stack = $slotSettings->GetReelStrips($winType, $betline * $lines, -1);
                    if($stack == null){
                        $response = 'unlogged';
                        exit( $response );
                    }
                ...
                if($slotSettings->GetGameData($slotSettings->slotId . 'BonusStep') == 0){
                    if($ind == 1)
                    {
                        $stack = $slotSettings->GetReelStrips('bonus', $betline * $lines, 1);
                        $slotSettings->SetGameData($slotSettings->slotId . 'TumbAndFreeStacks', $stack);
                        $slotSettings->SetGameData($slotSettings->slotId . 'TotalSpinCount', 1);
                        $arr_g = $stack[0]['g'];
                        $rs_p = $stack[0]['rs_p'];
                ...
                {
                    $stack = $slotSettings->GetReelStrips('bonus', $betline * $lines, 0);
                    $slotSettings->SetGameData($slotSettings->slotId . 'TumbAndFreeStacks', $stack);
                    $slotSettings->SetGameData($slotSettings->slotId . 'TotalSpinCount', 1);
                    $arr_g = $stack[0]['g'];
                    $str_trail = $stack[0]['trail'];
