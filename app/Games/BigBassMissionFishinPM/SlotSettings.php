...
namespace VanguardLTE\Games\BigBassMissionFishinPM
{
    class SlotSettings
    {
        ...
        {
            // if($winType == 'bonus'){
                // $stack = \VanguardLTE\PPGameStackModel\PPGameBigBassMissionFishinStack::where('id', 33347)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    if($pur >= 0)
                    {
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameBigBassMissionFishinStack::where(['spin_type' => 1, 'pur_level' => $pur])->whereNotIn('id', $existIds);
                        if($pur == 0){
                            $stacks = $stacks->where('fsmax', $this->GetGameData($this->slotId . 'ScatterCount'));
                        }
                    }
                    else
                    {
                        ...
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameBigBassMissionFishinStack::where(['spin_type'=>2, 'fsmax'=>$scatterCount])->whereNotIn('id', $existIds);    
                    }
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameBigBassMissionFishinStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
                $index = mt_rand(0, 28000);
                if($winType == 'win'){
                    $stacks = $stacks->where('odd', '>', 0);
                    $index = mt_rand(0, 87000);
                }
                if($isLowBank == true){
                    if($pur >= 0){
