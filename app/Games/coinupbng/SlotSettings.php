...
namespace VanguardLTE\Games\coinupbng
{
    class SlotSettings
    {
        ...
        public function GetAchievements($moneyCount){
            $number = $moneyCount;
            if($moneyCount > 120){
                return ['level'=> 4, 'level_percent'=> 1, 'number'=> 120, 'total_percent'=> 1];
            }
            $totalPercent = number_format($moneyCount / 120, 3);
            $levels = [0, 30, 60, 90, 120];
            $level = 0;
            $levelPercent = 0;
            if($moneyCount > 0){
                for($k = 0; $k < 5; $k++){
        ...
        {
            // if($winType == 'bonus'){
                // $stack = \VanguardLTE\BNGGameStackModel\BNGGameCoinUpStack::where('id', 21)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    ...
                    //     $stacks = \VanguardLTE\BNGGameStackModel\BNGGameCoinUpStack::where('spin_type', 2);
                    // }else{
                        $stacks = \VanguardLTE\BNGGameStackModel\BNGGameCoinUpStack::where('spin_type',2)->where('pur_level', $pur);
                    // }
                }else{
                    $stacks = \VanguardLTE\BNGGameStackModel\BNGGameCoinUpStack::where('spin_type', 0);
                }
