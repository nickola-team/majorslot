    ...
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
           /* if( config('LicenseDK.APL_INCLUDE_KEY_CONFIG') != 'wi9qydosuimsnls5zoe5q298evkhim0ughx1w16qybs2fhlcpn' ) 
            ...
            $this->CurrentDenom = $this->game->denomination;      //다국어기능시 외화환률 설정(례 : usd가 1인경우 cny는 7로 설정)cq는 1로 설정하여 사용
            $this->scaleMode = 0;
            $this->numFloat = 0;
            $this->Paytable['W']  = [0,0,0,0,0,0];
            $this->Paytable['SC']  = [0,0,0,0,0,0];
            $this->Paytable['1']  = [0,0,0,40,120,400];    //심볼이 당첨될때 배당률
            ...
            $this->WinGamble = $game->rezerv;         //구매하기시 limit값 설정값
        ...
        {
            // if($winType == 'bonus'){
                //  $stack = \VanguardLTE\CQ9GameStackModel\CQ9GameHotSpinStack::where('id', 1980)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    $stacks = \VanguardLTE\CQ9GameStackModel\CQ9GameHotSpinStack::where('spin_type','>', 0)->whereNotIn('id', $existIds);
                }else{
                    $stacks = \VanguardLTE\CQ9GameStackModel\CQ9GameHotSpinStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
                if($pur >= 0){
