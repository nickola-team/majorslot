...
namespace VanguardLTE\Games\grabmoregoldbng
{
    class SlotSettings
    {
        ...
        {
            // if($winType == 'bonus'){
                // $stack = \VanguardLTE\BNGGameStackModel\BNGGameGrabMoreGoldStack::where('id', 111833)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    $currentsac = $this->GetGameData($this->slotId . 'SAC') ?? [3, 0];
                    $stacks = \VanguardLTE\BNGGameStackModel\BNGGameGrabMoreGoldStack::where('spin_type','>', 0);
                    if($currentsac[0] >= 10){
