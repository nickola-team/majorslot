<?php 
namespace VanguardLTE\Games\SmugglersCovePM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->setGameData($slotSettings->slotId . 'LastReel', [4,9,4,6,8,3,7,9,4,7,5,8,7,5,8]);
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                $allBet = $betline * $lines;
                if($bl == 1 && $slotEvent['slotEvent'] != 'freespin'){
                    $allBet = $betline * 25;
                }
                $tumbAndFreeStacks = []; 
                ...
                if($scatterCount >= 3 && $slotEvent['slotEvent'] != 'freespin'){
                    $fsmax = 8;
                    if($str_iaw != ''){
                        $arr_iaw = explode('~', $str_iaw);
                        $fsmax = $arr_iaw[1];
                    }else{
                        $str_iaw = 'fs~8~0~8,10,12,15,20';
                    }
                ...
                $reelB = [];
                for($i = 0; $i < 5; $i++){
                    $reelA[$i] = mt_rand(4, 10);
                    $reelB[$i] = mt_rand(4, 10);
                }
                ...
                if($apv > 0){
                    $strOtherResponse = $strOtherResponse . '&apwa='. $coinWin .'&apt=ma&apv=' . $apv;
                }
                $response = 'tw='.$slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') . $strOtherResponse . $strWinLine .'&ls=0&balance='.$Balance. '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na='.$spinType .'&reel_set='. $currentReelSet.'&bl='. $bl .'&stime=' . floor(microtime(true) * 1000) .'&sa='.$strReelSa.'&sb='.$strReelSb.'&sh=3&st=rect&c='.$betline.'&sw=5&sver=5&counter='. ((int)$slotEvent['counter'] + 1) .'&l=20&w='.$totalWin.'&s=' . $strLastReel;
                if( ($slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') + 1 <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') > 0)) 
                {
                    //$slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                    $slotSettings->SetGameData($slotSettings->slotId . 'BonusWin', 0); 
