<?php 
namespace VanguardLTE\Games\MadameDestinyPM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                if($stack != null){
                    ...
                    if($aw >= 0){
                        $strOtherResponse = $strOtherResponse . '&awt=rsf&aw=' . $aw;
                    }
                    if($strWinLine != ''){
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                if($strWinLine != ''){
                    ...
                    for($k = 0; $k < count($arr_lines); $k++){
                        ...
                        $totalWin = $totalWin + $arr_sub_lines[1] * $fsmul;
                        $arr_lines[$k] = implode('~', $arr_sub_lines);
                    }
                    $strWinLine = implode('&', $arr_lines);
                } 
                if($str_psym != ''){
                    ...
                    for($k = 0; $k < count($arr_psyms); $k++){
                        $arr_sub_psyms = explode('~', $arr_psyms[$k]);
                        $arr_sub_psyms[1] = str_replace(',', '', $arr_sub_psyms[1]) / $original_bet * $betline;
                        $totalWin = $totalWin + $arr_sub_psyms[1];
                        $arr_psyms[$k] = implode('~', $arr_sub_psyms);
                    }
                ...
                {                    
                    ...
                    {
                        $strOtherResponse = '&fs_total='.$slotSettings->GetGameData($slotSettings->slotId . 'FreeGames').'&fswin_total=' . ($slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') / $fsmul) . '&fsend_total=1&fsmul_total='. $fsmul .'&fsres_total=' . $slotSettings->GetGameData($slotSettings->slotId . 'BonusWin');
                    }
                    else
                    {
                        $isState = false;
                ...
                }else
                {
                    if($scatterCount >= 3 ){
                        $isState = false;
                        $spinType = 's';
                        $strOtherResponse = '&fsmul='. $fsmul .'&fsmax='.$slotSettings->GetGameData($slotSettings->slotId . 'FreeGames').'&fswin=0.00&fs=1&fsres=0.00';
                    }
                }
                if($aw >= 0){
                    $strOtherResponse = $strOtherResponse . '&awt=rsf&aw=' . $aw;
                ...
                $response = 'tw='.$slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') . $strOtherResponse .'&balance='.$Balance. '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na='.$spinType .'&stime=' . floor(microtime(true) * 1000) .'&reel_set='.$currentReelSet .'&sa='.$strReelSa.'&sb='.$strReelSb.'&sh=3&c='.$betline.'&sver=5&counter='. ((int)$slotEvent['counter'] + 1) .'&l=10&s='.$strLastReel .'&w='.($totalWin / $fsmul);
