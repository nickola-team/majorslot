...
namespace VanguardLTE\Games\MoneyStacksPM
{
    class SlotSettings
    {
        ...
        {
            // if($winType == 'bonus'){
                // $stack = \VanguardLTE\PPGameStackModel\PPGameMoneyStacksStack::where('id', 15940)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    if($ind >= 0)
                    {
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameMoneyStacksStack::where(['spin_type' => 1, 'pur_level'=> $ind, 'fsmax' => $this->GetGameData($this->slotId . 'ScatterCount')])->whereNotIn('id', $existIds);
                    }
                    else
                    {
                        $scatterCounts = [3,4,5,6];
                        if($pur >= 0){
                            $scatterCount = $scatterCounts[$pur];
                        }else{
                            $scatterCount = 3;
                            if(mt_rand(0, 100) > 97){
                                $scatterCount = 4;
                        ...
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameMoneyStacksStack::where(['spin_type'=>2, 'fsmax'=>$scatterCount])->whereNotIn('id', $existIds);    
                    }
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameMoneyStacksStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
                ...
                if($isLowBank == true){
                    ...
                    if($ind >= 0){
                        if($scatterCount == 3)
                            ...
                            $stacks = $stacks->where('odd', '<=', 60);    
                        else if($scatterCount == 5)
                            $stacks = $stacks->where('odd', '<=', 120);    
                        else if($scatterCount == 6)
                            $stacks = $stacks->where('odd', '<=', 150);
                    }
