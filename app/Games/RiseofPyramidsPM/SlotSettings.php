...
namespace VanguardLTE\Games\RiseofPyramidsPM
{
    class SlotSettings
    {
        ...
        {
            // if($fsmax > 0){
                // $stack = \VanguardLTE\PPGameStackModel\PPGameRiseofPyramidsStack::where('id', 34121)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameRiseofPyramidsStack::where('spin_type', 1)->whereNotIn('id', $existIds);
                    if($pur >= 0){
                        $stacks = $stacks->where('pur_level', $pur);
                    }
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameRiseofPyramidsStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
