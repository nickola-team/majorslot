...
namespace VanguardLTE\Games\BookofTutRespinPM
{
    class SlotSettings
    {
        ...
        public function GetReelStrips($winType, $bet, $pur=-1, $ind=-1)
        {
            // if($winType == 'bonus' && $ind == -1){
            //     $stack = \VanguardLTE\PPGameStackModel\PPGameBookOfTutRespinStack::where('id', 121)->first();
            //     return json_decode($stack->spin_stack, true);
            // }
            if(($winType == 'bonus' && $pur == -1) || $ind > -1){
                $winAvaliableMoney = $this->GetBank('bonus');
            ...
            while(true){
                ...
                if(($winType == 'bonus' && $pur == -1) || $ind > -1){
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameBookOfTutRespinStack::where(['spin_type' => 1, 'pur_level' => $ind])->whereNotIn('id', $existIds);
                    ...
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameBookOfTutRespinStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
                if($winType == 'win'){
