<?php 
namespace VanguardLTE\Games\BookofTutRespinPM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->setGameData($slotSettings->slotId . 'LastReel', [6,8,5,6,8,5,11,3,3,7,4,8,10,4,10]);
                ...
                if(isset($stack)){
                    ...
                    if($rs_t > 0 && $arr_g != null && isset($arr_g['ep'])){
                        ...
                        {
                            if($vl != ''){
                                $arr_sub_lines = explode('~', $vl);
                                $arr_sub_lines[1] = str_replace(',', '', $arr_sub_lines[1]) / $original_bet * $bet;
                                $moneyWin = $moneyWin + $arr_sub_lines[1];
                                $arr_ep[$key] = implode('~', $arr_sub_lines);
                            }
                            
                        }
                        $arr_g['ep'] = $arr_ep;                        
                        if($rs_p > 0 || $rs_t > 0){
                            $strOtherResponse = $strOtherResponse . '&rs_win=' . $moneyWin;
                        }
                    }
                    if($arr_g != null){
                        if(!isset($arr_g['bb'])){
                        ...
                        $strOtherResponse = $strOtherResponse . '&g={bb:{reel_set:"23"}}';
                    }
                    if($strWinLine != ''){
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                if($slotEvent['slotEvent'] == 'freespin' || $slotSettings->GetGameData($slotSettings->slotId . 'CurrentRespin') >= 0){
                    ...
                    $stack = $slotSettings->GetReelStrips($winType, $betline * $lines, $pur, -1);
                    if($stack == null){
                        $response = 'unlogged';
                        exit( $response );
                    }
                ...
                $response = 'tw='.$slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') . $strOtherResponse  .'&balance='.$Balance. '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na='.$spinType .'&reel_set='. $currentReelSet .'&stime=' . floor(microtime(true) * 1000) .'&sa='.$strReelSa.'&sb='.$strReelSb.'&sh=3&c='.$betline .'&sver=5&counter='. ((int)$slotEvent['counter'] + 1) .'&l=10&w='.$totalWin.'&s=' . $strLastReel;
                ...
                $isState = false;
                $stack = $slotSettings->GetReelStrips('bonus', $betline * $lines, 0, $ind);
                if($stack == null){
                    $response = 'unlogged';
                    exit( $response );
                }
                ...
                $response = 'fsmul=1&balance='.$Balance . $strOtherResponse . '&fsmax=' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') . '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na=s&fswin=0.00&stime=' . floor(microtime(true) * 1000) .'&fs=1&fsres=0.00&sver=5&counter='. ((int)$slotEvent['counter'] + 1);
