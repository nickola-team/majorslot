<?php 
namespace VanguardLTE\Games\_8DragonsPM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                $response = 'fsmul=1&fs_opt_mask='. $str_fs_opt_mask .'&balance='.$Balance . $strOtherResponse . '&fsmax=' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') . '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na=s&fswin=0.00&stime=' . floor(microtime(true) * 1000) .'&fs=1&fs_opt='. $str_fs_opt .'&fsres=0.00&sver=5&n_reel_set='. $currentReelSet .'&counter='. ((int)$slotEvent['counter'] + 1) . '&fsopt_i=' . $ind;
                ...
                $response = 'balance='.$Balance . '&coef=' . $coef . $strOtherResponse . '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na='. $spinType .'&stime=' . floor(microtime(true) * 1000) .'&sver=5&counter='. ((int)$slotEvent['counter'] + 1);
