...
namespace VanguardLTE\Games\lordfortune2bng
{
    class SlotSettings
    {
        ...
        {
            // if($winType == 'bonus'){
                // $stack = \VanguardLTE\BNGGameStackModel\BNGGameLordFortune2Stack::where('id', 251)->first();
                // return json_decode($stack->spin_stack, true);
            // }
            $spintype = 0;
            if($winType == 'bonus'){
                $winAvaliableMoney = $this->GetBank('bonus');
                if($selectedMod == 'freespins'){
                    $spintype = 1;
                }else if($selectedMod == 'bonus'){
                    $spintype = 2;
                }else{
                    $currentHill = $this->GetGameData($this->slotId . 'Hill') ?? [0, 0];
                    if($currentHill[0] * 10 + $currentHill[1] >= 85){
                        $spintype = 2;
                    }else{
                        $spintype = mt_rand(1, 2);
                    }
                }
            }else if($winType == 'win'){
