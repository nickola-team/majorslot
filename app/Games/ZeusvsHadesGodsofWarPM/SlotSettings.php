...
namespace VanguardLTE\Games\ZeusvsHadesGodsofWarPM
{
    class SlotSettings
    {
        ...
        public function GetReelStrips($winType, $bet, $ind, $pur=-1)
        {
            // if($fsmax > 0){
                // $stack = \VanguardLTE\PPGameStackModel\PPGameZeusvsHadesGodsofWarStack::where('id', 22588)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameZeusvsHadesGodsofWarStack::where('spin_type', 1)->whereNotIn('id', $existIds);
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameZeusvsHadesGodsofWarStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
                $stacks = $stacks->where('ind', $ind);
                if($pur > -1){
