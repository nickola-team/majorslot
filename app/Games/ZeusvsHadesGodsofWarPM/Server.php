<?php 
namespace VanguardLTE\Games\ZeusvsHadesGodsofWarPM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->setGameData($slotSettings->slotId . 'LastReel', [4,10,9,7,9,6,8,10,6,11,3,11,5,3,8,5,8,9,4,9,4,9,7,6,11]);
                $slotSettings->SetGameData($slotSettings->slotId . 'ReplayGameLogs', []); //ReplayLog
                $slotSettings->SetGameData($slotSettings->slotId . 'TumbAndFreeStacks', []); //FreeStacks
                $slotSettings->SetGameData($slotSettings->slotId . 'BuyFreeSpin', -1);
                $slotSettings->SetGameData($slotSettings->slotId . 'Ind', 0);
                ...
                if(isset($stack)){
                    ...
                    if($str_ch_h != ''){
                        $strOtherResponse = $strOtherResponse . '&ch_h=' . $str_ch_h;
                    }
                    if($str_ch_v != ''){
                        $strOtherResponse = $strOtherResponse . '&ch_v=' . $str_ch_v;
                    }
                    if($str_ch_h != ''){
                        $strOtherResponse = $strOtherResponse . '&ch_k=' . $str_ch_k;
                    }
                    if($str_lmv != ''){
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                if($slotEvent['slotEvent'] == 'freespin'){
                    ...
                    $stack = $slotSettings->GetReelStrips($winType, $betline * $lines, $ind, $pur);
                    if($stack == null){
                        $response = 'unlogged';
                        exit( $response );
                    }
                    ...
                    $str_sty = $stack[0]['sty'];
                    $str_sts = $stack[0]['sts'];
                    $str_ch_h = $stack[0]['ch_h'];
                    $str_ch_v = $stack[0]['ch_v'];
                    $str_ch_k = $stack[0]['ch_k'];
                    $str_lmi = $stack[0]['lmi'];
                    $str_lmv = $stack[0]['lmv'];
                ...
                $response = 'tw='.$slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') . $strOtherResponse  .'&balance='.$Balance. '&index='.$slotEvent['index'] .'&balance_cash='.$Balance.'&reel_set='. $currentReelSet .'&balance_bonus=0.00&na='.$spinType .'&stime=' . floor(microtime(true) * 1000) .'&st=rect&c='.$betline.'&sa='. $str_sa .'&sb='. $str_sb .'&sh=5&sw=5&sver=5&counter='. ((int)$slotEvent['counter'] + 1) .'&l=15&s='. $strLastReel .'&w='.$totalWin;
                if( ($slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') + 1 <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') > 0)) 
                {
                    //$slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                    // $slotSettings->SetGameData($slotSettings->slotId . 'BonusWin', 0); 
