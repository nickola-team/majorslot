    ...
    class Server
    {
        ...
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->SetGameData($slotSettings->slotId . 'BonusSymbol', '10');
                ...
                $spinType = 's';
                $strOtherResponse = '';
                $bonusSymbol = explode(',', $slotSettings->GetGameData($slotSettings->slotId . 'BonusSymbol'));
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                for($i = 0; $i < 5; $i++){
                    ...
                    for($j = 0; $j < 3; $j++){
                        ...
                        if($lastReel[$j * 5 + $i] == $scatter){
                            ...
                            for($k = 0; $k < count($bonusSymbol); $k++){
                                if(!isset($me_bonusCount[$k])){
                                    $me_bonusCount[$k] = 0;
                                }
                                if(!isset($me_bonusPoses[$k])){
                                    $me_bonusPoses[$k] = [];
                                }
                                if($lastReel[$j * 5 + $i] == $bonusSymbol[$k]){
                                    $me_bonusCount[$k]++;
                                    $me_bonusPoses[$k][] = $j * 5 + $i; 
                                    if($me_bonusReelPoses[$i] == 0)  {
                                        $me_bonusReelPoses[$i] = $k + 1;
                                    }else{
                                        $me_bonusReelPoses[$i] = 3;
                                    }
                                }
                            }
                        }
                ...
                for($i = 0; $i < count($bonusSymbol); $i++){                    
                    $me_bonusWin[$i] = $slotSettings->Paytable[$bonusSymbol[$i]][$me_bonusCount[$i]] * $betline * 10;
                    $me_reels[$i] = [];
                    $me_bonusChangedPoses[$i] = [];
                    if($me_bonusWin[$i] > 0){
                        for( $r = 0; $r < 5; $r++ ) 
                        {
                            $me_reels[$i][$r] = [];
                            for( $k = 0; $k <= 2; $k++ ) 
                            {
                                if($me_bonusReelPoses[$r] == $i + 1 || $me_bonusReelPoses[$r] == 3){
                                    $me_reels[$i][$r][$k] = $bonusSymbol[$i];
                                    if($reels[$r][$k] != $bonusSymbol[$i]){
                                        array_push($me_bonusChangedPoses[$i], $k * 5 + $r);
                                    }
                                }else{
                                    $me_reels[$i][$r][$k] = $reels[$r][$k];
                                }
                            }
                        }
                        $totalWin = $totalWin + $me_bonusWin[$i];
                    }
                }
                ...
                for($i = 0; $i < count($bonusSymbol); $i++){
                    if($me_bonusWin[$i] > 0){
                        ...
                        for($k = 0; $k < 3; $k++){
                            for($j = 0; $j < 5; $j++){
                                $lastBonusReel[$j + $k * 5] = $me_reels[$i][$j][$k];
                            }
                        }
                        $me_arr[] = $bonusSymbol[$i] . '~' . implode(',', $me_bonusPoses[$i]) . '~' . implode(',', $me_bonusChangedPoses[$i]);
                        $mes_arr[] = $bonusSymbol[$i] . '~' . implode(',', $lastBonusReel);
                        $psym_arr[] = $bonusSymbol[$i] .'~'.$me_bonusWin[$i].'~' . implode(',', $me_bonusPoses[$i]);
                    }
                }
                if( $slotEvent['slotEvent'] == 'freespin' ) 
                {
                ...
                }else
                {
                    ...
                    if($scatterCount >= 3 ){
                        ...
                        $psym_arr[] = '1~' . $scatterWin.'~' . implode(',', $scatterPoses);
                        if($pur >= 0){
                ...
                $slotSettings->SetGameData($slotSettings->slotId . 'BonusSymbol', implode(',', $bonusSymbol));
                //------------ ReplayLog ---------------
                $replayLog = $slotSettings->GetGameData($slotSettings->slotId . 'ReplayGameLogs');
                ...
                $response = 'balance='.$Balance. '&ms='.$slotSettings->GetGameData($slotSettings->slotId . 'BonusSymbol') . '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na=s&rid='. $slotSettings->GetGameData($slotSettings->slotId . 'RoundID') .'&stime=' . floor(microtime(true) * 1000) .'&sver=5&counter='. ((int)$slotEvent['counter'] + 1);
                if($slotSettings->GetGameData($slotSettings->slotId . 'BuyFreeSpin') == 0){
