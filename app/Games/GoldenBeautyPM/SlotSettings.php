...
namespace VanguardLTE\Games\GoldenBeautyPM
{
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
            ...
            $this->Paytable[2] = [0,0,0,0,0,70];
            $this->Paytable[3] = [0,0,0,0,0,0];
            $this->Paytable[4] = [0,0,1,5,15,70];
            $this->Paytable[5] = [0,0,0,4,8,30];
            $this->Paytable[6] = [0,0,0,4,8,30];
            $this->Paytable[7] = [0,0,0,3,6,25];
            $this->Paytable[8] = [0,0,0,3,6,25];
            $this->Paytable[9] = [0,0,0,2,4,20];
            $this->Paytable[10] = [0,0,0,2,4,20];
            $this->Paytable[11] = [0,0,0,1,2,8];
            $this->Paytable[12] = [0,0,0,1,2,8];
            $this->Paytable[13] = [0,0,0,1,2,8];
            $this->Paytable[14] = [0,0,0,1,2,8];
            $this->Paytable[15] = [0,0,0,1,2,8];
        ...
        public function GetIndexReelStrips($id)
        {
            $stack = \VanguardLTE\PPGameStackModel\PPGameGoldenBeautyStack::where('id', $id)->first();
        ...
        public function GetReelStrips($winType, $bet, $fsmax = 0, $ind=-1)
        {
            // if($winType == 'bonus' && $fsmax == 0){
            //     $stack = \VanguardLTE\PPGameStackModel\PPGameGoldenBeautyStack::where('id', 87)->first();
            //     return [87, json_decode($stack->spin_stack, true)];
            // }
            if($fsmax > 0){
                $winAvaliableMoney = $this->GetBank('bonus');
            ...
            while(true){
                ...
                if($fsmax > 0){
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameGoldenBeautyStack::where(['spin_type' => 1, 'pur_level' => $ind, 'fsmax' => $fsmax])->whereNotIn('id', $existIds);
                }else if($winType == 'bonus'){
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameGoldenBeautyStack::where('spin_type', 2)->whereNotIn('id', $existIds);
                }else{
            ...
            return [$stack->id, json_decode($stack->spin_stack, true)];
        }
    }
}
