<?php 
namespace VanguardLTE\Games\WildWestDuelsPM
{
    class SlotSettings
    {
        ...
        public function GetReelStrips($winType, $bet, $pur_level = -1)
        {
            // if($fsmax > 0){
                // $stack = \VanguardLTE\PPGameStackModel\PPGameWildWestDuelsStack::where('id', 13377)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameWildWestDuelsStack::where('spin_type', 1)->whereNotIn('id', $existIds);
                    if($pur_level >= 0){
                        ...
                        if($rand < 800){
                            ...
                            $stacks = $stacks->where('pur_level', '<', 2); 
                        }
                        // $stacks = $stacks->where('pur_level', '<', 1);
                    }
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameWildWestDuelsStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
