...
namespace VanguardLTE\Games\AztecBlazePM
{
    class SlotSettings
    {
        ...
        public function GetReelStrips($winType, $bet, $bl)
        {
            ...
            while(true){
                if($winType == 'bonus'){
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameAztecBlazeStack::where('spin_type',1)->whereNotIn('id', $existIds);
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameAztecBlazeStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
                if($winType == 'win'){
