<?php 
namespace VanguardLTE\Games\StarlightPrincess1000PM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                if(isset($stack)){
                    ...
                    $str_sty = $stack['sty'];
                    $str_psym = $stack['psym'];
                    $str_tmb_win = $stack['tmb_win'];
                    $str_tmb_res = $stack['tmb_res'];
                    $str_accv = $stack['accv'];
                    $apv = $stack['apv'];
                    $str_apt = $stack['apt'];
                    $str_apwa = $stack['apwa'];
                    ...
                    if($str_tmb_win != ''){
                        $str_tmb_win = str_replace(',', '', $str_tmb_win) / $original_bet * $bet;
                        $strOtherResponse = $strOtherResponse . '&tmb_win=' . $str_tmb_win;
                    }
                    if($str_tmb_res != ''){
                        $str_tmb_res = str_replace(',', '', $str_tmb_res) / $original_bet * $bet;
                        $strOtherResponse = $strOtherResponse . '&tmb_res=' . $str_tmb_res;
                    }
                    if($apv > 0){
                        $str_apwa = str_replace(',', '', $str_apwa) / $original_bet * $bet;
                        $strOtherResponse = $strOtherResponse . '&apwa=' . $str_apwa . '&apt=' . $str_apt . '&apv=' . $apv;
                    }
                    if($str_accv != ''){
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                if($slotEvent['slotEvent'] == 'freespin' || $slotSettings->GetGameData($slotSettings->slotId . 'CurrentRespin') >= 0){
                    ...
                    $str_tmb_win = $stack[0]['tmb_win'];
                    $str_tmb_res = $stack[0]['tmb_res'];
                    $apv = $stack[0]['apv'];
                    $str_apt = $stack[0]['apt'];
                ...
                $extraWin = 0;
                if($str_apwa != ''){
                    $str_apwa = str_replace(',', '', $str_apwa) / $original_bet * $betline;
                    $totalWin = $totalWin + $str_apwa;
                }
                $spinType = 's';
                if( $totalWin > 0) 
                {
                ...
                if($str_tmb_win != ''){
                    $str_tmb_win = str_replace(',', '', $str_tmb_win) / $original_bet * $betline;
                    $strOtherResponse = $strOtherResponse . '&tmb_win=' . $str_tmb_win;
                }
                if($str_tmb_res != ''){
                    $str_tmb_res = str_replace(',', '', $str_tmb_res) / $original_bet * $betline;
                    $strOtherResponse = $strOtherResponse . '&tmb_res=' . $str_tmb_res;
                }
                if($apv > 0){
                    $strOtherResponse = $strOtherResponse . '&apwa=' . $str_apwa . '&apt=' . $str_apt . '&apv=' . $apv;
