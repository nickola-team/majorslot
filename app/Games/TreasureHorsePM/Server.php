<?php 
namespace VanguardLTE\Games\TreasureHorsePM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->setGameData($slotSettings->slotId . 'LastReel', [3,6,4,4,3,6,5,4,3]);
                ...
                if($stack != null){
                    ...
                    $strOtherResponse = $strOtherResponse . '&awt=6rl';
                }
                ...
                $Balance = $slotSettings->GetBalance();
                $response = 'def_s=3,6,4,4,3,6,5,4,3&balance='. $Balance .'&cfgs=2257&ver=2&index=1&balance_cash='. $Balance .'&reel_set_size=2&def_sb=4,6,5&def_sa=8,9,6&balance_bonus=0.00&na=s&scatters=1~0,0,0~8,0,0~1,1,1&fs_aw=m~2;m~3;m~4;m~5;m~6&gmb=0,0,0&rt=d&stime=' . floor(microtime(true) * 1000) . $strOtherResponse . '&sa=8,9,6&sb=4,6,5&sc='. implode(',', $slotSettings->Bet) .'&defc=50.00&sh=3&wilds=2~1000,0,0~1,1,1&bonuses=0&fsbonus=&c='.$bet.'&sver=5&n_reel_set=0&counter=2&paytable=0,0,0;0,0,0;0,0,0;400,0,0;150,0,0;75,0,0;40,0,0;20,0,0;10,0,0;5,0,0&l=18&reel_set0=9,6,3,9,1,8,5,4,8,2,7,9,8,6,7,8,5,1,2,5,8,4,3,8~7,1,8,3,9,5,6,7,2,8,8,1,3,4,5,2,6,8,1~5,3,9,2,6,8,1,7,9,3,2,9,4,6,8,1,5,7,2&s='.$lastReelStr.'&reel_set1=5,7,7,7,4,5,5,5,6,3,3,3,1,9,8,8,8,4,4,4,7,6,6,6,1,8,5,3,2,2,2,9,9,9,8,3,8~4,1,9,9,9,6,6,6,5,2,2,2,8,6,7,4,9,3,1,6,5,5,5,8,8,8,2,7,7,7,3,9,7,4,4,4,3,3,3~1,8,4,2,9,6,7,3,9,9,9,5,3,3,3,7,7,7,8,8,8,2,2,2,8,6,6,6,9,1,8,6,5,5,5,9,4,7';
            }
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                $response = 'tw='.$slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') . $strOtherResponse .'&balance='.$Balance. '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na='.$spinType .'&stime=' . floor(microtime(true) * 1000) .'&n_reel_set='.$currentReelSet .'&sa='.$strReelSa.'&sb='.$strReelSb.'&sh=3&c='.$betline.'&sver=5&counter='. ((int)$slotEvent['counter'] + 1) .'&l=18&s='.$strLastReel .'&w='.$totalWin;
