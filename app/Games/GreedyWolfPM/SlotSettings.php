...
namespace VanguardLTE\Games\GreedyWolfPM
{
    class SlotSettings
    {
        ...
        {
            // if($fsmax > 0){
                // $stack = \VanguardLTE\PPGameStackModel\PPGameGreedyWolfStack::where('id', 696)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameGreedyWolfStack::where('spin_type', 1)->whereNotIn('id', $existIds);
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameGreedyWolfStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
                $stacks = $stacks->where('pur_level', $bl);    
                $index =  mt_rand(0, 38000);
                if($bl == 1){
                    $index =  mt_rand(200, 250000);
                }
                if($winType == 'win'){
