<?php 
namespace VanguardLTE\Games\StrawberryCocktailPM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->setGameData($slotSettings->slotId . 'LastReel', [5,9,4,3,7,4,8,5,4,8,5,9,4,5,7]);
                ...
                if(isset($stack)){
                    ...
                    if($arr_g != null){
                        if(isset($arr_g['bg']) && isset($arr_g['bg']['rw'])){
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                $response = 'balance='.$Balance . $strOtherResponse .'&tw='. $slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') .'&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na='. $spinType .'&rid='. $slotSettings->GetGameData($slotSettings->slotId . 'RoundID') .'&stime=' . floor(microtime(true) * 1000) .'&sver=5&g='. preg_replace('/"(\w+)":/i', '\1:', json_encode($arr_g)) . '&counter='. ((int)$slotEvent['counter'] + 1);
