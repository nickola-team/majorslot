...
namespace VanguardLTE\Games\godstempledeluxebng
{
    class SlotSettings
    {
        ...
        public function GetReelStrips($winType, $bet, $ach_scatter)
        {
            ...
            $is_achieve = false;
            $pur_level = 3;
            if($ach_scatter >= 1194 && $winType == 'bonus'){
                $winType = 'win';
            }
            if($ach_scatter == 1198){
                if(mt_rand(0, 100) < 50){
            ...
            $spintype = 0;
            if($winType == 'bonus' || $is_achieve == true){
            ...
            while(true){
                if($is_achieve == true){
                    $stacks = \VanguardLTE\BNGGameStackModel\BNGGameGodsTempleDeluxeStack::where([
                        'spin_type' => 2,
                        'pur_level' => $pur_level
                    ]);
                }else if($winType == 'bonus'){                    
                    $stacks = \VanguardLTE\BNGGameStackModel\BNGGameGodsTempleDeluxeStack::where('spin_type', 1);
                }else{
                    $stacks = \VanguardLTE\BNGGameStackModel\BNGGameGodsTempleDeluxeStack::where('spin_type', 0)->where('pur_level','<=', $pur_level);
                }
