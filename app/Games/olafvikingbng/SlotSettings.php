...
namespace VanguardLTE\Games\olafvikingbng
{
    class SlotSettings
    {
        ...
        public function GetAchievements($moneyCount){
            $number = $moneyCount;
            if($moneyCount >= 460){
                return ['level'=> 6, 'level_percent'=> 1, 'number'=> $number, 'total_percent'=> 1];
            }
            $totalPercent = number_format($moneyCount / 460, 3);
            $levels = [35, 115, 185, 275, 370, 460];
            $level = 0;
            $levelPercent = 0;
            if($moneyCount > 35){
                for($k = 5; $k >= 0; $k--){
                    if($moneyCount >= $levels[$k]){
                        $level = $k + 1;
                        $levelPercent = number_format(($moneyCount - $levels[$k]) / ($levels[$k + 1] - $levels[$k]), 3);
                        break;
                    }
                }
            }else{
                $levelPercent = number_format($moneyCount / $levels[0], 3);
            }
        ...
        {
            // if($winType == 'bonus'){
                // $stack = \VanguardLTE\BNGGameStackModel\BNGGameOlafVikingStack::where('id', 1436)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    if($this->GetGameData($this->slotId . 'MoneyCount') >= 250){
                        $stacks = \VanguardLTE\BNGGameStackModel\BNGGameOlafVikingStack::where('spin_type',2);
                        $limitBonusOdd = 25;
                    }else{
                        $stacks = \VanguardLTE\BNGGameStackModel\BNGGameOlafVikingStack::where('spin_type','>', 0);
                    }
                }else{
                    $stacks = \VanguardLTE\BNGGameStackModel\BNGGameOlafVikingStack::where('spin_type', 0);
                }
