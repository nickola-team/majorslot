    ...
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
           /* if( config('LicenseDK.APL_INCLUDE_KEY_CONFIG') != 'wi9qydosuimsnls5zoe5q298evkhim0ughx1w16qybs2fhlcpn' ) 
            ...
            $this->Paytable[2] = [0,0,0,5,20,80];
            $this->Paytable[3] = [0,0,0,5,20,80];
            $this->Paytable[4] = [0,0,0,5,20,80];
            $this->Paytable[5] = [0,0,0,5,20,80];
            $this->Paytable[6] = [0,0,0,5,20,80];
            $this->Paytable[7] = [0,0,0,10,30,150];
            $this->Paytable[8] = [0,0,0,10,30,150];
            $this->Paytable[9] = [0,0,0,15,40,200];
            $this->Paytable[10] = [0,0,0,15,40,200];
            $this->Paytable[11] = [0,0,0,20,60,300];
            $this->Paytable[12] = [0,0,0,25,100,400];
        ...
        public function IsAvaliableScarabs($slotEvent){
        ...
        public function GetWildScarabReels($reels, $wildScarabCount){
            if($wildScarabCount > 0){
                $wildPoses = $this->GetRandomNumber(0, 14, $wildScarabCount);
                for($i = 0; $i < count($wildPoses); $i++){
                    $reels[$wildPoses[$i] % 5][intdiv($wildPoses[$i], 5)] = 12;
                }
            }
            return $reels;
        }
        public function GetReelStrips($winType, $slotEvent, $isDoubleScatter = false)
        {
            ...
            foreach( $_obf_reelStripCounts as $index => $value ) 
            {
                ...
                if($isScatter == false){
                    ...
                    $reel[$reel_id][0] = intval($key[abs($value - $scatterPos * $diffNum) % $rc]);
                    $reel[$reel_id][1] = intval($key[abs($value + (1 - $scatterPos) * $diffNum) % $rc]);
                    $reel[$reel_id][2] = intval($key[abs($value + (2 - $scatterPos) * $diffNum) % $rc]);
                }
            }
            return $reel;
        }
