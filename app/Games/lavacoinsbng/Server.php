<?php 
namespace VanguardLTE\Games\lavacoinsbng
{
    class Server
    {
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            if($slotEvent['command'] == 'login'){
                ...
                if($action['name'] == 'freespin_init' || $action['name'] == 'freespin_stop' || $action['name'] == 'bonus_init' || $action['name'] == 'bonus_spins_stop' || $action['name'] == 'bonus_freespins_stop'){
                    ...
                    for($k = 0; $k < 3; $k++){
                        ...
                        if($stack[$spin_type] != ''){
                            if($stack[$spin_type] != ''){
                                ...
                                $stack[$spin_type]['round_bet'] = $betline * $DENOMINATOR * $LINES * 2;
                                    ...
                                    for($i = 0; $i < count($stack[$spin_type]['origin_bs_v']); $i++){
                                        for($j = 0; $j < 3; $j++){
                                            if(is_numeric(str_replace(',', '', $stack[$spin_type]['origin_bs_v'][$i][$j]))){
                                                $stack[$spin_type]['origin_bs_v'][$i][$j] = str_replace(',', '', $stack[$spin_type]['origin_bs_v'][$i][$j]) * $betline * $DENOMINATOR / 2;
                                            }
                                        }
                                    }
                                }
                    ...
                    if($moneyCount > 0){
                        $currentHill[1]++;
                        if(($currentHill[0] + 1) < 3){
                            if((($currentHill[0] + 1) * 4 -2) <= $currentHill[1])
                            {
                                $currentHill[0]++;
                                $currentHill[1] = 0;
                            }
                        }else if($currentHill[0] < 9){
                            if((($currentHill[0] + 1) * 5 - 5) <= $currentHill[1]){
                                $currentHill[0]++;
                                $currentHill[1] = 0;
                            }
                        }else{
                    ...
                    $objRes = [
                        ...
                        'context' => [                            
                            ...
                            'current' => 'spins',
                            'last_args' => [
                                'bet_per_line' => $betline * $DENOMINATOR * 2,
                                'lines' => 5
                            ],
                            'last_win' => ($slotSettings->GetGameData($slotSettings->slotId . 'LastWin') ?? 0) * $DENOMINATOR,
                            'math_version' => 'a',
                ...
                $slotSettings->SaveLogReport(json_encode($objRes), $betline * $LINES * 2, $LINES, $slotSettings->GetGameData($slotSettings->slotId . 'TotalWin'), $slotEvent['slotEvent'], $isState);
                if($action['name'] == 'spin' || $action['name'] == 'freespin' || $action['name'] == 'freespin_stop' || $action['name'] == 'respin' || $action['name'] == 'bonus_spins_stop' || $action['name'] == 'bonus_freespins_stop'){
                    if($action['name'] == 'spin'){
                        $this->SaveBNGLogParse($beforeBalance, $BALANCE, $betline * $LINES * 2, $totalWin, $objRes, $slotSettings);
                    }else{
