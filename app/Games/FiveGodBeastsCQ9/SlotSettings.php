...
namespace VanguardLTE\Games\FiveGodBeastsCQ9
{
    class SlotSettings
    {
        ...
        public function GetReelStrips($winType, $bet, $gameRound=1,$selId = -1)
        {
            // if($winType == 'bonus'){
                // if($gameRound == 1){
                    // $stack = \VanguardLTE\CQ9GameStackModel\CQ9GameFiveGodBeastsStack::where('id', 98819)->first(); //59 :  Free,7971 : symbol spin
                    // return json_decode($stack->spin_stack, true);                    
                // }else if($gameRound == 2){
                //     $stack = \VanguardLTE\CQ9GameStackModel\CQ9GameFiveGodBeastsStack::where('id', 1049)->first();
                //     return json_decode($stack->spin_stack, true);
                // }else if($gameRound == 3){
                //     $stack = \VanguardLTE\CQ9GameStackModel\CQ9GameFiveGodBeastsStack::where('id', 622)->first();
                //     return json_decode($stack->spin_stack, true);
                //  }
            // }
            if($selId > -1){
