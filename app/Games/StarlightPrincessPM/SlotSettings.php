        ...
        public function GetReelStrips($winType, $slotEvent, $lastReels, $proposedScatterCount, $lastMULTIPLIERCollection)
        {
            ...
            if ($isTumble) {
                ...
                foreach ($lastFlattenSymbols as $pos => $symbol) {
                    ...
                    $oldSymbols[$reelId][$reelPos] = $symbol;
                }
                ...
                for ($reelId=0; $reelId < $REELCOUNT; $reelId++) { 
                    ...
                    $issame = false;
                    $lastnewsymbol = $oldSymbols[$reelId][0];
                    if($lastnewsymbol >= 3 && $lastnewsymbol < 12 && $lastnewsymbol != $oldSymbols[$reelId][1]){
                        $issame = true;
                    }
                    if ($removedSymbolCount > 0) {
                        ...
                        for ($i=0; $i < $removedSymbolCount; $i++) { 
                            ...
                            if ($multiplierPos == $i) {
                                ...
                                if($issame == true){
                                    $newSymbol = $lastnewsymbol;
                                    $issame = false;
                                }else{
                                    $newSymbol = random_int(3, 11);
                                    if($newSymbol != $lastnewsymbol){
                                        $issame = true;
                                        $lastnewsymbol = $newSymbol;
                                    }else{
                                        $newSymbol = random_int(3, 11);
                                        if($newSymbol != $lastnewsymbol){
                                            $issame = true;
