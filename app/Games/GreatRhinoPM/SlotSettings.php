    ...
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
           /* if( config('LicenseDK.APL_INCLUDE_KEY_CONFIG') != 'wi9qydosuimsnls5zoe5q298evkhim0ughx1w16qybs2fhlcpn' ) 
            ...
            $this->base_rhino_chance = 14;

            $this->CurrentDenom = $this->game->denomination;
            $this->scaleMode = 0;
            $this->numFloat = 0;
            $this->Paytable[1] = [0,0,0,0,0,0];
            $this->Paytable[2] = [0,0,4,50,150,500];
            $this->Paytable[3] = [0,0,2,50,150,400];
            $this->Paytable[4] = [0,0,0,15,50,200];
            $this->Paytable[5] = [0,0,0,10,40,150];
        ...
        public function SaveLogReport($spinSymbols, $bet, $lines, $win, $slotState, $isState = true)
        {
            ...
            {
                $_obf_slotstate = $this->slotId . ' RespinBet';
            }
            $game = $this->game;
        ...
        public function GetMoonsRise($value){
            if(rand(0, 100) < $this->base_rhino_chance){
                return 11;
            }else{
                return $value;
            }
        }
        public function GetMoonWin(){
            $percent = rand(0, 100);
            $sum = $this->money_respin[0][0];
            for($i = 1; $i < count($this->money_respin[0]); $i++){
                if($sum > $percent){
                    return $this->money_respin[1][$i - 1];
                }
        ...
        public function GetReelStrips($winType, $slotEvent, $bonusType)
        {
            ...
            foreach( $_obf_reelStripCounts as $index => $value ) 
            {
                ...
                if($isScatter == false){
                    ...
                    for($k = 0; $k < 3; $k++){
                        if($reel['reel' . $index][$k] >= 4 && $reel['reel' . $index][$k] <= 7){
                            $reel['reel' . $index][$k] = rand(4, 7);
                        }else if($reel['reel' . $index][$k] > 7 && $reel['reel' . $index][$k] <= 13){
                            $reel['reel' . $index][$k] = rand(8, 11);
                        }
                    }
                    $scatterPos = -1;
                    $wildPos = -1;
                    ...
                    if($scatterPos >= 0){
                        for($k = 0; $k < 3; $k++){
                            if($k != $scatterPos){
                                $reel['reel' . $index][$k] = rand(7, 11);
                            }
                        }
                    }
                }
                if($reel['reel' . $index][0] == $reel['reel' . $index][1] && ($reel['reel' . $index][0] > 3)){
                    $reel['reel' . $index][1] = $this->GetNoDuplicationSymbol($reel['reel' . $index][0], $reel['reel' . $index][2]);
                }
                if($reel['reel' . $index][0] == $reel['reel' . $index][2] && ($reel['reel' . $index][0] > 3)){
                    $reel['reel' . $index][2] = $this->GetNoDuplicationSymbol($reel['reel' . $index][0], $reel['reel' . $index][1]);
                }
                if($reel['reel' . $index][1] == $reel['reel' . $index][2] && ($reel['reel' . $index][1] > 3)){
                    $reel['reel' . $index][2] = $this->GetNoDuplicationSymbol($reel['reel' . $index][0], $reel['reel' . $index][1]);
                }
