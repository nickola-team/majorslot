<?php 
namespace VanguardLTE\Games\TheDogHouseMultiholdPM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->setGameData($slotSettings->slotId . 'LastReel', [4,13,7,7,10,3,11,4,5,13,6,12,8,4,10]);
                ...
                if(isset($stack)){
                    ...
                    for($i = 1; $i < 5; $i++){
                        $init_g['screen' . $i] = [
                            'def_s' => '4,13,7,7,10,3,11,4,5,13,6,12,8,4,10',
                            'def_sa' => '7,13,12,4,8',
                            'def_sb' => '3,12,4,3,12',
                            'reel_set' => '1',
                            'sh' => '3',
                    ...
                    if($arr_g != null){
                        foreach( $arr_g as $key => $vl ) 
                        {
                            for($k = 0; $k < 20; $k++){
                                $lineIndex = 'l' . $k;
                                if(isset($vl[$lineIndex])){
                                    $arr_sub_line = explode('~', $vl[$lineIndex]);
                                    $arr_sub_line[1] = str_replace(',', '', $arr_sub_line[1]) / $original_bet * $bet;
                                    $vl[$lineIndex] = implode('~', $arr_sub_line);
                                }
                            }
                            foreach($vl as $sub_key => $sub_vl){
                                $init_g[$key][$sub_key] = $sub_vl;
                            }
                        }
                    }
                    $strOtherResponse = $strOtherResponse . '&g=' . preg_replace('/"(\w+)":/i', '\1:', json_encode($init_g));
                    if($strWinLine != ''){
                ...
                if($arr_g != null){
                    foreach( $arr_g as $key => $vl ) 
                    {
                        for($k = 0; $k < 20; $k++){
                            ...
                            if(isset($vl[$lineIndex])){
                                $arr_sub_line = explode('~', $vl[$lineIndex]);
                                $arr_sub_line[1] = str_replace(',', '', $arr_sub_line[1]) / $original_bet * $betline;
                                $totalWin = $totalWin + $arr_sub_line[1];
                                $vl[$lineIndex] = implode('~', $arr_sub_line);
                            }
                        }
                    }
