    ...
    class Server
    {
        ...
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->setGameData($slotSettings->slotId . 'LastReel', [13,11,9,11,7,13,12,11,5,7,12,12,8,7,5,4,12,12,8,9,5,4,12,12,8,9,12,9,6,7,6,5,11,10,13,8,12,9,13,10,13,13,13,3,13,13,13,13]);
                ...
                if(isset($stack)){
                    ...
                    if($apv > 0){
                        $strOtherResponse = $strOtherResponse . '&apwa='. ($apwa / $original_bet * $bet) .'&apt='. $str_apt .'&apv=' . $apv;
                    }
                    if($str_accm != ''){
                        $strOtherResponse = $strOtherResponse . '&accm=' . $str_accm;
                    }else{
                        $strOtherResponse = $strOtherResponse . '&accm=cp';
                    }
                    if($str_accv != ''){
                        $strOtherResponse = $strOtherResponse . '&accv=' . $str_accv;
                    }else{
                        $strOtherResponse = $strOtherResponse . '&accv=0';
                    }
                    if($str_acci != ''){
                    ...
                    if($rs_t > 0){
                        $strOtherResponse = $strOtherResponse . '&rs_t=' . $rs_t . '&tmb_res=' . $slotSettings->GetGameData($slotSettings->slotId . 'TumbWin') . '&tmb_win=' . ($slotSettings->GetGameData($slotSettings->slotId . 'TumbWin') - $apwa);
                    }
                    if($strWinLine != ''){
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                if($apwa > 0){
                    $apwa = $apwa / $original_bet * $betline; // * $mul;
                    $totalWin = $totalWin + $apwa;
                }
                if($scatterCount >= 4 && $rs_p < 0){
                    $scatterMuls = [0,0,0,0,5,20,100];
                ...
                $response = 'tw='.$slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') . $strOtherResponse  .'&balance='.$Balance. '&index='.$slotEvent['index'].'&balance_cash='.$Balance .'&balance_bonus=0.00&bl='. $bl .'&na='.$spinType .'&stime=' . floor(microtime(true) * 1000) .'&sa='.$strReelSa.'&sb='.$strReelSb.'&sh=8&st=rect&c='.$betline.'&sw=6&sver=5&counter='. ((int)$slotEvent['counter'] + 1) .'&l=20&w='.$totalWin.'&s=';
