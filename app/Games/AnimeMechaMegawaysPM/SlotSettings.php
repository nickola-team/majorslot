<?php 
namespace VanguardLTE\Games\AnimeMechaMegawaysPM
{
    class SlotSettings
    {
        ...
        public function BonusWinChance($currentIndex, $bet)
        {
            ...
            if($currentIndex == 1 && $limitOdd < 30){
                return false;
            }else if($currentIndex == 2 && $limitOdd < 80){
                return false;
            }else if($currentIndex == 3 && $limitOdd < 120){
                return false;
        ...
        public function GetReelStrips($winType, $fsmax, $bet, $ind = -1)
        {
            ...
            //         $id = 13692;
            //     }
            //     $stack = \VanguardLTE\PPGameStackModel\PPGamePowerofMerlinMegawaysStack::where([
            //         'fsmax' => $fsmax,
            //         'spin_type' => 1,
            //         'id' => $id
            //     ])->first();
            ...
            while(true){
                if($fsmax > 0){
                    $stacks = \VanguardLTE\PPGameStackModel\PPGamePowerofMerlinMegawaysStack::where([
                        'fsmax' => $fsmax,
                        'spin_type' => 1
                    ]);
                }else if($fsmax == 0 && $winType == 'bonus'){
                    if($limitOdd < 20){
                        $stacks = \VanguardLTE\PPGameStackModel\PPGamePowerofMerlinMegawaysStack::where('spin_type', 2)->where('fsmax', '<', 18)->whereNotIn('id', $existIds);
                    }else{
                        $stacks = \VanguardLTE\PPGameStackModel\PPGamePowerofMerlinMegawaysStack::where('spin_type', 2)->whereNotIn('id', $existIds);
                    }
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGamePowerofMerlinMegawaysStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
