    ...
    class Server
    {
        ...
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                $response = 'tw='.$slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') . $strOtherResponse .'&balance='.$Balance. '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na='.$spinType .'&stime=' . floor(microtime(true) * 1000) .'&sa='.$strReelSa.'&sb='.$strReelSb.'&sh=3&c='.$betline.'&sw=5&sver=5&reel_set='.$currentReelSet.'&counter='. ((int)$slotEvent['counter'] + 1) .'&l=25&s='.$strLastReel.'&w='.$totalWin;
                if($slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') + 1 <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') > 0 && $tmp_whm != 'jp') 
                {
                    //$slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                    $slotSettings->SetGameData($slotSettings->slotId . 'BonusWin', 0); 
                ...
                $response = 'bgid=0&trail='. $str_trail . $strOtherResponse .'&balance='. $Balance .'&wins='. implode(',', $new_wins) .'&coef='. $coef .'&level='. $level .'&index='.$slotEvent['index'].'&balance_cash='. $Balance .'&balance_bonus=0.00&na='. $spinType .'&whi='. $whi .'&whm='. $str_whm .'&status='. implode(',', $new_status) .'&rw='. $totalWin .'&whw='. $str_whw .'&stime=' . floor(microtime(true) * 1000) .'&bgt='. $bgt .'&sa=4,6,7,6,12&sb=3,12,4,9,10&sh=3&wins_mask='. implode(',', $new_wins_mask) .'&st=rect&sw=5&sver=5&counter='. ((int)$slotEvent['counter'] + 1) .'&s=' . implode(',', $lastReel);
