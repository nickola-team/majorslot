...
namespace VanguardLTE\Games\aztecfire2bng
{
    class SlotSettings
    {
        ...
        public function GetAchievements($moneyCount){
            ...
            $totalPercent = number_format($moneyCount / 180, 3);
            $levels = [0, 20, 40, 120, 180];
            $level = 0;
            $levelPercent = 0;
            if($moneyCount > 0){
                for($k = 0; $k < 5; $k++){
        ...
        {
            // if($winType == 'bonus'){
                // $stack = \VanguardLTE\BNGGameStackModel\BNGGameAztecFire2Stack::where('id', 290)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    if($this->GetGameData($this->slotId . 'MoneyCount') >= 179){
                        $stacks = \VanguardLTE\BNGGameStackModel\BNGGameAztecFire2Stack::where('spin_type',2);
                        $limitBonusOdd = 25;
                    }else{
