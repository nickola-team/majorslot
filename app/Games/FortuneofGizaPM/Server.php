<?php 
namespace VanguardLTE\Games\FortuneofGizaPM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                if(isset($stack)){
                    ...
                    if($bgt > 0 && $bw == 0){
                        $strOtherResponse = $strOtherResponse . '&bgid=0&win_fs='. $win_fs .'&wins='. $str_wins .'&bgt='. $bgt .'&end=1&wins_mask=' . $str_wins_mask . $str_mb;
                    }else{
                        if($bw == 1){
                            $strOtherResponse = $strOtherResponse . '&bgid=0&win_fs='. $win_fs .'&wins='. $str_wins .'&bgt='. $bgt .'&bw=1&end=0&wins_mask=' . $str_wins_mask;
                            $spinType = 'b';
                        }
                        if($str_initReel != ''){
                        ...
                        if($str_mbri != ''){
                            $strOtherResponse = $strOtherResponse . '&mbri=' . $str_mbri;
                        }
                        if($str_mbr != ''){
                            $strOtherResponse = $strOtherResponse . '&mbr=' . $str_mbr;
                        }
                        if($str_mbp != ''){
                            $strOtherResponse = $strOtherResponse . '&mbp=' . $str_mbp;
                        }
                        if($str_mbv != ''){
                            $strOtherResponse = $strOtherResponse . '&mbv=' . $str_mbv;
                        }
                        if($str_sty != ''){
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                if($str_mbri != ''){
                    $strOtherResponse = $strOtherResponse . '&mbri=' . $str_mbri;
                    $str_mb = $str_mb . '&mbri=' . $str_mbri;
                }
                if($str_mbr != ''){
                    $strOtherResponse = $strOtherResponse . '&mbr=' . $str_mbr;
                    $str_mb = $str_mb . '&mbr=' . $str_mbr;
                }
                if($str_mbp != ''){
                    $strOtherResponse = $strOtherResponse . '&mbp=' . $str_mbp;
                    $str_mb = $str_mb . '&mbp=' . $str_mbp;
                }
                if($str_mbv != ''){
                    $strOtherResponse = $strOtherResponse . '&mbv=' . $str_mbv;
                    $str_mb = $str_mb . '&mbv=' . $str_mbv;
                }
                if($str_sty != ''){
                    $strOtherResponse = $strOtherResponse . '&sty=' . $str_sty;
                }
                ...
                $response = 'tw='.$slotSettings->GetGameData($slotSettings->slotId . 'TotalWin')  . '&n_reel_set='. $currentReelSet . $strOtherResponse .'&balance='.$Balance. '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na='.$spinType .'&stime=' . floor(microtime(true) * 1000) .'&sa='.$strReelSa.'&sb='.$strReelSb.'&sh=3&st=rect&c='.$betline.'&sver=5&counter='. ((int)$slotEvent['counter'] + 1) .'&l=20&s=' . $strLastReel;
                if( ($slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') + 1 <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') > 0)) 
                {
                    //$slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                    $slotSettings->SetGameData($slotSettings->slotId . 'BonusWin', 0); 
                ...
                $response = 'fsmul=1&bgid=0&balance='.$Balance.'&win_fs='. $win_fs .'&wins='. $str_wins .'&fsmax=' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') . '&index='.$slotEvent['index'].'&balance_cash='.$Balance.'&balance_bonus=0.00&na=s&fswin=0.00&stime=' . floor(microtime(true) * 1000) .'&fs=1&bgt='. $bgt .'&wins_mask='. $str_wins_mask .'&end=1&fsres=0.00&sver=5&n_reel_set=1&counter='. ((int)$slotEvent['counter'] + 1);
