    ...
    class Server
    {
        ...
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                $response = 'tw='. $slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') . $strOtherResponse .'&bgid=0&balance='. $Balance .'&coef='. $coef .'&level=1&index='.$slotEvent['index'].'&balance_cash='. $Balance .'&balance_bonus=0.00&na='. $spinType .'&rw='. $rw .'&stime=' . floor(microtime(true) * 1000) .'&bgt=22&lifes=0&wp='. $wp .'&end=1&sver=5&counter='. ((int)$slotEvent['counter'] + 1) . '&wof_wi=' . $wof_wi;
