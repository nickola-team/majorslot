    ...
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
           /* if( config('LicenseDK.APL_INCLUDE_KEY_CONFIG') != 'wi9qydosuimsnls5zoe5q298evkhim0ughx1w16qybs2fhlcpn' ) 
            ...
            $this->Paytable[4]  = [0,0,5,50,200,2000];
            $this->Paytable[5]  = [0,0,0,30,150,1000];
            $this->Paytable[6]  = [0,0,0,20,100,500];
            $this->Paytable[7]  = [0,0,0,20,100,500];
            ...
            $this->Paytable[13] = [0,0,0,5,25,100];
        ...
        public function GetHoldMoneyWin(){
            ...
            for($i = 0; $i < count($this->hold_money_respin[0]); $i++){
                ...
                if($sum >= $percent){
                    return $this->hold_money_respin[1][$i];
                }
            }
                ...
                else
                {
                    if($isGenerateRespinSpin == false){
                        ...
                        for( $i = 0; $i < count($_obf_reelStripNumber); $i++ ) 
                        {
                            if( $i == 2 ) 
                            {
                                $_obf_reelStripCounts[$_obf_reelStripNumber[$i]] = 6;
                            }
                            else
                            {
                                $_obf_reelStripCounts[$_obf_reelStripNumber[$i]] = rand(0, count($this->{'reelStrip' . $_obf_reelStripNumber[$i]}) - 3);
