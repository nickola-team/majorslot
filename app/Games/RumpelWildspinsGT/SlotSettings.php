    ...
    class SlotSettings
    {
        ...
        public function GetHistory()
        {
            ...
            foreach( $history as $log ) 
            {
                ...
                $_obf_0D212A1922173D2B302D1002282B3E332B312C5B130811 = json_decode($log->str);
                if( $_obf_0D1F212D1B1A262C271C2C331110342F041A3014182301->responseEvent != 'gambleResult' ) 
                {
                    $_obf_0D212A1922173D2B302D1002282B3E332B312C5B130811->serverResponse->FreeSeries = '';
                    $this->lastEvent = json_encode($_obf_0D212A1922173D2B302D1002282B3E332B312C5B130811);
                    break;
                }
            }
        ...
        public function GetBonusPreset($bet, $lines)
        {
            ...
            global $dblink;
            $bank = $this->Bank / $bet;
            $_obf_0D36152A142D3640231A1D361D113D0D310B0121323711 = \VanguardLTE\BonusPreset::whereRaw('win <= ? and game = ? and game_lines = ?', [
                $bank, 
                $this->slotId, 
                $lines
            ])->get();
            $presets = [];
            foreach( $_obf_0D36152A142D3640231A1D361D113D0D310B0121323711 as $a => $b ) 
            {
                $presets[] = ['info' => $b->info];
            }
            if( count($presets) > 0 ) 
            {
                $_obf_0D1E0B2706352522323E370F3003182F31033B190D0D22 = array_rand($presets);
                return $presets[$_obf_0D1E0B2706352522323E370F3003182F31033B190D0D22];
            }
            else
            {
                return 'NONE';
