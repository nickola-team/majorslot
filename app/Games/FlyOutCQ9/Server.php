        ...
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            if(isset($paramData['req'])){
                if($paramData['req'] == 1){ // init
                    ...
                    $slotSettings->SetGameData($slotSettings->slotId . 'FreeEnd',0);
                    ...
                    for($i = 0; $i < count($gameDatas); $i++){
                        ...
                        if($packet_id == 11){
                            ...
                            $result_val['GameExtraData'] = "";
                            $slotSettings->SetGameData($slotSettings->slotId . 'FreeEnd',1);
                        }
        ...
        public function generateResult($slotSettings, $result_val, $slotEvent, $betline, $lines, $totalbet, $respinReelNo){
            ...
            if($isEnd == true){
                ...
                if($scatterReelNumberCount >= 5){
                    $reelWins = [0,0,0,0,0];
                    for($i=0;$i<5;$i++){
                        $reelWins[$i] += floor($betline * 407 * ($scattersCount - 4)) + mt_rand(0, 8); 
                    }
                    if($slotEvent != 'freespin'){
