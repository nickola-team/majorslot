...
namespace VanguardLTE\Games\RevengeofLokiMegawaysPM
{
    class SlotSettings
    {
        ...
        public function BonusWinChance($currentIndex, $bet)
        {
            ...
            $fsChance = [63, 69, 73, 50]; // [10, 14, 18, 22, 26]
        ...
        {
            // if($winType='bonus'){
                // $stack = \VanguardLTE\PPGameStackModel\PPGameRevengeofLokiMegawaysStack::where([
                //     'id' => 28817
                // ])->first();
                // return json_decode($stack->spin_stack, true);
            // }
            if($winType == 'bonus' && $pur >= 0){
                $winAvaliableMoney = $this->GetBank('bonus');
            ...
            while(true){
                if($winType == 'bonus'){
                    if($pur >= 0)
                    {
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameRevengeofLokiMegawaysStack::where('spin_type', 1)->where('pur_level', $pur)->whereNotIn('id', $existIds);
                    }
                    else
                    {
                        $stacks = \VanguardLTE\PPGameStackModel\PPGameRevengeofLokiMegawaysStack::where('spin_type', 2)->whereNotIn('id', $existIds);
                    }
                }else{
                    $stacks = \VanguardLTE\PPGameStackModel\PPGameRevengeofLokiMegawaysStack::where('spin_type', 0)->whereNotIn('id', $existIds);
                }
