<?php 
namespace VanguardLTE\Games\AfricanElephantPM
{
    class Server
    {
        public $winLines = [];
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->setGameData($slotSettings->slotId . 'LastReel', [10,8,7,4,4,7,7,10,6,9,11,11,9,11,10,10,4,4,7,8]);
                ...
                if(isset($stack)){
                    ...
                    if($arr_g != null){
                        ...
                        $strOtherResponse = $strOtherResponse . '&g={purchase_s:{def_s:"5,7,3,6,7,4,8,8,4,9,5,10,6,3,7,4,11,1,4,9",def_sa:"5,11,8,4,10",def_sb:"5,9,7,3,7",s:"5,7,3,6,7,4,8,8,4,9,5,10,6,3,7,4,11,1,4,9",sa:"5,11,8,4,10",sb:"5,9,7,3,7",sh:"4",st:"rect",sw:"5"}}';
                    }
                    if($strWinLine != ''){
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                $response = 'tw='.$slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') . $strOtherResponse  .'&balance='.$Balance. '&index='.$slotEvent['index'] .'&balance_cash='.$Balance.'&reel_set='. $currentReelSet .'&balance_bonus=0.00&na='.$spinType .'&stime=' . floor(microtime(true) * 1000) .'&st=rect&c='.$betline.'&sa='. $str_sa .'&sb='. $str_sb .'&sh=4&sw=5&sver=5&counter='. ((int)$slotEvent['counter'] + 1) .'&l=20&s='. $strLastReel .'&w='.$totalWin;
                if( ($slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') + 1 <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') > 0)) 
                {
                    //$slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                    // $slotSettings->SetGameData($slotSettings->slotId . 'BonusWin', 0); 
