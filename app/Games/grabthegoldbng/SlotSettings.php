...
namespace VanguardLTE\Games\grabthegoldbng
{
    class SlotSettings
    {
        ...
        {
            // if($winType == 'bonus'){
                // $stack = \VanguardLTE\BNGGameStackModel\BNGGameGrabTheGoldStack::where('id', 77411)->first();
            ...
            while(true){
                if($winType == 'bonus'){
                    $currentsac = $this->GetGameData($this->slotId . 'SAC') ?? [4, 0];
                    $stacks = \VanguardLTE\BNGGameStackModel\BNGGameGrabTheGoldStack::where('spin_type','>', 0);
                    if($currentsac[0] >= 9){
