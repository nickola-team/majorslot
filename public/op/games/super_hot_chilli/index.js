...
var BONUS_ID = 12;
var JACKPOTS_NAME = ['mini', 'minor', 'midi', 'major', 'grand'];
var GameSymbol = {
    WILD_ID: 9,
    SCATTER_ID: 10,
    BONUS_ID: 11,
    ...
    EXTRA_MULTIPLIER: 13,
    BOOST_ID: 14,
    EXTRA_ID_ARRAY: [12, 13, 14],
    BONUS_SYMBOLS_ID_ARRAY: [11]
};
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function _background(snap, state) {
            ...
            } else if (this.details.context.bs_count === 20) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#6B8E23'
                })
            } else {
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && this.get_pearls_count(details.context.board) === 20 || details.state == 'freespins' && details.context.winscatters != undefined && details.context.winscatters.length > 0
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && this.get_pearls_count(details.context.board) === 20) {
                labelLeft = 'Jackpot!';
                if (this.width > 200) {
            ...
            } else if (details.state == 'freespins' && details.context.winscatters != undefined && details.context.winscatters.length) {
                ...
                group.text('50%', '56%', '+' + roundsGranted + ' FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
            }
            this._panel(group, labelLeft, labelRight);
            return group
        }
    }, {
        key: 'make_bac_win_snap',
        value: function make_bac_win_snap(details, combsGroup, margin, snap) {
            var group = snap.group();
            this._background(group, details, details.state);
            margin += this.width;
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            ...
            group.transform('T' + margin + ',0');
            group.text('48%', '48%', 'BAC Win').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
            return group
        }
    }, {
        key: 'make_x2_snap',
        value: function make_x2_snap(details, margin, snap) {
            var group = snap.group();
            this._background(group, details, details.state);
            margin += this.width;
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            ...
            group.transform('T' + margin + ',0');
            group.text('40%', '46%', 'x2 board').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
            return group
        }
    }, {
        key: 'get_text_for_action',
        ...
        value: function get_pearls_count(board) {
            var count = 0;
            for (var i in board) {
                for (var j in board[i]) {
                    if (GameSymbol.BONUS_ID_ARRAY.includes(board[i][j])) {
                        count = count + 1
                    }
                }
            }
        ...
        value: function make_spins(details) {
            ...
            game_bs_values.push(details.context.bs_v);
            if (details.state != 'bonus' && details.context.bac_win != undefined && details.context.bac_win) {
                game_boards.push(details.context.orig_board);
                game_bs_values.push(details.context.bs_v);
                combs.push({
                    'trigger': 'bac_v2_feature_before',
                    'positions': details.context.bac_pos,
                    'amount': details.round_win
                });
                game_boards.push(details.context.board);
                game_bs_values.push(details.context.bs_v);
                combs.push({
                    'trigger': 'bac_v2_feature_after',
                    'positions': details.context.bac_pos,
                    'amount': details.round_win
                })
            }
            if (this.get_pearls_count(details.context.board) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                var winbonus = {
                    'trigger': 'bonus',
                    'positions': [],
                    'amount': 0
                ...
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if (GameSymbol.BONUS_ID_ARRAY.includes(game_board[i][j])) {
                            winbonus.positions.push([i, j])
                        }
                    }
                }
                combs.push(winbonus)
            } else if (this.is_nudging(details)) {
                var wild_before_nudging = {
                    'trigger': 'wild_nudging',
                    'positions': []
                };
                var _board2 = details.context.board;
                _board2.map(function(reel, i) {
                    reel.map(function(symbol, j) {
                        symbol === GameSymbol.WILD_ID && wild_before_nudging.positions.push([i, j])
                    })
                });
                combs.unshift(wild_before_nudging);
                var wild_nudging = {
                    'trigger': 'wild_nudging',
                    'positions': []
                };
                _board2 = details.context.orig_board;
                _board2.map(function(reel, i) {
                    reel.map(function(symbol, j) {
                        symbol === GameSymbol.WILD_ID && wild_nudging.positions.push([i, j])
                    })
                });
                combs.unshift(wild_nudging)
            }
            if (!combs) {
                combs = []
            }
            ...
            var board = this.board(group, game_boards[0], -1, game_bs_values[0]);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            ...
            snap.click(function(e) {
                ...
                var board_index = counter;
                if (counter >= game_boards.length) {
                    board_index = game_boards.length - 1
                }
                cur_board = game_boards[board_index];
                bs_v = game_bs_values[board_index];
                board.remove();
                if (counter === 1 && _this2.is_nudging(details)) cur_board = details.context.orig_board;
                if (counter === 2 && _this2.is_bac_win(details)) cur_board = details.context.board;
                board = _this2.board(group, cur_board, counter - 1, bs_v);
        ...
        value: function make_bonus(details) {
            ...
            if (details.state === 'bonus') {
                for (var reel_index in game_board) {
                    details.context.board[reel_index].unshift(details.context.extra_reel[reel_index]);
                    details.context.bs_v[reel_index].unshift(details.context.extra_reel_values[reel_index]);
                    if (details.context.orig_board != undefined) {
                        details.context.orig_board[reel_index].unshift(details.context.extra_reel[reel_index]);
                        details.context.orig_bs_v[reel_index].unshift(details.context.extra_reel_values[reel_index])
                    }
                }
            }
            var snap = this.make_snap(details);
            this._background(snap, details.state);
            if (details.context.orig_board === undefined) {
                details.context.orig_board = details.context.board;
                details.context.orig_bs_v = details.context.bs_v
            }
            var game_boards = [];
            var game_bs_values = [];
            var group = snap.group().attr({
            ...
            game_boards.push(details.context.board);
            game_bs_values.push(details.context.bs_v);
            if (details.state === 'bonus' && details.context.new_bs_extra !== undefined) {
                var after_apply_multiplier = false;
                var multiplier_colum = -1;
                var original_multipliers = this.get_original_multipliers(details);
                var orig_board_board = details.context.orig_board.map(function(arr) {
                    return arr.slice()
                });
                var orig_bs_v = details.context.orig_bs_v.map(function(arr) {
                    return arr.slice()
                });
                game_boards.push(orig_board_board);
                game_bs_values.push(orig_bs_v);
                combs.push({
                    'trigger': 'apply_extra_reel',
                    'positions': [],
                    'amount': details.round_win,
                    'after_apply_multiplier': after_apply_multiplier,
                    'multiplier_colum': multiplier_colum,
                    'original_multipliers': original_multipliers
                });
                var _loop = function _loop(_reel_index2) {
                    _reel_index2 *= 1;
                    orig_board_board = JSON.parse(JSON.stringify(orig_board_board));
                    orig_bs_v = JSON.parse(JSON.stringify(orig_bs_v));
                    var isReelHasBonus = details.context.board[_reel_index2].some(function(s) {
                        return GameSymbol.BONUS_SYMBOLS_ID_ARRAY.includes(s)
                    });
                    if (details.context.extra_reel[_reel_index2] === 0 || !isReelHasBonus && details.context.extra_reel[_reel_index2] !== GameSymbol.BOOST_ID) {
                        return 'continue'
                    }
                    var positions = [];
                    positions.push([_reel_index2, 0]);
                    if (details.context.extra_reel[_reel_index2] === GameSymbol.BOOST_ID) {
                        details.context.orig_board.map(function(reel, col) {
                            reel.map(function(symbolId, row) {
                                if (GameSymbol.BONUS_ID_ARRAY.includes(symbolId)) positions.push([col, row])
                            })
                        })
                    } else {
                        for (var row_index = 0; row_index <= details.context.board[0].length; row_index++) {
                            if (details.context.board[_reel_index2][row_index] === GameSymbol.EXTRA_MULTIPLIER) {
                                after_apply_multiplier = true;
                                multiplier_colum = _reel_index2
                            }
                            if (GameSymbol.BONUS_ID_ARRAY.includes(details.context.board[_reel_index2][row_index])) {
                                positions.push([_reel_index2, row_index])
                            }
                        }
                    }
                    orig_board_board[_reel_index2] = details.context.board[_reel_index2];
                    orig_bs_v[_reel_index2] = details.context.bs_v[_reel_index2];
                    game_boards.push(orig_board_board);
                    game_bs_values.push(orig_bs_v);
                    combs.push({
                        'trigger': 'apply_extra_reel',
                    ...
                    _reel_index = _reel_index2
                };
                for (var _reel_index in details.context.extra_reel) {
                    var _ret = _loop(_reel_index);
                    if (_ret === 'continue') continue
                }
            }
            if (details.state === 'bonus' && details.context.rounds_left === 0 && this.is_multipliers(details)) {
                var _positions = [];
                var _original_multipliers = this.get_original_multipliers(details);
                details.context.board.map(function(reel, col) {
                    reel.map(function(symbol, row) {
                        var multiplier = details.context.game_bonus_multipliers[col][row - 1];
                        if (multiplier) _positions.push([col, row])
                    })
                });
                game_boards.push(details.context.orig_board);
                game_bs_values.push(details.context.orig_bs_v);
                combs.push({
                    'trigger': 'last_respin',
                    'positions': _positions,
                    'amount': details.round_win,
                    'original_multipliers': _original_multipliers
                });
                game_boards.push(details.context.board);
                game_bs_values.push(details.context.bs_v);
                combs.push({
            ...
            var combGroup = void 0;
            var margin = 0;
            this._combs_bs(combsGroup, combs, details);
            if (combs.length > 1 || game_boards.length > 1) {
                for (var i in combs) {
                    combGroup = this._comb(combsGroup, combs, combs[i], details);
                    margin += this.width;
            ...
            var partSize = this.width / combsGroup.children().length;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(Drawer.STYLE_PROGRESS);
            var board = this.board(group, details.context.board, -1, details.context.bs_v);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            ...
            snap.click(function(e) {
                ...
                bs_v = game_bs_values[board_index];
                board.remove();
                board = _this3.board(group, cur_board, counter - 1, bs_v, combs);
        ...
        key: 'get_original_multipliers',
        value: function get_original_multipliers(details) {
            var originalMultipliers = details.context.game_bonus_multipliers.map(function(reel, i) {
                return reel.map(function(multiplier, j) {
                    var mult = multiplier;
                    if (details.context.extra_reel[i] === GameSymbol.EXTRA_MULTIPLIER && mult !== 0) {
                        var newMultValue = details.context.extra_reel_values[i];
                        mult = mult - newMultValue
                    }
                    return mult
                })
            });
        ...
        key: 'is_multipliers',
        value: function is_multipliers(details) {
            var is_multipliers_count = 0;
            details.context.game_bonus_multipliers.map(function(reel) {
                reel.map(function(mult) {
                    return mult && is_multipliers_count++
                })
            });
            return !!is_multipliers_count
        }
    }, {
        key: 'board',
        value: function board(snap, _board) {
            ...
            var _this4 = this;
            var bs_v = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : undefined;
            var combs = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : [];
            if (!combs.length) {
                combs = [].concat(this.details.context.winlines, this.details.context.winscatters);
                if (this.get_pearls_count(_board) >= 6 && ['spin', 'freespin', 'respin'].includes(this.details.last_action)) {
                    var winbonus = {
                        'trigger': 'bonus',
                        'positions': []
                    };
                    for (var i in _board) {
                        for (var j in _board[i]) {
                            if ([].concat(_toConsumableArray(GameSymbol.BONUS_ID_ARRAY))) {
                                winbonus.positions.push([i, j])
                            }
                        }
                    }
            ...
            if (this.is_bac_win(this.details)) {
                var winbac = {
                    'trigger': 'bac_v2_feature',
                    'positions': this.details.context.bac_pos,
                    'amount': this.details.round_win
                };
                combs.push(winbac)
            }
            if (this.is_nudging(this.details)) {
                var wild_before_nudging = {
                    'trigger': 'wild_nudging',
                    'positions': []
                };
            ...
            } else if (combs[currentWinLine % combs.length]) {
                positions = combs[currentWinLine % combs.length].positions
            }
            var group = snap.group();
            var imageUrl = void 0;
            var _loop2 = function _loop2(_i2) {
                var reel = _board[_i2];
                var lastSymbolWild = reel[reel.length - 1] === GameSymbol.WILD_ID;
                var wildPart = lastSymbolWild ? 0 : 4 - _this4.wildCountOnReel(reel);
                var _loop3 = function _loop3(_j) {
                    var symbolId = _board[_i2][_j];
                    imageUrl = _this4.url(symbolId + '.png');
                    if (_this4.details.state === 'bonus' && ![].concat(_toConsumableArray(GameSymbol.BONUS_ID_ARRAY)).includes(symbolId) && !GameSymbol.EXTRA_ID_ARRAY.includes(symbolId)) {
                        imageUrl = _this4.url('0.png')
                    }
                    if (_this4.details.state === 'freespins' && symbolId === GameSymbol.WILD_ID) {
                        wildPart++;
                        imageUrl = _this4.url('9_' + wildPart + '.png')
                    }
                    if ([].concat(_toConsumableArray(GameSymbol.BONUS_ID_ARRAY)).includes(symbolId) && JACKPOTS_NAME.includes(bs_v[_i2][_j])) {
                        imageUrl = _this4.url('jackpot.png')
                    }
                    var origValue = _this4.details.context.orig_bs_v ? _this4.details.context.orig_bs_v[_i2][_j] : 0;
                    if ([].concat(_toConsumableArray(GameSymbol.BONUS_ID_ARRAY)).includes(symbolId) && JACKPOTS_NAME.includes(origValue) && _this4.details.state === 'bonus' && _this4.details.context.rounds_left === 0) {
                        imageUrl = _this4.url('jackpot.png')
                    }
                    var img = group.image(imageUrl, (_this4.cellWidth + _this4.cellOffsetX) * _i2, (_this4.cellHeight + _this4.cellOffsetY) * _j, _this4.cellWidth, _this4.cellHeight);
                    var value = void 0;
                    if (bs_v != undefined) {
                        value = bs_v[_i2][_j]
                    } else if (_this4.details.context.bs_v != undefined) {
                        value = _this4.details.context.bs_v[_i2][_j]
                    } else {
                        return 'continue'
                    }
                    if (_board[_i2][_j] === GameSymbol.BOOST_ID) value = 0;
                    if (_board[_i2][_j] === GameSymbol.EXTRA_PLUS) value = 0;
                    var number_value_in_string = void 0;
                    if (symbolId === GameSymbol.EXTRA_PLUS) {
                        number_value_in_string = '+' + (value / 100).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    } else if (symbolId === GameSymbol.EXTRA_MULTIPLIER) {
                        number_value_in_string = 'x' + value.toString()
                    } else if (typeof value === 'number') {
                        number_value_in_string = (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    }
                    if (!GameSymbol.BONUS_ID_ARRAY.includes(symbolId) && !GameSymbol.EXTRA_ID_ARRAY.includes(symbolId)) value = 0;
                    var mult_value = 1;
                    if (_this4.details.state === 'bonus' && currentWinLine === -1 && _this4.details.context.game_bonus_multipliers[_i2][_j - 1]) {
                        mult_value = _this4.details.context.game_bonus_multipliers[_i2][_j - 1]
                    }
                    if (combs[currentWinLine] && currentWinLine !== -1 && combs[currentWinLine].multiplier_colum < _i2) {
                        mult_value = combs[currentWinLine].original_multipliers[_i2][_j - 1]
                    }
                    if (_this4.details.state === 'bonus' && currentWinLine !== -1 && _this4.details.context.game_bonus_multipliers[_i2][_j - 1] && combs[currentWinLine] && combs[currentWinLine].after_apply_multiplier && combs[currentWinLine].multiplier_colum >= _i2) {
                        mult_value = _this4.details.context.game_bonus_multipliers[_i2][_j - 1]
                    }
                    if (_this4.details.state === 'bonus' && _this4.details.context.rounds_left === 0 && currentWinLine === 0 && combs[currentWinLine].original_multipliers) {
                        mult_value = combs[currentWinLine].original_multipliers[_i2][_j - 1]
                    }
                    if (mult_value > 1 && _this4.details.round_win === 0) {
                        var offsetX = -1;
                        var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i2 + _this4.cellWidth / 1.77 - offsetX;
                        var rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j - _this4.cellOffsetY + 0.5;
                        var rect_width = _this4.cellWidth / 2.3 + 2 * offsetX;
                        var text_x_x = rect_x + rect_width / 2;
                        var text_x_y = (_this4.cellHeight + _this4.cellOffsetY) * _j + _this4.cellOffsetY + _this4.cellHeight * 0.17;
                    ...
                    if (mult_value > 1 && _this4.details.round_win > 0 && combs[currentWinLine] !== undefined) {
                        if (combs[currentWinLine].trigger != 'mult_after') {
                            var _offsetX = -1;
                            var _rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i2 + _this4.cellWidth / 1.8 - _offsetX;
                            var _rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j - _this4.cellOffsetY + 0.5;
                            var _rect_width = _this4.cellWidth / 2.3 + 2 * _offsetX;
                            var _text_x_x = _rect_x + _rect_width / 2;
                            var _text_x_y = (_this4.cellHeight + _this4.cellOffsetY) * _j + _this4.cellOffsetY + _this4.cellHeight * 0.18;
                    ...
                    if (value !== 0 && [0, GameSymbol.WILD_ID, GameSymbol.SCATTER_ID].concat(_toConsumableArray(GameSymbol.BONUS_ID_ARRAY), _toConsumableArray(GameSymbol.EXTRA_ID_ARRAY)).includes(symbolId)) {
                        var gapX = 1;
                        var _rect_x2 = (_this4.cellWidth + _this4.cellOffsetX) * _i2 - _this4.cellOffsetX;
                        var _rect_y2 = (_this4.cellHeight + _this4.cellOffsetY) * _j + _this4.cellHeight / 3;
                        var _rect_width2 = _this4.cellWidth + 2 * _this4.cellOffsetX;
                        var text_x = _rect_x2 + _rect_width2 / 2;
                        var text_y = (_this4.cellHeight + _this4.cellOffsetY) * _j + _this4.cellHeight / 2;
                        group.rect(_rect_x2 + gapX, _rect_y2, _rect_width2 - gapX * 2, _this4.cellHeight / 3).attr({
                            'fill': 'white'
                        });
                        if (value == 'mini') {
                            if (_this4.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MINI_70)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MINI_40)
                            }
                        } else if (value == 'minor') {
                            if (_this4.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MINOR_70)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MINOR_40)
                            }
                        } else if (value == 'major') {
                            if (_this4.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MAJOR_70)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MAJOR_40)
                            }
                        } else if (value == 'midi') {
                            if (_this4.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MIDI_70)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MIDI_40)
                            }
                        } else {
                            if (value >= 10000000000) {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_31)
                                } else {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_15)
                                }
                            } else if (value >= 1000000000) {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_31)
                                } else {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_18)
                                }
                            } else if (value >= 100000000) {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_40)
                                } else {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_23)
                                }
                            } else if (value >= 10000000) {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_47)
                                } else {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_27)
                                }
                            } else if (value >= 1000000) {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_54)
                                } else {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_31)
                                }
                            } else if (value >= 100000) {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_60)
                                } else {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_34)
                                }
                            } else {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_70)
                                } else {
                                    group.text(text_x, text_y, number_value_in_string).attr(Drawer.BS_VALUE_40)
                                }
                            }
                    ...
                    if ((currentWinLine !== -1 || combs.length && ['bonus', 'freespins', 'bac_v2_feature'].includes(combs[0].trigger) || ['spins', 'freespins'].includes(_this4.details.state) && combs.length === 1) && positions.length) {
                        img.attr({
                            opacity: 0.5
                        });
                        positions.forEach(function(pos) {
                            if (pos[0] == _i2 && pos[1] == _j) {
                                img.attr({
                                    opacity: 1
                                })
                            }
        ...
        key: 'wildCountOnReel',
        value: function wildCountOnReel(reel) {
            var wildCount = 0;
            reel.map(function(id) {
                return id === GameSymbol.WILD_ID && wildCount++
            });
            return wildCount
        }
    }, {
        ...
        value: function _combs(snap, combs, details) {
            ...
            var totalWin = details.round_win;
            var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin);
            var labelRight = combs.length && combs.length > 1 ? 'all/' + combs.length : '';
            this._panel(group, labelLeft, labelRight);
            for (var i = 0; i < combs.length; ++i) {
                if (combs[i].trigger !== 'wild_nudging') {
                    console.log('combs[i].', combs[i]);
                    this.drawe_border_win_symbols(group, combs[i]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
                }
            }
            for (var _i3 = 0; _i3 < combs.length; ++_i3) {
                this.drawe_line_patern(group, combs[_i3]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            return group
        }
    }, {
        key: '_combs_bs',
        value: function _combs_bs(snap, combs, details) {
            var group = snap.group();
            this.bordered_cells = [];
            var totalWin = details.round_win;
            var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin);
            ...
            this._panel(group, labelLeft, labelRight);
            for (var i = 0; i < combs.length; ++i) {
                this.drawe_line_patern(group, combs[i]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            return group
        }
    }, {
        ...
        value: function _panel(snap, labelLeft, labelRight) {
            ...
            group.rect(0, 0, '100%', this.panelHeight).attr(BaseDrawer.STYLE_PANEL);
            var middleLine = this.panelHeight / 2 + 1;
            if (labelLeft.length >= 24) {
                group.text('2%', middleLine, labelLeft).attr(BaseDrawer.STYLE_PANEL_TEXT).attr({
                    'font-size': this.panelFont * 0.9
                });
                group.text('98%', middleLine, labelRight).attr(BaseDrawer.STYLE_PANEL_TEXT).attr({
                    'font-size': this.panelFont * 0.9,
                    'textAnchor': 'end'
        ...
        value: function _calculateSizes(cols, rows) {
            this.panelHeight = this.height > 150 ? 20 : 12;
            this.panelFont = this.height > 150 ? '1em' : '0.8em';
            this.progressHeight = 3;
            this.boardHeight = this.height - (this.panelHeight + this.progressHeight + 1);
            this.cellHeight = parseInt(this.boardHeight / rows, 10);
            this.cellWidth = this.cellHeight * 1.278;
            this.cellOffsetX = 0;
            this.cellOffsetY = 0;
        ...
        value: function is_boost(details) {
            return Boolean(details.context.boost_pay) && details.context.extra_reel.includes(GameSymbol.BOOST_ID)
        }
    }, {
        key: 'is_nudging',
        value: function is_nudging(details) {
            return details.state == 'freespins' && details.last_action !== 'freespin_init' && this.nudging_reels(details).length
        }
    }, {
        key: 'nudging_reels',
        value: function nudging_reels(details) {
            var reels = [];
            var board = details.context.orig_board;
            board.map(function(reel, i) {
                var wildCount = 0;
                reel.map(function(symbol) {
                    symbol === GameSymbol.WILD_ID && wildCount++
                });
                if (wildCount > 0 && wildCount < 4) reels.push(i)
            });
            return reels
        }
    }, {
        key: 'is_bac_win',
        value: function is_bac_win(details) {
            return Boolean(details.context.bac_win)
        }
    }, {
        key: 'make_boost_snap',
        value: function make_boost_snap(details, combsGroup, margin, snap) {
            ...
            if (this.width > 200) {
                ...
                group.text('50%', '70%', this.coins(details.context.boost_pay)).attr(Drawer.FREESPINS_STYLE_TEXT_CENTER_2)
            } else {
                group.text('50%', '50%', 'Boost win').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
                group.text('50%', '70%', this.coins(details.context.boost_pay)).attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
            }
            this._panel(group, labelLeft, labelRight);
            return snap
        }
