...
var BONUS_ID = 12;
var BONUS_SUPER_ID = 13;
var BOOST_ID = 14;
var EXTRA_PLUS = 15;
var EXTRA_MULTIPLIER = 16;
var EXTRA_TRANSFORM = 17;
var GameSymbol = {
    WILD_ID: 10,
    SCATTER_ID: 11,
    BONUS_ID: 12,
    BONUS_SUPER_ID: 13,
    BONUS_ID_ARRAY: [12, 13, 14],
    BONUS_SYMBOLS_ID_ARRAY: [12, 13],
    BOOST_ID: 14,
    EXTRA_PLUS: 15,
    EXTRA_MULTIPLIER: 16,
    EXTRA_TRANSFORM: 17,
    EXTRA_ID_ARRAY: [15, 16, 17]
};
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && details.context.bonus_gold_count >= 2 && details.context.rounds_left == 0 || details.state == 'freespins' && details.context.winscatters != undefined && details.context.winscatters.length > 0
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && details.context.bonus_gold_count >= 2) {
                labelLeft = jackpots[details.context.bonus_gold_count] + ' Jackpot';
                group.text('50%', '56%', jackpots[details.context.bonus_gold_count] + ' Jackpot!').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                ...
                group.text('50%', '56%', '+8 FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
            }
            this._panel(group, labelLeft, labelRight);
            return group
        }
        ...
        value: function make_spins(details) {
            ...
            if (details.state != 'bonus' && details.context.bac_win != undefined && details.context.bac_win) {
                if (details.context.orig_board !== undefined) game_boards.push(details.context.orig_board.board);
                game_bs_values.push(details.context.bs_v);
                combs.push({
                ...
                game_boards.push(details.context.orig_board.board);
                game_bs_values.push(details.context.bs_v)
            }
            if (this.get_pearls_count(details.context.board) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                ...
                for (var _i in game_board) {
                    for (var _j in game_board[_i]) {
                        if (GameSymbol.BONUS_ID_ARRAY.includes(game_board[_i][_j])) {
            ...
            snap.click(function(e) {
                ...
                board.remove();
                if (combs[(counter - 1 + game_boards.length) % game_boards.length] != undefined && combs[(counter - 1 + game_boards.length) % game_boards.length].trigger == 'bonus' && details.context.bac_win) {
                    board = _this2.make_bac_win_snap(details, combsGroup, margin, snap)
                } else {
        ...
        value: function make_bonus(details) {
            ...
            if (details.state === 'bonus') {
                for (var reel_index in game_board) {
                    ...
                    details.context.bs_v[reel_index].unshift(details.context.extra_reel_v[reel_index]);
                    if (details.context.orig_board != undefined) {
                        details.context.orig_board.board[reel_index].unshift(details.context.extra_reel[reel_index]);
                        details.context.orig_board.bs_v[reel_index].unshift(details.context.extra_reel_v[reel_index])
                    }
                }
            }
            var snap = this.make_snap(details);
            this._background(snap, details.state);
            if (details.context.orig_board === undefined) {
                details.context.orig_board = {};
                details.context.orig_board.board = details.context.board;
                details.context.orig_board.bs_v = details.context.bs_v
            }
            var game_boards = [];
            var game_bs_values = [];
            var group = snap.group().attr({
            ...
            game_boards.push(details.context.board);
            game_bs_values.push(details.context.bs_v);
            if (details.state === 'bonus' && details.context.is_full_board !== undefined && details.context.is_full_board) {
                game_boards.push(details.context.orig_board.board);
                game_bs_values.push(details.context.orig_board.bs_v);
                var positions = [];
                for (var i = 0; i < details.context.board.length; i++) {
                    for (var j = 1; j < details.context.board[i].length; j++) {
                        positions.push([i, j])
                    }
                }
                combs.push({
                    'trigger': 'origin_board_X2',
                    'positions': positions,
                    'amount': details.round_win
                })
            }
            if (details.state === 'bonus' && details.context.new_bs_extra !== undefined) {
                var isReelsHasBonus = false;
                details.context.new_bs_extra.forEach(function(bsInfo) {
                    isReelsHasBonus = details.context.board[bsInfo.pos].some(function(s) {
                        return GameSymbol.BONUS_SYMBOLS_ID_ARRAY.includes(s)
                    })
                });
                if (!combs.length && isReelsHasBonus) {
                    game_boards.push(details.context.orig_board.board);
                    game_bs_values.push(details.context.orig_board.bs_v);
                    combs.push({
                        'trigger': 'origin_board',
                        'positions': [],
                        'amount': details.round_win
                    })
                }
                var orig_board_board = details.context.orig_board.board.map(function(arr) {
                    return arr.slice()
                });
                var orig_bs_v = details.context.orig_board.bs_v.map(function(arr) {
                    return arr.slice()
                });
                var _loop = function _loop(_reel_index2) {
                    ...
                    if (details.context.extra_reel[_reel_index2] === 0 || !isReelHasBonus) {
                        return 'continue'
                    }
                    var positions = [];
                    positions.push([_reel_index2, 0]);
                    if (details.context.extra_reel[_reel_index2] === GameSymbol.EXTRA_TRANSFORM) {
                        var transformed = (details.context.bs_transform_pos || []).find(function(pos) {
                            return pos[0] === _reel_index2
                        });
                        if (!transformed) {
                            return 'continue'
                        }
                        var row_index = transformed[1];
                        if (details.context.orig_board.board[_reel_index2][row_index] !== details.context.board[_reel_index2][row_index]) {
                            positions.push([_reel_index2, row_index])
                        }
                    } else {
                        for (var _row_index = 0; _row_index <= details.context.board[0].length; _row_index++) {
                            if (GameSymbol.BONUS_ID_ARRAY.includes(details.context.board[_reel_index2][_row_index])) {
                                positions.push([_reel_index2, _row_index])
                            }
                        }
                    }
            ...
            if (this.is_boost(details)) {
                ...
                for (var _i3 in game_board) {
                    for (var _j2 in game_board[_i3]) {
                        if (GameSymbol.BONUS_ID_ARRAY.includes(game_board[_i3][_j2])) {
                            winboost.positions.push([_i3, _j2])
                        }
                    }
                }
                combs.push(winboost)
            ...
            snap.click(function(e) {
                ...
                bs_v = game_bs_values[board_index];
                board.remove();
                if (combs[(counter - 1 + game_boards.length) % game_boards.length] != undefined && combs[(counter - 1 + game_boards.length) % game_boards.length].trigger == 'x2_snap') {
                    board = _this3.make_x2_snap(details, margin, snap)
                } else {
        ...
        value: function board(snap, _board) {
            ...
            if (!combs.length) {
                ...
                if (this.get_pearls_count(_board) >= 6 && ['spin', 'freespin', 'respin'].includes(this.details.last_action)) {
                    ...
                    for (var i in _board) {
                        for (var j in _board[i]) {
                            if ([].concat(_toConsumableArray(GameSymbol.BONUS_ID_ARRAY), _toConsumableArray(GameSymbol.EXTRA_ID_ARRAY)).includes(_board[i][j])) {
                                winbonus.positions.push([i, j])
                            }
                        }
                    }
            ...
            if (this.is_boost(this.details)) {
                ...
                for (var _i5 in _board) {
                    for (var _j3 in _board[_i5]) {
                        if (GameSymbol.BONUS_ID_ARRAY.includes(_board[_i5][_j3])) {
                            winboost.positions.push([_i5, _j3])
                        }
                    }
                }
                combs.push(winboost)
            ...
            var _loop2 = function _loop2(_i6) {
                var _loop3 = function _loop3(_j4) {
                    imageUrl = _this4.url(_board[_i6][_j4] + '.png');
                    if (_this4.details.state === 'bonus' && ![].concat(_toConsumableArray(GameSymbol.BONUS_ID_ARRAY), _toConsumableArray(GameSymbol.EXTRA_ID_ARRAY)).includes(_board[_i6][_j4])) {
                        imageUrl = _this4.url('0.png')
                    }
                    var img = group.image(imageUrl, (_this4.cellWidth + _this4.cellOffsetX) * _i6, (_this4.cellHeight + _this4.cellOffsetY) * _j4, _this4.cellWidth, _this4.cellHeight);
                    var value = void 0;
                    if (_board[_i6][_j4] === GameSymbol.EXTRA_TRANSFORM) {
                        value = 0
                    } else if (bs_v != undefined) {
                        value = bs_v[_i6][_j4]
                    } else if (_this4.details.context.bs_v != undefined) {
                        value = _this4.details.context.bs_v[_i6][_j4]
                    } else {
                        return 'continue'
                    }
                    var number_value_in_string = void 0;
                    if (_board[_i6][_j4] === GameSymbol.EXTRA_PLUS) {
                        number_value_in_string = '+' + (value / 100).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    } else if (_board[_i6][_j4] === GameSymbol.EXTRA_MULTIPLIER) {
                        number_value_in_string = 'x' + value.toString()
                    } else if (_board[_i6][_j4] === GameSymbol.EXTRA_TRANSFORM) {
                        number_value_in_string = (value + 1).toString()
                    } else if (typeof value === 'number') {
                        number_value_in_string = (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    }
                    if (value !== 0 && [0, GameSymbol.WILD_ID, GameSymbol.SCATTER_ID].concat(_toConsumableArray(GameSymbol.BONUS_ID_ARRAY), _toConsumableArray(GameSymbol.EXTRA_ID_ARRAY)).includes(_board[_i6][_j4])) {
                        var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i6 - _this4.cellOffsetX;
                        var rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j4 + _this4.cellHeight / 3;
                        var rect_width = _this4.cellWidth + 2 * _this4.cellOffsetX;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = (_this4.cellHeight + _this4.cellOffsetY) * _j4 + _this4.cellHeight / 2;
                        ...
                        if (value == 'mini') {
                            ...
                            if (_this4.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MULT_120)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MULT_80)
                            }
                        } else {
                            if (value >= 1000000000) {
                                if (_this4.width > 200) {
                ...
                for (var _j4 in _board[_i6]) {
                    var _ret3 = _loop3(_j4);
        ...
        value: function _comb(snap, combs, comb, details) {
            ...
            var group = snap.group();
            var labelLeft = this.title(details) + ', Win:' + this.coins(comb.amount);
            if (comb.trigger === 'origin_board_X2') {
        ...
        value: function _combs(snap, combs, details) {
            ...
            for (var _i7 = 0; _i7 < combs.length; ++_i7) {
                this.drawe_line_patern(group, combs[_i7]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            return group
        }
    }, {
