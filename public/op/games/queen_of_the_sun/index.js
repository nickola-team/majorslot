    ...
    _createClass(Drawer, [{
        ...
        value: function make_boost_snap(details, combsGroup, margin, snap) {
            ...
            if (this.width > 200) {
                ...
                group.text('50%', '70%', details.context.boost_pay / 100).attr(Drawer.FREESPINS_STYLE_TEXT_CENTER_2)
            } else {
                group.text('50%', '50%', 'Boost win').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
                group.text('50%', '70%', details.context.boost_pay / 100).attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
            }
            this._panel(group, labelLeft, labelRight);
            return snap
        }
                ...
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if ([11, 12].includes(game_board[i][j])) {
                            winboost.positions.push([i, j])
                        }
                    }
                }
                combs.push(winboost)
            } else if (this.get_pearls_count(this.details) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                var winbonus = {
                    'trigger': 'bonus',
                    'positions': [],
        ...
        value: function board(snap, _board) {
            ...
            if (this.is_boost(this.details)) {
                ...
                for (var _i3 in _board) {
                    for (var _j2 in _board[_i3]) {
                        if ([11, 12].includes(_board[_i3][_j2])) {
                            winbonus.positions.push([_i3, _j2])
                        }
                    }
                }
                combs.push(winbonus)
            ...
            var _loop = function _loop(_i4) {
                var _loop2 = function _loop2(_j3) {
                    ...
                    if (_this3.details.state === 'bonus' && bonus_symbols.indexOf(_board[_i4][_j3]) == -1) {
                        imageUrl = _this3.url('0.png')
                    }
                    if (_this3.details.state === 'freespins' && _board[_i4][_j3] == 9) {
                    ...
                    if (_this3.details.context.bs_v != undefined) {
                        value = _this3.details.context.bs_v[_i4][_j3]
                    } else {
                        value = _this3.details.context.bonus_symbols_v[_i4][_j3]
                    }
                    var is_last_respin = _this3.details.context.last_respin;
                    if (typeof value === 'number') {
                        value = (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    }
                    if (value != 0 && [11, 12].includes(_board[_i4][_j3])) {
