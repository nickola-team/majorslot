...
var WILD_ID = 9;
var MYSTERY_ID = 10;
var COLLECCTOR_ID = 12;
var DOUBLE_ID = 11;
var BONUS_ID = 13;
var SPECIAL_SYMBOLS = [MYSTERY_ID, COLLECCTOR_ID, DOUBLE_ID];
var BONUS_ID_ARRAY = [MYSTERY_ID, COLLECCTOR_ID, DOUBLE_ID, BONUS_ID];
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function title(details) {
            if (details.last_action == 'restore') {
                return 'Restore'
            } else if (details.last_action == 'bonus_init' || details.last_action == 'bonus_freespins_stop' || details.last_action == 'bonus_stop') {
        ...
        value: function _background(snap, state) {
            ...
            } else if (this.details.context.winscatters.length > 0) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#4682B4'
                })
            } else if (this.get_bonus_symbols_count(this.details) === 15) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#6B8E23'
                })
            } else {
        ...
        key: '_combsExtra',
        value: function _combsExtra(snap, combs, details) {
            var special = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
            var group = snap.group();
            this.bordered_cells = [];
            var totalWin = details.round_win;
            var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin);
            var labelRight = combs.length ? special ? 'all/' + combs.length : combs.length + '/' + combs.length : '';
            this._panel(group, labelLeft, labelRight);
            for (var i = 0; i < combs.length; ++i) {
                if (special) continue;
                this.drawe_border_win_symbols(group, combs[i]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
        ...
        value: function _combs(snap, combs, details) {
            var special = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
            if (combs.length && combs[0].isPopUp) {
                var _group = snap.group();
                var _totalWin = details.round_win || 0;
                var _labelLeft = this.title(details) + ', Win:' + this.coins(_totalWin);
                var _labelRight = combs.length ? 'all/' + combs.length : '';
                if (!special && combs.length === 1 && details.state !== 'bonus') _labelRight = '';
                this._panel(_group, _labelLeft, _labelRight);
                _group.text('50%', '56%', '').attr(Drawer.STYLE_TEXT_CENTER_RED);
                return _group
            }
            var group = snap.group();
            this.bordered_cells = [];
            ...
            var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin);
            var labelRight = combs.length ? 'all/' + combs.length : '';
            if (!special && combs.length === 1 && details.state !== 'bonus') labelRight = '';
            this._panel(group, labelLeft, labelRight);
            for (var i = 0; i < combs.length; ++i) {
                if (special) continue;
                this.drawe_border_win_symbols(group, combs[i]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            for (var _i2 = 0; _i2 < combs.length; ++_i2) {}
            return group
        }
    }, {
        key: '_comb',
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && details.context.bs.filter(function(v) {
                return v > 0
            }).length === 15
        }
    }, {
        key: 'get_bonus_symbols_count',
        value: function get_bonus_symbols_count(details) {
            var count = 0;
            var board = details.context.board;
            for (var i in board) {
                for (var j in board[i]) {
                    if (BONUS_ID_ARRAY.includes(board[i][j])) {
                        count = count + 1
                    }
                }
            }
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && this.get_bonus_symbols_count(details) === 15) {
                labelLeft = 'Jackpot!';
                if (this.width > 200) {
            ...
            } else if (details.state == 'freespins' && details.context.winscatters.length) {
        ...
        value: function make_spins(details) {
            ...
            var combs = [].concat(details.context.winlines, details.context.winscatters);
            if (this.get_bonus_symbols_count(this.details) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                var winbonus = {
                    'trigger': 'bonus',
                    'positions': []
                };
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if (BONUS_ID_ARRAY.includes(game_board[i][j])) {
            ...
            var combGroup = void 0;
            var margin = 0;
            var isExtraBonus = this.details.context.is_extra_bonus;
            var isSpinsOrFreespins = this.details.state === 'spins' || this.details.state === 'freespins';
            var special_board = details.context.board_origin ? Array.from(details.context.board_origin) : null;
            this._combs(combsGroup, combs, details, isExtraBonus && isSpinsOrFreespins);
            if (combs.length > 1) {
                for (var _i3 in combs) {
                    combGroup = this._comb(combsGroup, combs, combs[_i3], details);
                    margin += this.width;
                    combGroup.transform('T' + margin + ',0')
                }
            }
            var combsCount = isExtraBonus && isSpinsOrFreespins ? 2 : combs.length;
            combsCount = combsCount === 0 ? 1 : combsCount;
            var partSize = this.width / combsCount;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(Drawer.STYLE_PROGRESS);
            var actual_board = isExtraBonus && isSpinsOrFreespins && special_board ? special_board : game_board;
            var board = this.board(group, actual_board, -1, isExtraBonus && isSpinsOrFreespins);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            ...
            var counter = 0;
            snap.click(function(e) {
                if (isExtraBonus && isSpinsOrFreespins) {
                    counter = (counter + 1) % 2;
                    progress.animate({
                        width: partSize * (counter + 1)
                    }, 100);
                    combsGroup.remove();
                    combsGroup = snap.group();
                    _this2._combsExtra(combsGroup, combs, details, counter !== 1);
                    board.remove();
                    var _special_board = Array.from(details.context.board_origin);
                    var cur_board = counter === 1 ? Array.from(game_board) : _special_board;
                    board = _this2.board(group, cur_board, -1, !(counter === 1));
                    ...
                    progress.animate({
                        width: partSize * counter
                    }, 100);
                    var _cur_board = Array.from(game_board);
                    board.remove();
                    board = _this2.board(group, _cur_board, counter - 1);
                    board.transform('T' + _this2.boardMarginLeft + ',' + (_this2.panelHeight + 1))
                }
            });
        ...
        key: 'proceedSpecialSymbols',
        value: function proceedSpecialSymbols() {
            var specialInfo = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
            var mystery = [];
            var doubles = [];
            var collectors = [];
            ...
            try {
                for (var _iterator = specialInfo[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    var symbol = _step.value;
                    if (symbol.old_value_by_bet !== 0) continue;
                    switch (symbol.symbol_id) {
                        case 12:
                            collectors.push(symbol);
                            break;
                        case 11:
                            doubles.push(symbol);
                            break;
        ...
        value: function make_bonus(details) {
            ...
            var specialInfo = this.proceedSpecialSymbols(details.context.bonus_collection_events);
            var nonSpecialSymbols = !(specialInfo.mystery.length !== 0 || specialInfo.doubles.length !== 0 || specialInfo.collectors.length !== 0);
            var isBonusStart = details.last_action === 'bonus_init';
            var isExtraSnap = this.is_extra_snap(details);
            var isLastBonusJackSpin = this.details.context.rounds_left === 0 && this.details.context.bs_jackpots.find(function(el) {
                return el > -1
            }) >= 0;
            var combs = [];
            if (specialInfo.mystery.length > 0 && specialInfo.mystery.find(function(el) {
                    return el.old_value_by_bet === 0
                })) {
                if (isBonusStart && combs.length === 0) combs.push({
                    positions: [],
            ...
            if (specialInfo.doubles.length > 0 && specialInfo.doubles.find(function(el) {
                    return el.old_value_by_bet === 0
                })) {
                if (isBonusStart && combs.length === 0) combs.push({
                    positions: [],
            ...
            if (specialInfo.collectors.length > 0 && specialInfo.collectors.find(function(el) {
                    return el.old_value_by_bet === 0
                })) {
                if (isBonusStart && combs.length === 0) combs.push({
                    positions: [],
            ...
            if (isLastBonusJackSpin && combs.length === 0) {
                combs.push({
                    positions: [],
                    amount: details.round_win,
                    isPopUp: true
            ...
            this._combs(snap.group(), combs, details);
            var combsGroup = snap.group();
            var combGroup = void 0;
            var margin = 0;
            if (isExtraSnap) {
            ...
            var partSize = this.width / combsGroup.children().length;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(Drawer.STYLE_PROGRESS);
            var board = this.bonusBoard(group, game_board, -1, false);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            if (details.last_action === 'bonus_init' || details.last_action === 'bonus_spins_stop') {
                board.remove();
                if (nonSpecialSymbols) this._background(snap, 'bonus');
                if (nonSpecialSymbols) this._panel(snap, details.last_action, '')
            }
            var counter = 0;
            snap.click(function(e) {
                if (isLastBonusJackSpin && combsGroup.children().length === 1) {
                    counter = (counter + 1) % 2;
                    progress.animate({
                        width: partSize * (counter + 1)
                    }, 100);
                    ...
                    board.remove();
                    if (counter === 0) {
                        _snap = snap.text('50%', '56%', _this3.get_text_for_action(details)).attr(Drawer.STYLE_TEXT_CENTER_RED)
                    } else {
                        _snap.remove()
                    }
                    var isSpecial = isBonusStart ? counter > 1 : true;
                    board = _this3.bonusBoard(snap, cur_board, counter, isSpecial, counter === combsGroup.children().length - 1 && isExtraSnap || counter === combsGroup.children().length && !isExtraSnap && _this3.details.context.rounds_left === 0);
                    ...
                    var _cur_board2 = Array.from(game_board);
                    board.remove();
                    if (counter === 0) {
                        _snap = snap.text('50%', '56%', _this3.get_text_for_action(details)).attr(Drawer.STYLE_TEXT_CENTER_RED)
                    } else {
                    ...
                    if ((counter || !isBonusStart) && !(counter === combsGroup.children().length - 1 && isExtraSnap)) {
                        var _isSpecial = isBonusStart ? counter > 1 : true;
                        board = _this3.bonusBoard(snap, _cur_board2, counter, _isSpecial, counter === combsGroup.children().length - 2 && isExtraSnap || counter === combsGroup.children().length - 1 && !isExtraSnap && _this3.details.context.rounds_left === 0)
                    }
                    board.transform('T' + _this3.boardMarginLeft + ',' + (_this3.panelHeight + 1))
                }
            });
            var _snap = snap.text('50%', '56%', this.get_text_for_action(details)).attr(Drawer.STYLE_TEXT_CENTER_RED);
            return snap.node
        }
    }, {
        key: 'board',
        value: function board(snap, _board) {
            ...
            combs = combs.concat(this.details.context.winscatters);
            var mults = this.details.context.mults;
            var win_mult_pos = [];
            if (this.get_bonus_symbols_count(this.details) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                var winbonus = {
                    'trigger': 'bonus',
                ...
                for (var i in _board) {
                    for (var j in _board[i]) {
                        if (BONUS_ID_ARRAY.includes(_board[i][j])) {
                            winbonus.positions.push([i, j])
                        }
                    }
                }
                combs.push(winbonus)
            } else if (this.details.context.mults != undefined) {
                for (var _i4 in mults) {
                    if (this.details.context.bs_v[mults[_i4][0]][mults[_i4][1]] != 0) {
                        win_mult_pos.push(mults[_i4])
                    }
                }
            }
            if (!combs) {
            ...
            var _loop = function _loop(_i5) {
                var _loop2 = function _loop2(_j) {
                    var x2 = '';
                    if (_board[_i5][_j] == WILD_ID && _this4.details.state === 'freespins') {
                        x2 = '_x2'
                    }
                    imageUrl = _this4.url(_board[_i5][_j] + x2 + '.png');
                    if (_this4.details.state === 'bonus' && BONUS_ID_ARRAY.indexOf(_board[_i5][_j]) == -1) {
                        imageUrl = _this4.url('0.png');
                        return 'continue'
                    }
                    var img = group.image(imageUrl, (_this4.cellWidth + _this4.cellOffsetX) * _i5, (_this4.cellHeight + _this4.cellOffsetY) * _j, _this4.cellWidth, _this4.cellHeight);
                    if (combs.length == 1) {
                        currentWinLine = 0
                    }
                    var value = void 0;
                    if (BONUS_ID_ARRAY.includes(_board[_i5][_j]) && _this4.details.context.bs != undefined) {
                        value = _this4.details.context.bs[parseInt(_i5) * 3 + parseInt(_j)]
                    } else {
                        value = 0
                    }
                    var mult_value = -1;
                    if (_this4.details.context.mult_values === undefined) {
                        mult_value = 1
                    } else {
                        mult_value = _this4.details.context.mult_values[_i5][_j]
                    }
                    if (!special && mult_value !== 1 && (_this4.details.context.rounds_left != 0 || !win_mult_pos.includes((_i5, _j)))) {
                        var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i5 + _this4.cellWidth / 1.8;
                        var rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j - _this4.cellOffsetY;
                        var rect_width = _this4.cellWidth / 2.1 + 2 * _this4.cellOffsetX;
                        var text_x_x = rect_x + rect_width / 2;
                        var text_x_y = (_this4.cellHeight + _this4.cellOffsetY) * _j + _this4.cellOffsetY + _this4.cellHeight * 0.2;
                    ...
                    if (value != 0) {
                        var _rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i5 - _this4.cellOffsetX;
                        var _rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j + _this4.cellHeight / 3;
                        var _rect_width = _this4.cellWidth + 2 * _this4.cellOffsetX;
                        var text_x = _rect_x + _rect_width / 2;
                        ...
                        var jackpots = [15, 50, 150, 5000];
                        var jackpotIndex = jackpots.indexOf(parseInt(value));
                        if (jackpotIndex != -1 && _this4.details.context.rounds_left != 0 && _board[_i5][_j] == 10) {
                            if (jackpotIndex === 0) value = 'mini';
                            if (jackpotIndex === 1) value = 'minor';
                            if (jackpotIndex === 2) value = 'major';
                            if (jackpotIndex === 3) value = 'grand'
                        }
                        if (value == 'mini') {
                            if (_this4.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MINI_70)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MINI_40)
                            }
                        } else if (value == 'minor') {
                            if (_this4.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MINOR_70)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MINOR_40)
                            }
                        } else if (value == 'major') {
                            if (_this4.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MAJOR_70)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MAJOR_40)
                            }
                        } else if (value == 'grand') {
                            if (_this4.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_GRAND_70)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_GRAND_40)
                            }
                        } else {
                            value = value * _this4.details.round_bet / 100;
                            if (typeof value === 'number') {
                                value = value.toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                            }
                            if (value < 100000) {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_70)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_40)
                                }
                            } else {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_44)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_23)
                                }
                            }
                        }
                    } else {}
                ...
                for (var _j in _board[_i5]) {
                    var _ret2 = _loop2(_j);
                    if (_ret2 === 'continue') continue
                }
            };
            for (var _i5 in _board) {
                _loop(_i5)
            }
            return group
        }
        ...
        key: 'bonusBoard',
        value: function bonusBoard(snap, board) {
            var currentWinLine = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -1;
            var _this5 = this;
            var special = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
            var jpRevile = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
            var combs = [].concat(this.details.context.winlines, this.details.context.winscatters);
            var isBonusGame = this.details.state === 'bonus';
            var isBonusStart = this.details.last_action === 'bonus_init';
            var specialInfo = this.proceedSpecialSymbols(this.details.context.bonus_collection_events);
            if (this.details.context.orig_board && !newBonusBoard && isBonusGame && mysteryIndex == -1) {
                board = this.details.context.orig_board
            }
            if (this.get_bonus_symbols_count(this.details) >= 6 && 'spin' == this.details.last_action) {
                var winbonus = {
                    trigger: 'bonus',
                    positions: [],
                    amount: 0
            ...
            var _loop3 = function _loop3(_i6) {
                var _loop4 = function _loop4(_j2) {
                    var isMysterySymbol = board[_i6][_j2] === MYSTERY_ID;
                    var isDoubleSymbol = board[_i6][_j2] === DOUBLE_ID;
                    var isCollectorSymbol = board[_i6][_j2] === COLLECCTOR_ID;
                    var nubOfSpecialSymbols = 0;
                    for (var key in specialInfo) {
                        specialInfo[key].length > 0 && nubOfSpecialSymbols++
                    }
                    var x2 = '';
                    if (board[_i6][_j2] === WILD_ID && _this5.details.state === 'freespins') {
                        x2 = '_x2'
                    }
                    imageUrl = _this5.url(board[_i6][_j2] + x2 + '.png');
                    if (_this5.details.state === 'bonus' && BONUS_ID_ARRAY.indexOf(board[_i6][_j2]) === -1) {
                        imageUrl = _this5.url('0.png');
                        return 'continue'
                    }
                    var img = group.image(imageUrl, (_this5.cellWidth + _this5.cellOffsetX) * _i6, (_this5.cellHeight + _this5.cellOffsetY) * _j2, _this5.cellWidth, _this5.cellHeight);
                    if (combs.length === 1) {
                        currentWinLine = 0
                    }
                    var value = void 0;
                    if (BONUS_ID_ARRAY.includes(board[_i6][_j2]) && _this5.details.context.bs !== undefined) {
                        value = _this5.details.context.bs[parseInt(_i6) * 3 + parseInt(_j2)];
                        if (!special && SPECIAL_SYMBOLS.includes(board[_i6][_j2])) {
                            if (isMysterySymbol && specialInfo.mystery.length > 0 && specialInfo.mystery.find(function(el) {
                                    return el.position.toString() === [_i6, _j2].toString() && el.old_value_by_bet === 0
                                })) {
                                value = 0
                            } else if (isDoubleSymbol && specialInfo.doubles.length > 0 && specialInfo.doubles.find(function(el) {
                                    return el.position.toString() === [_i6, _j2].toString() && el.old_value_by_bet === 0
                                })) {
                                value = 0
                            } else if (isCollectorSymbol && specialInfo.collectors.length > 0 && specialInfo.collectors.find(function(el) {
                                    return el.position.toString() === [_i6, _j2].toString() && el.old_value_by_bet === 0
                                })) {
                                value = 0
                            } else {
                    ...
                    if (special && SPECIAL_SYMBOLS.includes(board[_i6][_j2]) && _this5.details.context.bs !== undefined) {
                        if (isMysterySymbol && (currentWinLine >= 1 || specialInfo.mystery.length === 0 || !specialInfo.mystery.find(function(el) {
                                return el.position.toString() === [_i6, _j2].toString() && el.old_value_by_bet === 0
                            }))) {
                            value = _this5.details.context.bs[parseInt(_i6) * 3 + parseInt(_j2)]
                        } else if (isDoubleSymbol && (specialInfo.mystery.length > 0 && currentWinLine >= 2 + (isBonusStart ? 1 : 0) || specialInfo.mystery.length === 0 && currentWinLine >= 1 || specialInfo.doubles.length === 0 || !specialInfo.doubles.find(function(el) {
                                return el.position.toString() === [_i6, _j2].toString() && el.old_value_by_bet === 0
                            }))) {
                            value = _this5.details.context.bs[parseInt(_i6) * 3 + parseInt(_j2)]
                        } else if (isCollectorSymbol && (specialInfo.mystery.length > 0 && specialInfo.doubles.length > 0 && currentWinLine >= 3 + (isBonusStart ? 1 : 0) || specialInfo.mystery.length === 0 && specialInfo.doubles.length > 0 && currentWinLine >= 3 || specialInfo.mystery.length > 0 && specialInfo.doubles.length === 0 && currentWinLine >= 3 || specialInfo.mystery.length === 0 && specialInfo.doubles.length === 0 && currentWinLine >= 1 || specialInfo.collectors.length === 0 || !specialInfo.collectors.find(function(el) {
                                return el.position.toString() === [_i6, _j2].toString() && el.old_value_by_bet === 0
                            }))) {
                            value = _this5.details.context.bs[parseInt(_i6) * 3 + parseInt(_j2)]
                        } else {
                    ...
                    var mult_value = -1;
                    if (_this5.details.context.mult_values === undefined) {
                        mult_value = 1
                    } else {
                        mult_value = _this5.details.context.mult_values[_i6][_j2]
                    }
                    if (!special && mult_value !== 1 && (_this5.details.context.rounds_left != 0 || !win_mult_pos.includes((_i6, _j2)))) {
                        var rect_x = (_this5.cellWidth + _this5.cellOffsetX) * _i6 + _this5.cellWidth / 1.8;
                        var rect_y = (_this5.cellHeight + _this5.cellOffsetY) * _j2 - _this5.cellOffsetY;
                        var rect_width = _this5.cellWidth / 2.1 + 2 * _this5.cellOffsetX;
                        var text_x_x = rect_x + rect_width / 2;
                        var text_x_y = (_this5.cellHeight + _this5.cellOffsetY) * _j2 + _this5.cellOffsetY + _this5.cellHeight * 0.2;
                    ...
                    if (value !== 0) {
                        var _rect_x2 = (_this5.cellWidth + _this5.cellOffsetX) * _i6 - _this5.cellOffsetX;
                        var _rect_y2 = (_this5.cellHeight + _this5.cellOffsetY) * _j2 + _this5.cellHeight / 3;
                        var _rect_width2 = _this5.cellWidth + 2 * _this5.cellOffsetX;
                        var text_x = _rect_x2 + _rect_width2 / 2;
                        var text_y = (_this5.cellHeight + _this5.cellOffsetY) * _j2 + _this5.cellHeight / 2;
                        group.rect(_rect_x2, _rect_y2, _rect_width2, _this5.cellHeight / 3).attr({
                            'fill': 'white'
                        });
                        var jackpots = _this5.details.context.bs_jackpots;
                        var jackpotIndex = jackpots[+_j2 + +_i6 * 3];
                        if (jackpotIndex != -1 && !jpRevile) {
                            if (jackpotIndex === 0) value = 'mini';
                            if (jackpotIndex === 1) value = 'minor';
                            if (jackpotIndex === 2) value = 'major';
                            if (jackpotIndex === 3) value = 'grand'
                        ...
                        if (value == 'mini') {
                            ...
                            value = value * _this5.details.round_bet / 100;
                            if (typeof value === 'number') {
                                value = value.toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                            }
                            if (value < 100000) {
                ...
                for (var _j2 in board[_i6]) {
                    var _ret4 = _loop4(_j2);
                    if (_ret4 === 'continue') continue
                }
            };
            for (var _i6 in board) {
                _loop3(_i6)
            }
            return group
        }
