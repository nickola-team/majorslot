...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function make_spins(details) {
            ...
            if (details.state == 'bonus' && details.last_action == 'respin' && details.context.bs_count !== 40) {
                this.make_unlock_snap(details, combsGroup, margin, snap)
            }
            if (details.last_action == 'freespin_init' || details.last_action == 'freespin_stop') {
                board.remove();
                this._background(snap, 'freespins');
        ...
        value: function board(snap, _board) {
            ...
            if (this.details.state === 'bonus') {
                unlockedRow = this.key_thresholds.map(function(numToUnlock) {
                    var bsCount = _this4.details.context.keys;
                    var num = numToUnlock - bsCount < 0 ? 0 : numToUnlock - bsCount;
                    return num
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(_j) {
                    var value = _this4.details.context.bs ? _this4.details.context.bs.filter(function(el) {
                        return el.reel == i && el.position == _j
                    })[0] : [];
                    value = value ? value.value : 0;
                    var meteorType = value == _this4.mini_jp * round_bet || value == _this4.minor_jp * round_bet || value == _this4.major_jp * round_bet || value == _this4.grand_jp * round_bet ? '2' : '1';
                    var postfix = _this4.details.state === 'bonus' ? '_b' + meteorType : _board[i][_j] === 11 ? '_' + meteorType : '';
                    var symName = 'sym' + _board[i][_j] + postfix + '.png';
                    imageUrl = _this4.url(symName);
                    var img = group.image(imageUrl, (_this4.cellWidth + _this4.cellOffsetX) * i, (_this4.cellHeight + _this4.cellOffsetY) * _j, _this4.cellWidth, _this4.cellHeight);
                    if (_this4.details.state === 'bonus' && _board[i][_j] === 11) {
                        img.attr({
            ...
            if (typeof this.details.context.bs !== 'undefined') {
                ...
                try {
                    for (var _iterator = this.details.context.bs[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        ...
                        var j = bs.position;
                        var value = void 0;
                        if (bs.value == this.mini_jp * round_bet) {
                            value = 'mini'
                        } else if (bs.value == this.minor_jp * round_bet) {
                            value = 'minor'
                        } else if (bs.value == this.major_jp * round_bet) {
                            value = 'major'
                        } else if (bs.value == this.grand_jp * round_bet) {
                            value = 'grand'
                        } else {
                            value = (+bs.value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                        }
                        var rect_x = (this.cellWidth + this.cellOffsetX) * i - this.cellOffsetX + 2;
                        ...
                        if (['mini', 'minor', 'major', 'grand'].includes(value)) {
                            attr = Object.assign({}, this.width > 200 ? Drawer['PEARL_' + value.toUpperCase() + '_55'] : Drawer['PEARL_' + value.toUpperCase() + '_40'], {
                                'opacity': opacity
                            });
                            ...
                            if (numValue < 999999) {
                                attr = Object.assign({}, this.width > 200 ? Drawer.PEARL_VALUE_52 : Drawer.PEARL_VALUE_28, {
                                    'opacity': opacity
                                });
                                group.text(text_x, text_y, value).attr(attr)
                            } else {
                                attr = Object.assign({}, this.width > 200 ? Drawer.PEARL_VALUE_34 : Drawer.PEARL_VALUE_22, {
                                    'opacity': opacity
                                });
                                group.text(text_x, text_y, value).attr(attr)
                            }
