...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function title(details) {
            ...
            if ('achievements_init' == details.last_action) {
                return 'Free Achievements (' + details.context.rounds_left + ')'
            }
            switch (details.state) {
                case 'spins':
                    return 'Spin';
                case 'freespins':
                    return 'Free (' + details.context.rounds_left + ')';
            }
        }
    }, {
        key: 'draw',
        value: function draw(element, details) {
            details = this.migrate(details);
            _get(Drawer.prototype.__proto__ || Object.getPrototypeOf(Drawer.prototype), 'draw', this).call(this, element, details)
        }
    }, {
        key: 'migrate',
        value: function migrate(details) {
            if (details.version === 3) return details;
            if (details.context.version === 2) return details;
            if ('matrix' in details.context) {
                details.context.board = details.context.matrix;
                delete details.context.matrix;
                if (details.state === 'freespins') {
                    details.context.rounds_left = details.context.left;
                    delete details.context.left
                }
                for (var i in details.context.winlines) {
                    var line = details.context.winlines[i];
                    line.positions = line.positions.map(function(v, i) {
                        return [i, v - 1]
                    }).filter(function(i) {
                        return i[1] !== -1
                    })
                }
                for (var _i in details.context.winscatters) {
                    var _line = details.context.winscatters[_i];
                    _line.positions = _line.positions.map(function(v, i) {
                        return [i, v - 1]
                    }).filter(function(i) {
                        return i[1] !== -1
                    })
            ...
            if ('board' in details.context) {
                if (details.state === 'freespins' && 'rounds_lefts' in details.context) {
                    details.context.rounds_left = details.context.rounds_lefts
                }
            }
            return details
        ...
        value: function make_spins(details) {
            var _this2 = this;
            this.cols = details.context.board.length;
            if (details.context.board[5].length > details.context.board[0].length) {
                this.rows = details.context.board[5].length
            } else {
                this.rows = details.context.board[0].length
            }
            this._calculateSizes(this.cols, this.rows);
            var snap = new Snap(this.width, this.height);
            if (details.state == 'freespins' || details.last_action == 'freespin_stop') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#C1FFC1'
                });
                this.boardMarginTop = this.cellHeight
            } else {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#E8E8E8'
                });
                this.boardMarginTop = 0
            }
            var group = snap.group().attr({
                width: this.width,
                height: this.height
            ...
            var partSize = this.width / combsGroup.children().length;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(BaseDrawer.STYLE_PROGRESS);
            var board = this.board(group, details.context);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1 + this.boardMarginTop).toString());
            ...
            if (details.state == 'spins' && details.last_action !== 'freespin_stop') {
                group.text('50%', '87%', 'Achievements progress: ').attr(Drawer.STYLE_TEXT_CENTER);
                group.text('50%', '95%', this.details.achievements.scatters + '/' + this.details.achievements.scatters_to_collect).attr(Drawer.STYLE_TEXT_CENTER)
            }
        ...
        key: 'board',
        value: function board(snap, ctx) {
            var board = ctx.board;
            var group = snap.group();
            var imageUrl = void 0;
            for (var i in board) {
                for (var j in board[i]) {
                    imageUrl = this.url(board[i][j] + '.png');
                    group.image(imageUrl, (this.cellWidth + this.cellOffsetX) * i, (this.cellHeight + this.cellOffsetY) * j + this.cellHeight / 2 * (7 - board[i].length), this.cellWidth, this.cellHeight)
                }
            }
            return group
        }
            ...
            for (var i = 0; i < line.positions.length; ++i) {
                ...
                if (!isItemInArray(this.bordered_cells, cell)) {
                    if (this.details.state == 'spins') {
                        group.rect(cell[0] * this.cellWidth, cell[1] * this.cellHeight + this.cellHeight / 2 * (5 - cell[0]), this.cellWidth, this.cellHeight).attr(BaseDrawer.STYLE_LINE_SYMBOL)
                    } else {
                        group.rect(cell[0] * this.cellWidth, cell[1] * this.cellHeight + this.cellHeight / 2 * cell[0] + this.boardMarginTop, this.cellWidth, this.cellHeight).attr(BaseDrawer.STYLE_LINE_SYMBOL)
                    }
                    this.bordered_cells.push(cell)
                }
            }
        ...
        value: function drawe_line_patern(snap, line) {
            ...
            if (typeof line['trigger'] === 'undefined') {
                ...
                var path = 'M' + this.cellWidth / 2 + ',';
                if (this.details.state == 'spins') {
                    path += temp[0] * this.cellHeight + this.cellHeight / 2 + this.cellHeight / 2 * 5 + ' ';
                    for (var _i3 = 0; _i3 < temp.length; ++_i3) {
                        path += 'L' + (_i3 * this.cellWidth + this.cellWidth / 2) + ',';
                        path += temp[_i3] * this.cellHeight + this.cellHeight / 2 + this.cellHeight / 2 * (5 - _i3) + ' '
                    }
                } else {
                    path += temp[0] * this.cellHeight + this.cellHeight / 2 + this.boardMarginTop + ' ';
                    for (var _i4 = 0; _i4 < temp.length; ++_i4) {
                        path += 'L' + (_i4 * this.cellWidth + this.cellWidth / 2) + ',';
                        path += temp[_i4] * this.cellHeight + this.cellHeight / 2 + this.cellHeight / 2 * _i4 + this.boardMarginTop + ' '
                    }
                }
                group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
                group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER)
...
Drawer.STYLE_RESPIN_LINE = {
    'stroke': '#FF850F',
    'stroke-width': 4,
    'fill-opacity': 0
};
Drawer.STYLE_TEXT_CENTER = {
    'text-anchor': 'middle',
    'font-size': '0.7em'
};
