...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function title(details) {
            if (details.last_action == 'restore') {
                return 'Restore'
            } else if (['freespin_init', 'freespin_stop', 'spin_pick'].includes(details.last_action)) return details.last_action;
        ...
        key: '_background',
        value: function _background(snap, state) {
            if (state === 'freespins' || this.details.last_action === 'freespins_stop') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#4682B4'
                })
            } else if (this.details.last_action === 'spin_pick') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#7e871b'
                })
            } else if (state === 'spins') {
            ...
            } else if (this.details.context.pearls_count === 15 || this.details.context.bs_count === 15) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#6B8E23'
                })
            } else {
        ...
        key: '_calculateSizes',
        value: function _calculateSizes(cols) {
            this.panelHeight = this.height > 150 ? 20 : 12;
            this.panelFont = this.height > 150 ? '1em' : '0.7em';
            this.progressHeight = 3;
            this.boardHeight = this.height - (this.panelHeight + this.progressHeight + 1);
            ...
            this.cellOffsetY = -1;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10) + 2;
            this.boardMarginLeftES = parseInt((this.width - this.colWidth * cols) / 2, 10) + 2
        }
    }, {
        key: '_calculateHeight',
        value: function _calculateHeight(rows) {
            this.cellHeight = [];
            for (var i in rows) {
                this.cellHeight.push(this.height * MULT / rows[i])
            }
        }
    }, {
        key: '_get',
        value: function _get(object, key, default_value) {
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            margin += this.width;
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(BaseDrawer.STYLE_PROGRESS);
            progress.animate({
                width: 186
            }, 100);
        ...
        key: 'get_symbol_win',
        value: function get_symbol_win(symbol, length) {
            if (symbol in paytable) {
                return paytable[symbol][length] * this.details.context.bet_per_line
            } else {
                return 0
            }
        }
        ...
        value: function get_text_for_action(details) {
            ...
            switch (action) {
                ...
                case 'freespin_init':
                    return 'FS count: ' + details.context.rounds_left;
                case 'spin_pick':
                    return this.get_spin_pick_text(details);
                default:
                    return '';
            }
        }
        ...
        value: function make_snap(details) {
            this.cols = details.context.board.length;
            this.rows = details.context.reels_len;
            this._calculateSizes(this.cols, this.rows);
        ...
        value: function make_spins(details) {
            ...
            this._background(snap, details.state);
            this._calculateHeight(details.context.reels_len);
            var action_list = [{
                'action': true,
                'board': this.details.context.board,
                'winlines': this.details.context.winlines ? this.details.context.winlines : [],
                'winscatters': this.details.context.winscatters ? this.details.context.winscatters : [],
                'round_win': 0,
                'win': this.details.context.win ? this.details.context.win : 0,
                'mult': this.details.context.mult - (this.details.context.avalanche ? this.details.context.avalanche.length : 0) ? this.details.context.mult : 1
            }];
            var totalWin = this.details.context.win ? this.details.context.win : 0;
            var combs = [];
            combs = combs.concat(this.details.context.winlines);
            combs.forEach(function(elem) {
                var mult_values = _this2.details.context.mult_values;
                var prev_values = {
                    1: 1
                };
                var cur_values = {};
                var pos_dict = {};
                elem.positions.forEach(function(pos) {
                    pos_dict[pos[0]] = [].concat(_this2._get(pos_dict, pos[0], []), [pos[1]])
                });
                ...
                try {
                    var _loop = function _loop() {
                        ...
                        value.forEach(function(i) {
                            var mult_value = mult_values[key][i];
                            mult_value = mult_value ? mult_value : 1;
                            mult_amount[mult_value] = _this2._get(mult_amount, mult_value, 0) + 1
                        });
                        var _iteratorNormalCompletion3 = true;
                        var _didIteratorError3 = false;
                        var _iteratorError3 = undefined;
                        try {
                            for (var _iterator3 = Object.entries(prev_values)[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                                var _step3$value = _slicedToArray(_step3.value, 2),
                                    prev_mult = _step3$value[0],
                                    prev_amount = _step3$value[1];
                                var _iteratorNormalCompletion4 = true;
                                var _didIteratorError4 = false;
                                var _iteratorError4 = undefined;
                                try {
                                    for (var _iterator4 = Object.entries(mult_amount)[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
                                        var _step4$value = _slicedToArray(_step4.value, 2),
                                            mult = _step4$value[0],
                                            amount = _step4$value[1];
                                        var new_mult = prev_mult * mult;
                                        var new_amount = prev_amount * amount;
                                        cur_values[new_mult] = _this2._get(cur_values, new_mult, 0) + new_amount
                                    }
                                } catch (err) {
                    ...
                    for (var _iterator = Object.entries(pos_dict)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        _loop()
                    }
                } catch (err) {
                ...
                var win_values = {};
                var totalWin = 0;
                var win_per_line = _this2.get_symbol_win(elem.symbol, elem.occurrences);
                var _iteratorNormalCompletion2 = true;
                var _didIteratorError2 = false;
                var _iteratorError2 = undefined;
                try {
                    for (var _iterator2 = Object.entries(prev_values)[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                        var _step2$value = _slicedToArray(_step2.value, 2),
                            mult = _step2$value[0],
                            amount = _step2$value[1];
                        win_values[mult] = win_per_line * amount * mult;
                        totalWin += win_per_line * amount * mult
                    }
                } catch (err) {
                ...
                var snap = {
                    'action': false,
                    'symbol': elem.symbol,
                    'count': elem.occurrences,
                    'lines': prev_values,
                    'wins': win_values,
                    'win': totalWin
                };
                action_list = action_list.concat(snap)
            });
            if (this.details.context.winscatters.length > 0) {
                var scatters = [{
                    'is_scatters': true,
                    'action': true,
                    'board': this.details.context.board,
                    'winscatters': this.details.context.winscatters
                }];
                action_list = action_list.concat(scatters)
            }
            var group = snap.group().attr({
                width: this.width,
                height: this.height
            ...
            if (details.last_action === 'respin') {
                ...
                var labelLeft = 'Respin, Win:' + this.coins(totalWin);
                var labelRight = '';
                this._panel(group, labelLeft, labelRight);
                return snap.node
            }
            ...
            if (action_list.length > 1) {
                for (var i in action_list) {
                    if (i == 0) continue;
                    combGroup = this._comb(combsGroup, action_list, action_list[i], details);
                    margin += this.width;
                    combGroup.transform('T' + margin + ',0')
                }
            }
            var partSize = this.width / combsGroup.children().length;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(BaseDrawer.STYLE_PROGRESS);
            if (['freespin_init', 'freespin_stop', 'spin_pick'].includes(details.last_action)) {
                board.remove();
                if (details.last_action === 'spin_pick') {
                    this._background(snap, 'spin_pick')
                } else {
                    this._background(snap, 'freespins')
                }
                this._panel(snap, details.last_action, '')
            ...
            snap.click(function(e) {
                ...
                board.remove();
                if (action_list[counter].action) {
                    board = _this2.board(group, action_list[counter]);
                    board.transform('T' + _this2.boardMarginLeft + ',' + (_this2.panelHeight + 1));
                    if (action_list[counter].is_scatters) {
                        _this2.drawe_border_win_symbols(group, action_list[counter]).transform('T' + _this2.boardMarginLeft + ',' + (_this2.panelHeight + 1))
                    }
                } else {
                    board = _this2.draw_extra_snap(group, action_list, counter);
                    board.transform('T' + _this2.boardMarginLeftES + ',' + (_this2.panelHeight + 1))
                }
            });
            snap.text('50%', '56%', this.get_text_for_action(details)).attr(BaseDrawer.STYLE_TEXT_CENTER_RED);
            return snap.node
        ...
        value: function draw_extra_snap(snap, action_list, currentAction) {
            ...
            var group = snap.group();
            var multAmount = one_action.lines ? Object.keys(one_action.lines).length : 0;
            var imageHeight = this.height * MULT / 7;
            var counter = 0;
            var height_alpha = -0.055;
            var line_start = this.height * 0.08;
            var line_step = this.height * 0.8 / Object.keys(one_action.lines).length;
            if (this.height > 200) {
                group.text(this.width * 0.018, this.height * 0.05, 'Len').attr(Drawer.HEADER_TEXT_CENTER_2);
                group.text(this.width * 0.17, this.height * 0.05, 'Symbol').attr(Drawer.HEADER_TEXT_CENTER_2);
                group.text(this.width * 0.36, this.height * 0.05, 'Lines').attr(Drawer.HEADER_TEXT_CENTER_2);
                group.text(this.width * 0.52, this.height * 0.05, 'Mult').attr(Drawer.HEADER_TEXT_CENTER_2);
                group.text(this.width * 0.78, this.height * 0.05, 'Win').attr(Drawer.HEADER_TEXT_CENTER_2)
            } else {
                group.text(this.width * 0.018, this.height * 0.05, 'Len').attr(Drawer.HEADER_TEXT_CENTER);
                group.text(this.width * 0.17, this.height * 0.05, 'Symbol').attr(Drawer.HEADER_TEXT_CENTER);
                group.text(this.width * 0.36, this.height * 0.05, 'Lines').attr(Drawer.HEADER_TEXT_CENTER);
                group.text(this.width * 0.52, this.height * 0.05, 'Mult').attr(Drawer.HEADER_TEXT_CENTER);
                group.text(this.width * 0.78, this.height * 0.05, 'Win').attr(Drawer.HEADER_TEXT_CENTER)
            }
            var path = 'M' + this.width * 0.07 + ',' + 0;
            path += 'L' + this.width * 0.07 + ',' + this.height * 0.87 + ' ';
            group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
            group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER);
            path = 'M' + this.width * 0.27 + ',' + 0;
            path += 'L' + this.width * 0.27 + ',' + this.height * 0.87 + ' ';
            group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
            group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER);
            path = 'M' + this.width * 0.44 + ',' + 0;
            path += 'L' + this.width * 0.44 + ',' + this.height * 0.87 + ' ';
            group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
            group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER);
            path = 'M' + this.width * 0.6 + ',' + 0;
            path += 'L' + this.width * 0.6 + ',' + this.height * 0.87 + ' ';
            group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
            group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER);
            path = 'M' + this.width * -0.1 + ',' + this.height * 0.07;
            path += 'L' + this.width + ',' + this.height * 0.08 + ' ';
            group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
            group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER);
            for (var i = 1; i < multAmount; i++) {
                path = 'M' + this.width * 0.27 + ',' + (line_start + i * line_step);
                path += 'L' + this.width + ',' + (line_start + i * line_step) + ' ';
                group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
                group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER)
            }
            var imageUrl = this.url(one_action.symbol + '_x7.png');
            group.image(imageUrl, this.width * 0.095 + (this.colWidth - this.cellWidth) / 2, this.height * 0.44, this.cellWidth * 0.9, imageHeight * 0.9);
            if (this.height > 200) {
                group.text(this.width * 0.02, this.height * 0.44 + imageHeight / 1.5, one_action.count).attr(Drawer.EXTRA_SNAP_TEXT_CENTER_2)
            } else {
                group.text(this.width * 0.02, this.height * 0.44 + imageHeight / 1.5, one_action.count).attr(Drawer.EXTRA_SNAP_TEXT_CENTER)
            }
            var TextStyle = Drawer.EXTRA_SNAP_TEXT_CENTER;
            switch (true) {
                case this.height > 200 && multAmount > 5:
                    TextStyle = Drawer.EXTRA_SNAP_TEXT_SMALL_2;
                    break;
                case this.height > 200 && multAmount <= 5:
                    TextStyle = Drawer.EXTRA_SNAP_TEXT_CENTER_2;
                    break;
                case this.height <= 200 && multAmount > 5:
                    TextStyle = Drawer.EXTRA_SNAP_TEXT_SMALL;
                    break;
                case this.height <= 200 && multAmount <= 5:
                    TextStyle = Drawer.EXTRA_SNAP_TEXT_CENTER;
                    break;
            }
            ...
            try {
                for (var _iterator5 = Object.entries(one_action.lines)[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
                    var _step5$value = _slicedToArray(_step5.value, 2),
                        _i = _step5$value[0],
                        value = _step5$value[1];
                    counter += 1;
                    group.text(this.width * 0.2 + this.colWidth, line_start - this.height * (0.03 + height_alpha) + (counter - 0.5) * line_step, value).attr(TextStyle);
                    group.text(this.width * 0.36 + this.colWidth, line_start - this.height * (0.03 + height_alpha) + (counter - 0.5) * line_step, _i).attr(TextStyle);
                    group.text(this.width * 0.615 + this.colWidth, line_start - this.height * (0.03 + height_alpha) + (counter - 0.5) * line_step, this.coins(one_action.wins[_i])).attr(TextStyle)
                }
            } catch (err) {
        ...
        value: function drawe_border_win_symbols(snap, one_action) {
            var group = snap.group().attr(BaseDrawer.STYLE_LINE_GROUP);
            var height_shift = this.height * 0.05;
            var combs = [];
            if (this.details.context.winscatters) {
            ...
            for (var i in combs) {
                var line = combs[i];
                for (var _i2 = 0; _i2 < line.positions.length; ++_i2) {
                    var cell = line.positions[_i2];
                    if (!isItemInArray(this.bordered_cells, cell)) {
                        group.rect(cell[0] * (this.cellWidth + this.cellOffsetX), cell[1] * this.cellHeight[cell[0]] + height_shift, this.cellWidth + this.cellOffsetX, this.cellHeight[cell[0]] + this.cellOffsetY).attr(Drawer.STYLE_LINE_SYMBOL);
                        this.bordered_cells.push(cell)
                    }
                }
            }
        ...
        value: function _combs(snap, action_list, details) {
            ...
            var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin);
            var labelRight = '';
            labelRight = action_list.length > 1 ? '1/' + action_list.length : '';
            this._panel(group, labelLeft, labelRight);
        ...
        key: '_comb',
        value: function _comb(snap, combs, comb, details) {
            var multiply = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : undefined;
            this.bordered_cells = [];
            var group = snap.group();
            var labelLeft = this.title(details) + ', Win:' + this.coins(comb.win);
            var labelRight = void 0;
            labelRight = combs.indexOf(comb) + 1 + '/' + combs.length;
            if (comb.action) {
                this.drawe_border_win_symbols(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            this._panel(group, labelLeft, labelRight);
        ...
        value: function board(snap, one_action) {
            ...
            var group = snap.group();
            var imageUrl = void 0;
            var reels_len = this.details.context.reels_len;
            var pos_set = new Set;
            var height_shift = this.height * 0.05;
            ...
            var _loop2 = function _loop2(i) {
                var _loop3 = function _loop3(j) {
                    var img_name = board[i][j] + '_x' + reels_len[i];
                    if (_this3.details.state === 'freespins' && (board[i][j] === 11 || board[i][j] === 12 || board[i][j] === 13)) {
                        img_name += '_fs'
                    }
                    imageUrl = _this3.url(img_name + '.png');
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * i, _this3.cellHeight[i] * j + height_shift, _this3.cellWidth, _this3.cellHeight[i]);
        ...
        key: 'get_spin_pick_text',
        value: function get_spin_pick_text(details) {
            if (details.context.is_gambled) {
                if (details.context.gamble_win) {
                    if (details.context.winscatters[0].freespins === 30) {
                        return 'Gamble win, ' + details.context.winscatters[0].freespins + 'FS'
                    } else if (details.context.gamble_end) {
                        return 'Taked ' + details.context.winscatters[0].freespins + 'FS'
                    } else {
                        return 'Gamble win, ' + details.context.winscatters[0].freespins + 'FS'
                    }
                } else {
                    if (details.context.winscatters.freespins === 0) {
                        return 'Gamble loss'
                    } else {
                        return 'Gamble loss, ' + details.context.winscatters[0].freespins + 'FS'
                    }
                }
            } else if (details.context.winscatters[0].freespins === 0) {
                return 'Gamble loss'
            } else {
                return 'Taked ' + details.context.winscatters[0].freespins + 'FS'
            }
...
Drawer.EXTRA_SNAP_TEXT_CENTER_COLORED = {
    'stroke': '#474747',
    'text-anchor': 'middle',
    'fill': '#474747',
    'font-size': '0.5em'
...
Drawer.EXTRA_SNAP_TEXT_SMALL = {
    'stroke': '#ffffff',
    'text-anchor': 'middle',
    'fill': '#fff',
    'font-size': '0.4em'
};
Drawer.EXTRA_SNAP_TEXT_SMALL_2 = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#fff',
    'font-size': '0.65em'
