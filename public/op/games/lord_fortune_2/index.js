...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function title(details) {
            ...
            } else if (details.last_action == 'freespin_init' || details.last_action == 'freespin_stop' || details.last_action == 'buy_spin') return details.last_action;
        ...
        value: function _combs(snap, combs, details) {
            ...
            var totalWin = details.round_win;
            var labelLeft = '';
            if (details.state === 'freespins' && combs.length === 1) {
                var line_multiplier = 1;
                var is_wild_in_line = false;
                for (var i in combs[0].positions) {
                    var pos = combs[0].positions[i];
                    if (details.context.board[pos[0]][pos[1]] === 9) {
                        line_multiplier *= details.context.wild_symbols_values[pos[0]][pos[1]];
                        is_wild_in_line = true
                    }
                }
                if (is_wild_in_line) {
                    labelLeft = this.title(details) + ', Win:' + this.coins(totalWin) + ',x' + line_multiplier
                } else {
        ...
        value: function _comb(snap, combs, comb, details) {
            ...
            if (details.state === 'freespins') {
                var line_multiplier = 1;
                var is_wild_in_line = false;
                for (var i in comb.positions) {
                    var pos = comb.positions[i];
                    if (details.context.board[pos[0]][pos[1]] === 9) {
                        line_multiplier *= details.context.wild_symbols_values[pos[0]][pos[1]];
                        is_wild_in_line = true
                    }
                ...
                if (is_wild_in_line) {
                    labelLeft = this.title(details) + ', Win:' + this.coins(comb.amount) + ',x' + line_multiplier
                } else {
                    labelLeft = this.title(details) + ', Win:' + this.coins(comb.amount)
                }
            } else {
                labelLeft = this.title(details) + ', Win:' + this.coins(comb.amount)
            }
        ...
        value: function drawe_border_win_symbols(snap, line) {
            var group = snap.group().attr(BaseDrawer.STYLE_LINE_GROUP);
            if (line.positions === undefined || line.positions === []) {
                return group
            }
            for (var i = 0; i < line.positions.length; ++i) {
                var cell = line.positions[i];
        ...
        value: function make_spins(details) {
            ...
            var combs = [].concat(details.context.winlines, details.context.winscatters);
            if (this.get_pearls_count(this.details) >= 6 && ['spin', 'freespin', 'buy_spin'].includes(this.details.last_action)) {
                var winbonus = {
                    'trigger': 'bonus',
                    'positions': [],
                    'amount': 0
        ...
        value: function board(snap, _board) {
            ...
            if (this.get_pearls_count(this.details) >= 6 && ['spin', 'freespin', 'buy_spin'].includes(this.details.last_action)) {
                ...
                for (var i in _board) {
                    for (var j in _board[i]) {
                        if ([11, 12, 13, 14].includes(_board[i][j])) {
                            winbonus.positions.push([i, j])
                        }
                    }
                }
            ...
            var _loop = function _loop(_i4) {
                var _loop2 = function _loop2(_j) {
                    imageUrl = _this3.url(_board[_i4][_j] + '.png');
                    if (_this3.details.state === 'bonus' && bonus_symbols.indexOf(_board[_i4][_j]) == -1) {
                        imageUrl = _this3.url('0.png')
                    }
                    if (_this3.details.state === 'freespins' && _board[_i4][_j] == 9 && _this3.details.last_action != 'freespin_init') {
                        imageUrl = _this3.url('9_x' + _this3.details.context.wild_symbols_values[_i4][_j] + '.png')
                    }
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * _i4, (_this3.cellHeight + _this3.cellOffsetY) * _j, _this3.cellWidth, _this3.cellHeight);
                    var value = void 0;
                    if (_this3.details.context.bs_v != undefined) {
                        value = _this3.details.context.bs_v[_i4][_j]
                    } else {
                        value = _this3.details.context.bonus_symbols_v[_i4][_j]
                    }
                    var is_last_respin = _this3.details.context.last_respin;
                    if (typeof value === 'number') {
                        value = (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    }
                    if (value != 0 && [11, 12, 13].includes(_board[_i4][_j])) {
