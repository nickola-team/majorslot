...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function _background(snap, state) {
            ...
            } else if (this.details.state == 'freespins' && this.details.context.round_index === 0 && this.details.context.last_action != 'freespin_init') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#4682B4'
                })
            } else {
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && this.get_pearls_count(details) === 15 || details.state == 'freespins' && details.context.round_index === 0 && details.context.last_action != 'freespin_init' || details.state == 'bonus' && details.context.last_respin && this.is_extra_respins(details)
        }
    }, {
        key: 'get_arr_sum',
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            } else if (details.state == 'freespins' && details.context.round_index === 0 && details.context.last_action != 'freespin_init') {
                this._background(group, details, 'freespins');
                labelLeft = 'Additional FreeSpins!';
                var roundsGranted = add_fs;
        ...
        value: function make_spins(details) {
            ...
            if (details.state === 'freespins' && ctx.collects_on_board > 0) {
                var wincollectors = {
                    'positions': ctx.collectors_pos,
                    'amount': ctx.collect_pay,
                    'trigger': 'collect'
                };
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if ([6].includes(game_board[i][j])) {
                            wincollectors.positions.push([i, j])
                        }
                    }
                }
                combs.push(wincollectors)
            }
            if (!combs) {
                combs = []
            }
        ...
        value: function _combs(snap, combs, details) {
            ...
            this._panel(group, labelLeft, labelRight);
            for (var i = 0; i < combs.length; ++i) {
                if (combs[i].trigger === 'collect' && combs[i].amount === 0) {
                    continue
                }
                this.drawe_border_win_symbols(group, combs[i]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            for (var _i2 = 0; _i2 < combs.length; ++_i2) {
                if (combs[_i2].trigger === 'collect' && combs[_i2].amount === 0) {
                    continue
                }
        ...
        value: function board(snap, _board) {
            ...
            var _loop = function _loop(_i3) {
                var _loop2 = function _loop2(_j) {
                    ...
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * _i3, (_this3.cellHeight + _this3.cellOffsetY) * _j, _this3.cellWidth, _this3.cellHeight);
                    ...
                    if (value !== 0 && (_board[_i3][_j] === 13 || _board[_i3][_j] === 12 || _board[_i3][_j] === 6)) {
                        var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i3 - _this3.cellOffsetX;
                        var rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j + _this3.cellHeight / 1.6;
                        var rect_width = _this3.cellWidth + 2 * _this3.cellOffsetX;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = (_this3.cellHeight + _this3.cellOffsetY) * _j + _this3.cellHeight / 1.3;
                        if (_board[_i3][_j] === 6) {
                            group.rect(rect_x, rect_y, rect_width, _this3.cellHeight / 3).attr({
                                'fill': 'white'
                            })
                        }
                        if (_this3.details.state === 'freespins') {
                            var details = _this3.details;
                            var is_add_fs = details.state == 'freespins' && details.context.round_index === 0 && details.context.last_action != 'freespin_init';
                            var game_level = _this3.details.context.game_level;
                            if (is_add_fs) {
                                game_level = game_level - 1
                            }
                            if (_board[_i3][_j] === 6 && game_level > 0) {
                                rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i3 + _this3.cellWidth / 1.8;
                                rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j - _this3.cellOffsetY;
                                rect_width = _this3.cellWidth / 2.2 + 2 * _this3.cellOffsetX;
                                ...
                                var coins_mult = collect_multipliers[game_level];
                                if (_this3.width > 200) {
                                    group.text(text_x_x, text_x_y, 'x' + coins_mult).attr(Drawer.PEARL_VALUE_70)
                                } else {
                                    group.text(text_x_x, text_x_y, 'x' + coins_mult).attr(Drawer.PEARL_VALUE_40)
                                }
                            }
                        }
                        if ([12, 13].includes(_board[_i3][_j])) {
                            value = (_this3.details.context.collect_pay / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                            if (value > 0) {
                                group.rect(rect_x, rect_y, rect_width, _this3.cellHeight / 3).attr({
                                    'fill': 'white'
                                });
                                if (value < 100000) {
                                    if (_this3.width > 200) {
                                        group.text(text_x, text_y, value).attr(Drawer.COLLECT_70)
                                    } else {
                                        group.text(text_x, text_y, value).attr(Drawer.COLLECT_40)
                                    }
                                } else {
                                    if (_this3.width > 200) {
                                        group.text(text_x, text_y, value).attr(Drawer.COLLECT_48)
                                    } else {
                                        group.text(text_x, text_y, value).attr(Drawer.COLLECT_27)
                                    }
                                }
                            }
                        } else {
