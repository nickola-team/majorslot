...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        key: '_background',
        value: function _background(snap, state) {
            var isSuperBonus = this.details.context.is_super_state || this.details.context.is_sb_trigger;
            if (state === 'bonus' || this.details.last_action === 'bonus_spins_stop' || this.details.last_action === 'bonus_freespins_stop') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': isSuperBonus ? '#FF4DFF' : '#B300B3'
                })
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && this.get_pearls_count(details) === 15 || details.state == 'freespins' && details.context.winscatters.length > 0 || details.state == 'bonus' && details.context.last_respin && this.is_extra_respins(details)
        }
    }, {
        key: 'get_arr_sum',
        ...
        value: function is_extra_respins(details) {
            var bonus_symbols_v = void 0;
            if (details.context.bonus_symbols_v != undefined) {
                bonus_symbols_v = details.context.bonus_symbols_v
            } else {
                bonus_symbols_v = details.context.bs_v
            }
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            } else if (details.state == 'bonus' && details.context.is_sb_trigger === true) {
                labelLeft = 'SuperBonus start!';
                group.text('50%', '56%', 'Super Bonus Game start').attr(Drawer.STYLE_TEXT_CENTER_RED)
            }
        ...
        key: 'make_extra_sb_snap',
        value: function make_extra_sb_snap(details, combsGroup, margin, snap) {
            var group = combsGroup.group();
            var labelLeft = '';
            var labelRight = '';
            this._background(group, details, details.state);
        ...
        value: function get_text_for_action(details) {
            ...
            switch (action) {
                case 'bonus_init':
                    return details.context.is_super_state ? 'Super Bonus Game start' : 'Bonus Game start';
                case 'bonus_spins_stop':
                    return details.context.is_super_state ? 'Super Bonus Game end' : 'Bonus Game end';
        ...
        value: function make_spins(details) {
            ...
            if (this.get_pearls_count(this.details) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                ...
                combs.push(winbonus)
            } else if (this.details.context.is_sb_trigger) {
                var _winbonus = {
                    'trigger': 'sb_state',
                    'positions': [],
                    'amount': 0
            ...
            var margin = 0;
            this._combs(combsGroup, combs, details);
            if (combs.length > 1 || this.details.context.is_sb_trigger) {
                if (this.details.state == 'bonus' && this.details.context.is_sb_trigger) {
                    this.make_extra_sb_snap(details, combsGroup, margin, snap);
                    margin += this.width
                }
                for (var _i in combs) {
            ...
            if (details.last_action === 'respin' && details.context.is_super_state) {
                var rect = snap.children().filter(function(el) {
                    return el.type === 'rect'
                })[0];
                var fillAttrIndex = Object.values(rect.node.attributes).map(function(attr) {
                    return attr.nodeName
                }).indexOf('fill');
                rect.node.attributes[fillAttrIndex].value = '#FF4DFF'
            }
            var counter = 0;
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
        ...
        value: function board(snap, _board) {
            ...
            var bs_v = sb_first_snap ? this.details.context.old_bs_v : this.details.context.bs_v;
            var _loop = function _loop(_i2) {
                var _loop2 = function _loop2(_j) {
                    var value = bs_v[_i2][_j];
                    if (combs.length === 1 && combs[0].trigger !== 'sb_state') {
                        currentWinLine = 0
                    }
                    if (_board[_i2][_j] === 12) {
                        if (sb_first_snap || !_this3.details.context.is_super_state) {
                            value = 0
                        }
                    }
                    ...
                    imageUrl = _this3.url('sym' + _board[_i2][_j] + '.png');
                    if (_board[_i2][_j] === 11) {
                        var type = ['mini', 'minor', 'major', 'grand'].includes(value) ? '_green' : '';
                        imageUrl = _this3.url('sym11' + type + '.png')
                    }
                    if (_board[_i2][_j] === 12 && !sb_first_snap && ['mini', 'minor', 'major', 'grand'].includes(value)) {
                        imageUrl = _this3.url('sym11_green.png')
                    }
                    if (_this3.details.state === 'bonus' && bonus_symbols.indexOf(_board[_i2][_j]) == -1) {
                        imageUrl = _this3.url('sym0.png')
                    } else if (_board[_i2][_j] === 11 && !['mini', 'minor', 'major', 'grand'].includes(value) && _this3.details.context.is_super_state && !sb_first_snap) {
                        imageUrl = _this3.url('sym12.png')
                    }
                    var convertedMystery = false;
                    if (_this3.details.state === 'bonus' && sb_first_snap && [11].includes(_board[_i2][_j]) && _this3.details.context.mystery_pos && _this3.details.context.mystery_pos.some(function(arr) {
                            return arr[0] == _i2 && arr[1] == _j
                        })) {
                        imageUrl = _this3.url('sym13.png');
                        convertedMystery = true
                    }
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * _i2, (_this3.cellHeight + _this3.cellOffsetY) * _j, _this3.cellWidth, _this3.cellHeight);
                    if (value != 0 && (_board[_i2][_j] == 11 || _board[_i2][_j] == 12) && !convertedMystery) {
                        var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i2 - _this3.cellOffsetX;
                        var rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j + _this3.cellHeight / 3;
