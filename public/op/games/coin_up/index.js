...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function title(details) {
            if (details.last_action == 'restore') {
                return 'Restore'
            } else if (details.last_action == 'bonus_init' || details.last_action == 'bonus_spins_stop') {
        ...
        value: function _background(snap, state) {
            ...
            } else if (this.details.context.bs_count == 9) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#AF2D2D'
                })
            } else {
        ...
        key: 'is_extra_snap_reel',
        value: function is_extra_snap_reel(details) {
            return details.state == 'bonus' && details.last_action != 'restore' && details.context.reel_up && Array.isArray(details.context.reel_up) && this.get_number_of_features(details.context) > 0
        }
    }, {
        key: 'is_extra_snap_jackpot',
        value: function is_extra_snap_jackpot(details) {
            return details.state == 'bonus' && details.context.bs_count == 9 && details.last_action != 'restore'
        }
    }, {
        key: 'make_extra_snap_jackpot',
        value: function make_extra_snap_jackpot(details, combsGroup, margin, snap, ce) {
            var group = combsGroup.group();
            var labelLeft = '';
            var labelRight = '';
            this._background(group, details, details.state);
            margin += this.width * ce;
            ...
            labelLeft = 'Jackpot!';
            group.text('50%', '56%', 'Jackpot !').attr(Drawer.JACKPOT_STYLE_TEXT_CENTER);
            this._panel(group, labelLeft, labelRight);
            return snap
        }
    }, {
        key: 'make_extra_snap_reel',
        value: function make_extra_snap_reel(details, combsGroup, margin, snap, ce, feature, finalResult) {
            var group = combsGroup.group();
            ...
            group.transform('T' + margin + ',0');
            var reel_up = details.context.reel_up;
            var reelup = this.draw_reel_up(group, reel_up, feature, finalResult);
            reelup && reelup.transform('T' + this.boardMarginLeft + ',' + (this.boardMarginTop - 25));
            var reelupExtra = {
                name: feature,
                final: finalResult
            };
            var board = this.board(group, details.context.board, false, reelupExtra);
            board.transform('T' + this.boardMarginLeft + ',' + this.boardMarginTop);
            var total = this.get_number_of_features(details.context) + 1;
            var level = !feature ? 1 : feature == 'coins' || feature == 'multi' && total == 2 ? 2 : 3;
            var labelLeft = finalResult ? feature === 'coins' ? 'Coin Up Feature' : 'Multi Up Feature' : 'Bonus(' + details.context.rounds_left + '), Win:0.00';
            var labelRight = level + '/' + total;
            this._panel(group, labelLeft, labelRight);
            return snap
        }
    }, {
        key: 'get_number_of_features',
        value: function get_number_of_features(ctx) {
            return Number(ctx.reel_up.includes('multi')) + Number(Array.isArray(ctx.coins_up))
        }
    }, {
        key: 'get_text_for_action',
        value: function get_text_for_action(details) {
            ...
            switch (action) {
                case 'bonus_init':
                    return details.context.bg_type === 5 ? 'Ultra Bonus game start' : 'Bonus game start';
                case 'bonus_spins_stop':
                    return details.context.bg_type === 5 ? 'Ultra Bonus game end' : 'Bonus game end';
                default:
                    return '';
            }
        }
        ...
        key: '_combs',
        value: function _combs(snap, combs, details) {
            var hideFrames = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
            var group = snap.group();
            this.bordered_cells = [];
            var totalWin = details.round_win;
            var ctx = details.context;
            var numFeatures = ctx.reel_up ? this.get_number_of_features(ctx) : 0;
            var bonusFeatures = details.state === 'bonus' && details.last_action !== 'restore' ? numFeatures : 0;
            var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin);
            var lastRespin = details.state === 'bonus' && ctx.rounds_left === 0;
            var bonusInit = details.last_action === 'spin' && ctx.winlines && ctx.winlines[0].trigger === 'bonus';
            var labelRight = bonusFeatures ? 'all/' + (bonusFeatures + 1) : bonusInit || lastRespin || combs.length <= 1 ? '' : 'all/' + combs.length;
            this._panel(group, labelLeft, labelRight);
            if (!hideFrames) {
                for (var i = 0; i < combs.length; ++i) {
                    this.drawe_border_win_symbols(group, combs[i]).transform('T' + this.boardMarginLeft + ',' + this.boardMarginTop)
                }
            }
            return group
        }
        ...
        key: 'drawe_border_win_symbols',
        value: function drawe_border_win_symbols(snap, line) {
            var feature = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
            var group = snap.group().attr(BaseDrawer.STYLE_LINE_GROUP);
            this.bordered_cells = [];
            for (var i = 0; i < line.positions.length; ++i) {
                var cell = line.positions[i];
                if (!isItemInArray(this.bordered_cells, cell)) {
                    group.rect(cell[0] * (this.cellWidth + this.cellOffsetX), cell[1] * (this.cellHeight + this.cellOffsetY), this.cellWidth + this.cellOffsetX, this.cellHeight + this.cellOffsetY).attr(feature ? BaseDrawer.FEATURE_LINE_SYMBOL : BaseDrawer.STYLE_LINE_SYMBOL);
                    this.bordered_cells.push(cell)
                }
            }
            return group
        ...
        value: function _calculateSizes(cols, rows) {
            ...
            this.progressHeight = 3;
            this.boardHeight = this.height - (this.panelHeight + this.progressHeight + 1);
            this.cellWidth = this.isBonus ? this.boardHeight * 0.86 / 3 : this.boardHeight * 0.97 / 3;
            this.cellHeight = this.cellWidth;
            this.cellOffsetX = -1;
            this.cellOffsetY = -1;
            this.boardMarginTop = this.isBonus ? this.panelHeight * 2.4 : this.panelHeight * 1.4;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10) + 2
        }
    }, {
        key: 'make_spins',
        value: function make_spins(details) {
            ...
            this._background(snap, details.state);
            var extraMultipliersSlide = false;
            var group = snap.group().attr({
                width: this.width,
                height: this.height
            });
            var combs = (details.context.winlines ? details.context.winlines : []).concat(details.context.winscatters ? details.context.winscatters : []);
            var combsGroup = snap.group();
            var combGroup = void 0;
            var margin = 0;
            this._combs(combsGroup, combs, details, extraMultipliersSlide);
            if (combs.length > 1) {
                for (var i in combs) {
                    combGroup = this._comb(combsGroup, combs, combs[i], details);
                    margin += this.width;
            ...
            var reelup = void 0;
            var ce = 1;
            var showMainMultipliers = !this.isBonus && this.isValidMultiplier(details);
            reelup = this.draw_reel_up(group, details.context.reel_up);
            reelup && reelup.transform('T' + this.boardMarginLeft + ',' + (this.boardMarginTop - 25));
            board = this.board(group, details.context.board, showMainMultipliers, null);
            board.transform('T' + this.boardMarginLeft + ',' + this.boardMarginTop);
            if (this.is_extra_snap_reel(details)) {
                var finalResult = false;
                this.make_extra_snap_reel(details, combsGroup, margin, snap, ce, null, finalResult);
                ce += 1;
                finalResult = true;
                if (details.context.reel_up.includes('coins') && details.context.coins_up) {
                    this.make_extra_snap_reel(details, combsGroup, margin, snap, ce, 'coins', finalResult);
                    ce += 1
                }
                if (details.context.reel_up.includes('multi')) {
                    this.make_extra_snap_reel(details, combsGroup, margin, snap, ce, 'multi', finalResult);
                    ce += 1
                }
            }
            if (this.is_extra_snap_jackpot(details)) {
                this.make_extra_snap_jackpot(details, combsGroup, margin, snap, ce)
            }
            if (['bonus_init', 'bonus_stop', 'bonus_spins_stop'].includes(details.last_action)) {
                board && board.remove();
            ...
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
                counter = (counter + 1) % combsGroup.children().length;
                if (extraMultipliersSlide && counter === combsGroup.children().length - 1) {
                    counter = 0
                }
                ...
                if (board) {
                    board.remove();
                    board = _this2.board(group, cur_board, counter - 1, null);
                    board.transform('T' + _this2.boardMarginLeft + ',' + _this2.boardMarginTop)
                }
            });
            snap.text('50%', '56%', this.get_text_for_action(details)).attr(details.context.bg_type === 5 ? Drawer.STYLE_TEXT_SB_WHITE : Drawer.STYLE_TEXT_CENTER_WHITE);
            return snap.node
        }
    }, {
        key: 'isValidMultiplier',
        value: function isValidMultiplier(details) {
            if (details.context.mps && details.context.mps.flat().some(function(m) {
                    return m > 1
                })) {
                return true
            } else {
        ...
        key: 'board',
        value: function board(snap, _board, showMainMultipliers, reelupExtra) {
            var group = snap.group();
            this.draw_board(group, _board, false, false, showMainMultipliers, reelupExtra);
            return group
        }
    }, {
        key: 'draw_reel_up',
        value: function draw_reel_up(snap, reel_up) {
            var feature = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'multi';
            var finalResult = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
            var group = snap.group();
            if (!this.isBonus) return;
            if (!reel_up || !Array.isArray(reel_up) || reel_up.length !== 3) return;
            for (var i = 0; i < reel_up.length; i++) {
                var symbolLink = undefined;
                if (reel_up[i] === 'multi') symbolLink = 'sym10';
                if (reel_up[i] === 'coins') symbolLink = 'sym9';
                if (!symbolLink) continue;
                var imageUrl = this.url(symbolLink + '.png');
                var img = group.image(imageUrl, (this.cellWidth + this.cellOffsetX) * i, this.height > 150 ? -14 : 0, this.cellWidth, this.cellHeight);
                if (feature) img.attr({
                    opacity: !finalResult || reel_up[i] === feature ? 1 : 0.5
                })
            }
            return group
        }
        ...
        key: 'draw_board',
        value: function draw_board(group, board) {
            var extraSlide = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
            var addOffset = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
            var _this3 = this;
            var showMainMultipliers = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
            var reelupExtra = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : null;
            var round_bet = this.details.round_bet_not_restored ? this.details.round_bet_not_restored : this.details.round_bet;
            var lastExtraSlide = !extraSlide && addOffset;
            var lastRestoredRespin = this.details.last_action == 'restore' && this.details.context.rounds_left === 0;
            if (this.details.context.bs) {
                ...
                try {
                    for (var _iterator = this.details.context.bs[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        ...
                        id = id === 1 && _bs.value == this.mini_jp * round_bet && !(lastExtraSlide && this.details.context.mps[_bs.reel][_bs.position] > 1) ? 2 : id === 1 && _bs.value == this.minor_jp * round_bet && !(lastExtraSlide && this.details.context.mps[_bs.reel][_bs.position] > 1) ? 3 : id === 1 && _bs.value == this.major_jp * round_bet && !(lastExtraSlide && this.details.context.mps[_bs.reel][_bs.position] > 1) ? 4 : id;
                        board[_bs.reel][_bs.position] = id
                    }
                } catch (err) {
                ...
                if (extraSlide) {
                    if (this.details.context.mysteries) {
                        var _iteratorNormalCompletion2 = true;
                        var _didIteratorError2 = false;
                        var _iteratorError2 = undefined;
                        try {
                            for (var _iterator2 = this.details.context.mysteries[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                                var bs = _step2.value;
                                board[bs.reel][bs.position] = bs.value < this.mini_jp * round_bet || bs.type == 'collector' ? 6 : 7
                            }
                        } catch (err) {
            ...
            if (reelupExtra) {
                var bpl = this.details.context.bet_per_line;
                if (reelupExtra.name === null && !reelupExtra.final && this.details.context.coins_up) {
                    this.details.context.coins_up.forEach(function(bs) {
                        board[bs.reel][bs.position] = bs.from / bpl <= 4 ? 1 : bs.from / bpl == 5 ? 5 : bs.from == _this3.mini_jp ? 2 : bs.from == _this3.minor_jp ? 3 : bs.from == _this3.major_jp ? 4 : 8
                    })
                }
            }
            var imageUrl = void 0;
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    imageUrl = _this3.url('sym' + board[i][j] + '.png');
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * i + (addOffset ? _this3.boardMarginLeft : 0), (_this3.cellHeight + _this3.cellOffsetY) * j + (addOffset ? _this3.panelHeight : 0), _this3.cellWidth, _this3.cellHeight);
                    var comb = {
                        positions: []
                    };
                    var showCoinsUp = reelupExtra && reelupExtra.name === 'coins' && reelupExtra.final && _this3.details.context.coins_up;
                    var showMultiUp = reelupExtra && reelupExtra.name === 'multi' && reelupExtra.final && _this3.details.context.reel_up;
                    if (showCoinsUp) {
                        var isCoinUp = _this3.details.context.coins_up.find(function(bs) {
                            return bs.reel == i && bs.position == j
                        });
                        img.attr({
                            opacity: isCoinUp ? 1 : 0.5
                        });
                        if (isCoinUp) comb.positions.push([i, j])
                    }
                    if (showMultiUp) {
                        var isReelUp = _this3.details.context.reel_up[i] == 'multi' && _this3.details.context.mps[i][j] > 1;
                        img.attr({
                            opacity: isReelUp ? 1 : 0.5
                        });
                        if (isReelUp) comb.positions.push([i, j])
                    }
                    if (showCoinsUp || showMultiUp) {
                        _this3.drawe_border_win_symbols(group, comb, true)
                    }
                };
            ...
            if (!this.details.context.bs) return group;
            this.draw_symbols_values(group, board, lastExtraSlide, addOffset, reelupExtra);
            var showMultipliers = !!(this.details.state === 'bonus' && this.details.context.mps) || showMainMultipliers;
            if (showMultipliers) {
                this.draw_bg_multipliers(group, !lastExtraSlide, addOffset, reelupExtra)
            }
        }
    }, {
        key: 'draw_symbols_values',
        value: function draw_symbols_values(group, board, lastExtraSlide) {
            var _this4 = this;
            var addOffset = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
            var reelupExtra = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : null;
            var round_bet = this.details.last_action == 'restore' && this.details.state == 'spins' && this.details.context.hasOwnProperty('bg_type') && this.details.context.last_bet ? this.details.context.last_bet : this.details.round_bet_not_restored ? this.details.round_bet_not_restored : this.details.round_bet;
            var lastRestoredRespin = this.details.last_action == 'restore' && this.details.context.rounds_left === 0;
            var firstRestoredPreInit = false;
            if (this.details.last_action == 'restore' || this.details.last_action === 'buy_spin') {
                var bs_new = [];
                if (this.details.context && this.details.context.bs_new && this.details.context.bs_new.length > 0) bs_new = bs_new.concat(this.details.context.bs_new);
                if (this.details.context && this.details.context.bs_ultra && this.details.context.bs_ultra.length > 0) bs_new = bs_new.concat(this.details.context.bs_ultra);
                var count = 0;
                bs_new.forEach(function(data) {
                    if (data.position === 1) count += 1
                });
                firstRestoredPreInit = count === 3
            }
            var lastRespin = this.details.last_action == 'respin' && this.details.context.rounds_left === 0;
            var filledMiddleRow = this.details.context.bs.filter(function(bs) {
                return bs.position == 1
            }).length == 3;
            ...
            try {
                var _loop3 = function _loop3() {
                    ...
                    var j = bs.position;
                    var id = board[i][j];
                    if (id < 8 || _this4.details.last_action !== 'spin' && !(_this4.details.last_action == 'restore' && _this4.details.state == 'spins' && !filledMiddleRow)) {
                        var value = bs.value == _this4.mini_jp * round_bet && id < 8 && !lastExtraSlide ? 'mini' : bs.value == _this4.minor_jp * round_bet && id < 8 && !lastExtraSlide ? 'minor' : bs.value == _this4.major_jp * round_bet && id < 8 && !lastExtraSlide ? 'major' : +bs.value;
                        if (typeof value === 'number') {
                            value = (lastExtraSlide || lastRespin && !reelupExtra || lastRestoredRespin) && _this4.isValidMultiplier(_this4.details) ? (value * _this4.details.context.mps[i][j] / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                        }
                        if (reelupExtra && reelupExtra.name == null && !reelupExtra.final && _this4.details.context.coins_up) {
                            var coinUpBs = _this4.details.context.coins_up.filter(function(bs) {
                                return bs.reel == i && bs.position == j
                            })[0];
                            value = coinUpBs ? (coinUpBs.from / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : value
                        }
                        var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * i - _this4.cellOffsetX + (addOffset ? _this4.boardMarginLeft : 0);
                        var rect_y = (_this4.cellHeight + _this4.cellOffsetY) * j + _this4.cellHeight / 3 + (addOffset ? _this4.panelHeight : 0);
                        var rect_width = _this4.cellWidth + 2 * _this4.cellOffsetX;
                        var rect_height = _this4.cellHeight / 3;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = (_this4.cellHeight + _this4.cellOffsetY) * j + _this4.cellHeight / 2 + (addOffset ? _this4.panelHeight : 0);
                        var showValues = ![2, 3, 4, 6, 7].includes(id) && value !== '0.00';
                        if (id === 8 && firstRestoredPreInit) showValues = false;
                        if (showValues) {
                            var rect = group.rect(rect_x, rect_y, rect_width, rect_height).attr({
                                'fill': 'white'
                            });
                            var text = void 0;
                            if (['mini', 'minor', 'major'].includes(value)) {
                                text = group.text(text_x, text_y, value).attr(_this4.width > 200 ? Drawer['PEARL_' + value.toUpperCase() + '_65'] : Drawer['PEARL_' + value.toUpperCase() + '_45'])
                            } else {
                                if (value < 100000) {
                                    text = group.text(text_x, text_y, value).attr(_this4.width > 200 ? Drawer.PEARL_VALUE_52 : Drawer.PEARL_VALUE_35)
                                } else {
                                    text = group.text(text_x, text_y, value).attr(_this4.width > 200 ? Drawer.PEARL_VALUE_37 : Drawer.PEARL_VALUE_23)
                                }
                            }
                            var ctx = _this4.details.context;
                            if (reelupExtra && reelupExtra.final) {
                                var isCoinUp = reelupExtra.name === 'coins' && ctx.coins_up && ctx.coins_up.find(function(bs) {
                                    return bs.reel == i && bs.position == j
                                });
                                var isReelUp = reelupExtra.name === 'multi' && ctx.reel_up[i] == 'multi';
                                rect.attr({
                                    opacity: isCoinUp || isReelUp ? 1 : 0.5
                                });
                                text.attr({
                                    opacity: isCoinUp || isReelUp ? 1 : 0.5
                                })
                ...
                for (var _iterator3 = this.details.context.bs[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                    _loop3()
                }
            } catch (err) {
        ...
        key: 'draw_bg_multipliers',
        value: function draw_bg_multipliers(group, showMultipliers) {
            var _this5 = this;
            var addOffset = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
            var reelupExtra = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;
            if (!showMultipliers) return;
            var lastRestoredRespin = this.details.last_action == 'restore' && this.details.context.rounds_left === 0;
            var lastRespin = this.details.last_action == 'respin' && this.details.context.rounds_left === 0;
            var mps = reelupExtra ? this.details.context.mps.map(function(arr) {
                return [].concat(_toConsumableArray(arr))
            }) : this.details.context.mps;
            var ctx = this.details.context;
            var _loop4 = function _loop4(_i) {
                var _loop5 = function _loop5(_j) {
                    if ((lastRestoredRespin || lastRespin) && !reelupExtra && ![2, 3, 4].includes(ctx.board[_i][_j])) {
                        return 'continue'
                    }
                    if (reelupExtra && (!reelupExtra.final || reelupExtra.name === 'coins') && ctx.reel_up[_i] == 'multi' && mps[_i][_j] > 1) {
                        mps[_i][_j] -= 1
                    }
                    if (mps[_i][_j] > 1) {
                        var value_rect_width = _this5.cellWidth + 2 * _this5.cellOffsetX;
                        var mltp_rect_x = -0.5 + (_this5.cellWidth + _this5.cellOffsetX) * _i - _this5.cellOffsetX + (addOffset ? _this5.boardMarginLeft : 0) + value_rect_width - value_rect_width / 2.5;
                        var mltp_rect_y = 1.5 + (_this5.cellHeight + _this5.cellOffsetY) * _j + (addOffset ? _this5.panelHeight : 0);
                        var mltp_rect_width = value_rect_width / 2.5;
                        var mltp_rect_height = -1 + _this5.cellHeight / 3;
                        var mltp_text_x = mltp_rect_x + mltp_rect_width / 2;
                        var mltp_text_y = -0.5 + (_this5.cellHeight + _this5.cellOffsetY) * _j + (addOffset ? _this5.panelHeight : 0) + _this5.cellHeight / 2 - mltp_rect_height;
                        var rect = group.rect(mltp_rect_x, mltp_rect_y, mltp_rect_width, mltp_rect_height).attr({
                            'fill': 'white'
                        });
                        var text = group.text(mltp_text_x, mltp_text_y, 'x' + mps[_i][_j]).attr(Drawer.MULTIPLIER_VALUE_47);
                        if (reelupExtra && reelupExtra.name === 'coins' && reelupExtra.final && ctx.coins_up) {
                            var isCoinUp = ctx.coins_up.find(function(bs) {
                                return bs.reel == _i && bs.position == _j
                            });
                            rect.attr({
                                opacity: isCoinUp ? 1 : 0.5
                            });
                        ...
                        } else if (reelupExtra && reelupExtra.name === 'multi' && reelupExtra.final && ctx.reel_up) {
                            var isReelUp = ctx.reel_up[_i] == 'multi';
                            rect.attr({
                                opacity: isReelUp ? 1 : 0.5
                            });
                            text.attr({
        ...
        key: 'isBonus',
        get: function get() {
            return this.details.state === 'bonus' || this.details.last_action == 'bonus_stop'
        }
    }]);
    return Drawer
}(BaseDrawer);
Drawer.MULTIPLIER_VALUE_47 = {
    'font-size': '47%',
...
Drawer.STYLE_TEXT_CENTER_WHITE = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#0000',
    'font-size': '1.2em'
};
Drawer.STYLE_TEXT_SB_WHITE = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#0000',
    'font-size': '1.05em'
...
BaseDrawer.FEATURE_LINE_SYMBOL = {
    'stroke': 'green',
    'stroke-width': 2,
    'fill-opacity': 0
};
...
Drawer.JACKPOT_STYLE_TEXT_CENTER = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#0000',
    'font-size': '1.2em'
