...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function _background(snap, state) {
            ...
            } else if (this.details.context.moons_count == 15) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#7D3478'
                })
            } else {
        ...
        value: function is_extra_snap(details) {
            return details.context.blast_win || details.state == 'bonus' && details.context.moons_count == 15 || details.state == 'freespins' && details.context.winscatters.length
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.context.blast_win) {
                labelLeft = 'Boost win!';
                group.text('50%', '56%', 'Boost win').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
            } else if (details.state == 'bonus' && details.context.moons_count == 15) {
                labelLeft = 'JackPot!';
            ...
            if (details.context.blast_win) {
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if (game_board[i][j] == this.blast_id) {
        ...
        value: function board(snap, _board) {
            ...
            var group = snap.group();
            var imageUrl = void 0;
            if (this.details.state == 'freespins' && _board[2][1] < this.moon_id) {
                var _loop = function _loop(i) {
                    var _loop2 = function _loop2(j) {
                        if (i == 0 || i == 4) {
                            imageUrl = _this3.url(_board[i][j] + '.png');
                            var _img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * i, (_this3.cellHeight + _this3.cellOffsetY) * j, _this3.cellWidth, _this3.cellHeight);
                            if (combs.length == 1) {
                                currentWinLine = 0
                            }
                            if (currentWinLine !== -1) {
                ...
                imageUrl = this.url('giant_' + _board[2][2] + '.png');
                var img = group.image(imageUrl, this.cellWidth + this.cellOffsetX, 0, 3 * this.cellWidth + 2 * this.cellOffsetX, 3 * this.cellHeight + 2 * this.cellOffsetY);
                img.attr({
                    opacity: 1
                })
            } else {
            ...
            var round_bet = this.details.round_bet;
            var _iteratorNormalCompletion = true;
            var _didIteratorError = false;
            var _iteratorError = undefined;
            try {
                for (var _iterator = this.details.context.moons[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        key: 'drawe_border_win_symbols',
        value: function drawe_border_win_symbols(snap, line) {
            if (this.details.state != 'freespins' || this.details.context.winscatters.length == 0 || this.details.context.winscatters.length && this.details.context.winscatters[0].symbol == this.moon_id) {
                var group = snap.group().attr(BaseDrawer.STYLE_LINE_GROUP);
                for (var i = 0; i < line.positions.length; ++i) {
                    var cell = line.positions[i];
                    if (!isItemInArray(this.bordered_cells, cell)) {
                ...
                for (var _i2 = 0; _i2 < line.positions.length; ++_i2) {
                    var _cell = line.positions[_i2];
                    var x_pos = _cell[0];
                    var y_pos = _cell[1];
                    if (this.details.state === 'freespins' && x_pos != 0 && x_pos != 4) {
                        if (!isItemInArray(this.bordered_cells, _cell)) {
                            _group.rect(1 * (this.cellWidth + this.cellOffsetX), 0 * (this.cellHeight + this.cellOffsetY), (this.cellWidth + this.cellOffsetX) * 3, (this.cellHeight + this.cellOffsetY) * 3).attr(BaseDrawer.STYLE_LINE_SYMBOL);
