...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function _contextCutMigration(details) {
            ...
            if (!details.add_respins_pos) {
                details.add_respins_pos = []
            }
            if (!details.probs) {
                details.probs = []
            }
        ...
        value: function is_extra_snap(details) {
            var extra_respins_count = this.get_arr_count(details.context.board, 14);
            return details.state == 'bonus' && this.get_pearls_count(details) === 15 || details.state == 'freespins' && details.context.winscatters.length > 0 || details.state == 'bonus' && extra_respins_count > 0
        }
    }, {
        key: 'get_arr_count',
        value: function get_arr_count(arr, symbol) {
            var count = 0;
            for (var i in arr) {
                for (var j in arr[i]) {
                    if (arr[i][j] == symbol) {
        ...
        value: function get_pearls_count(details) {
            ...
            for (var i in board) {
                for (var j in board[i]) {
                    if (board[i][j] == 11 || board[i][j] == 12 || board[i][j] == 13) {
                        count = count + 1
                    }
                }
            }
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            var extra_respins_count = this.get_arr_count(details.context.board, 14);
            if (details.state == 'bonus' && extra_respins_count != 0) {
                group.text('50%', '60%', '+' + extra_respins_count + ' Respins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
            }
            this._panel(group, labelLeft, labelRight);
            return snap
        }
