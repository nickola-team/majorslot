    ...
    _createClass(Drawer, [{
        ...
        value: function _combs(snap, combs, details) {
            ...
            if (details.last_action === 'respin' && details.context.rounds_left !== 0) {
                ...
                if (this.details.state == 'freespins') {
                    var mult_list = [1, 2, 3, 10];
        ...
        value: function get_pearls_count(details) {
            ...
            for (var i in board) {
                for (var j in board[i]) {
                    if (this.get_collect_symbols().includes(board[i][j])) {
                        count = count + 1
                    }
                }
            }
        ...
        key: 'get_collect_symbols',
        value: function get_collect_symbols() {
            return [12, 13]
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'freespins' && details.context.add_fs) {
                labelLeft = 'Additional FreeSpins!';
                var roundsGranted = 10;
                var mult_list = [1, 2, 3, 10];
                var game_level = this.details.context.game_level;
                var mult_value = mult_list[game_level];
                group.text('50%', '55%', '+' + roundsGranted + ' FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
        ...
        value: function make_spins(details) {
            ...
            var combs = [].concat(details.context.winlines, details.context.winscatters);
            if (this.get_pearls_count(this.details) > 0 && this.details.context.collects_on_board > 0) {
                var winmoney = {
                    'trigger': 'collect',
                    'positions': []
                };
                var collect_symbols = this.get_collect_symbols().concat([10]);
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if (collect_symbols.includes(game_board[i][j])) {
                            winmoney.positions.push([i, j])
                        }
                    }
                }
                combs.push(winmoney)
            }
            if (this.details.context.collects_on_board > 0 && this.details.state == 'freespins') {
                var collectors = {
                    'trigger': 'collectors',
                    'positions': []
                };
                var collect_symbol = 10;
                for (var _i2 in game_board) {
                    for (var _j in game_board[_i2]) {
                        if (collect_symbol == game_board[_i2][_j]) {
                            collectors.positions.push([_i2, _j])
                        }
                    }
                }
        ...
        value: function board(snap, _board) {
            ...
            if (this.get_pearls_count(this.details) > 0 && this.details.context.collects_on_board > 0) {
                ...
                for (var i in _board) {
                    for (var j in _board[i]) {
                        if (collect_symbols.includes(_board[i][j])) {
                            winmoney.positions.push([i, j])
                        }
                    }
                }
            ...
            if (this.details.context.collects_on_board > 0 && this.details.state == 'freespins') {
                ...
                for (var _i4 in _board) {
                    for (var _j2 in _board[_i4]) {
                        if (collect_symbol == _board[_i4][_j2]) {
                            collectors.positions.push([_i4, _j2])
                        }
                    }
                }
                combs.push(collectors)
            ...
            var _loop = function _loop(_i5) {
                var _loop2 = function _loop2(_j3) {
                    ...
                    if (_this3.details.context.bs_v != undefined) {
                        value = _this3.details.context.bs_v[_i5][_j3]
                    } else {
                        value = 0
                    }
                    ...
                    if ([12, 13].includes(_board[_i5][_j3]) && _this3.details.state == 'freespins') {
                        var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i5 - _this3.cellOffsetX + _this3.cellWidth * 0.425;
                        var rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j3 + _this3.cellHeight * 0.04;
                        var rect_width = _this3.cellWidth * 0.5;
                        var text_x = (_this3.cellWidth + _this3.cellOffsetX) * _i5 - _this3.cellOffsetX + _this3.cellWidth * 0.655;
                        var text_y = (_this3.cellHeight + _this3.cellOffsetY) * _j3 + _this3.cellHeight * 0.23;
                        var mult_list = [1, 2, 3, 10];
                    ...
                    if (_board[_i5][_j3] === 10 && _this3.get_pearls_count(_this3.details) > 0) {
                        var _rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i5 - _this3.cellOffsetX;
                        var _rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j3 + _this3.cellHeight * 0.64;
                        var _rect_width = _this3.cellWidth + 2 * _this3.cellOffsetX;
                        var _text_x = _rect_x + _rect_width / 2;
                        var collect_y_delta = _this3.cellHeight * 0.8;
                        var _text_y = (_this3.cellHeight + _this3.cellOffsetY) * _j3 + collect_y_delta;
                        var _collect_value = _this3.details.context.collect_pay;
                        group.rect(_rect_x, _rect_y, _rect_width, _this3.cellHeight / 3).attr({
                            'fill': 'white'
                        });
                        _collect_value = (_collect_value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                        if (_collect_value < 100000) {
                            if (_this3.width > 200) {
                                group.text(_text_x, _text_y, _collect_value).attr(Drawer.PEARL_VALUE_60)
                            } else {
                                group.text(_text_x, _text_y, _collect_value).attr(Drawer.PEARL_VALUE_38)
                            }
                        } else {
                            if (_this3.width > 200) {
                                group.text(_text_x, _text_y, _collect_value).attr(Drawer.PEARL_VALUE_35)
                            } else {
                                group.text(_text_x, _text_y, _collect_value).attr(Drawer.PEARL_VALUE_22)
                            }
                        }
                    }
                    if (value != 0) {
                        var _rect_x2 = (_this3.cellWidth + _this3.cellOffsetX) * _i5 - _this3.cellOffsetX;
                        var _rect_y2 = (_this3.cellHeight + _this3.cellOffsetY) * _j3 + _this3.cellHeight * 0.65;
                        var _rect_width2 = _this3.cellWidth + 2 * _this3.cellOffsetX;
                        var _text_x2 = _rect_x2 + _rect_width2 * 0.49;
                        var text_y_money = (_this3.cellHeight + _this3.cellOffsetY) * _j3;
                        var money_y_delta = _this3.cellHeight * 0.82;
                        var _text_y2 = text_y_money + money_y_delta;
                        if (_board[_i5][_j3] === 13 && _this3.details.context.collects_on_board > 0) {
                            group.rect(_rect_x2, _rect_y2, _rect_width2, _this3.cellHeight / 3).attr({
                                'fill': 'white'
                            });
                            var jackpot_value = _this3.details.context.bs_values[_i5][_j3] * _this3.details.round_bet;
                            jackpot_value = (jackpot_value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                            if (value == 'mini') {
                                if (jackpot_value < 100000) {
                                    if (_this3.width > 200) {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MINI_60)
                                    } else {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MINI_38)
                                    }
                                } else {
                                    if (_this3.width > 200) {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MINI_35)
                                    } else {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MINI_22)
                                    }
                                }
                            } else if (value == 'minor') {
                                if (jackpot_value < 100000) {
                                    if (_this3.width > 200) {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MINOR_60)
                                    } else {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MINOR_38)
                                    }
                                } else {
                                    if (_this3.width > 200) {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MINOR_35)
                                    } else {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MINOR_22)
                                    }
                                }
                            } else if (value == 'major') {
                                if (jackpot_value < 100000) {
                                    if (_this3.width > 200) {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MAJOR_60)
                                    } else {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MAJOR_38)
                                    }
                                } else {
                                    if (_this3.width > 200) {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MAJOR_35)
                                    } else {
                                        group.text(_text_x2, _text_y2, jackpot_value).attr(Drawer.PEARL_MAJOR_22)
                                    }
                                }
                            } else if (value == 'grand') {
                                if (jackpot_value < 100000) {
                                    if (_this3.width > 200) {
                                        group.text(_text_x2, _text_y2, collect_value).attr(Drawer.PEARL_GRAND_60)
                                    } else {
                                        group.text(_text_x2, _text_y2, collect_value).attr(Drawer.PEARL_GRAND_38)
                                    }
                                } else {
                                    if (_this3.width > 200) {
                                        group.text(_text_x2, _text_y2, collect_value).attr(Drawer.PEARL_GRAND_35)
                                    } else {
                                        group.text(_text_x2, _text_y2, collect_value).attr(Drawer.PEARL_GRAND_22)
                                    }
                                }
                            }
                        } else if (_board[_i5][_j3] === 12) {
                            var _rect_y3 = (_this3.cellHeight + _this3.cellOffsetY) * _j3 + _this3.cellHeight * 0.65;
                            group.rect(_rect_x2, _rect_y3, _rect_width2, _this3.cellHeight / 3).attr({
                                'fill': 'white'
                            });
                            if (value < 100000) {
                                if (_this3.width > 200) {
                                    group.text(_text_x2, _text_y2, value).attr(Drawer.PEARL_VALUE_60)
                                } else {
                                    group.text(_text_x2, _text_y2, value).attr(Drawer.PEARL_VALUE_38)
                                }
                            } else {
                                if (_this3.width > 200) {
                                    group.text(_text_x2, _text_y2, value).attr(Drawer.PEARL_VALUE_35)
                                } else {
                                    group.text(_text_x2, _text_y2, value).attr(Drawer.PEARL_VALUE_22)
                                }
                            }
                        }
                    } else {}
