...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function is_extra_snap(details) {
            return details.state === 'freespins' && details.context.winscatters && details.context.winscatters.length > 0
        }
    }, {
        key: 'get_mystery_position',
        value: function get_mystery_position(details) {
            if (!details.context.mystery || details.context.mystery.length === 0) {
                return []
            }
            var board = details.context.board;
            var mystery = details.context.mystery ? details.context.mystery.length : 0;
            var dict = {
                'occurrences': mystery.length,
                'positions': [],
                'symbol': 8,
                'trigger': 'mystery'
            };
            for (var i in board) {
                for (var j in board[i]) {
                    if (board[i][j] === 8) {
                        dict['positions'].push([parseInt(i), parseInt(j)])
                    }
                }
            }
            return [dict]
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (this.details.context.mystery && this.details.context.mystery.length > 0) {
                labelLeft = 'Mystery'
        ...
        value: function make_spins(details) {
            ...
            var combs = [].concat(this.get_mystery_position(details), details.context.winlines, details.context.winscatters);
            var combsGroup = snap.group();
            var combGroup = void 0;
            var margin = 0;
            this._combs(combsGroup, combs, details);
            ...
            var counter = 0;
            snap.click(function(e) {
                if (_this2.details.last_action == 'freespin_stop' || _this2.details.last_action == 'freespin_init') return;
                var mystery = _this2.get_mystery_position(_this2.details) ? _this2.get_mystery_position(_this2.details) : [];
                if (combsGroup.children().length < 2 && mystery.length == 0) return;
                var comb_len = combsGroup.children().length;
                if (combsGroup.children().length < 2) {
                    comb_len = 3
                }
                counter = (counter + 1) % comb_len;
                combsGroup.transform('T-' + _this2.width * counter);
                progress.animate({
                    width: partSize * (counter + 1)
                }, 100);
                var cur_board = Array.from(game_board);
                if (details.context.mystery && details.context.mystery.length > 0 && counter !== 0 && counter !== 1) {
                    cur_board = details.context.mystery_board
                }
                board.remove();
                if (details.context.mystery && details.context.mystery.length > 0 && counter === 1) {
                    board = _this2.mystery_board(group, cur_board)
                } else {
                    board = _this2.board(group, cur_board, counter - 1)
                }
                board.transform('T' + _this2.boardMarginLeft + ',' + (_this2.panelHeight + 1));
                if (combsGroup.children().length < 2 && _this2.get_mystery_position(_this2.details).length > 0) {
                    var labelLeft = '';
                    var labelRight = '';
                    if (counter == 1) {
                        var _labelLeft = _this2.title(details) + ', Win:' + _this2.coins(combs[0].amount);
                        var _group = snap.group();
                        _this2._panel(_group, _labelLeft, labelRight);
                        _this2.drawe_border_win_symbols(_group, _this2.get_mystery_position(_this2.details)[0]).transform('T' + _this2.boardMarginLeft + ',' + (_this2.panelHeight + 1))
                    }
                    if (counter == 2) {
                        var _group2 = snap.group();
                        var _labelLeft2 = _this2.title(details) + ', Win:' + _this2.coins(combs[0].amount);
                        _this2._panel(_group2, _labelLeft2, labelRight)
                    }
                }
            });
        ...
        value: function board(snap, _board) {
            ...
            var combs = [].concat(this.get_mystery_position(this.details), winlines, winscatters);
            if (!combs) combs = [];
            var group = snap.group();
            var imageUrl = void 0;
            if (combs && combs.length == 1) {
            ...
            if (this.details.context.mystery && this.details.context.mystery.length > 0 && winlines.length == 0 && winscatters.length == 0 && combs.length == currentWinLine + 1) {
                currentWinLine = -1
            }
        ...
        key: 'mystery_board',
        value: function mystery_board(snap, board) {
            var _this4 = this;
            var mystery = this.details.context.mystery ? this.details.context.mystery : [];
            var combs = [].concat(this.get_mystery_position(this.details));
            var group = snap.group();
            var imageUrl = void 0;
            var _loop3 = function _loop3(i) {
                var _loop4 = function _loop4(j) {
                    imageUrl = _this4.url(board[i][j] + '.png');
                    if (_this4.details.context.board[i][j] === 8) {
                        group.rect(i * (_this4.cellWidth + _this4.cellOffsetX), j * (_this4.cellHeight + _this4.cellOffsetY), _this4.cellWidth + _this4.cellOffsetX, _this4.cellHeight + _this4.cellOffsetY).attr(BaseDrawer.STYLE_LINE_SYMBOL)
                    }
                    if (board[i][j] === 8) {
                        for (var index in mystery) {
                            if (mystery[index][i] !== undefined) {
                                imageUrl = _this4.url(mystery[index][i] + '.png')
                            }
                        }
                    }
                    var img = group.image(imageUrl, (_this4.cellWidth + _this4.cellOffsetX) * i, (_this4.cellHeight + _this4.cellOffsetY) * j, _this4.cellWidth, _this4.cellHeight);
                    if (!combs[0]) return 'continue';
                    var positions = combs[0].positions;
                    img.attr({
                        opacity: 0.5
                    });
                    positions.forEach(function(pos) {
        ...
        value: function drawe_line_patern(snap, line) {
            ...
            if (typeof line['trigger'] === 'undefined') {
                ...
                var path = 'M' + this.cellWidth / 2.4 + ',';
                path += temp[0] * this.cellHeight + this.cellHeight / 2.1 + ' ';
                for (var _i2 = 0; _i2 < temp.length; ++_i2) {
                    path += 'L' + (_i2 * this.cellWidth + this.cellWidth / 2.4) + ',';
                    path += temp[_i2] * this.cellHeight + this.cellHeight / 2.1 + ' '
                }
                group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
                group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER)
            }
