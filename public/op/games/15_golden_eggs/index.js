...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function make_freespins(details) {
            ...
            if (details.context.egg_pos && details.context.egg_pos.length === 15) {
                var _group = combsGroup.group();
                var labelLeft = this.title(details);
                var labelRight = '';
                this._background(_group, details.state);
                this._panel(_group, labelLeft, labelRight);
                _group.text('50%', '56%', 'JackPot!').attr(BaseDrawer.STYLE_TEXT_CENTER_RED);
                margin += this.width;
            ...
            if (!noBoard) {
                var _board2 = this.board(group, details.context.board);
                _board2.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            var counter = 0;
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
        ...
        key: 'board',
        value: function(_board) {
            function board(_x, _x2) {
        ...
        value: function drawe_line_patern(snap, line) {
            var group = snap.group().attr(BaseDrawer.STYLE_LINE_GROUP);
            var temp = line.positions.slice().sort();
            if ('line' in line) {
                var path = 'M' + (temp[0][0] * this.cellWidth + this.cellWidth / 2).toString() + ',';
                path += temp[0][1] * this.cellHeight + this.cellHeight / 2 + ' ';
                for (var i = 0; i < temp.length; ++i) {
                    path += 'L' + (temp[i][0] * this.cellWidth + this.cellWidth / 2) + ',';
                    path += temp[i][1] * this.cellHeight + this.cellHeight / 2 + ' '
                }
                group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
                group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER)
            }
