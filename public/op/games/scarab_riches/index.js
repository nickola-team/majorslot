...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function make_spins(details) {
            ...
            var partSize = this.width / combsGroup.children().length;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(BaseDrawer.STYLE_PROGRESS);
            var board = this.board(group, details.context.feature_board);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            var counter = 0;
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
                ...
                var cur_board = Array.from(_this2.details.context.feature_board);
                board.remove();
        ...
        value: function make_freespins(details) {
            ...
            if (details.context.winscatters && details.context.winscatters.length) {
                ...
                var labelRight = '';
                this._background(_group, details);
                this._panel(_group, labelLeft, labelRight);
                _group.text('50%', '56%', '+' + roundsGranted + ' freespins').attr(BaseDrawer.STYLE_TEXT_CENTER_RED);
                margin += this.width;
            ...
            snap.click(function(e) {
                ...
                var cur_board = Array.from(_this3.details.context.feature_board);
                board.remove();
        ...
        value: function title(details) {
            ...
            switch (details.state) {
                case 'spins':
                    return 'Spin';
                case 'freespins':
                    var rounds_left = details.context.rounds_left;
                    return 'Free,(' + rounds_left + ')';
            }
        }
    }, {
        key: '_background',
        value: function _background(snap, details) {
            var action = details.last_action;
            if (details.state === 'spins') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#F0E68C'
                })
            }
            if (details.state === 'freespins' || action === 'freespin_stop' || action === 'freespin_init') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#87CEFA'
                })
            }
        ...
        value: function board(snap, _board) {
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    imageUrl = _this4.url(_board[i][j] + '.png');
        ...
        value: function sort_winlines() {
            ...
            var winscatters = this.details.context.winscatters;
            if (winscatters) {
