...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function sort_winlines() {
            ...
            if (winlines && winlines.length) {
                if (this.details.state === 'spins') {
                    ...
                    if (without_special_id && without_special_id.length) {
                        combs = combs.concat(without_special_id.sort(function(a, b) {
                            return b.symbol - a.symbol
                        }))
                    }
                    if (with_special_id && with_special_id.length) {
        ...
        value: function board(snap, _board) {
            ...
            if (details.context.winscatters && details.last_action !== 'freespin_init') {
        ...
        value: function make_freespins(details) {
            ...
            if (details.context.winscatters && details.context.winscatters.length) {
                ...
                _group.text('50%', '40%', '+' + roundsGranted + ' freespins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
                var extra = details.context['extra'];
                _group.image(this.url('extra_' + extra[extra.length - 1] + '.png'), this.width / 3, this.height / 2 - 3, this.width / 3, this.height / 2);
                margin += this.width;
            ...
            var counter = 0;
            snap.click(function(e) {
                if (_this4.details.last_action == 'freespin_stop' || _this4.details.last_action == 'freespin_init') return;
                if (combsGroup.children().length < 2) return;
                counter = (counter + 1) % combsGroup.children().length;
            ...
            if (details.last_action === 'freespin_init') {
                board.remove();
                snap.image(this.url('extra_' + details.context.extra + '.png'), this.width / 3, this.height / 2 - 5, this.width / 3, this.height / 2)
            }
