...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function title(details) {
            ...
            } else if (details.last_action == 'freespin_init' || details.last_action == 'freespin_stop') return details.last_action;
        ...
        value: function _background(snap, state) {
            ...
            } else if (this.details.context.bs_count == 40) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#992fbe'
                })
            } else {
        ...
        key: '_calculateSizes',
        value: function _calculateSizes(cols, rows) {
            var isBonus = this.details.state === 'bonus';
            this.panelHeight = this.height > 150 ? 20 : 12;
            this.panelFont = this.height > 150 ? '1em' : '0.7em';
            this.progressHeight = 3;
            this.boardHeight = this.height - (this.panelHeight + this.progressHeight + 1);
            this.cellWidth = parseInt(this.boardHeight / rows, 10) + (isBonus ? 16 : 2);
            this.cellHeight = this.cellWidth - (isBonus ? 15.5 : 2);
            this.cellOffsetX = 2;
            this.cellOffsetY = -1;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10) - (isBonus ? 3 : 4)
        }
    }, {
        key: '_makeMigrations',
        value: function _makeMigrations(details) {
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && details.context.bs_count == 40 && this.promotedTokens.length == 0 && details.last_action != 'restore' || details.state == 'freespins' && typeof details.context.winscatters !== 'undefined' && details.context.winscatters.length > 0 && (details.context.winscatters[0].trigger != 'bonus' || details.context.winscatters[1] && details.context.winscatters[1].trigger == 'freespins')
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && details.context.bs_count == 40) {
                labelLeft = 'Jackpot!';
                group.text('50%', '56%', 'Jackpot!').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
            } else if (details.state == 'freespins' && details.context.winscatters.length) {
                if (details.context.winscatters.some(function(obj) {
                        return obj.symbol == _this2.scat_id
                    })) {
        ...
        key: 'getTokenSteps',
        value: function getTokenSteps() {
            if (this.details.state == 'bonus') {
                return this.details.context.steps ? this.details.context.steps.filter(function(step) {
                    return step.action === 'token_handling'
                }) : []
            }
            return []
        }
    }, {
        key: 'getPromotedTokens',
        value: function getPromotedTokens() {
            return this.tokenSteps.filter(function(token) {
                return token.data.clean === false
            })
        }
    }, {
        key: 'make_snap',
        value: function make_snap(details) {
            this.cols = details.context.board.length;
            this.rows = details.context.board[0].length;
            this.tokenSteps = this.getTokenSteps();
            this.promotedTokens = this.getPromotedTokens();
            this.mini_jp = 15;
            this.minor_jp = 30;
            this.major_jp = 100;
            this.grand_jp = 1000;
            this.scat_id = 10;
            this.moon_id = 11;
            this.key_thresholds = [10, 15, 20, 25].reverse();
            this.minStartBsNum = 6;
            this._calculateSizes(this.cols, this.rows);
            return new Snap(this.width, this.height)
        }
    }, {
        key: 'make_bonus_board_snap',
        value: function make_bonus_board_snap(details, combsGroup, margin, snap, withTokens, royalFromPromoted) {
            var _this3 = this;
            var group = combsGroup.group();
            margin += this.width;
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            ...
            group.transform('T' + margin + ',0');
            var board = details.context.board.slice();
            var bs = void 0;
            if (withTokens) {
                bs = this.details.context.bs.slice();
                this.details.context.new_bs.forEach(function(newSym) {
                    if (!bs.some(function(sym) {
                            return newSym.reel === sym.reel && newSym.position === sym.position
                        })) {
                        bs.push(newSym)
                    }
                });
                this.tokenSteps.forEach(function(token) {
                    if (!bs.some(function(sym) {
                            return token.data.bs.reel === sym.reel && token.data.bs.position === sym.position
                        })) {
                        bs.push(token.data.bs)
                    }
                })
            } else {
                bs = this.details.context.bs.slice()
            ...
            var unlockedRow = this.key_thresholds.map(function(numToUnlock) {
                var bsCount = _this3.details.context.keys;
                var num = numToUnlock - bsCount < 0 ? 0 : numToUnlock - bsCount;
                return num
            }).concat([0, 0, 0, 0]).map(function(num) {
                return !Boolean(num)
            });
            var _loop = function _loop(i) {
                var _loop3 = function _loop3(_j) {
                    var value = bs ? bs.filter(function(el) {
                        return el.reel == i && el.position == _j
                    })[0] : [];
                    value = value ? value.value : 0;
                    var isJackpotValue = [_this3.mini_jp * round_bet, _this3.minor_jp * round_bet, _this3.major_jp * round_bet, _this3.grand_jp * round_bet].includes(value);
                    var symName = board[i][_j] === 0 && !isJackpotValue ? 'sym0_b.png' : !isJackpotValue || unlockedRow[_j] && (!_this3.tokenSteps.some(function(token) {
                        return token.data.bs.reel == i && token.data.bs.position == _j
                    }) || !withTokens) ? 'sym11_b.png' : 'sym11_b_token.png';
                    imageUrl = _this3.url(symName);
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * i + _this3.boardMarginLeft, (_this3.cellHeight + _this3.cellOffsetY) * _j + _this3.panelHeight, _this3.cellWidth, _this3.cellHeight);
                    if (board[i][_j] === 11) {
                        img.attr({
                            opacity: unlockedRow[_j] ? 1 : 0.5
                        })
                    }
                };
                for (var _j in board[i]) {
                    _loop3(_j)
                }
            };
            for (var i in board) {
            ...
            if (typeof this.details.context.bs !== 'undefined') {
                ...
                try {
                    var _loop2 = function _loop2() {
                        ...
                        var i = bs.reel;
                        var j = bs.position;
                        var value = bs.value == _this3.mini_jp * round_bet ? 'mini' : bs.value == _this3.minor_jp * round_bet ? 'minor' : bs.value == _this3.major_jp * round_bet ? 'major' : bs.value == _this3.grand_jp * round_bet ? 'grand' : (+bs.value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                        var isJackpotSymbol = ['mini', 'minor', 'major', 'grand'].includes(value);
                        if (!(withTokens && _this3.tokenSteps.some(function(t) {
                                return t.data.bs.reel == i && t.data.bs.position == j
                            }))) {
                            var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * i - _this3.cellOffsetX + 2 + _this3.boardMarginLeft;
                            var rect_y = (_this3.cellHeight + _this3.cellOffsetY) * j + _this3.cellHeight / 3 + _this3.panelHeight;
                            var rect_width = _this3.cellWidth + 2 * _this3.cellOffsetX - 3;
                            var text_x = rect_x + rect_width / 2;
                            var text_y = (_this3.cellHeight + _this3.cellOffsetY) * j + _this3.cellHeight / 2 + _this3.panelHeight;
                            var opacity = !unlockedRow ? 1 : unlockedRow[j] ? 1 : isJackpotSymbol ? 0 : 0.5;
                            ...
                            var attr = void 0;
                            if (isJackpotSymbol) {
                                attr = Object.assign({}, _this3.width > 200 ? Drawer['BS_' + value.toUpperCase() + '_65'] : Drawer['BS_' + value.toUpperCase() + '_55'], {
                                    'opacity': opacity
                                });
                                group.text(text_x, text_y, value).attr(attr)
                            } else {
                                var numValue = parseInt(value.replace(/\,/g, ''), 10);
                                if (numValue < 999999) {
                                    attr = Object.assign({}, _this3.width > 200 ? Drawer.BS_VALUE_52 : Drawer.BS_VALUE_28, {
                                        'opacity': opacity
                                    });
                                    group.text(text_x, text_y, value).attr(attr)
                                } else {
                                    attr = Object.assign({}, _this3.width > 200 ? Drawer.BS_VALUE_34 : Drawer.BS_VALUE_22, {
                                        'opacity': opacity
                                    });
                                    group.text(text_x, text_y, value).attr(attr)
                                }
            ...
            if (royalFromPromoted) {
                ...
                for (var _i = 0; _i < combs.length; ++_i) {
                    this.drawe_border_win_symbols(group, combs[_i]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
                }
            }
            var labelLeft = details.last_action == 'restore' ? 'Restore' : 'Bonus(' + details.context.rounds_left + ')';
            var labelRight = royalFromPromoted ? 'all/1' : '';
            this._panel(group, labelLeft + ', Win:' + this.coins(details.round_win), labelRight);
            return snap
        }
    }, {
        key: 'make_bonus_init_snap',
        value: function make_bonus_init_snap(details, combsGroup, margin, snap) {
            var group = combsGroup.group();
            margin += this.width;
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            progress.animate({
            ...
            group.transform('T' + margin + ',0');
            group.text('50%', '56%', this.get_text_for_action(details)).attr(Drawer.STYLE_TEXT_CENTER_RED);
            this._panel(group, details.last_action, '');
            return snap
        }
    }, {
        key: 'make_unlock_snap',
        value: function make_unlock_snap(details, combsGroup, margin, snap) {
            var group = combsGroup.group();
            var labelLeft = 'Rows to unlock:';
            var labelRight = '';
            this._background(group, details.state);
            margin += this.width;
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            ...
            group.transform('T' + margin + ',0');
            this.key_thresholds.forEach(function(numToUnlock, i) {
                var bsCount = details.context.keys;
                var num = numToUnlock - bsCount < 0 ? 0 : numToUnlock - bsCount;
                if (num > 0) {
                    group.text('50%', 26 + i * 20 + '%', 8 - i + 'th - ' + num + ' Bonus to unlock').attr(Drawer.BONUS_UNLOCK_ROW_STYLE)
                } else {
                    group.text('50%', 26 + i * 20 + '%', 8 - i + 'th - Unlocked').attr(Drawer.BONUS_UNLOCK_ROW_STYLE)
                }
            });
            this._panel(group, labelLeft, labelRight);
            return snap
        ...
        key: 'make_jackpot_progress_snap',
        value: function make_jackpot_progress_snap(details, combsGroup, margin, snap) {
            var group = combsGroup.group();
            this._background(group, details.state);
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            progress.animate({
            ...
            margin += this.width;
            group.transform('T' + margin + ',0');
            var jackpots = ['grand', 'major', 'minor', 'mini'];
            jackpots.forEach(function(jackpot, i) {
                var name = jackpot.charAt(0).toUpperCase() + jackpot.slice(1);
                var num = details.context.jp_tokens[jackpot];
                group.text('50%', 25 + i * 20 + '%', name + ': ' + num).attr(Drawer.BONUS_UNLOCK_ROW_STYLE)
            });
            this._panel(group, 'Jackpot tokens progress:', '');
            return snap
        }
    }, {
        key: 'make_jackpot_win_snap',
        value: function make_jackpot_win_snap(details, combsGroup, margin, snap) {
            var group = combsGroup.group();
            this._background(group, details.state);
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            progress.animate({
            ...
            group.transform('T' + margin + ',0');
            var jackpots = {};
            this.promotedTokens.forEach(function(token) {
                var type = token.data.type;
                jackpots[type] = !jackpots[type] ? 1 : jackpots[type] + 1
            });
            Object.entries(jackpots).forEach(function(arr, i, entries) {
                var jackpot = arr[0];
                var num = arr[1];
                var name = jackpot.charAt(0).toUpperCase() + jackpot.slice(1);
                group.text('50%', 100 / (1 + entries.length) * (i + 1) + 5 + '%', num + ' ' + name).attr(Drawer.BONUS_TOKEN_JACKPOT_WIN)
            });
            this._panel(group, 'Jackpot granted:', '');
            return snap
        }
    }, {
        key: 'make_royal_win_snap',
        value: function make_royal_win_snap(details, combsGroup, margin, snap) {
            var group = combsGroup.group();
            this._background(group, details.state);
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            progress.animate({
            ...
            group.transform('T' + margin + ',0');
            group.text('50%', '56%', 'Jackpot!').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
            this._panel(group, 'Jackpot!', '');
            return snap
        }
    }, {
        key: '_combs',
        value: function _combs(snap, combs, details, royalFromPromoted) {
            var group = snap.group();
            this.bordered_cells = [];
            var totalWin = details.round_win;
            var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin);
            ...
            for (var _i2 = 0; _i2 < combs.length; ++_i2) {
                if (!royalFromPromoted) {
                    this.drawe_border_win_symbols(group, combs[_i2]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
                }
            }
        ...
        value: function make_spins(details) {
            ...
            var combGroup = void 0;
            var margin = 0;
            var royalFromPromoted = details.context.bs_count && details.context.bs_count === 40 && this.promotedTokens.length > 0;
            this._combs(combsGroup, combs, details, royalFromPromoted);
            if (combs.length > 1) {
                for (var _i4 in combs) {
                    combGroup = this._comb(combsGroup, combs, combs[_i4], details);
                    margin += this.width;
                    combGroup.transform('T' + margin + ',0')
                }
            }
            ...
            if (details.last_action === 'restore' && ['spins', 'freespins'].includes(details.state) && details.context.bs && details.context.bs.some(function(bs) {
                    return bs.position >= 4
                })) {
                details.context.bs = details.context.bs.filter(function(bs) {
                    return bs.position >= 4
                }).map(function(bs) {
                    bs.position -= 4;
                    return bs
                });
                details.context.bs.forEach(function(bs) {
                    details.context.board[bs.reel][bs.position] = 11
                })
            }
            var ce = 0;
            var board = void 0;
            if (details.last_action == 'bonus_init') {
                ce = -1;
                this.make_bonus_init_snap(details, combsGroup, margin + this.width * ce, snap);
                ce += 1;
                if (this.tokenSteps.length > 0) {
                    var withTokens = true;
                    this.make_bonus_board_snap(details, combsGroup, margin + this.width * ce, snap, withTokens);
                    ce += 1;
                    if (this.promotedTokens.length > 0) {
                        this.make_jackpot_win_snap(details, combsGroup, margin + this.width * ce, snap);
                        ce += 1;
                        this.make_bonus_board_snap(details, combsGroup, margin + this.width * ce, snap);
                        ce += 1;
                        this.make_jackpot_progress_snap(details, combsGroup, margin + this.width * ce, snap);
                        ce += 1;
                        this.make_unlock_snap(details, combsGroup, margin + this.width * ce, snap);
                        ce += 1
                    } else {
                        this.make_jackpot_progress_snap(details, combsGroup, margin + this.width * ce, snap);
                        ce += 1;
            ...
            } else if (this.tokenSteps.length == 0 || details.state == 'bonus' && details.last_action == 'restore') {
                board = this.board(group, details.context.board);
                board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            if (this.is_extra_snap(details)) {
                this.make_extra_snap(details, combsGroup, margin, snap);
                partSize = this.width / combsGroup.children().length
            }
            if (details.state == 'bonus' && details.last_action == 'respin' && details.last_action !== 'restore' && (details.context.bs_count !== 40 || royalFromPromoted)) {
                if (this.tokenSteps.length > 0) {
                    board && board.remove();
                    ce = -1;
                    var _withTokens = true;
                    this.make_bonus_board_snap(details, combsGroup, margin + this.width * ce, snap, _withTokens);
                    ce += 1;
                    if (this.promotedTokens.length > 0) {
                        this.make_jackpot_win_snap(details, combsGroup, margin + this.width * ce, snap);
                        ce += 1;
                        this.make_bonus_board_snap(details, combsGroup, margin + this.width * ce, snap, !_withTokens, royalFromPromoted);
                        ce += 1;
                        if (!royalFromPromoted) {
                            this.make_jackpot_progress_snap(details, combsGroup, margin + this.width * ce, snap);
                            ce += 1;
                        ...
                        if (details.context.bs_count == 40) {
                            this.make_royal_win_snap(details, combsGroup, margin + this.width * ce, snap);
                            ce += 1
                        }
                    } else {
                        this.make_jackpot_progress_snap(details, combsGroup, margin + this.width * ce, snap);
            ...
            if (['freespin_init', 'freespin_stop', 'bonus_stop', 'bonus_spins_stop', 'bonus_freespins_stop'].includes(details.last_action)) {
                board && board.remove();
                this._background(snap, ['freespin_init', 'freespin_stop'].includes(details.last_action) ? 'freespins' : 'bonus');
                this._panel(snap, details.last_action, '')
            }
            var counter = 0;
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
                counter = (counter + 1) % combsGroup.children().length;
                if ((_this4.tokenSteps.length > 0 || details.last_action === 'bonus_init' || details.last_action === 'restore' && details.state == 'bonus') && counter === combsGroup.children().length - 1) {
                    board && board.remove();
                    counter = 0
                }
        ...
        value: function board(snap, _board) {
            ...
            var combs = this.details.context.winlines ? this.details.context.winlines : [];
            combs = combs.concat(this.details.context.winscatters ? this.details.context.winscatters : []);
            var group = snap.group();
            var imageUrl = void 0;
            var round_bet = this.details.round_bet_not_restored ? this.details.round_bet_not_restored : this.details.round_bet;
            var isBonus = this.details.state === 'bonus';
            var unlockedRow = void 0;
            if (isBonus) {
                unlockedRow = this.key_thresholds.map(function(numToUnlock) {
                    var bsCount = _this5.details.context.keys;
                    var num = numToUnlock - bsCount < 0 ? 0 : numToUnlock - bsCount;
                    return num
                }).concat([0, 0, 0, 0]).map(function(num) {
                    return !Boolean(num)
            ...
            var _loop4 = function _loop4(_i5) {
                var _loop5 = function _loop5(_j3) {
                    var value = _this5.details.context.bs ? _this5.details.context.bs.filter(function(el) {
                        return el.reel == _i5 && el.position == _j3
                    })[0] : [];
                    value = value ? value.value : 0;
                    var restoreTokenAfterBonus = _this5.details.last_action == 'restore' && !isBonus && _board[_i5][_j3] === 11 && _this5.details.context.bs && !_this5.details.context.bs.some(function(sym) {
                        return sym.reel == _i5 && sym.position == _j3
                    });
                    var restoreBSAfterBonus = _this5.details.last_action == 'restore' && !isBonus && _board[_i5][_j3] === 11 && _board.flat().filter(function(id) {
                        return id == 11
                    }).length !== _this5.details.context.bs_count && _this5.details.context.bs && _this5.details.context.bs.some(function(sym) {
                        return sym.reel == _i5 && sym.position == _j3
                    });
                    var tokenType = [_this5.mini_jp * round_bet, _this5.minor_jp * round_bet, _this5.major_jp * round_bet, _this5.grand_jp * round_bet].includes(value) ? restoreBSAfterBonus ? '' : '_token' : '';
                    var postfix = isBonus ? unlockedRow[_j3] ? '_b' : '_b' + tokenType : _board[_i5][_j3] === 11 ? restoreTokenAfterBonus ? '_token' : '' + tokenType : '';
                    var symName = 'sym' + _board[_i5][_j3] + postfix + '.png';
                    imageUrl = _this5.url(symName);
                    var img = group.image(imageUrl, (_this5.cellWidth + _this5.cellOffsetX) * _i5, (_this5.cellHeight + _this5.cellOffsetY) * _j3, _this5.cellWidth, _this5.cellHeight);
                    if (isBonus && _board[_i5][_j3] === 11) {
                        img.attr({
                            opacity: unlockedRow[_j3] ? 1 : 0.5
                        })
                    }
                    if (combs.length == 1) {
                        currentWinLine = 0
                ...
                for (var _j3 in _board[_i5]) {
                    var _ret5 = _loop5(_j3);
            ...
            if (typeof this.details.context.bs !== 'undefined') {
                ...
                try {
                    for (var _iterator2 = this.details.context.bs[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                        var bs = _step2.value;
                        var _i6 = bs.reel;
                        var _j2 = bs.position;
                        var _value = bs.value == this.mini_jp * round_bet ? 'mini' : bs.value == this.minor_jp * round_bet ? 'minor' : bs.value == this.major_jp * round_bet ? 'major' : bs.value == this.grand_jp * round_bet ? 'grand' : (+bs.value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                        var rect_x = (this.cellWidth + this.cellOffsetX) * _i6 - this.cellOffsetX + 2;
                        var rect_y = (this.cellHeight + this.cellOffsetY) * _j2 + this.cellHeight / 3;
                        var rect_width = this.cellWidth + 2 * this.cellOffsetX - 3;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = (this.cellHeight + this.cellOffsetY) * _j2 + this.cellHeight / 2;
                        var _isJackpotSymbol = ['mini', 'minor', 'major', 'grand'].includes(_value);
                        var restoreBSAfterBonus = this.details.last_action == 'restore' && !isBonus && !(_board.flat().filter(function(id) {
                            return id == 11
                        }).length == this.details.context.bs_count);
                        var opacity = isBonus ? !unlockedRow ? 1 : unlockedRow[_j2] ? 1 : _isJackpotSymbol ? 0 : 0.5 : _isJackpotSymbol && !restoreBSAfterBonus ? 0 : 1;
                        ...
                        var attr = void 0;
                        if (_isJackpotSymbol) {
                            attr = Object.assign({}, this.width > 200 ? Drawer['BS_' + _value.toUpperCase() + '_65'] : Drawer['BS_' + _value.toUpperCase() + '_55'], {
                                'opacity': opacity
                            });
                            group.text(text_x, text_y, _value).attr(attr)
                        } else {
                            var numValue = parseInt(_value.replace(/\,/g, ''), 10);
                            if (numValue < 999999) {
                                attr = Object.assign({}, this.width > 200 ? Drawer.BS_VALUE_52 : Drawer.BS_VALUE_28, {
                                    'opacity': opacity
                                });
                                group.text(text_x, text_y, _value).attr(attr)
                            } else {
                                attr = Object.assign({}, this.width > 200 ? Drawer.BS_VALUE_34 : Drawer.BS_VALUE_22, {
                                    'opacity': opacity
                                });
                                group.text(text_x, text_y, _value).attr(attr)
                            }
        ...
        value: function drawe_border_win_symbols(snap, line) {
            var group = snap.group().attr(BaseDrawer.STYLE_LINE_GROUP);
            for (var _i7 = 0; _i7 < line.positions.length; ++_i7) {
                var cell = line.positions[_i7];
                if (!isItemInArray(this.bordered_cells, cell)) {
                    group.rect(cell[0] * (this.cellWidth + this.cellOffsetX) - 1, cell[1] * (this.cellHeight + this.cellOffsetY), this.cellWidth + this.cellOffsetX, this.cellHeight + this.cellOffsetY).attr(BaseDrawer.STYLE_LINE_SYMBOL);
                    this.bordered_cells.push(cell)
                }
            }
            return group
...
Drawer.BONUS_TOKEN_JACKPOT_WIN = {
    'fill': '#fff',
    'font-family': '"Skia", Gadget, sans-serif',
    'text-anchor': 'middle',
    'font-weight': 'bold',
