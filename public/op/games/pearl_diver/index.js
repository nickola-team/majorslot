...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function is_extra_snap(details) {
            return this.is_diver_feature_win(details) || this.is_addFS_win(details)
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (this.is_diver_feature_win(details)) {
                if (this.is_addFS_win(details)) {
                    labelLeft = 'Diver feature, additional freespins!';
                    ...
                    if (this.width > 200) {
                        group.text('50%', '50%', 'Diver feature: ' + details.context.diver_feature.total_win / 100).attr(Drawer.FEATURE_STYLE_TEXT_CENTER_BIG);
                        group.text('50%', '64%', '+' + roundsGranted + ' FreeSpins').attr(Drawer.FEATURE_STYLE_TEXT_CENTER_BIG)
                    } else {
                        group.text('50%', '50%', 'Diver feature: ' + details.context.diver_feature.total_win / 100).attr(Drawer.FEATURE_STYLE_TEXT_CENTER);
                        group.text('50%', '64%', '+' + roundsGranted + ' FreeSpins').attr(Drawer.FEATURE_STYLE_TEXT_CENTER)
                    }
                } else {
                    labelLeft = 'Diver feature!';
                    if (this.width > 200) {
                        group.text('50%', '56%', 'Diver feature: ' + details.context.diver_feature.total_win / 100).attr(Drawer.FEATURE_STYLE_TEXT_CENTER_BIG)
                    } else {
                        group.text('50%', '56%', 'Diver feature: ' + details.context.diver_feature.total_win / 100).attr(Drawer.FEATURE_STYLE_TEXT_CENTER)
                    }
                }
            } else if (this.is_addFS_win(details)) {
                labelLeft = 'Additional FreeSpins!';
                var _roundsGranted = details.context.winscatters.reduce(function(a, _ref3) {
                    var _ref3$freespins = _ref3.freespins,
                        freespins = _ref3$freespins === undefined ? 0 : _ref3$freespins;
                    return a + freespins
                }, 0);
                if (this.width > 200) {
                    group.text('50%', '56%', '+' + _roundsGranted + ' FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER_BIG)
                } else {
                    group.text('50%', '56%', '+' + _roundsGranted + ' FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                }
            }
            this._panel(group, labelLeft, labelRight);
            return snap
        ...
        value: function make_snap(details) {
            ...
            this.rows = details.context.board[0].length;
            this.scat_id = 1;
            this.super_pearl_id = 15;
            this.diver_feature_ids = [1, 6, 7, 8, 9, 15];
            this._calculateSizes(this.cols, this.rows);
        ...
        value: function board(snap, _board) {
            ...
            var group = snap.group();
            var imageUrl = void 0;
            if (this.details.context.super_pearl !== undefined) {
                var _iteratorNormalCompletion = true;
                var _didIteratorError = false;
                var _iteratorError = undefined;
                try {
                    for (var _iterator = this.details.context.super_pearl[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        var super_pearl = _step.value;
                        _board[super_pearl[0]][super_pearl[1]] = this.super_pearl_id
                    }
                } catch (err) {
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    if (_this3.details.state == 'freespins') {
                        imageUrl = _this3.url(_board[i][j] + '_f.png')
                    } else {
        ...
        key: 'drawe_pearl_values',
        value: function drawe_pearl_values(group) {
            var round_bet = this.details.context.lines * this.details.context.bet_per_line;
            if (typeof this.details.context.diver_feature.pearls !== 'undefined') {
                var _iteratorNormalCompletion2 = true;
                var _didIteratorError2 = false;
                var _iteratorError2 = undefined;
                try {
                    for (var _iterator2 = this.details.context.diver_feature.pearls[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                        var pearl = _step2.value;
                        var value = (+pearl.value * round_bet / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                        var i = pearl.positions[0];
                        var j = pearl.positions[1];
