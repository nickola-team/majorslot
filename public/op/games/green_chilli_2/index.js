...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function _combs(snap, combs, details) {
            ...
            this.bordered_cells = [];
            var totalWin = details.round_win;
            var labelLeft = void 0;
            if (details.last_action === 'respin' && details.context.rounds_left !== 0) {
                labelLeft = this.title(details)
            } else {
                labelLeft = this.title(details) + ', Win:' + this.coins(totalWin)
            }
            var labelRight = combs.length ? 'all/' + combs.length : '';
            this._panel(group, labelLeft, labelRight);
            for (var i = 0; i < combs.length; ++i) {
        ...
        key: '_combs_bonus',
        value: function _combs_bonus(snap, combs, details) {
            var group = snap.group();
            this.bordered_cells = [];
            var totalWin = details.round_win;
            var labelLeft = void 0;
            ...
            var labelRight = combs.length ? 'final' : '';
            this._panel(group, labelLeft, labelRight);
            for (var i = 0; i < combs.length; ++i) {
        ...
        key: '_comb_bonus',
        value: function _comb_bonus(snap, combs, comb, details) {
            this.bordered_cells = [];
            var group = snap.group();
            var labelLeft = this.title(details) + ', Win:' + this.coins(comb.amount);
            var labelRight = '';
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && (details.context.bs_count === 15 || details.context.bonus_symbols_count === 15) || details.state == 'freespins' && details.context.winscatters != undefined && details.context.winscatters.length > 0
        }
    }, {
        ...
        value: function get_pearls_count(details) {
            ...
            for (var i in board) {
                for (var j in board[i]) {
                    if ([11, 12, 13].includes(board[i][j])) {
                        count = count + 1
                    }
                }
            }
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && this.get_pearls_count(details) === 15) {
                labelLeft = 'Jackpot!';
                if (this.width > 200) {
        ...
        value: function make_spins(details) {
            ...
            if (details.last_action === 'respin') {
                ...
                if (this.details.context.rounds_left == 0 && mults != undefined) {
                    for (var _i2 in mults) {
                        if (this.details.context.bs_v[mults[_i2][0]][mults[_i2][1]] != 0) {
                            win_mult_pos.push(mults[_i2])
                        }
                    }
                    if (win_mult_pos.length > 0) {
                        var winmults = {
                            'trigger': 'winmult',
                            'positions': []
                        };
                        winmults.positions = win_mult_pos;
                        combs.push(winmults);
                        var winmults2 = {
                            'trigger': 'winmult_fin',
                            'positions': []
                        };
                        winmults2.positions = win_mult_pos;
                        combs.push(winmults2)
                    }
                }
            }
            var combsGroup = snap.group();
            ...
            var margin = 0;
            if (win_mult_pos.length > 0) {
                this._combs_bonus(combsGroup, combs, details)
            } else {
                this._combs(combsGroup, combs, details)
            }
            if (combs.length > 1) {
                for (var _i3 in combs) {
                    if (win_mult_pos.length > 0) {
                        combGroup = this._comb_bonus(combsGroup, combs, combs[_i3], details)
                    } else {
            ...
            if (win_mult_pos.length > 0) {
                partSize = this.width / (combsGroup.children().length - 1);
                progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(Drawer.STYLE_PROGRESS)
            }
            ...
            var counter = 0;
            snap.click(function(e) {
                if (win_mult_pos.length > 0 && !_this2.is_extra_snap(details)) {
                    var newLocal = counter = (counter + 1) % (combsGroup.children().length - 1);
                    ...
                    var _newLocal = counter = (counter + 1) % combsGroup.children().length;
                    combsGroup.transform('T-' + _this2.width * counter);
                    progress.animate({
                        width: partSize * (counter + 1)
                    }, 100);
        ...
        value: function board(snap, _board) {
            ...
            var extra_flag = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
            var bonus_symbols = [11, 12, 13];
            var combs = this.details.context.winlines;
            ...
            if (this.details.last_action === 'respin') {
                var _mults = this.details.context.mults;
                if (this.details.context.rounds_left == 0 && _mults != undefined) {
                    var _win_mult_pos = [];
                    for (var _i5 in _mults) {
                        if (this.details.context.bs_v[_mults[_i5][0]][_mults[_i5][1]] != 0) {
                            _win_mult_pos.push(_mults[_i5])
                        }
                    }
                    if (_win_mult_pos.length > 0) {
                        var winmults = {
                            'trigger': 'winmult',
                            'positions': []
                        };
                        winmults.positions = _win_mult_pos;
                        combs.push(winmults);
                        var winmults2 = {
                            'trigger': 'winmult_fin',
                            'positions': []
                        };
                        winmults2.positions = _win_mult_pos;
                        combs.push(winmults2)
                    }
                }
            }
            var new_mults = [];
            if (this.details.context.new_mult != undefined) {
                new_mults = this.details.context.new_mult
            }
            var group = snap.group();
            var imageUrl = void 0;
            var _loop = function _loop(_i6) {
                var _loop2 = function _loop2(_j) {
                    imageUrl = _this3.url(_board[_i6][_j] + '.png');
                    if (_this3.details.state === 'freespins' && _board[_i6][_j] === 9) {
                        imageUrl = _this3.url('9_fs.png')
                    }
                    if (_this3.details.state === 'bonus' && bonus_symbols.indexOf(_board[_i6][_j]) == -1) {
                        imageUrl = _this3.url('0.png')
                    }
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * _i6, (_this3.cellHeight + _this3.cellOffsetY) * _j, _this3.cellWidth, _this3.cellHeight);
                    if (combs.length == 1) {
                        currentWinLine = 0
                    }
                    var value = void 0;
                    if (_this3.details.context.bs_v != undefined) {
                        value = _this3.details.context.bs_v[_i6][_j]
                    } else {
                        value = 0
                    }
                    var mult_value = -1;
                    if (_this3.details.context.mult_values === undefined) {
                        mult_value = 1
                    } else {
                        mult_value = _this3.details.context.mult_values[_i6][_j];
                        if (mult_value > 1) value = value / mult_value
                    }
                    if (currentWinLine !== -1) {
                        if (combs[currentWinLine] !== undefined) {
                            if (combs[currentWinLine].trigger !== undefined) {
                                if (combs[currentWinLine].trigger == 'winmult_fin') {
                                    mult_value = 1
                                }
                            }
                        }
                    }
                    if (mult_value !== 1 && (_this3.details.context.rounds_left != 0 || !win_mult_pos.includes((_i6, _j)))) {
                        if (currentWinLine !== -1 || win_mult_pos.length === 0 || _this3.details.context.rounds_left != 0) {
                            var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i6 + _this3.cellWidth / 1.8;
                            var rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j - _this3.cellOffsetY;
                            var rect_width = _this3.cellWidth / 2.1 + 2 * _this3.cellOffsetX;
                            var text_x_x = rect_x + rect_width / 2;
                            var text_x_y = (_this3.cellHeight + _this3.cellOffsetY) * _j + _this3.cellOffsetY + _this3.cellHeight * 0.2;
                            var mult_txt = 'x' + mult_value;
                            var add_mult_flag = false;
                            for (var l in new_mults) {
                                if (_i6 == new_mults[l].pos[0] && _j == new_mults[l].pos[1]) {
                                    var old_mult = mult_value - new_mults[l].value
                                }
                            }
                            if (add_mult_flag) {
                                var delta = _this3.cellWidth * 0.2;
                                rect_x = rect_x - delta;
                                rect_width = rect_width + delta;
                                text_x_x = text_x_x - delta / 2
                            }
                            group.rect(rect_x, rect_y, rect_width, _this3.cellHeight / 3).attr({
                                'fill': 'white'
                            });
                            if (_this3.width > 200) {
                                group.text(text_x_x, text_x_y, mult_txt).attr(Drawer.MULT_VALUE_70)
                            } else {
                                group.text(text_x_x, text_x_y, mult_txt).attr(Drawer.MULT_VALUE_40)
                            }
                        }
                    }
                    var is_win_pos = false;
                    if (currentWinLine !== -1) {
                        if (combs[currentWinLine] !== undefined) {
                            if (combs[currentWinLine].trigger !== undefined) {
                                if (combs[currentWinLine].trigger == 'winmult_fin') {
                                    for (var q in win_mult_pos) {
                                        if (win_mult_pos[q][0] == _i6 && win_mult_pos[q][1] == _j) {
                                            is_win_pos = true
                                        }
                                    }
                                    if (is_win_pos) {
                    ...
                    if (currentWinLine == -1) {
                        for (var _q in win_mult_pos) {
                            if (win_mult_pos[_q][0] == _i6 && win_mult_pos[_q][1] == _j) {
                                is_win_pos = true
                            }
                        }
                        if (is_win_pos) {
                    ...
                    if (value != 0) {
                        var _rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i6 - _this3.cellOffsetX;
                        var _rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j + _this3.cellHeight / 3;
                        var _rect_width = _this3.cellWidth + 2 * _this3.cellOffsetX;
                        ...
                        if (value == 'mini') {
                            ...
                            if (value.length < 7) {
                                if (_this3.width > 200) {
                                    ...
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_50)
                                }
                            } else if (7 <= value.length && value.length < 10) {
                                if (_this3.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_60)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_40)
                                }
                            } else if (10 <= value.length && value.length < 13) {
                                if (_this3.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_50)
                                } else {
                                ...
                                if (_this3.width > 200) {
                                    ...
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_25)
                                }
                            }
                        }
                    } else {}
