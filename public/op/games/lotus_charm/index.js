    ...
    _createClass(Drawer, [{
        ...
        value: function _background(snap, state) {
            ...
            if (state === 'bonus' || this.details.last_action === 'bonus_stop') {
                ...
                if (this.details.context.is_extra_feature) {
                    color = '#b7daf7'
                } else {
                    color = '#4682B4'
                }
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && details.context.bs_count === 15 || details.state == 'freespins' && details.context.winscatters != undefined && details.context.winscatters.length > 0
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && details.context.bs_count === 15) {
                labelLeft = 'Jackpot';
        ...
        value: function make_spins(details) {
            ...
            if (details.state == 'bonus' && details.context.rounds_left == 0 && details.context.mystery_pos != undefined) {
                ...
                try {
                    for (var _iterator = details.context.mystery_pos[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        ...
                        board_with_mystery[mys_pos[0]][mys_pos[1]] = 12
                    }
                } catch (err) {
            ...
            if (details.context.is_extra_feature) {
                game_boards.push(details.context.feature_board)
            } else {
            ...
            if (combs.length > 1 || game_boards.length > 1) {
                for (var _i in combs) {
                    if (combs[_i].type === 'random_wild') {
                        combGroup = this._combs_random_wild(combsGroup, combs, details);
                        margin += this.width;
                        combGroup.transform('T' + margin + ',0');
                        continue
                    } else {
        ...
        key: '_combs_random_wild',
        value: function _combs_random_wild(snap, combs, details) {
            var group = snap.group();
            this.bordered_cells = [];
            var totalWin = details.round_win;
            var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin);
        ...
        value: function board(snap, _board) {
            ...
            var _loop = function _loop(_i3) {
                var _loop2 = function _loop2(_j) {
                    ...
                    if (_this3.details.state === 'bonus' && ![11, 12].includes(_board[_i3][_j])) {
                    ...
                    if (_this3.details.context.bs_v != undefined) {
                        value = _this3.details.context.bs_v[_i3][_j]
                    } else if (_this3.details.context.bonus_symbols_v != undefined) {
                        value = _this3.details.context.bonus_symbols_v[_i3][_j]
                    } else {
                        return 'continue'
                    }
