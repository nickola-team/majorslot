...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function _calculateSizes(cols, rows) {
            ...
            this.cellOffsetY = -1;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10) + 2;
            this.bestWidth = Math.min(this.width, this.height * 3 / 2);
            this.bestMargin = Math.max((this.width - this.bestWidth) / 2, 0)
        }
    }, {
        key: '_makeMigrations',
        value: function _makeMigrations(details) {
            ...
            if (details.last_action == 'restore') {
                details.context.winlines = [];
                details.context.winscatters = [];
                details.context.mask_positions = [];
                details.round_win = 0;
                details.round_bet = 0
            }
            return details
        ...
        value: function _combs(snap, combs, details) {
            ...
            var labelLeft = this.title(details) + ',Win:' + this.coins(totalWin);
            var labelRight = combs.length ? 'all/' + combs.length : '';
            if (combs.length == 1 && typeof combs[0].ways !== 'undefined') {
                labelLeft += ',Ways:' + combs[0].ways
            }
            this._panel(group, labelLeft, labelRight);
            for (var i = 0; i < combs.length; ++i) {
                this.drawe_border_win_symbols(group, combs[i]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
        ...
        value: function _comb(snap, combs, comb, details) {
            ...
            var labelLeft = this.title(details) + ',Win:' + this.coins(comb.amount);
            var labelRight = combs.indexOf(comb) + 1 + '/' + combs.length;
            if (typeof comb.ways !== 'undefined') {
                labelLeft += ',Ways:' + comb.ways
            }
            this._panel(group, labelLeft, labelRight);
            this.drawe_border_win_symbols(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            this.drawe_line_patern(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
        ...
        value: function is_extra_snap(details) {
            return this.is_addFS_win(details)
        }
    }, {
        key: 'make_extra_snap',
        ...
        key: 'make_level_up_snap',
        value: function make_level_up_snap(details, combsGroup, margin, snap, level) {
            var group = combsGroup.group();
            var labelLeft = '';
            var labelRight = '';
            this._background(group, details, details.state);
            ...
            group.transform('T' + margin + ',0');
            labelLeft = 'Hero Rise Feature!';
            group.image(this.url(this.king_id - level + '.png'), this.bestMargin + 1 / 6 * this.bestWidth, 1 / 6 * this.height, this.bestWidth / 4, this.height / 3);
            group.image(this.url('redarrow.png'), this.bestMargin + 5 / 12 * this.bestWidth, 2 / 9 * this.height, this.bestWidth / 6, this.height / 4.5);
            group.image(this.url(this.king_id + '.png'), this.bestMargin + 7 / 12 * this.bestWidth, 1 / 6 * this.height, this.bestWidth / 4, this.height / 3);
            if (this.width > 200) {
                group.text('50%', '68%', '+1 FreeSpin').attr(Drawer.FEATURE_STYLE_TEXT_CENTER_BIG)
            } else {
                group.text('50%', '68%', '+1 FreeSpin').attr(Drawer.FEATURE_STYLE_TEXT_CENTER)
            }
            this._panel(group, labelLeft, labelRight);
            return snap
        }
    }, {
        key: 'make_progress_snap',
        value: function make_progress_snap(details, combsGroup, margin, snap, keys) {
            var group = combsGroup.group();
            var labelLeft = '';
            var labelRight = '';
            this._background(group, details, details.state);
            ...
            try {
                for (var _iterator = this.threshold_sym_ids[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    var threshold_sym_id = _step.value;
                    var threshold_sym_num = this.king_id - threshold_sym_id - 1;
                    var rect_w = this.bestMargin + (1 / 6 + 5 / 12 * (threshold_sym_num % 2)) * this.bestWidth;
                    var rect_h = (1 / 9 + 4 / 9 * Math.floor(threshold_sym_num / 2)) * this.height;
                    if (keys >= this.keys_for_level_up * (threshold_sym_num + 1)) {
                        group.image(this.url(this.king_id + '.png'), rect_w, rect_h, this.bestWidth / 4, this.height / 3);
                        group.text((7 / 24 + 5 / 12 * (threshold_sym_num % 2)) * 100 + '%', (1 / 2 + 4 / 9 * Math.floor(threshold_sym_num / 2)) * 100 + '%', this.keys_for_level_up + '/' + this.keys_for_level_up).attr(Drawer.FEATURE_STYLE_TEXT_CENTER)
                    } else {
                        group.image(this.url(threshold_sym_id + '.png'), rect_w, rect_h, this.bestWidth / 4, this.height / 3);
                        if (keys >= this.keys_for_level_up * threshold_sym_num) {
                            var progress_cur_level = keys - this.keys_for_level_up * threshold_sym_num;
                            group.text((7 / 24 + 5 / 12 * (threshold_sym_num % 2)) * 100 + '%', (1 / 2 + 4 / 9 * Math.floor(threshold_sym_num / 2)) * 100 + '%', progress_cur_level + '/' + this.keys_for_level_up).attr(Drawer.FEATURE_STYLE_TEXT_CENTER)
                        } else {
                            group.text((7 / 24 + 5 / 12 * (threshold_sym_num % 2)) * 100 + '%', (1 / 2 + 4 / 9 * Math.floor(threshold_sym_num / 2)) * 100 + '%', '0/' + this.keys_for_level_up).attr(Drawer.FEATURE_STYLE_TEXT_CENTER)
                        }
                    }
                }
            } catch (err) {
        ...
        value: function make_snap(details) {
            this.cols = details.context.board.length;
            this.rows = details.context.board[0].length;
            this.threshold_sym_ids = [10, 9, 8, 7];
            this.king_id = 11;
            this.scat_id = 13;
            this.keys_for_level_up = 10;
            this._calculateSizes(this.cols, this.rows);
        ...
        value: function make_spins(details) {
            ...
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            var ce = 0;
            if (details.state == 'freespins' && details.last_action != 'restore') {
                var keys = this.details.context.keys;
                if (typeof this.details.context.mask_positions !== 'undefined' && this.details.context.mask_positions.length > 0) {
                    var level_old = Math.floor((keys - this.details.context.mask_positions.length) / this.keys_for_level_up) + 1;
                    for (var level = level_old; level < this.details.context.threshold_num; ++level) {
                        this.make_level_up_snap(details, combsGroup, margin + this.width * ce, snap, level);
                        ce += 1
                    }
                }
                if (this.is_addFS_win(details)) {
                    var roundsGranted = details.context.winscatters.reduce(function(a, _ref3) {
                        var _ref3$freespins = _ref3.freespins,
                            freespins = _ref3$freespins === undefined ? 0 : _ref3$freespins;
                        return a + freespins
                    }, 0);
                    this.make_extra_snap(details, combsGroup, margin + this.width * ce, snap, 'Additional freespins', '+' + roundsGranted + ' FreeSpins');
                    ce += 1
                }
                this.make_progress_snap(details, combsGroup, margin + this.width * ce, snap, this.details.context.keys);
                ce += 1
            }
            if (details.last_action == 'freespin_init' || details.last_action == 'freespin_stop') {
                board.remove();
        ...
        key: 'has_mask',
        value: function has_mask(details, i, j) {
            var mask_positions = details.context.mask_positions;
            if (typeof mask_positions !== 'undefined') {
                var _iteratorNormalCompletion2 = true;
                var _didIteratorError2 = false;
                var _iteratorError2 = undefined;
                try {
                    for (var _iterator2 = mask_positions[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                        var mask_position = _step2.value;
                        if (mask_position[0] == i && mask_position[1] == j) {
                            return true
                        }
                    }
                } catch (err) {
        ...
        value: function board(snap, _board) {
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    if (_board[i][j] == _this3.king_id && _this3.has_mask(_this3.details, i, j)) {
                        imageUrl = _this3.url(_this3.king_id + '_active.png')
                    } else {
