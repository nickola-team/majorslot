...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && details.context.bs_count === 20 || details.state == 'freespins' && details.context.winscatters != undefined && details.context.winscatters.length > 0
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && details.context.bs_count === 20) {
                labelLeft = 'Jackpot';
        ...
        value: function make_spins(details) {
            ...
            if (details.state === 'bonus') {
                for (var reel_index in game_board) {
                    details.context.board[reel_index].unshift(0);
                    var mult = 1;
                    if (details.context.reel_multipliers[reel_index]) {
                        mult = details.context.reel_multipliers[reel_index]
                    }
                    details.context.bs_v[reel_index].unshift('x' + mult);
                    if (details.context.final_bs_v) {
                        details.context.final_bs_v[reel_index].unshift(0)
                    }
                }
                if (details.context.new_multipliers) {
                    details.context.new_multipliers.forEach(function(new_mult) {
                        return new_mult['pos'][1] += 1
                    })
                }
            }
            ...
            if (details.state == 'bonus' && details.context.rounds_left == 0) {
                game_boards.push(details.context.board);
                var mysterys = {
                    'trigger': 'final_bonus_board',
                    'positions': [],
            ...
            snap.click(function(e) {
                ...
                if (counter === 1) {
                    ...
                    bs_v = details.context.final_bs_v
                } else {
        ...
        value: function board(snap, _board) {
            ...
            var _loop = function _loop(_i2) {
                var _loop2 = function _loop2(_j) {
                    imageUrl = _this3.url(_board[_i2][_j] + '.png');
                    if (_this3.details.state === 'bonus' && ![11, 12].includes(_board[_i2][_j])) {
                        imageUrl = _this3.url('0.png')
                    }
                    if (_this3.details.state === 'freespins' && _board[_i2][_j] === 10) {
                        imageUrl = _this3.url('10.png')
                    }
                    ...
                    if (_board[_i2][_j] === 12) {
                        ...
                        try {
                            for (var _iterator = _this3.details.context.new_multipliers[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                                var new_mult = _step.value;
                                if (new_mult['pos'][0] === parseInt(_i2) & new_mult['pos'][1] === parseInt(_j)) {
                                    value = 'x' + new_mult['val'];
                                    break
                                }
                            }
                        } catch (err) {
                    ...
                    } else if (_this3.details.state === 'freespins' && _board[_i2][_j] === 9) {
                        value = 'x' + _this3.details.context.wild_mult
                    } else if (bs_v != undefined) {
                        value = bs_v[_i2][_j]
                    } else if (_this3.details.context.bs_v != undefined) {
                        value = _this3.details.context.bs_v[_i2][_j]
                    } else if (_this3.details.context.bonus_symbols_v != undefined) {
                        value = _this3.details.context.bonus_symbols_v[_i2][_j]
                    } else {
                        return 'continue'
                    }
                    ...
                    if (value != 0 && [0, 9, 11, 12].includes(_board[_i2][_j])) {
                        ...
                        if (value == 'mini') {
                            ...
                            if (_this3.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MULT_70)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.BS_MULT_40)
                            }
                        } else {
                            console.log(value);
                            console.log(number_value_in_string);
                            if (value >= 1000000000) {
                                if (_this3.width > 200) {
