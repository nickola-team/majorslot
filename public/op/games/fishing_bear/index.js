...
var CONDITION = {
    ...
    WIN: "win",
    WIN_ALL: "win_all",
    WIN_ADDITIONAL_FS: "win_additional_fs",
    WIN_SCATTERS: "win_scatters",
    BUY_BONUS: "bonus_buy",
    REWIND_PREVIEW: "rewind_preview",
    REWIND: "rewind",
    REWIND_COLLECT: "rewind_collect",
    REWIND_RESULT: "rewind_result",
    BEAR_COLLECT: "bear_collect",
...
var CONFIG = {
    premods: (_premods = {}, _defineProperty(_premods, PREMOD.REWIND, {
        condition: CONDITION.REWIND_RESULT
    }), _defineProperty(_premods, PREMOD.COLLECT, {
        condition: CONDITION.REWIND_RESULT
    }), _premods),
    states: (_states = {}, _defineProperty(_states, GAME.SPINS, [{
        ...
        condition: CONDITION.REWIND_PREVIEW,
        pages: [PAGE.REWIND_PREVIEW]
    }, {
        condition: CONDITION.REWIND_COLLECT,
        pages: [PAGE.REWIND_COLLECT]
    }, {
        condition: CONDITION.REWIND,
        pages: [PAGE.REWIND]
    }, {
        condition: CONDITION.REWIND_RESULT,
        pages: [PAGE.REWIND_RESULT],
        enum: true
    }, {
        condition: CONDITION.BEAR_COLLECT,
        pages: [PAGE.BEAR_COLLECT]
    }, {
        condition: CONDITION.WIN,
        pages: [PAGE.WIN],
        enum: true
    }, {
        condition: CONDITION.WIN_SCATTERS,
        pages: [PAGE.GAME_FREESPINS]
    }, {
        ...
        condition: CONDITION.INFO_FREESPINS_OUTRO,
        pages: [PAGE.INFO_FREESPINS_OUTRO]
    }]), _defineProperty(_states, GAME.FREESPINS, [{
        condition: CONDITION.WIN_ALL,
        pages: [PAGE.WIN_ALL]
    }, {
        condition: CONDITION.REWIND_PREVIEW,
        ...
        condition: CONDITION.WIN_ADDITIONAL_FS,
        pages: [PAGE.INFO_FREESPINS_ADDITIONAL]
    }, {
        condition: CONDITION.NOWIN,
        pages: [PAGE.NOWIN]
    }, {
        condition: CONDITION.INFO_FREESPINS_INTRO,
        pages: [PAGE.INFO_FREESPINS_INTRO]
    }]), _states),
...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function board(snap, pageData) {
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    ...
                    if (_this3.isBonusSymbol(id) && _this3.jackpotTypeGet(bet, bs, i, j) === "mini") imageName = "sym12_mn.png";
                    if (_this3.isBonusSymbol(id) && _this3.jackpotTypeGet(bet, bs, i, j) === "minor") imageName = "sym12_mr.png";
                    if (_this3.isBonusSymbol(id) && _this3.jackpotTypeGet(bet, bs, i, j) === "major") imageName = "sym12_mj.png";
                    if (_this3.isBonusSymbol(id) && _this3.jackpotTypeGet(bet, bs, i, j) === "grand") imageName = "sym12_gr.png";
                    imageUrl = _this3.url(imageName);
                    var yShift = id === 11 ? _this3.cellHeigthSize * 0.093 : 0;
                    var cellX = _this3.cellWidthSize * i;
                    var cellY = _this3.cellHeigthSize * j - yShift;
            ...
            if (state === GAME.SPINS || state === GAME.FREESPINS) {
                var line = lines && lines.find(function(line) {
                    return line.symbol === _this3.scat_id && line.value > 0
                });
                if (line) {
                    var sc_value = line.value;
                    this.boardJackpotSymbolDraw(group, sc_value, 1, bet, 2, 1)
                }
                bs.forEach(function(bsSymbol) {
                    var column = bsSymbol.reel;
                    var row = bsSymbol.position;
                    var value = bsSymbol.value;
                    var id = board[column][row];
                    var multi = 1;
                    if (!_this3.isBonusSymbol(id)) return;
                    if (!!_this3.jackpotTypeGet(bet, bs, column, row)) return;
                    if (!value) return;
        ...
        value: function boardJackpotSymbolDraw(group, value, multi, bet, column, row) {
            var cellWidth = this.cellWidth;
            var cellHeight = this.cellWidth;
            var cellHeightPosition = cellHeight * 0.33;
            var cellHeightPositionText = cellHeight * 0.5;
            var rect_x = this.cellWidthSize * column;
            var rect_y = this.cellHeigthSize * row + cellHeightPosition;
            var rect_width = cellWidth;
            var rect_height = this.cellHeight * 0.33;
            var text_x = rect_x + rect_width * 0.5;
            var text_y = this.cellHeigthSize * row + cellHeightPositionText;
        ...
        value: function drawe_border_win_symbols(snap, line) {
            ...
            for (var i = 0; i < line.positions.length; ++i) {
                ...
                if (!isItemInArray(this.bordered_cells, cell)) {
                    ...
                    var y = cell[1] * this.cellHeigthSize;
        ...
        value: function screenBetGet(details) {
            var context = details.context,
                round_bet = details.round_bet,
                round_bet_not_restored = details.round_bet_not_restored,
                state = details.state;
            var last_bet = context.last_bet,
                bet_per_line = context.bet_per_line;
            var bet = round_bet || round_bet_not_restored || 0;
            if (state === "spins") {
                if (last_bet > 0 && bet_per_line > 0 && last_bet !== bet_per_line) {
                    bet = bet * last_bet / bet_per_line
                }
            }
            return bet
        }
        ...
        value: function screenContextGet(details) {
            ...
            var board = context.board,
                bs = context.bs,
                rewind_collect = context.rewind_collect;
            rewind_collect && rewind_collect.forEach(function(symbolData) {
                if (!symbolData.id) symbolData.id = _this4.scat_id
            });
            if (last_action === "restore") {
                bs && bs.forEach(function(symbolData) {
                    var id = symbolData.id,
                        reel = symbolData.reel,
                        position = symbolData.position;
                    board[reel][position] = id || _this4.bonus_id
                });
                rewind_collect && rewind_collect.forEach(function(symbolData) {
                    var id = symbolData.id,
                        reel = symbolData.reel,
                        position = symbolData.position;
                    board[reel][position] = id || _this4.scat_id
                })
            }
            return context
        }
        ...
        value: function screenConditionsGet(screenData) {
            ...
            conditionCandidates.map(function(condition) {
                var isPass = _this5.screenConditionIsPass(condition.condition, screenData);
                if (!isPass) return;
        ...
        value: function screenPassMethodGet(condition) {
            switch (condition) {
                ...
                case CONDITION.REWIND_PREVIEW:
                    return this.passRewindPreview.bind(this);
                case CONDITION.REWIND:
                    return this.passRewind.bind(this);
                case CONDITION.REWIND_COLLECT:
                    return this.passRewindCollect.bind(this);
                case CONDITION.REWIND_RESULT:
                    return this.passBonusSymbolsResult.bind(this);
                case CONDITION.BEAR_COLLECT:
                    return this.passBearCollect.bind(this);
                case CONDITION.WIN_ALL:
                    return this.passWinAll.bind(this);
                case CONDITION.WIN:
                    return this.passWin.bind(this);
                case CONDITION.WIN_SCATTERS:
                    return this.passFreeSpinsGameWin.bind(this);
                case CONDITION.BUY_BONUS:
                    return this.passBonusGameBuy.bind(this);
                case CONDITION.WIN_ADDITIONAL_FS:
                    return this.passAdditionalFreeSpins.bind(this);
                case CONDITION.NOWIN:
                    return this.passNoWin.bind(this);
            }
        }
    }, {
        key: "sortWinlines",
        ...
        value: function passDefault(screenData) {
            ...
            return isReel && !isInfo && (win === 0 || restore)
        }
    }, {
        key: "passNoWin",
        value: function passNoWin(screenData) {
            var win = screenData.win,
                restore = screenData.restore,
                context = screenData.context,
                action = screenData.action;
            var is_collect = context.is_collect;
            var isReel = this.passReelGameType(screenData);
            var isInfo = this.passInfo(screenData);
            var isWinlines = context.winlines && context.winlines.length > 0;
            var isFSInit = action === "freespin_init";
            var isBuyBonusInit = action === "buy_spin";
            return isReel && !isInfo && !restore && win === 0 && !isWinlines && !isFSInit && !is_collect && !isBuyBonusInit
        }
    }, {
        key: "passWin",
        ...
        value: function passWinAll(screenData) {
            ...
            var winlines = context.winlines,
                winscatters = context.winscatters,
                rewind_bs = context.rewind_bs,
                rewind_collect = context.rewind_collect;
            var isReel = this.passReelGameType(screenData);
            var isWinlines = winlines && winlines.length > 1;
            var isWinline = winlines && winlines.length === 1 ? 1 : 0;
            var isWinscatters = winscatters && Array.isArray(winscatters) && winscatters.filter(function(data) {
                return data.amount > 0
            }).length;
            var isRewind = rewind_bs && rewind_bs.length > 0 ? 1 : 0;
            var isRewindCollect = rewind_collect && rewind_collect.length > 0 ? 1 : 0;
            var bonusSymbolsSum = isRewind * 2 + isRewindCollect * 2 || isWinscatters;
            var isWinSum = isWinline + bonusSymbolsSum;
            return isReel && win > 0 && state !== GAME.BONUS && (isWinlines || isWinSum > 1)
        }
    }, {
        key: "passFreeSpinsGameWin",
        value: function passFreeSpinsGameWin(screenData) {
            var action = screenData.action,
                restore = screenData.restore,
                context = screenData.context;
            ...
            var list = winscatters && winscatters.find(function(data) {
                return data.trigger === GAME.FREESPINS && data.freespins > 0
            });
            var isFSScatters = isWinscatters && !!list;
            return isReel && !restore && !isInfo && isFSScatters && action === "spin"
        }
    }, {
        key: "passAdditionalFreeSpins",
        value: function passAdditionalFreeSpins(screenData) {
            var win = screenData.win,
                restore = screenData.restore,
                context = screenData.context;
            var winscatters = context.winscatters;
            ...
            var isFSScatters = isWinscatters && !!list;
            return isReel && !restore && !isInfo && isFSScatters
        }
    }, {
        ...
        value: function passBonusGameBuy(screenData) {
            ...
            var round_win = details.round_win,
                round_bet = details.round_bet,
                round_started = details.round_started;
            var bs_count = context.bs_count,
                from_buy_spin = context.from_buy_spin;
            var isBonusBuy = action === "buy_spin" && round_started === true;
            var isBonusBuyResore = action === "restore" && round_win === 0 && round_bet === 0 && bs_count === 0 && from_buy_spin === true;
            var isReel = this.passReelGameType(screenData);
            var isInfo = this.passInfo(screenData);
            return isReel && (isInfo || action === "restore") && (isBonusBuy || isBonusBuyResore)
        }
    }, {
        key: "passRewindPreview",
        value: function passRewindPreview(screenData) {
            var isRewind = this.passRewind(screenData);
            var isRewindCollect = this.passRewindCollect(screenData);
            return isRewind || isRewindCollect
        }
    }, {
        key: "passRewind",
        value: function passRewind(screenData) {
            var restore = screenData.restore,
                context = screenData.context;
            var is_collect = context.is_collect,
                rewind_bs = context.rewind_bs;
            var isInfo = this.passInfo(screenData);
            var isReel = this.passReelGameType(screenData);
            var isRewind = rewind_bs && Array.isArray(rewind_bs) && rewind_bs.length > 0;
            return !restore && isReel && !isInfo && is_collect && isRewind
        }
    }, {
        key: "passRewindCollect",
        value: function passRewindCollect(screenData) {
            var restore = screenData.restore,
                context = screenData.context;
            var is_collect = context.is_collect,
                rewind_collect = context.rewind_collect;
            var isInfo = this.passInfo(screenData);
            var isReel = this.passReelGameType(screenData);
            var isRewindCollect = rewind_collect && Array.isArray(rewind_collect) && rewind_collect.length === 1;
            return !restore && isReel && !isInfo && is_collect && isRewindCollect
        }
    }, {
        key: "passBonusSymbolsResult",
        value: function passBonusSymbolsResult(screenData) {
            var restore = screenData.restore,
                context = screenData.context;
            var is_collect = context.is_collect,
                bs = context.bs;
            var isInfo = this.passInfo(screenData);
            var isReel = this.passReelGameType(screenData);
            var isBS = bs && Array.isArray(bs) && bs.length > 0;
            return !restore && isReel && !isInfo && is_collect && isBS
        }
    }, {
        key: "passBearCollect",
        value: function passBearCollect(screenData) {
            var action = screenData.action,
                restore = screenData.restore,
                context = screenData.context;
            var is_collect = context.is_collect,
            ...
            var isInfo = this.passInfo(screenData);
            var isReel = this.passReelGameType(screenData);
            var isNoBonus = !bs && !rewind_bs && !rewind_collect;
            var isFreespinsAction = action && action.indexOf("freespins") >= 0;
            var isBear = board[2][1] === this.scat_id;
            var isWinlines = winlines && winlines.length > 0;
            var isWinscatters = winscatters && !!winscatters.find(function(data) {
                return data.amount > 0
            });
            return !restore && isReel && !isInfo && is_collect && isNoBonus && isBear && isFreespinsAction && !isWinlines && !isWinscatters
        }
    }, {
        key: "passScatterWin",
            ...
            if (premodOptions.length > 0) {
                ...
                pageList.map(function(page) {
                    premodOptions.map(function(option) {
                        if (completeList.includes(option.name)) {
                            option.makeAftermod(page, screenData)
                        } else {
                            var isComplete = option.makePremod(page, screenData);
        ...
        value: function premodListGet(screenData) {
            ...
            Object.keys(config).map(function(name) {
                ...
                premods.push({
                    name: name,
                    premod: premod,
                    makePremod: methods.makePremod,
                    makeAftermod: methods.makeAftermod
                })
            });
            return premods
        }
        ...
        value: function premodMethodGet(premodName) {
            switch (premodName) {
                case PREMOD.REWIND:
                    return {
                        check: this.premodRewindCheck.bind(this),
                        makePremod: this.premodRewindMake.bind(this),
                        makeAftermod: this.aftermodRewindMake.bind(this)
                    };
                case PREMOD.REWIND_AND_COLLECT:
                    return {
                        check: this.premodRewindCheck.bind(this),
                        makePremod: this.premodRewindMake.bind(this),
                        makeAftermod: this.aftermodRewindMake.bind(this)
                ...
                case PREMOD.COLLECT:
                    return {
                        check: this.premodCollectCheck.bind(this),
                        makePremod: this.premodCollectMake.bind(this),
                        makeAftermod: this.aftermodCollectMake.bind(this)
                    };
            }
            return undefined
        }
    }, {
        key: "premodRewindCheck",
        value: function premodRewindCheck(screenData) {
            var context = screenData.context,
                state = screenData.state,
                restore = screenData.restore;
            var board = context.board,
                is_collect = context.is_collect,
                rewind_bs = context.rewind_bs;
            var isRewind = rewind_bs && Array.isArray(rewind_bs) && rewind_bs.length > 0;
            return !restore && is_collect && isRewind
        }
    }, {
        key: "premodRewindMake",
        value: function premodRewindMake(page, screenData) {
            var name = page.name;
            return name === PAGE.REWIND
        }
    }, {
        key: "aftermodRewindMake",
        value: function aftermodRewindMake(page, screenData) {
            var context = screenData.context;
            var rewind_bs = context.rewind_bs;
            var name = page.name,
                board = page.board;
            if (!board) return false;
            if (!rewind_bs || !Array.isArray(rewind_bs)) return;
            var positions = this.rewindPositionsGet(rewind_bs);
            var rewindBoard = this.swapSymbolsOnBoardAtPositions(board, positions, this.bonus_id);
            page.board = rewindBoard;
            return name === PAGE.REWIND
        }
    }, {
        key: "premodCollectCheck",
        value: function premodCollectCheck(screenData) {
            var context = screenData.context,
                state = screenData.state,
                restore = screenData.restore;
            var board = context.board,
                is_collect = context.is_collect,
                rewind_collect = context.rewind_collect;
            var isRewind = rewind_collect && Array.isArray(rewind_collect) && rewind_collect.length === 1;
            return !restore && is_collect && isRewind
        }
    }, {
        key: "premodCollectMake",
        value: function premodCollectMake(page, screenData) {
            var name = page.name;
            return name === PAGE.REWIND_COLLECT
        }
    }, {
        key: "aftermodCollectMake",
        value: function aftermodCollectMake(page, screenData) {
            var context = screenData.context;
            var rewind_bs = context.rewind_bs,
                rewind_collect = context.rewind_collect;
            var name = page.name,
                board = page.board;
            if (!board) return false;
            if (!rewind_collect || !Array.isArray(rewind_collect)) return;
            var positions = this.rewindPositionsGet(rewind_collect);
            var rewindAllBoard = this.swapSymbolsOnBoardAtPositions(board, positions, this.scat_id);
            if (rewind_bs && rewind_bs.length > 0) {
                var _positions = this.rewindPositionsGet(rewind_bs);
                rewindAllBoard = this.swapSymbolsOnBoardAtPositions(rewindAllBoard, _positions, this.bonus_id)
            }
            page.board = rewindAllBoard;
            return name === PAGE.REWIND_COLLECT
        }
    }, {
        key: "pagesGet",
        ...
        value: function pageListGet(screenData) {
            ...
            screenData.conditions.map(function(condition) {
                condition.pages.map(function(pageName) {
                    var pageData = _this7.pagesByNameGet({
                        pageName: pageName,
                        condition: condition,
            ...
            if (screenData.conditions.length === 0) {
                var pageDefault = this.pagesByNameGet({
                    pageName: "default",
                    condition: "default",
                    screenData: screenData
                });
                list.push(pageDefault)
            }
            return list
        }
    }, {
        ...
        value: function pagesUpdateEnumerates(pageDataList) {
            ...
            pageDataList.map(function(pageData) {
                if (pageData.condition.enum) pageCount++;
        ...
        value: function pagesByNameGet(pageOptions) {
            ...
            switch (pageName) {
                case PAGE.REWIND_PREVIEW:
                    return this.pageRewindPreview(pageOptions);
                case PAGE.REWIND:
                    return this.pageRewind(pageOptions);
                case PAGE.REWIND_COLLECT:
                    return this.pageRewindCollect(pageOptions);
                case PAGE.REWIND_RESULT:
                    return this.pageRewindResult(pageOptions);
                case PAGE.BEAR_COLLECT:
                    return this.pageBearCollect(pageOptions);
                ...
                case PAGE.INFO_FREESPINS_INTRO:
                    return this.pageFreeSpinsInfo(pageOptions);
        ...
        value: function pageDefault(pageOptions) {
            ...
            var board = this.boardClone(screenData.context.board);
        ...
        key: "pageFreeSpinsInfo",
        value: function pageFreeSpinsInfo(pageOptions) {
            var pageData = this.pageInfo(pageOptions);
            var screenData = pageOptions.screenData;
            var context = screenData.context;
            var rounds_left = context.rounds_left,
                rounds_granted = context.rounds_granted;
            var info = CONFIG.info[pageData.name];
            var text = info.text;
            if (text.indexOf("{value}") >= 0) {
                var freeSpinsValue = rounds_left || rounds_granted || 8;
                text = text.replace("{value}", freeSpinsValue);
                pageData.info.text = text
            }
            return pageData
        }
    }, {
        ...
        value: function pageFreespinsWinInfo(pageOptions) {
            ...
            var isFSScatters = isWinscatters && !!list && list.length > 0;
            if (isFSScatters) {
                var fsLine = list[0];
                value = fsLine.freespins || value
            }
            pageData.info.text = "+" + value + " FreeSpins";
            return pageData
        }
    }, {
        key: "pageWinAll",
        value: function pageWinAll(pageOptions) {
            ...
            var pageData = this.pageDefault(pageOptions);
            var board = pageData.board;
            var lines = context.winlines ? context.winlines.map(function(line) {
                return line
            }) : [];
            var symbolDataList = [];
            if (this.isCollectInBonusSymbols(context)) {
                rewind_collect.forEach(function(symbolData) {
                    return symbolDataList.push(symbolData)
                })
            } else if (this.isCollectInWinscatters(context)) {
                symbolDataList.push({
                    id: this.scat_id,
                    reel: 2,
                    position: 1,
            ...
            if (this.isRewindInBonusSymbols(context)) {
                rewind_bs.forEach(function(symbolData) {
                    return symbolDataList.push(symbolData)
                })
            }
            if (!!symbolDataList.find(function(symbolData) {
                    return symbolData.id === _this8.scat_id
                }) && bs && bs.length > 0) {
                bs.forEach(function(symbolData) {
                    var isInSDL = symbolDataList.find(function(data) {
                        return data.position === symbolData.position && data.reel === symbolData.reel
                    });
                    if (isInSDL) return;
                    symbolDataList.push(symbolData)
                })
            }
            if (symbolDataList.length > 0) {
                symbolDataList.forEach(function(symbolData) {
                    var id = symbolData.id,
                        reel = symbolData.reel,
                        position = symbolData.position;
                    board[reel][position] = id
                });
                var coins = bs && bs.reduce(function(acc, data) {
                    return acc + data.value
                }, 0);
                var line = this.lineCreateBySymbolDataList(symbolDataList);
                line.symbol = this.scat_id;
                line.value = coins || 0;
                lines.push(line)
            }
        ...
        value: function pageWinList(pageOptions) {
            ...
            context.winlines.map(function(winline) {
                ...
                var title = _this9.titleWinPrefixAdd(pageData.title, win);
                var data = {
                    title: title,
                    name: name,
                    action: action,
        ...
        value: function pageFreespinsGameWin(pageOptions) {
            ...
            var line = context.winscatters.find(function(line) {
                return line.symbol === _this10.scat_id
            });
        ...
        key: "pageRewindPreview",
        value: function pageRewindPreview(pageOptions) {
            var pageData = this.pageDefault(pageOptions);
            var name = pageData.name;
            pageData.page = {
                screen: name,
        ...
        key: "pageRewind",
        value: function pageRewind(pageOptions) {
            var screenData = pageOptions.screenData;
            var context = screenData.context;
            var rewind_bs = context.rewind_bs;
            var pageData = this.pageDefault(pageOptions);
            ...
            pageData.title = this.titleTextPrefixAdd(pageData.title, "Fish rewind");
            return pageData
        }
    }, {
        key: "pageRewindCollect",
        value: function pageRewindCollect(pageOptions) {
            var screenData = pageOptions.screenData;
            var context = screenData.context;
            var rewind_collect = context.rewind_collect;
            var pageData = this.pageDefault(pageOptions);
            var board = pageData.board;
            var symbolDataList = [];
            if (this.isCollectInBonusSymbols(context)) {
            ...
            pageData.title = this.titleTextPrefixAdd(pageData.title, "Wild Rewind");
            return pageData
        }
    }, {
        key: "pageRewindResult",
        value: function pageRewindResult(pageOptions) {
            var screenData = pageOptions.screenData;
            var context = screenData.context;
            var rewind_bs = context.rewind_bs,
                rewind_collect = context.rewind_collect,
            ...
            var coins = symbolDataList.reduce(function(acc, data) {
                return acc + data.value
            }, 0);
            var line = this.lineCreateBySymbolDataList(symbolDataList);
            line.value = coins || 0;
            var collectTitle = "Collect";
            if (coins > 0) collectTitle += ": " + this.coins(coins);
            pageData.lines = [line];
            pageData.title = this.titleTextPrefixAdd(pageData.title, collectTitle);
            return pageData
        }
    }, {
        key: "pageBearCollect",
        value: function pageBearCollect(pageOptions) {
            var screenData = pageOptions.screenData;
            var context = screenData.context;
            var pageData = this.pageDefault(pageOptions);
            var symbolDataList = [{
            ...
            var line = this.lineCreateBySymbolDataList(symbolDataList);
            pageData.lines = [line];
            pageData.title = this.titleWinPrefixAdd(pageData.title, coins);
            return pageData
        }
    }, {
        key: "drawPage",
        ...
        value: function drawTitle(screenData, pageData) {
            ...
            var isPages = page && !page.screen && page.total > 1 && (page.current > 0 || page.current === "all");
        ...
        value: function drawBorders(group, screenData, pageData) {
            ...
            lines.filter(function(line) {
                ...
                return _this11.drawe_border_win_symbols(bordersGroup, line)
            });
        ...
        value: function drawLines(group, screenData, pageData) {
            ...
            lines.filter(function(line) {
                ...
                return _this12.drawe_line_patern(linesGroup, line)
            });
        ...
        key: "titleTextPrefixAdd",
        value: function titleTextPrefixAdd(title, text) {
            if (!text) text = "";
            var index = title.indexOf(", Win:");
            if (index >= 0) {
                return title.substring(0, index) + ", " + text
            }
            return title + ", " + text
        }
    }, {
        ...
        value: function background(group, backgroundName) {
            if (backgroundName === GAME.FREESPINS) {
        ...
        value: function make_award(details) {
            ...
            var group = snap.group();
            if (details.send_transaction) {
                group.text(parseInt(this.width / 2), parseInt(this.height * 0.6), this.coins(details.amount)).attr(Drawer.STYLE_AWARD_TEXT_CENTER_WHITE)
            } else {
                group.text(parseInt(this.width / 2), parseInt(this.height * 0.6), "Manual Mode").attr(Drawer.STYLE_AWARD_TEXT_CENTER_WHITE)
            }
            this._calculateSizes(this.cols, this.rows);
            this._panel(snap, "award", "");
            return snap.node
        ...
        key: "boardClone",
        value: function boardClone(board) {
            var result = board.map(function(reel) {
        ...
        key: "lineCreateBySymbolDataList",
        value: function lineCreateBySymbolDataList(symbolDataList) {
            var symbol = 0;
            var positions = [];
            var occurrences = 0;
            symbolDataList.forEach(function(symbolData) {
                symbol = symbolData.id;
                positions.push([symbolData.reel, symbolData.position]);
                occurrences++
            });
            return {
                symbol: symbol,
        ...
        key: "isRewindInBonusSymbols",
        value: function isRewindInBonusSymbols(context) {
            if (!context) return false;
            var list = this.rewindPositionsGet(context.rewind_bs);
            return list.length > 0
        }
    }, {
        key: "isCollectInBonusSymbols",
        value: function isCollectInBonusSymbols(context) {
            if (!context) return false;
            var list = this.rewindPositionsGet(context.rewind_collect);
            return list.length > 0
        }
    }, {
        key: "isCollectInWinscatters",
        value: function isCollectInWinscatters(context) {
            var _this13 = this;
            if (!context) return false;
            if (!context.winscatters) return false;
            return !!context.winscatters.find(function(data) {
                return data.symbol === _this13.scat_id
            })
        }
    }, {
        key: "rewindPositionsGet",
        value: function rewindPositionsGet(symbolDataList) {
            var list = [];
            if (!symbolDataList || !Array.isArray(symbolDataList) || symbolDataList.length === 0) return list;
            symbolDataList.forEach(function(symbolData) {
                return list.push([symbolData.reel, symbolData.position])
            });
            return list
        }
    }, {
        ...
        key: "jackpotTypeGet",
        value: function jackpotTypeGet(bet, bs, column, row) {
            var type = undefined;
            bs && bs.forEach(function(symbolData) {
                if (symbolData.reel !== column || symbolData.position !== row) return;
                var value = symbolData.value;
                if (bet * 10 === value) type = "mini";
                if (bet * 20 === value) type = "minor";
                if (bet * 50 === value) type = "major";
                if (bet * 1000 === value) type = "grand"
            });
            return type
        }
    }]);
...
Drawer.STYLE_AWARD_TEXT_CENTER_WHITE = {
    "text-anchor": "middle",
    "fill": "#fff",
    "font-size": "1.4em"
};
