...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && this.get_pearls_count(details, [11]) === 20 || details.state == 'freespins' && details.context.winscatters.length > 0 || details.state == 'bonus' && details.context.last_respin && this.is_extra_respins(details)
        }
    }, {
        key: 'get_arr_sum',
        ...
        value: function get_pearls_count(details) {
            var bonus_symbols = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [11, 12, 13];
            var count = 0;
        ...
        value: function make_spins(details) {
            ...
            if (this.get_pearls_count(this.details) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                ...
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if ([11, 12, 13].includes(game_board[i][j])) {
                ...
                for (var _i in game_board) {
                    for (var _j in game_board[_i]) {
                        if ([11, 13].includes(game_board[_i][_j])) {
            ...
            var margin = 0;
            this._combs(combsGroup, combs, details);
            if (combs.length > 1 || open_mystery) {
        ...
        value: function board(snap, _board) {
            ...
            if (this.is_boost(this.details)) {
                ...
                for (var _i4 in _board) {
                    for (var _j2 in _board[_i4]) {
                        if ([11, 13].includes(_board[_i4][_j2])) {
            ...
            var group = snap.group();
            var imageUrl = void 0;
            var bs_v = this.details.context.bs_v;
                ...
                var _loop2 = function _loop2(_j3) {
                    ...
                    if (is_show_mystery_case) {
                        _this3.details.context.mystery_pos.forEach(function(pos) {
                            if (_i5 == pos[0] && _j3 == pos[1]) {
                                show_mystery = true
                            }
                        })
                    }
                    imageUrl = _this3.url(_board[_i5][_j3] + '.png');
                    if (_this3.details.state === 'bonus' && bonus_symbols.indexOf(_board[_i5][_j3]) == -1) {
                        imageUrl = _this3.url('0.png')
                    }
                    if (show_mystery) {
                        imageUrl = _this3.url('12.png')
                    }
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * _i5, (_this3.cellHeight + _this3.cellOffsetY) * _j3, _this3.cellWidth, _this3.cellHeight);
                    ...
                    if (value != 0 && !show_mystery && _board[_i5][_j3] == 11) {
                        var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i5 - _this3.cellOffsetX;
                        var rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j3 + _this3.cellHeight / 3;
                        var rect_width = _this3.cellWidth + 2 * _this3.cellOffsetX;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = (_this3.cellHeight + _this3.cellOffsetY) * _j3 + _this3.cellHeight / 2;
                        ...
                        if (value == 'mini') {
                            ...
                            if (_this3.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MIDI_70)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MIDI_40)
                            }
                        } else if (value == 'major') {
                            if (_this3.width > 200) {
                    ...
                    if (_board[_i5][_j3] == 13 && boost_mult_value && !show_mystery) {
                        var _rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i5 + _this3.cellWidth / 1.8;
                        var _rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j3 - _this3.cellOffsetY + 0.5;
                        var _rect_width = _this3.cellWidth / 2.3 + 2 * _this3.cellOffsetX;
                        var text_x_x = _rect_x + _rect_width / 2;
                        var text_x_y = (_this3.cellHeight + _this3.cellOffsetY) * _j3 + _this3.cellOffsetY + _this3.cellHeight * 0.18;
