...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        key: 'get_bs_winscatters',
        value: function get_bs_winscatters(details) {
            var board = details.context.board;
            var possitions = [];
            var bs_count = 0;
            for (var i in board) {
                for (var j in board[i]) {
                    if (board[i][j] === 11 || board[i][j] === 12) {
                        possitions.push([parseInt(i), parseInt(j)]);
                        bs_count += 1
                    }
                }
            }
            var result = null;
            if (bs_count >= 6 && (details.state === 'spins' || details.state === 'freespins')) {
                result = [{
                    'amount': details.context.win,
                    'occurrences': bs_count,
        ...
        value: function make_spins(details) {
            ...
            var combs = [].concat(details.context.winlines, details.context.winscatters);
            var bs_winscatters = this.get_bs_winscatters(details);
            if (bs_winscatters) {
                combs = combs.concat(bs_winscatters)
            }
            if (!combs) {
                combs = []
            }
            ...
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + this.boardMarginTop));
            ...
            snap.click(function(e) {
                ...
                board.transform('T' + _this2.boardMarginLeft + ',' + (_this2.panelHeight + _this2.boardMarginTop))
            });
        ...
        value: function board(snap, _board) {
            ...
            var bs_winscatters = this.get_bs_winscatters(this.details);
            if (bs_winscatters) {
                combs = combs.concat(bs_winscatters)
            }
            if (!combs) {
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    ...
                    if (_this3.details.context.bs_v != undefined) {
                        value = _this3.details.context.bs_v[i][j]
                    } else {
                        value = _this3.details.context.bonus_symbols_v[i][j]
                    }
                    var high_bonus = '';
                    if (_board[i][j] == 11 && ['mini', 'minor', 'major'].includes(value)) {
                        high_bonus = '_' + value
                    }
                    imageUrl = _this3.url(_board[i][j] + high_bonus + '.png');
                    if (_this3.details.state === 'bonus' && bonus_symbols.indexOf(_board[i][j]) == -1) {
                    ...
                    if (value != 0 && (_board[i][j] == 11 || _board[i][j] == 12)) {
                        var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * i - _this3.cellOffsetX;
        ...
        value: function _combs(snap, combs, details) {
            ...
            this._panel(group, labelLeft, labelRight);
            for (var i = 0; i < combs.length; ++i) {
                this.drawe_border_win_symbols(group, combs[i]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + this.boardMarginTop))
            }
            for (var _i = 0; _i < combs.length; ++_i) {
                this.drawe_line_patern(group, combs[_i]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + this.boardMarginTop))
            }
            return group
        }
    }, {
        ...
        value: function _comb(snap, combs, comb, details) {
            ...
            var labelRight = combs.indexOf(comb) + 1 + '/' + combs.length;
            this._panel(group, labelLeft, labelRight);
            this.drawe_border_win_symbols(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + this.boardMarginTop));
            this.drawe_line_patern(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + this.boardMarginTop));
            return group
        }
    }]);
    return Drawer
