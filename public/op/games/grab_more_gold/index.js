...
var SYMBOLS = {
    ...
    MONEY_SUPER_ID: 14,
    JACKPOT_ID: 15,
    BONUS_ID_ARRAY: [13, 14, 15]
};
var PROGRESSIVE_FS_MULTS = [1, 2, 3, 10];
var JACKPOT_NAMES = ['grand', 'major', 'minor', 'mini'];
var BG_COLORS = {
    SPINS: '#B8860B',
    FREESPINS: '#4682B4',
    SUPER_FREESPINS: '#6B8E23'
};
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        key: 'title',
        value: function title(details) {
            if (details.last_action == 'freespin_init' || details.last_action == 'freespin_stop') details.last_action;
            else if (details.state == 'freespins') {
                return this._isSuperFreespins() ? 'Super FS(' + details.context.rounds_left + ')' : 'FS(' + details.context.rounds_left + ')'
            } else {
                return 'Spin'
            }
        }
        ...
        key: '_background',
        value: function _background(snap, state) {
            var color = BG_COLORS.SPINS;
            if (this._isSuperFreespins() && (state === 'freespins' || this.details.last_action === 'freespins_stop')) color = BG_COLORS.SUPER_FREESPINS;
            else if (state === 'freespins' || this.details.last_action === 'freespins_stop') color = BG_COLORS.FREESPINS;
            snap.rect(0, 0, this.width, this.height).attr({
                'fill': color
            })
        }
        ...
        value: function _combs(snap, combs, details) {
            ...
            if (details.last_action === 'respin' && details.context.rounds_left !== 0) {
                ...
                if (this._isFS()) {
                    var game_level = this.details.context.game_level;
                    if (this.details.context.add_fs != undefined) {
                        game_level -= 1
                    }
                    var mult_value = PROGRESSIVE_FS_MULTS[game_level];
                    labelLeft += ', x' + mult_value
                }
            }
            var labelRight = combs.length ? 'all/' + combs.length : '';
            this._panel(group, labelLeft, labelRight);
        ...
        value: function is_extra_snap(details) {
            return details.state == 'freespins' && details.context.add_fs != undefined
        }
    }, {
        key: '_getBonusSymbolCount',
        value: function _getBonusSymbolCount() {
            var count = 0;
            var board = this.details.context.board;
            for (var i in board) {
                for (var j in board[i]) {
                    if (SYMBOLS.BONUS_ID_ARRAY.includes(board[i][j])) {
                        count = count + 1
                    }
                }
            }
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            var roundsGranted = 10;
            var group = combsGroup.group();
            this._background(group, details.state);
            var labelLeft = this._isSuperFreespins() ? 'Additional Super FS!' : 'Additional FS!';
            var labelRight = '';
            ...
            group.text('50%', '55%', '+' + roundsGranted + ' ' + (this._isSuperFreespins() ? 'Super FS' : 'FS')).attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
            group.text('50%', '70%', 'x' + mult_value).attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
            this._panel(group, labelLeft, labelRight);
            return snap
        }
    }, {
        ...
        value: function get_text_for_action(details) {
            var action = details.last_action;
            if (action === 'freespin_stop' && this._isSuperFreespins()) return 'Super FS End';
            if (action === 'freespin_init' && this._isSuperFreespins()) return 'Super FS count: ' + this.details.context.rounds_left;
            if (action === 'freespin_stop') return 'FS End';
            if (action === 'freespin_init') return 'FS count: ' + this.details.context.rounds_left;
            return ''
        }
    }, {
        key: 'make_snap',
        ...
        value: function make_spins(details) {
            ...
            var combs = this._getCombs(game_board);
            var combsGroup = snap.group();
            var combGroup = void 0;
            var margin = 0;
            this._combs(combsGroup, combs, details);
        ...
        value: function board(snap, _board) {
            ...
            var combs = this._getCombs(_board);
            var group = snap.group();
            this._forEachSymbol(_board, function(symbol, col, row) {
                var rect = _this3._getRectDimensions(col, row);
                var textPos = _this3._getTextPosition(col, row);
                var imageUrl = _this3.url(symbol + '.png');
                var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * col, (_this3.cellHeight + _this3.cellOffsetY) * row, _this3.cellWidth, _this3.cellHeight);
                var value = _this3._getSymbolValue(col, row);
                if (combs.length == 1) currentWinLine = 0;
                if (_this3._isFS()) {
                    _this3._createProgressFS(group)
                }
                if (symbol === SYMBOLS.COLLECT_ID && _this3._isFS()) {
                    var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * col - _this3.cellOffsetX + _this3.cellWidth * 0.425;
                    var rect_y = (_this3.cellHeight + _this3.cellOffsetY) * row + _this3.cellHeight * 0.04;
                    var rect_width = _this3.cellWidth * 0.5;
                    var text_x = (_this3.cellWidth + _this3.cellOffsetX) * col - _this3.cellOffsetX + _this3.cellWidth * 0.655;
                    var text_y = (_this3.cellHeight + _this3.cellOffsetY) * row + _this3.cellHeight * 0.23;
                    var mult_list = PROGRESSIVE_FS_MULTS;
                    var game_level = _this3.details.context.game_level;
                    if (_this3.details.context.add_fs != undefined) {
                        game_level -= 1
                    }
                    var mult_value = mult_list[game_level];
                    var finalValue = combs.length - 1 === currentWinLine || currentWinLine === -1;
                    if (symbol === SYMBOLS.COLLECT_ID && mult_value > 1 && finalValue) {
                        group.rect(rect_x, rect_y, rect_width, _this3.cellHeight / 3).attr({
                            'fill': 'white'
                        });
                        if (_this3.width > 200) {
                            group.text(text_x, text_y, 'x' + mult_value).attr(Drawer.MULT_VALUE_50)
                        } else {
                            group.text(text_x, text_y, 'x' + mult_value).attr(Drawer.MULT_VALUE_35)
                        }
                    }
                }
                if (symbol === SYMBOLS.COLLECT_ID && _this3._getBonusSymbolCount() > 0) {
                    var _finalValue = combs.length - 1 === currentWinLine || currentWinLine === -1;
                    var collect_value = _this3.coins(_finalValue ? _this3.details.context.collect_pay : _this3._sumAllBonuses());
                    group.rect(rect.x, rect.y, rect.width, rect.height).attr({
                        'fill': 'white'
                    });
                    group.text(textPos.x, textPos.y, collect_value).attr(_this3._getTextAttr(collect_value)).attr(_finalValue ? {
                        'fill': 'red'
                    } : {})
                }
                if (value && _this3._showValueOnSymbol(symbol)) {
                    group.rect(rect.x, rect.y, rect.width, rect.height).attr({
                        'fill': 'white'
                    });
                    group.text(textPos.x, textPos.y, value).attr(_this3._getTextAttr(value))
                }
                if (currentWinLine !== -1 && combs[currentWinLine]) {
        ...
        key: '_isSuperFreespins',
        value: function _isSuperFreespins() {
            return false
        }
    }, {
        key: '_getCombs',
        value: function _getCombs(board) {
            var combs = [].concat(_toConsumableArray(this.details.context.winlines), _toConsumableArray(this.details.context.winscatters));
            if (this._getBonusSymbolCount() > 0 && this.details.context.collects_on_board > 0) {
                var winmoney = {
                    'trigger': 'collect',
                    'positions': []
                };
                var collect_symbols = [].concat(_toConsumableArray(SYMBOLS.BONUS_ID_ARRAY), [SYMBOLS.COLLECT_ID]);
                this._forEachSymbol(board, function(symbol, col, row) {
                    if (collect_symbols.includes(symbol)) {
                        winmoney.positions.push([col, row])
                    }
                });
                combs.push(winmoney)
            }
            if (this.details.context.collects_on_board > 0 && this._isFS()) {
                var collectors = {
                    'trigger': 'collectors',
                    'positions': []
                };
                this._forEachSymbol(board, function(symbol, col, row) {
                    if (SYMBOLS.COLLECT_ID == symbol) {
                        collectors.positions.push([col, row])
                    }
                });
                combs.push(collectors)
            }
            return combs
        }
    }, {
        key: '_forEachSymbol',
        value: function _forEachSymbol(board, callback) {
            for (var col = 0; col < board.length; col++) {
                for (var row = 0; row < board[col].length; row++) {
                    callback(board[col][row], col, row)
                }
            }
        }
    }, {
        key: '_getTextAttr',
        value: function _getTextAttr(value) {
            var attrValue = void 0;
            if (value < 100000) {
                attrValue = this.width > 200 ? Drawer.PEARL_VALUE_60 : Drawer.PEARL_VALUE_38
            } else {
                attrValue = this.width > 200 ? Drawer.PEARL_VALUE_35 : Drawer.PEARL_VALUE_22
            }
            return attrValue
        }
    }, {
        key: '_getRectDimensions',
        value: function _getRectDimensions(col, row) {
            var yOffset = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0.65;
            var x = (this.cellWidth + this.cellOffsetX) * col - this.cellOffsetX;
            var y = (this.cellHeight + this.cellOffsetY) * row + this.cellHeight * yOffset;
            var width = this.cellWidth + 2 * this.cellOffsetX;
            var height = this.cellHeight / 3;
            return {
                x: x,
                y: y,
                width: width,
        ...
        value: function _getTextPosition(col, row) {
            var yOffset = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0.8;
            var x = (this.cellWidth + this.cellOffsetX) * col + this.cellWidth / 2;
            var y = (this.cellHeight + this.cellOffsetY) * row + this.cellHeight * yOffset;
            return {
                x: x,
                y: y
        ...
        key: '_getSymbolValue',
        value: function _getSymbolValue(col, row) {
            var value = this.details.context.bs_v ? this.details.context.bs_v[col][row] : 0;
            if (typeof value === 'number') return this.coins(value);
            if (JACKPOT_NAMES.includes(value)) return this.coins(this.details.context.bs_values[col][row] * this.details.round_bet);
            return value
        }
    }, {
        key: '_showValueOnSymbol',
        value: function _showValueOnSymbol(symbol) {
            var isShow = symbol === SYMBOLS.MONEY_ID || symbol === SYMBOLS.MONEY_SUPER_ID;
            var isClosedShow = symbol === SYMBOLS.JACKPOT_ID && this.details.context.collects_on_board > 0;
            return isShow || isClosedShow
        }
    }, {
        key: '_sumAllBonuses',
        value: function _sumAllBonuses() {
            var totalSum = 0;
            var arr = this.details.context.bs_v;
            for (var i = 0; i < arr.length; i++) {
                for (var j = 0; j < arr[i].length; j++) {
                    if (JACKPOT_NAMES.includes(arr[i][j])) totalSum += this.details.context.bs_values[i][j] * this.details.round_bet;
                    else totalSum += arr[i][j]
                }
            }
            return totalSum
        }
        ...
        value: function _calculateSizes(cols, rows) {
            ...
            this.cellWidth = parseInt(this.boardHeight / rows, 10);
            this.cellHeight = this.cellWidth;
            this.cellOffsetX = 0;
            this.cellOffsetY = 0;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10);
            if (this._isFS()) this.boardMarginLeft += Math.floor(this.boardHeight / 15) * 2
        }
    }, {
        key: '_isFS',
        value: function _isFS() {
            return this.details.state == 'freespins'
        }
    }, {
        key: '_createProgressFS',
        value: function _createProgressFS(group) {
            var height = Math.floor(this.boardHeight / 15);
            var width = height * 2;
            var xOffset = width / 2;
            var yOffset = height;
            var yOffsetScale = height * 2;
            var scale = 2;
            var x = -xOffset * 4;
            var y = height / 2;
            var level = this.details.context.collects_count || 0;
            for (var i = 12; i >= 1; i--) {
                if (i % 4 === 0) {
                    group.rect(x - xOffset, y, width * scale, height * scale).attr('style', 'outline: thin solid #387920').attr(i <= level ? {
                        'fill': '#f3b33e'
                    } : {
                        'fill': 'white'
                    });
                    if (this.width > 200) {
                        group.text(x + xOffset, y + height, '10FS x' + PROGRESSIVE_FS_MULTS[i / 4]).attr(Drawer.MULT_VALUE_50)
                    } else {
                        group.text(x + xOffset, y + height, '10FS x' + PROGRESSIVE_FS_MULTS[i / 4]).attr(Drawer.MULT_VALUE_35)
                    }
                    y += yOffsetScale
                } else {
                    group.rect(x, y, width, height).attr('style', 'outline: thin solid #387920').attr(i <= level ? {
                        'fill': '#f3b33e'
                    } : {
                        'fill': 'white'
                    });
