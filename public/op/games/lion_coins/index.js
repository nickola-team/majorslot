...
var JACKPOT_ID = 10;
var COLLECTOR_ID = 11;
var BONUS_ID_ARRAY = [9, 10, 11];
var JACKPOT_VALUES = ['minor', 'mini', 'major', 'grand'];
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function make_spins(details) {
            ...
            var combGroup = void 0;
            var margin = 0;
            if (this.get_bonus_symbols_count(this.details) >= 3 && ['spin', 'freespin'].includes(this.details.last_action)) {
                var winbonus = {
                    'trigger': 'bonus',
                    'positions': [],
                    'amount': 0
        ...
        value: function make_bonus(details) {
            ...
            var combs = [];
            if (this.newBonusSymbolsToCollect()) {
                combs = combs.concat([{
                    positions: []
                }])
            }
            ...
            snap.click(function(e) {
                ...
                var cur_board = Array.from(game_board);
                board.remove();
                board = _this3.board(group, cur_board, counter, counter);
        ...
        value: function board(snap, _board) {
            ...
            var _loop = function _loop(_i2) {
                var _loop2 = function _loop2(_j) {
                    var isCollectorSymbol = _board[_i2][_j] === COLLECTOR_ID;
                    var isBonusSymbol = _board[_i2][_j] === BONUS_ID;
                    imageUrl = _this4.url(_board[_i2][_j] + '.png');
                    if (_this4.details.state === 'bonus' && !BONUS_ID_ARRAY.includes(_board[_i2][_j])) {
                        imageUrl = _this4.url('0.png')
                    }
                    if (_board[_i2][_j] === JACKPOT_ID) {
                        imageUrl = _this4.url(JACKPOT_ID + '_' + _this4.details.context.bs_v[_i2][_j] + '.png')
                    }
                    if (newBonusBoard && (_board[_i2][_j] === BONUS_ID || _board[_i2][_j] === JACKPOT_ID)) {
                        imageUrl = _this4.url('0.png')
                    }
                    ...
                    var value = _this4.details.context.bs_v[_i2][_j];
                    if (isBonusGame && isCollectorSymbol && !newBonusBoard) value = _this4.details.context.origin_bs_v[1][_j];
                    if (isBonusSymbol && !newBonusBoard || isCollectorSymbol) {
                        if (typeof value === 'number') {
                            value = (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                            if (value != 0) {
                                var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i2 - _this4.cellOffsetX + 4;
                                var rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j + _this4.cellHeight / 3;
                                var rect_width = _this4.cellWidth + 2 * _this4.cellOffsetX - 8;
