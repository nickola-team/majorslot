...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function _background(snap, state) {
            ...
            } else if (this.get_pearls_count(this.details) === 15) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#6B8E23'
                })
            } else {
        ...
        key: '_calculateSizes',
        value: function _calculateSizes(cols, rows) {
            var isBonus = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
            this.panelHeight = this.height > 150 ? 20 : 12;
            this.panelFont = this.height > 150 ? '1em' : '0.7em';
            this.progressHeight = 3;
            this.boardHeight = this.height - (this.panelHeight + this.progressHeight + 1);
            this.cellHeight = parseInt(this.boardHeight / rows, 10) - (isBonus ? 0 : 2);
            this.cellWidth = !isBonus ? this.cellHeight : this.cellHeight * 2;
            this.cellOffsetX = -1;
            this.cellOffsetY = -1;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10) + 2
        }
        ...
        value: function make_spins(details) {
            var _this2 = this;
            var game_board = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : details.context.board;
            this.cols = game_board.length;
            this.rows = game_board[0].length;
            this._calculateSizes(this.cols, this.rows, details.state == 'bonus');
            var snap = new Snap(this.width, this.height);
            snap.rect(0, 0, this.width, this.height).attr({
                'fill': '#FFDAB9'
            });
            ...
            var combs = [];
            if (this.details.context.winlines) {
                combs = combs.concat(this.details.context.winlines.sort(function(a, b) {
                    return b.symbol - a.symbol
                }))
            }
            if (this.details.context.winscatters && this.details.context.winscatters.length > 0 && this.details.context.winscatters.filter(function(v) {
                    return v.freespins > 0
                }).length && this.details.context.winscatters.filter(function(v) {
                    return v.freespins > 0
                })[0].trigger === 'freespins') {
                combs = combs.concat(this.details.context.winscatters.filter(function(v) {
                    return v.freespins > 0
                }))
            }
            ...
            var bonusViewModes = [0];
            bonusViewModes.push(1);
            var isJackpots = details.context.bs && details.context.bs.filter(function(v) {
                return v.type !== 'regular'
            }).length > 0;
            if (details.context.multiplied_rows && details.context.multiplied_rows.includes(1) || isBonus && isJackpots && details.context.rounds_left === 0) {
                bonusViewModes.push(2)
            }
            if (isBonus) {
                partSize = this.width / bonusViewModes.length;
                var totalWin = details.round_win;
                var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin);
                var labelRight = bonusViewModes.length > 1 ? 'all/' + (bonusViewModes.length - 1) : '';
                this._panel(combsGroup, labelLeft, labelRight)
            }
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(BaseDrawer.STYLE_PROGRESS);
            var counter = 0;
            snap.click(function(e) {
                if (isBonus) {
                    if (bonusViewModes.length < 2) return;
                    counter = (counter + 1) % bonusViewModes.length;
                    ...
                    board.remove();
                    var _labelRight = bonusViewModes.length > 1 ? counter + '/' + (bonusViewModes.length - 1) : '';
                    if (counter === 1) {
                        _this2._panel(group, 'Rows to unlock', _labelRight)
                    }
                    if (counter === 2) {
                        var _totalWin = details.round_win;
                        var _labelLeft = _this2.title(details) + ', Win:' + _this2.coins(_totalWin);
                        _this2._panel(group, _labelLeft, _labelRight)
                    }
        ...
        value: function _combs(snap, combs, details) {
            ...
            for (var _i2 = 0; _i2 < combs.length; ++_i2) {
                this.drawe_line_patern(group, combs[_i2]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            return group
        }
    }, {
        key: '_rows_to_unlock',
        value: function _rows_to_unlock(snap, details) {
            var trashHolds = [10, 15, 20, 25];
            var rowToUnlock = snap.group();
            for (var i = 0; i < 4; i++) {
                var vh = 30 + i * 18;
                var leftText = 8 - i + 'th';
                var rightText = details.context.keys >= trashHolds[3 - i] ? 'Unlocked' : trashHolds[3 - i] - details.context.keys + ' to unlock';
                rowToUnlock.text('50%', vh + '%', leftText + ' - ' + rightText).attr(Drawer.ROWS_TO_UNLOCK_TEXT_CENTER)
            }
            rowToUnlock.transform('T' + '-' + this.boardMarginLeft + ',' + '-' + (this.panelHeight + 1));
            return rowToUnlock
        }
    }, {
        key: 'is_extra_snap',
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && details.context.bs_count === 40 || details.state == 'freespins' && details.context.winscatters != undefined && details.context.winscatters.length > 0 && details.context.winscatters.filter(function(v) {
                return v.freespins > 0
            }).length && details.context.winscatters.filter(function(v) {
                return v.freespins > 0
            })[0].trigger === 'freespins'
        }
    }, {
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && this.get_pearls_count(details) === 40) {
                labelLeft = 'Jackpot!';
                if (this.width > 200) {
            ...
            } else if (details.state == 'freespins' && details.context.winscatters.length && details.context.winscatters.filter(function(v) {
                    return v.freespins > 0
                }).length && details.context.winscatters.filter(function(v) {
                    return v.freespins > 0
                })[0].trigger === 'freespins') {
        ...
        value: function sort_winlines() {
            ...
            var winscatters = this.details.context.winscatters && this.details.context.winscatters.length && this.details.context.winscatters.filter(function(v) {
                return v.freespins > 0
            }).length && this.details.context.winscatters.filter(function(v) {
                return v.freespins > 0
            })[0].trigger === 'freespins' ? this.details.context.winscatters : [];
            var combs = [];
            combs = combs.concat(winlines.sort(function(a, b) {
                return b.symbol - a.symbol
            }), winscatters.sort(function(a, b) {
        ...
        value: function board(snap, _board) {
            ...
            var trashHolds = [10, 15, 20, 25];
            var bonus_symbols = [11];
            var isBonus = this.details.state === 'bonus';
            var details = this.details;
            var combs = this.sort_winlines();
            var group = snap.group();
            var imageUrl = void 0;
            if (currentWinLine === 0 && isBonus) {
                this._rows_to_unlock(group, details);
                return group
            }
            if (this.get_pearls_count(this.details) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                var winbonus = {
            ...
            if (combs.length === 1 && !isBonus) {
                currentWinLine = 0
            }
            if (!combs) {
                combs = []
            }
            var multipliers = [10, 5, 3, 2];
            if (isBonus) {
                multipliers.forEach(function(multiplier, index) {
                    var mult = group.image(_this3.url('x' + multiplier + '.png'), (_this3.cellWidth + _this3.cellOffsetX) * 5, (_this3.cellHeight + _this3.cellOffsetY) * index, 100 * _this3.cellHeight / 60, _this3.cellHeight);
                    if (index == 0 && trashHolds[3] > details.context.keys && details.context.filled_row && details.context.filled_row[3] !== 1 || index == 1 && trashHolds[2] > details.context.keys && details.context.filled_row && details.context.filled_row[2] !== 1 || index == 2 && trashHolds[1] > details.context.keys && details.context.filled_row && details.context.filled_row[1] !== 1 || index == 3 && trashHolds[0] > details.context.keys && details.context.filled_row && details.context.filled_row[0] !== 1) {
                        mult.attr({
                            opacity: 0.3
                        })
                    } else if (details.context.filled_row && (index === 0 && details.context.filled_row[3] !== 1 || index === 1 && details.context.filled_row[2] !== 1 || index === 2 && details.context.filled_row[1] !== 1 || index === 3 && details.context.filled_row[0] !== 1)) {
                        mult.attr({
                            opacity: 0.3
                        })
                    }
            ...
            var _loop = function _loop(_i3) {
                var _loop2 = function _loop2(_j) {
                    var colorBG = '#402840';
                    if (_j == 0) colorBG = '#92194a';
                    if (_j == 1) colorBG = '#6f1395';
                    if (_j == 2) colorBG = '#09438b';
                    if (_j == 3) colorBG = '#1a5c01';
                    var bg = void 0;
                    if (isBonus) {
                        bg = group.rect((_this3.cellWidth + _this3.cellOffsetX) * _i3, (_this3.cellHeight + _this3.cellOffsetY) * _j, _this3.cellWidth, _this3.cellHeight).attr({
                            'fill': colorBG
                        })
                    }
                    if (_board[_i3][_j] === 11) {
                        var bonus = _this3.details.context.bs.filter(function(v) {
                            return v.reel == _i3 && v.position == _j
                        })[0];
                        var type = bonus && bonus.type || 'regular';
                        if (type === 'regular') {
                            imageUrl = _this3.url('11.png')
                        } else {
                            imageUrl = _this3.url(type + '.png')
                        }
                    } else {
                        imageUrl = _this3.url(_board[_i3][_j] + '.png')
                    }
                    var offsetX = isBonus && _board[_i3][_j] != 0 ? _this3.cellWidth / 4 : 0;
                    var img = void 0;
                    if (_board[_i3][_j] != 0) {
                        img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * _i3 + offsetX, (_this3.cellHeight + _this3.cellOffsetY) * _j, isBonus && _board[_i3][_j] != 0 ? _this3.cellWidth / 2 : _this3.cellWidth, _this3.cellHeight)
                    }
                    var value = void 0;
                    if (_this3.details.context.bs != undefined) {
                        value = _this3.details.context.bs.filter(function(v) {
                            return v.reel == _i3 && v.position == _j
                        })[0]
                    } else {
                        value = 0
                    ...
                    if (value) {
                        if (value.type !== 'regular') {
                            if (isBonus && currentWinLine === -1 && _this3.details.context.rounds_left === 0) {
                    ...
                    if (value && typeof value === 'number') {
                        var div = 100;
                        value = (value / div).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    }
                    var rect = void 0;
                    var rect2 = void 0;
                    var txt = void 0;
                    var txt2 = void 0;
                    if (value) {
                        var offsetY = isBonus ? _this3.cellHeight / 8 : 0;
                        var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i3 - _this3.cellOffsetX;
                        var rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j + _this3.cellHeight / 3 - offsetY;
                        var rect_width = _this3.cellWidth + 2 * _this3.cellOffsetX;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = (_this3.cellHeight + _this3.cellOffsetY) * _j + _this3.cellHeight / 2;
                        rect = group.rect(rect_x, rect_y, rect_width, 2 * offsetY + _this3.cellHeight / 3).attr({
                            'fill': 'white'
                        });
                        var isFilled = details.context.filled_row && details.context.filled_row[3 - _j] === 1;
                        var isFilledInThisSpin = details.context.multiplied_rows && details.context.multiplied_rows[_j] === 1;
                        if (isBonus && isFilled && _this3.isRowUnlocked(_j, details) && (currentWinLine === 1 && isFilledInThisSpin || typeof copyValue === 'string')) {
                            rect2 = group.rect(rect_x, rect_y - _this3.cellHeight / 3, rect_width / 2, _this3.cellHeight / 3).attr({
                                'fill': 'white'
                            });
                            txt2 = group.text(rect_x + rect_width / 4, text_y - _this3.cellHeight / 3 - 1, 'x' + multipliers[_j]).attr(_this3.width > 200 ? Drawer.PEARL_VALUE_40 : Drawer.PEARL_VALUE_30);
                            if (typeof copyValue === 'number') {
                                var _div = 100;
                                value = copyValue / multipliers[_j];
                                value = (value / _div).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                            }
                        }
                        if (value == 'mini' || value == 'midi') {
                            if (_this3.width > 200) {
                                txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_MINI_70)
                            } else {
                                txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_MINI_40)
                            }
                        } else if (value == 'minor') {
                            if (_this3.width > 200) {
                                txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_MINOR_70)
                            } else {
                                txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_MINOR_40)
                            }
                        } else if (value == 'major' || value == 'grand') {
                            if (_this3.width > 200) {
                                txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_MAJOR_70)
                            } else {
                                txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_MAJOR_40)
                            }
                        } else {
                            if (copyValue < 10000000) {
                                if (_this3.width > 200) {
                                    txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_70)
                                } else {
                                    txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_40)
                                }
                            } else if (copyValue < 1000000000) {
                                if (_this3.width > 200) {
                                    txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_44)
                                } else {
                                    txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_23)
                                }
                            } else {
                                if (_this3.width > 200) {
                                    txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_30)
                                } else {
                                    txt = group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_15)
                                }
                            }
                        }
                    } else {}
                    if (currentWinLine !== -1 && details.state !== 'bonus') {
                    ...
                    if (details.state === 'bonus') {
                        if (_j == 0 && trashHolds[3] > details.context.keys || _j == 1 && trashHolds[2] > details.context.keys || _j == 2 && trashHolds[1] > details.context.keys || _j == 3 && trashHolds[0] > details.context.keys) {
                            bg && bg.attr({
                                opacity: 0.3
                            });
                            img && img.attr({
        ...
        key: 'isRowUnlocked',
        value: function isRowUnlocked(row, details) {
            var trashHolds = [10, 15, 20, 25];
            if (details.state === 'bonus') {
                if (row == 0 && trashHolds[3] > details.context.keys || row == 1 && trashHolds[2] > details.context.keys || row == 2 && trashHolds[1] > details.context.keys || row == 3 && trashHolds[0] > details.context.keys) {
                    return false
                }
            }
            return true
...
Drawer.ROWS_TO_UNLOCK_TEXT_CENTER = {
    'text-anchor': 'middle',
    'fill': '#fff',
    'font-size': '0.9em'
};
