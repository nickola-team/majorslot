...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && this.get_pearls_count(details) === 15 || details.state == 'freespins' && details.context.winscatters && details.context.winscatters.length > 0
        }
    }, {
        key: 'make_extra_snap',
        ...
        value: function board(snap, _board) {
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    ...
                    if (_this3.details.state === 'bonus' && _board[i][j] != 11) {
                        imageUrl = _this3.url('0.png')
                    }
                    if ((_this3.details.state === 'freespins' || _this3.details.context.back_to === 'freespins') && i != 0 && i != 4) {
                        imageUrl = _this3.url('0.png')
                    }
                    var img = void 0;
                    ...
                    if ((_this3.details.state === 'freespins' || _this3.details.context.back_to === 'freespins') && i == 2 && j == 1) {
                        imageUrl = _this3.url(_board[1][0] + '.png');
                        img = group.image(imageUrl, _this3.cellWidth + _this3.cellOffsetX, (_this3.cellHeight + _this3.cellOffsetY) * 0, _this3.cellWidth * 3, _this3.cellHeight * 3)
                    }
                    var value = void 0;
                    ...
                    if (value != 0) {
                        ...
                        if (value == 'mini') {
                            ...
                            if (_this3.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MAJOR_JACKPOT_70)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MAJOR_JACKPOT_40)
                            }
                        } else {
                            if (value < 100000) {
                                if (_this3.width > 200) {
                    ...
                    if (currentWinLine !== -1) {
                        ...
                        positions.forEach(function(pos) {
                            ...
                            if (_this3.details.state === 'freespins' && i != 0 && i != _board.length - 1 && j != 0 && j != _board[i].length - 1) {
                                img.attr({
                                    opacity: 1
                                })
                            }
        ...
        key: 'drawe_border_win_symbols',
        value: function drawe_border_win_symbols(snap, line) {
            if (this.details.state === 'freespins' && this.details.context.winscatters && this.details.context.winscatters.length) {
                var group = snap.group().attr(BaseDrawer.STYLE_LINE_GROUP);
                for (var i = 0; i < line.positions.length; ++i) {
                    var cell = line.positions[i];
                    var x_pos = cell[0];
                    var y_pos = cell[1];
                    if (this.details.state === 'freespins' && x_pos != 0 && x_pos != 4) {
                        if (!isItemInArray(this.bordered_cells, cell)) {
                            group.rect(1 * (this.cellWidth + this.cellOffsetX), 0 * (this.cellHeight + this.cellOffsetY), (this.cellWidth + this.cellOffsetX) * 3, (this.cellHeight + this.cellOffsetY) * 3).attr(BaseDrawer.STYLE_LINE_SYMBOL);
                            this.bordered_cells.push(cell)
                        }
                    } else {
                        if (!isItemInArray(this.bordered_cells, cell)) {
                ...
                var _group = snap.group().attr(BaseDrawer.STYLE_LINE_GROUP);
                for (var _i = 0; _i < line.positions.length; ++_i) {
                    var _cell = line.positions[_i];
                    if (!isItemInArray(this.bordered_cells, _cell)) {
                        _group.rect(_cell[0] * (this.cellWidth + this.cellOffsetX), _cell[1] * (this.cellHeight + this.cellOffsetY), this.cellWidth + this.cellOffsetX, this.cellHeight + this.cellOffsetY).attr(BaseDrawer.STYLE_LINE_SYMBOL);
                        this.bordered_cells.push(_cell)
                    }
                }
                return _group
            }
