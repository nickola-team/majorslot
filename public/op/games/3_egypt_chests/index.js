...
var TOTAL_SYMBOLS_QUANTITY = 15;
var BONUS_SYMBOLS = [10, 11, 12, 13];
var BOOST_ID = 14;
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function title(details) {
            if (details.last_action == 'restore') {
                return 'Restore'
            } else if (details.last_action == 'bonus_init' || details.last_action == 'bonus_freespins_stop' || details.last_action == 'bonus_spins_stop') {
                return details.state
            } else if (details.last_action == 'freespin_init' || details.last_action == 'freespin_stop') details.last_action;
            else if (details.state == 'freespins') {
                return 'FS(' + details.context.rounds_left + ')'
            } else if (details.state == 'bonus') {
                return 'Bonus(' + details.context.rounds_left + ')'
            } else {
                return 'Spin'
            }
        ...
        key: '_background',
        value: function _background(snap, state) {
            if (state === 'bonus' || this.details.last_action === 'bonus_stop') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#6B8E23'
                })
            } else if (state === 'freespins' || this.details.last_action === 'freespins_stop') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#4682B4'
                })
            } else if (state === 'spins') {
            ...
            } else if (this.details.context.winscatters != undefined && this.details.context.winscatters.length > 0) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#4682B4'
                })
            } else if (this.details.context.bs_count === TOTAL_SYMBOLS_QUANTITY) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#6B8E23'
                })
            } else if (state === 'bonus' && this.details.context.bonus_mechanic.includes(2) && this.details.context.bs_count_2 === TOTAL_SYMBOLS_QUANTITY) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#6B8E23'
                })
            } else {
        ...
        key: 'is_extra_snap',
        value: function is_extra_snap(details) {
            if (details.state == 'bonus') {
                if (this.details.context.bonus_mechanic.includes(2)) {
                    if (details.context.bs_count_2 === TOTAL_SYMBOLS_QUANTITY) {
                        return true
                    }
                }
                if (details.context.bs_count === TOTAL_SYMBOLS_QUANTITY) {
                    return true
                }
            }
            return false
        ...
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            var group = combsGroup.group();
            var labelLeft = '';
            var labelRight = '';
            this._background(group, details.state);
            margin += this.width;
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            progress.animate({
                width: 186
            }, 100);
            group.transform('T' + margin + ',0');
            if (details.state == 'bonus' && this.details.context.bs_count === TOTAL_SYMBOLS_QUANTITY) {
                labelLeft = 'Jackpot!';
                if (this.width > 200) {
                    group.text('50%', '60%', 'Jackpot !').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER_2)
                } else {
                    group.text('50%', '60%', 'Jackpot !').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                }
            }
            if (details.state == 'bonus' && this.details.context.bonus_mechanic.includes(2) && this.details.context.bs_count_2 === TOTAL_SYMBOLS_QUANTITY) {
                labelLeft = 'Jackpot!';
                if (this.width > 200) {
                    group.text('50%', '60%', 'Jackpot !').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER_2)
                } else {
        ...
        value: function get_text_for_action(details) {
            ...
            switch (action) {
                case 'bonus_init':
                    return 'Bonus game start';
                case 'bonus_spins_stop':
                    return 'Bonus game End';
        ...
        key: 'get_pearls_count',
        value: function get_pearls_count(details) {
            var count = 0;
            var board = details.context.board;
            for (var i in board) {
                for (var j in board[i]) {
                    if (BONUS_SYMBOLS.includes(board[i][j])) {
                        count = count + 1
                    }
                }
            }
        ...
        value: function make_snap(details) {
            this.cols = details.context.board.length;
            this.rows = details.context.board[0].length;
            if (this.details.state === 'bonus') {
                if (this.details.context.bonus_mechanic.includes(2)) {
                    this._calculateSizesBonus(this.cols, 7)
                } else {
                    this._calculateSizes(this.cols, this.rows)
                }
            } else {
        ...
        value: function make_spins(details) {
            var _this2 = this;
            var game_board = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : details.context.board;
            var snap = this.make_snap(details);
            this._background(snap, details.state);
            var group = snap.group().attr({
                width: this.width,
                height: this.height
            ...
            var combs = [].concat(details.context.winlines, details.context.winscatters);
            if (this.get_pearls_count(this.details) >= 1 && ['spin', 'freespin'].includes(this.details.last_action) && this.details.context.bac_win == true) {
                var winbonus = {
                    'trigger': 'bonus',
                    'positions': []
                };
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if ([].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(game_board[i][j])) {
                            winbonus.positions.push([i, j])
                        }
                    }
                }
                combs.push(winbonus)
            } else if (this.is_boost(details)) {
                var winboost = {
                    'trigger': 'boost',
                    'positions': [],
                    'amount': details.context.boost_pay
                };
                for (var _i in game_board) {
                    for (var _j in game_board[_i]) {
                        if ([].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(game_board[_i][_j])) {
                            winboost.positions.push([_i, _j])
                        }
                    }
                }
                combs.push(winboost)
            ...
            var game_boards = [];
            game_boards.push(details.context.board);
            if (!combs) {
                combs = []
            }
            var combsGroup = snap.group();
            ...
            var margin = 0;
            this._combs(combsGroup, combs, details);
            if (combs.length > 1 || game_boards.length > 1) {
                for (var _i2 in combs) {
                    combGroup = this._comb(combsGroup, combs, combs[_i2], details);
                    margin += this.width;
                    combGroup.transform('T' + margin + ',0')
                }
            }
            var partSize = this.width / combsGroup.children().length;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(Drawer.STYLE_PROGRESS);
            var board = this.board(group, game_boards[0]);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            if (this.is_boost(details)) {
                this.make_boost_snap(details, combsGroup, margin, snap)
            }
            if (this.is_extra_snap(details)) {
                this.make_extra_snap(details, combsGroup, margin, snap);
                var _partSize = this.width / combsGroup.children().length
            }
            if (details.last_action === 'freespin_init' || details.last_action === 'freespin_stop') {
                board.remove();
                this._background(snap, 'freespins');
            ...
            if (details.last_action === 'bonus_init' || details.last_action === 'bonus_stop') {
                board.remove();
                this._background(snap, 'bonus');
                this._panel(snap, details.last_action, '')
            }
            if (details.last_action === 'bonus_spins_stop' || details.last_action === 'bonus_freespins_stop') {
                board.remove();
                this._background(snap, 'bonus');
                this._panel(snap, details.last_action, '')
            }
            ...
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
                counter = (counter + 1) % combsGroup.children().length;
                combsGroup.transform('T-' + _this2.width * counter);
                progress.animate({
                    width: partSize * (counter + 1)
                }, 100);
                if (counter === 1) {
                    cur_board = Array.from(game_boards[game_boards.length - 1])
                } else {
                    cur_board = Array.from(game_boards[0])
                }
                board.remove();
                board = _this2.board(group, cur_board, counter - 1);
                board.transform('T' + _this2.boardMarginLeft + ',' + (_this2.panelHeight + 1))
            });
            snap.text('50%', '56%', this.get_text_for_action(details)).attr(Drawer.STYLE_TEXT_CENTER_RED);
            return snap.node
        }
    }, {
        key: 'make_freespins',
        value: function make_freespins(details) {
            var game_board = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : details.context.board;
            return this.make_spins(details, game_board)
        }
    }, {
        key: 'make_bonus',
        value: function make_bonus(details) {
            ...
            if (this.details.context.bonus_mechanic.includes(2)) {
                for (var i in game_board) {
                    game_board[i] = details.context.board_2[i].concat([0], game_board[i])
                }
            }
            var group = snap.group().attr({
                width: this.width,
            ...
            var combs = [].concat(details.context.winlines, details.context.winscatters);
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(3)) {
                if (this.details.context.bonus_mechanic.includes(2)) {
                    if (this.details.context.mult_win_2 != undefined) {
                        var positions = this.details.context.mult_win_2.map(function(mult) {
                            return mult.pos
                        });
                        var mult_win_2 = {
                            trigger: 'none',
                        ...
                        mult_win_2.trigger = 'mult_win_2';
                        mult_win_2.positions = positions;
                        mult_win_orig_2.trigger = 'mult_win_2_orig';
                        mult_win_orig_2.positions = positions;
                        combs.push(mult_win_orig_2);
                        combs.push(mult_win_2)
                    }
                    if (this.details.context.mult_win != undefined) {
                        var _positions = this.details.context.mult_win.map(function(mult) {
                            return [mult.pos[0], mult.pos[1] + 4]
                        });
                        var mult_win = {
                            trigger: 'none',
                            positions: [],
                        ...
                        mult_win.trigger = 'mult_win';
                        mult_win.positions = _positions;
                        mult_win_orig.trigger = 'mult_win_orig';
                        mult_win_orig.positions = _positions;
                        combs.push(mult_win_orig);
                        combs.push(mult_win)
                    }
                } else if (this.details.context.mult_win != undefined && this.details.context.mult_win.length) {
                    var _positions2 = this.details.context.mult_win.map(function(mult) {
                        return mult.pos
                    });
                    var _mult_win = {
                        trigger: 'none',
                    ...
                    _mult_win.trigger = 'mult_win';
                    _mult_win.positions = _positions2;
                    _mult_win_orig.trigger = 'mult_win_orig';
                    _mult_win_orig.positions = _positions2;
                    combs.push(_mult_win_orig);
                    combs.push(_mult_win)
                }
            }
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(3)) {
                var rounds_left = this.details.context.rounds_left;
                if (this.details.context.bonus_mechanic.includes(2)) {
                    rounds_left += this.details.context.rounds_left_2
                }
                if (rounds_left == 0) {
                    var e_m = {
                        trigger: 'mult_before',
            ...
            if (this.is_boost_2(details)) {
                var winboost = {
                    trigger: 'boost',
                    positions: [],
                    amount: details.context.boost_pay
                };
                for (var _i3 in game_board) {
                    for (var j in game_board[_i3]) {
                        if (j < 3 && [].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(game_board[_i3][j])) {
                            winboost.positions.push([_i3, j])
                        }
                    }
                }
                combs.push(winboost)
            ...
            if (this.is_boost(details)) {
                ...
                for (var _i4 in game_board) {
                    for (var _j2 in game_board[_i4]) {
                        if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(2)) {
                            if (_j2 > 3 && [].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(game_board[_i4][_j2])) {
                                _winboost.positions.push([_i4, _j2])
                            }
                        } else {
                            if ([].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(game_board[_i4][_j2])) {
                                _winboost.positions.push([_i4, _j2])
                            }
                        }
                    }
                }
                combs.push(_winboost)
            }
            var game_boards = [];
            ...
            var margin = 0;
            this._combs(combsGroup, combs, details);
            if (combs.length > 0 || game_boards.length > 1) {
                for (var _i5 in combs) {
                    combGroup = this._comb(combsGroup, combs, combs[_i5], details);
                    margin += this.width;
                    combGroup.transform('T' + margin + ',0')
                }
            }
            var partSize = this.width / combsGroup.children().length;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(Drawer.STYLE_PROGRESS);
            var board = this.board_in_bonus(group, game_boards[0]);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            if (this.is_boost(details) || this.is_boost_2(details)) {
                this.make_boost_snap(details, combsGroup, margin, snap)
            }
            if (this.is_extra_snap(details)) {
                this.make_extra_snap(details, combsGroup, margin, snap);
                var _partSize2 = this.width / combsGroup.children().length
            }
            if (details.last_action === 'freespin_init' || details.last_action === 'freespin_stop') {
                board.remove();
                this._background(snap, 'freespins');
            ...
            snap.click(function(e) {
                ...
                var cur_board = Array.from(game_board);
                board.remove();
                board = _this3.board_in_bonus(group, cur_board, counter - 1);
        ...
        value: function board(snap, _board) {
            ...
            var combs = [].concat(this.details.context.winlines, this.details.context.winscatters);
            if (this.get_pearls_count(this.details) >= 1 && ['spin', 'freespin'].includes(this.details.last_action) && this.details.context.bac_win == true) {
                var winbonus = {
                    'trigger': 'bonus',
                    'positions': []
                ...
                for (var i in _board) {
                    for (var j in _board[i]) {
                        if ([].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(_board[i][j])) {
                            winbonus.positions.push([i, j])
                        }
                    }
                }
            ...
            if (this.is_boost(this.details) && this.get_pearls_count(this.details) >= 1) {
                var winboost = {
                    'trigger': 'boost',
                    'positions': [],
                    'amount': this.details.context.boost_pay
                };
                for (var _i6 in _board) {
                    for (var _j3 in _board[_i6]) {
                        if ([].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(_board[_i6][_j3])) {
                            winboost.positions.push([_i6, _j3])
                        }
                    }
                }
                combs.push(winboost)
            ...
            var scaled_mults = [];
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(2)) {
                for (var _i7 in _board) {
                    scaled_bs_v[_i7] = this.details.context.bs_v_2[_i7].concat([0], this.details.context.bs_v[_i7])
                }
                if (this.details.context.bonus_mechanic.includes(3)) {
                    for (var _i8 in _board) {
                        scaled_mults[_i8] = this.details.context.multipliers_2[_i8].concat([0], this.details.context.multipliers[_i8])
                    }
                }
            }
            var curr_board = {};
            Object.assign(curr_board, this.details.context.orig_board);
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(3)) {
                if (this.details.context.bonus_mechanic.includes(2)) {
                    if (this.details.context.mult_win_2 != undefined) {
                        var mult_win_2 = {
                        ...
                        mult_win_2.trigger = 'mult_win_2';
                        mult_win_2.val = this.details.context.mult_win_2.v;
                        mult_win_2.positions = this.details.context.mult_win_2.pos;
                        for (var _j4 in _board) {
                            if (this.details.context.mult_win != undefined) {
                                mult_win_2.bs_v.push(this.details.context.mult_orig_bs_v_2[_j4].concat([0], this.details.context.us_orig_bs_v[_j4]))
                            } else {
                                mult_win_2.bs_v.push(this.details.context.mult_orig_bs_v_2[_j4].concat([0], this.details.context.bs_v[_j4].slice(-3)))
                            }
                        }
                        combs.push(mult_win_2)
                    }
                    if (this.details.context.mult_win != undefined) {
                        ...
                        mult_win.trigger = 'mult_win';
                        mult_win.val = this.details.context.mult_win.mult;
                        for (var _i9 in this.details.context.mult_win.pos) {
                            var pos = this.details.context.mult_win.pos[_i9];
                            mult_win.positions.push(pos)
                        }
                        for (var _j5 in _board) {
                            if (this.details.context.mult_win_2 != undefined) {
                                mult_win.bs_v.push(this.details.context.mult_orig_bs_v_2[_j5].concat([0], this.details.context.us_orig_bs_v[_j5]))
                            } else {
                                var part_1 = this.details.context.bs_v[_j5].slice(0, 3);
                                mult_win.bs_v.push(part_1.concat([0], this.details.context.mult_orig_bs_v[_j5]))
                            }
                        }
                        combs.push(mult_win)
                    }
                    ...
                    if (this.details.context.mult_win != undefined) {
                        ...
                        _mult_win2.trigger = 'mult_win';
                        _mult_win2.val = this.details.context.mult_win.mult;
                        _mult_win2.positions = this.details.context.mult_win.pos;
                        _mult_win2.bs_v = this.details.context.mult_orig_bs_v;
                        combs.push(_mult_win2)
                    }
                }
            }
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(3)) {
                ...
                if (rounds_left == 0) {
                    ...
                    if (this.details.context.bonus_mechanic.includes(2)) {
                        for (var _i10 in _board) {
                            e_m.bs_v[_i10] = this.details.context.mult_orig_bs_v_2[_i10].concat([0], this.details.context.mult_orig_bs_v[_i10]);
                            for (var _j6 in _board[_i10]) {
                                if (e_m.bs_v[_i10][_j6] > 1) {
                                    e_m.positions.push([_i10, _j6])
                                }
                            }
                        }
                    } else {
                        e_m.bs_v = this.details.context.mult_orig_bs_v
                    }
                    for (var _i11 in _board) {
                        for (var _j7 in _board[_i11]) {
                            if (e_m.bs_v[_i11][_j7] > 1) {
                                e_m.positions.push([_i11, _j7])
                            }
                        }
                    }
                    combs.push(e_m)
                ...
                if (rounds_left == 0) {
                    ...
                    if (this.details.context.bonus_mechanic.includes(2)) {
                        for (var _i12 in _board) {
                            _e_m2.bs_v[_i12] = this.details.context.bs_v_2[_i12].concat([0], this.details.context.bs_v[_i12]);
                            for (var _j8 in _board[_i12]) {
                                if (_e_m2.bs_v[_i12][_j8] > 1) {
                                    _e_m2.positions.push([_i12, _j8])
                                }
                            }
                        }
                    } else {
                        _e_m2.bs_v = this.details.context.mult_orig_bs_v
                    }
                    for (var _i13 in _board) {
                        for (var _j9 in _board[_i13]) {
                            if (_e_m2.bs_v[_i13][_j9] > 1) {
                                _e_m2.positions.push([_i13, _j9])
                            }
                        }
                    }
                    combs.push(_e_m2)
            ...
            var group = snap.group();
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(2)) {
                var divider = group.rect(0, this.height / 2 - this.cellHeight / 2, this.width - this.cellWidth * 0.7, 2).attr(Drawer.STYLE_DIVIDER)
            }
            var imageUrl = void 0;
            var widthCoef = 1;
            if (this.details.state === 'bonus') {
            ...
            var _loop = function _loop(_i14) {
                var _loop2 = function _loop2(_j10) {
                    if (combs.length == 1 && _this4.details.state !== 'bonus') {
                        currentWinLine = 0
                    }
                    imageUrl = _this4.url(_board[_i14][_j10] + '.png');
                    var img = void 0;
                    if (_this4.details.state === 'bonus' && ![].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(_board[_i14][_j10])) {
                        imageUrl = _this4.url('0.png');
                        if (_this4.details.context.bonus_mechanic.includes(3)) {
                            var m_v = _this4.details.context.multipliers[_i14][_j10];
                            if (_this4.details.state === 'bonus' && _this4.details.context.bonus_mechanic.includes(2)) {
                                if (_this4.details.context.bonus_mechanic.includes(3)) {
                                    m_v = scaled_mults[_i14][_j10]
                                }
                            }
                            if (m_v > 1 && (currentWinLine != -1 || _this4.details.round_win === 0)) {
                                imageUrl = _this4.url('border.png')
                            }
                        }
                        var extraOffset = (_this4.cellWidth - _this4.cellWidth / widthCoef) / 2;
                        img = group.image(imageUrl, _this4.cellWidth * _i14, (_this4.cellHeight + _this4.cellOffsetY) * _j10, _this4.cellWidth, _this4.cellHeight)
                    } else {
                        var _extraOffset = (_this4.cellWidth - _this4.cellWidth / widthCoef) / 2;
                        img = group.image(imageUrl, (_this4.cellWidth + _this4.cellOffsetX) * _i14 + _extraOffset, (_this4.cellHeight + _this4.cellOffsetY) * _j10, _this4.cellWidth / widthCoef, _this4.cellHeight)
                    }
                    if (currentWinLine !== -1) {
                        if (!combs[currentWinLine]) return 'continue';
                        var _positions3 = combs[currentWinLine].positions;
                        img.attr({
                            opacity: 0.5
                        });
                        _positions3.forEach(function(pos) {
                            if (combs[currentWinLine] !== undefined) {
                                if (combs[currentWinLine].trigger == 'mult_before' || combs[currentWinLine].trigger == 'mult_after') {
                                    img.attr({
                                        opacity: 1
                                    })
                                }
                            }
                            if (pos[0] == _i14 && pos[1] == _j10) {
                                img.attr({
                                    opacity: 1
                                })
                            }
                    ...
                    var value = void 0;
                    if (_this4.details.context.bs_v != undefined) {
                        value = _this4.details.context.bs_v[_i14][_j10];
                        if (_this4.details.state === 'bonus' && _this4.details.context.bonus_mechanic.includes(2)) {
                            value = scaled_bs_v[_i14][_j10]
                        }
                        if (currentWinLine !== -1 && combs[currentWinLine].bs_v != undefined) {
                            var isUS = false;
                            for (var q in combs[currentWinLine].positions) {
                                var p = combs[currentWinLine].positions[q];
                                if (p[0] == _i14 && p[1] == _j10) {
                                    isUS = true
                                }
                            }
                            if (isUS == true) {
                                value = combs[currentWinLine].bs_v[_i14][_j10]
                            }
                        }
                    } else if (_this4.details.context.bonus_symbols_v != undefined) {
                        value = _this4.details.context.bonus_symbols_v[_i14][_j10]
                    }
                    if (combs[currentWinLine] !== undefined) {
                        if (combs[currentWinLine].trigger == 'mult_before') {
                            value = combs[currentWinLine].bs_v[_i14][_j10]
                        }
                    }
                    if (typeof value === 'number') {
                        value = (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    }
                    var mult_value = null;
                    if (_this4.details.context.multipliers) {
                        mult_value = _this4.details.context.multipliers[_i14][_j10]
                    }
                    if (_this4.details.state === 'bonus' && _this4.details.context.bonus_mechanic.includes(2)) {
                        if (_this4.details.context.bonus_mechanic.includes(3)) {
                            mult_value = scaled_mults[_i14][_j10]
                        }
                    }
                    if (mult_value && _this4.details.round_win === 0) {
                        var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i14 + _this4.cellWidth / 1.77;
                        var rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j10 - _this4.cellOffsetY + 0.5;
                        var rect_width = _this4.cellWidth / 2.3 + 2 * _this4.cellOffsetX;
                        var text_x_x = rect_x + rect_width / 2;
                        var text_x_y = (_this4.cellHeight + _this4.cellOffsetY) * _j10 + _this4.cellOffsetY + _this4.cellHeight * 0.18;
                        group.rect(rect_x, rect_y, rect_width, _this4.cellHeight / 3).attr({
                            'fill': 'white'
                        });
                        if (_this4.width > 200) {
                            group.text(text_x_x, text_x_y, 'x' + mult_value).attr(Drawer.MULT_VALUE_70)
                        } else {
                            group.text(text_x_x, text_x_y, 'x' + mult_value).attr(Drawer.MULT_VALUE_40)
                        }
                    }
                    if (mult_value && _this4.details.round_win > 0 && combs[currentWinLine] !== undefined) {
                        if (combs[currentWinLine].trigger != 'mult_after') {
                            var _rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i14 + _this4.cellWidth / 1.77;
                            var _rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j10 - _this4.cellOffsetY + 0.5;
                            var _rect_width = _this4.cellWidth / 2.3 + 2 * _this4.cellOffsetX;
                            var _text_x_x = _rect_x + _rect_width / 2;
                            var _text_x_y = (_this4.cellHeight + _this4.cellOffsetY) * _j10 + _this4.cellOffsetY + _this4.cellHeight * 0.18;
                            group.rect(_rect_x, _rect_y, _rect_width, _this4.cellHeight / 3).attr({
                                'fill': 'white'
                            });
                            if (_this4.width > 200) {
                                group.text(_text_x_x, _text_x_y, 'x' + mult_value).attr(Drawer.MULT_VALUE_70)
                            } else {
                                group.text(_text_x_x, _text_x_y, 'x' + mult_value).attr(Drawer.MULT_VALUE_40)
                            }
                        }
                    }
                    if (value != 0 && [10, 11, 12, 13].includes(_board[_i14][_j10])) {
                        var _rect_x2 = (_this4.cellWidth + _this4.cellOffsetX) * _i14 - _this4.cellOffsetX;
                        var _rect_y2 = (_this4.cellHeight + _this4.cellOffsetY) * _j10 + _this4.cellHeight / 2.7 - _this4.cellHeight * 0.04;
                        if (_this4.details.state === 'bonus' && _this4.details.context.bonus_mechanic.includes(2)) {
                            _rect_y2 = (_this4.cellHeight + _this4.cellOffsetY) * _j10 + _this4.cellHeight / 2.7 - _this4.cellHeight * 0.04
                        }
                        var _rect_width2 = _this4.cellWidth + 2 * _this4.cellOffsetX;
                        var text_x = _rect_x2 + _rect_width2 / 2;
                        var text_y = (_this4.cellHeight + _this4.cellOffsetY) * _j10 + _this4.cellHeight / 2;
                        group.rect(_rect_x2, _rect_y2, _rect_width2, _this4.cellHeight / 3 + 1).attr({
                            'fill': 'white'
                        });
                        if (value == 'mini') {
                            if (_this4.width > 200) {
                                group.text(text_x, text_y - _this4.cellHeight * 0.01, value).attr(Drawer.BS_MINI_70)
                            } else {
                                group.text(text_x, text_y - _this4.cellHeight * 0.01, value).attr(Drawer.BS_MINI_40)
                            }
                        } else if (value == 'minor') {
                            if (_this4.width > 200) {
                                group.text(text_x, text_y - _this4.cellHeight * 0.01, value).attr(Drawer.BS_MINOR_70)
                            } else {
                                group.text(text_x, text_y - _this4.cellHeight * 0.01, value).attr(Drawer.BS_MINOR_40)
                            }
                        } else if (value == 'major') {
                            if (_this4.width > 200) {
                                group.text(text_x, text_y - _this4.cellHeight * 0.01, value).attr(Drawer.BS_MAJOR_70)
                            } else {
                                group.text(text_x, text_y - _this4.cellHeight * 0.01, value).attr(Drawer.BS_MAJOR_40)
                            }
                        } else {
                            if (value.length <= 8) {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.BS_VALUE_65)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.BS_VALUE_40)
                                }
                            } else if (value.length > 8 && value.length <= 10) {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.BS_VALUE_50)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.BS_VALUE_30)
                                }
                            } else {
                                if (_this4.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.BS_VALUE_36)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.BS_VALUE_22)
                                }
                            }
                        }
                    } else if (_board[_i14][_j10] === 14) {
                        var _value = 'boost';
                        var _rect_x3 = (_this4.cellWidth + _this4.cellOffsetX) * _i14 - _this4.cellOffsetX;
                        var _rect_y3 = (_this4.cellHeight + _this4.cellOffsetY) * _j10 + _this4.cellHeight / 2.7 - _this4.cellHeight * 0.04;
                        if (_this4.details.state === 'bonus' && _this4.details.context.bonus_mechanic.includes(2)) {
                            _rect_y3 = (_this4.cellHeight + _this4.cellOffsetY) * _j10 + _this4.cellHeight / 2.7 - _this4.cellHeight * 0.04
                        }
                        var _rect_width3 = _this4.cellWidth + 2 * _this4.cellOffsetX;
                        var _text_x = _rect_x3 + _rect_width3 / 2;
                        var _text_y = (_this4.cellHeight + _this4.cellOffsetY) * _j10 + _this4.cellHeight / 2;
                        group.rect(_rect_x3, _rect_y3, _rect_width3, _this4.cellHeight / 3 + 1).attr({
                            'fill': 'white'
                        });
                        if (_this4.width > 200) {
                            group.text(_text_x, _text_y - _this4.cellHeight * 0.01, _value).attr(Drawer.BS_BOOST_70)
                        } else {
                            group.text(_text_x, _text_y - _this4.cellHeight * 0.01, _value).attr(Drawer.BS_BOOST_40)
                        }
                    }
                };
                for (var _j10 in _board[_i14]) {
                    var _ret2 = _loop2(_j10);
                    if (_ret2 === 'continue') continue
                }
            };
            for (var _i14 in _board) {
                _loop(_i14)
            }
            return group
        }
        ...
        key: 'board_in_bonus',
        value: function board_in_bonus(snap, board) {
            var _this5 = this;
            var currentWinLine = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -1;
            var combs = [].concat(this.details.context.winlines, this.details.context.winscatters);
            var scaled_bs_v = [];
            ...
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(2)) {
                for (var i in board) {
                    scaled_bs_v[i] = this.details.context.bs_v_2[i].concat([0], this.details.context.bs_v[i])
                }
                if (this.details.context.bonus_mechanic.includes(3)) {
                    for (var _i15 in board) {
                        scaled_mults[_i15] = this.details.context.multipliers_2[_i15].concat([0], this.details.context.multipliers[_i15])
                    }
                }
            }
            var curr_board = {};
            ...
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(3)) {
                if (this.details.context.bonus_mechanic.includes(2)) {
                    if (this.details.context.mult_win_2 != undefined) {
                        var _positions4 = this.details.context.mult_win_2.map(function(mult) {
                            return mult.pos
                        });
                        var mult_win_2 = {
                            trigger: 'mult_win_2',
                        ...
                        if (this.details.context.rounds_left === 0 && this.details.context.rounds_left_2 === 0) {
                            for (var j in board) {
                                mult_win_2.bs_v.push(this.details.context.mult_orig_bs_v_2[j].concat([0], this.details.context.mult_orig_bs_v[j]));
                                mult_win_2_orig.bs_v.push(this.details.context.mult_orig_bs_v_2[j].concat([0], this.details.context.mult_orig_bs_v[j]))
                            }
                        } else {
                            for (var _j11 in board) {
                                mult_win_2.bs_v.push(this.details.context.bs_v_2[_j11].concat([0], this.details.context.bs_v[_j11]));
                                mult_win_2_orig.bs_v.push(this.details.context.bs_v_2[_j11].concat([0], this.details.context.bs_v[_j11]))
                            }
                        }
                        this.details.context.multipliers_2.forEach(function(arr, i) {
                            arr.forEach(function(mult, j) {
                                var finalMult = mult;
                                var winMult = null;
                                var isWinDouble = _this5.details.context.mult_win_2;
                                var value = 0;
                                if (isWinDouble) winMult = _this5.details.context.mult_win_2.find(function(e) {
                                    return e.pos[0] === i && e.pos[1] === j
                                });
                                if (winMult) value = winMult.mult;
                                orig_values[i][j] = finalMult - value;
                                final_values[i][j] = finalMult
                            })
                        });
                        this.details.context.multipliers.forEach(function(arr, i) {
                            arr.forEach(function(mult, j) {
                                var finalMult = mult;
                                var winMult = null;
                                var isWinMult = _this5.details.context.mult_win;
                                var value = 0;
                                if (isWinMult) winMult = _this5.details.context.mult_win.find(function(e) {
                                    return e.pos[0] === i && e.pos[1] === j
                                });
                                if (winMult) value = winMult.mult;
                                orig_values[i][j + 4] = finalMult - value;
                                final_values[i][j + 4] = finalMult - value
                            })
                        });
                        mult_win_2_orig.orig_val = orig_values;
                        mult_win_2.final_val = final_values;
                        combs.push(mult_win_2_orig);
                        combs.push(mult_win_2)
                    }
                    if (this.details.context.mult_win != undefined) {
                        var _positions5 = this.details.context.mult_win.map(function(mult) {
                            return [mult.pos[0], mult.pos[1] + 4]
                        });
                        var mult_win = {
                            trigger: 'mult_win',
                        ...
                        mult_win.val = this.details.context.mult_win.mult;
                        if (this.details.context.rounds_left === 0 && this.details.context.rounds_left_2 === 0) {
                            for (var _j12 in board) {
                                mult_win.bs_v.push(this.details.context.mult_orig_bs_v_2[_j12].concat([0], this.details.context.mult_orig_bs_v[_j12]));
                                mult_win_orig.bs_v.push(this.details.context.mult_orig_bs_v_2[_j12].concat([0], this.details.context.mult_orig_bs_v[_j12]))
                            }
                        } else {
                            for (var _j13 in board) {
                                mult_win.bs_v.push(this.details.context.bs_v_2[_j13].concat([0], this.details.context.bs_v[_j13]));
                                mult_win_orig.bs_v.push(this.details.context.bs_v_2[_j13].concat([0], this.details.context.bs_v[_j13]))
                            }
                        }
                        for (var _i16 in this.details.context.mult_win.pos) {
                            var pos = this.details.context.mult_win.pos[_i16];
                            mult_win.positions.push(pos)
                        }
                        this.details.context.multipliers_2.forEach(function(arr, i) {
                            arr.forEach(function(mult, j) {
                                _orig_values[i][j] = mult
                            })
                        });
                        this.details.context.multipliers.forEach(function(arr, i) {
                            arr.forEach(function(mult, j) {
                                ...
                                if (winMult) value = winMult.mult;
                                _orig_values[i][j + 4] = finalMult - value
                            })
                        });
                        mult_win_orig.orig_val = _orig_values;
                        combs.push(mult_win_orig);
                        combs.push(mult_win)
                    }
                } else if (this.details.context.mult_win && this.details.context.mult_win.length) {
                    var _positions6 = this.details.context.mult_win.map(function(mult) {
                        return mult.pos
                    });
                    var _mult_win3 = {
                        trigger: 'mult_win',
                        positions: _positions6,
                        bs_v: [],
                        val: this.details.context.mult_win.mult
                    };
                    var _mult_win_orig2 = {
                        trigger: 'mult_win_orig',
                        positions: _positions6,
                    ...
                    if (this.details.context.rounds_left === 0) {
                        _mult_win3.bs_v = this.details.context.mult_orig_bs_v;
                        _mult_win_orig2.bs_v = this.details.context.mult_orig_bs_v
                    } else {
                        _mult_win3.bs_v = this.details.context.bs_v;
                        _mult_win_orig2.bs_v = this.details.context.bs_v
                    }
                    this.details.context.multipliers.forEach(function(r, ri) {
                        r.forEach(function(s, si) {
                            _orig_values2[ri][si] = s;
                            _orig_values2[ri][si] = s
                        })
                    });
                    this.details.context.mult_win.forEach(function(mult) {
                        var position = mult.pos;
                        var value = mult.mult;
                        var finalMultiplier = _this5.details.context.multipliers[position[0]][position[1]];
                        _orig_values2[position[0]][position[1]] = finalMultiplier - value
                    });
                    _mult_win_orig2.orig_val = _orig_values2;
                    combs.push(_mult_win_orig2);
                    combs.push(_mult_win3)
                }
            }
            if (this.is_boost(this.details) || this.is_boost_2(this.details)) {
                if (this.details.context.bonus_mechanic.includes(2)) {
                    if (this.details.context.boost_pay_2 != undefined) {
                        var winboost = {
                            'trigger': 'boost',
                            'positions': [],
                            'amount': this.details.context.boost_pay
                        };
                        for (var _i17 in board) {
                            for (var _j14 in board[_i17]) {
                                if (_j14 < 3 && [].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(board[_i17][_j14])) {
                                    winboost.positions.push([_i17, _j14])
                                }
                            }
                        }
                        combs.push(winboost)
                    }
                    if (this.details.context.boost_pay != undefined) {
                        var _winboost2 = {
                            'trigger': 'boost',
                            'positions': [],
                            'amount': this.details.context.boost_pay
                        };
                        for (var _i18 in board) {
                            for (var _j15 in board[_i18]) {
                                if (_j15 > 3 && [].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(board[_i18][_j15])) {
                                    _winboost2.positions.push([_i18, _j15])
                                }
                            }
                        }
                        combs.push(_winboost2)
                    }
                } else {
                    var _winboost3 = {
                        'trigger': 'boost',
                    ...
                    for (var _i19 in board) {
                        for (var _j16 in board[_i19]) {
                            if ([].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(board[_i19][_j16])) {
                                _winboost3.positions.push([_i19, _j16])
                            }
                        }
                    }
                    combs.push(_winboost3)
                }
            }
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(3)) {
                var rounds_left = this.details.context.rounds_left;
                ...
                if (rounds_left == 0) {
                    ...
                    if (this.details.context.bonus_mechanic.includes(2)) {
                        for (var _i20 in board) {
                            e_m.bs_v[_i20] = this.details.context.mult_orig_bs_v_2[_i20].concat([0], this.details.context.mult_orig_bs_v[_i20]);
                            for (var _j17 in board[_i20]) {
                                if (e_m.bs_v[_i20][_j17]) {
                                    e_m.positions.push([_i20, _j17])
                                }
                            }
                        }
                    } else {
                    ...
                    for (var _i21 in board) {
                        for (var _j18 in board[_i21]) {
                            if (e_m.bs_v[_i21][_j18]) {
                                e_m.positions.push([_i21, _j18])
                            }
                        }
                    }
                    combs.push(e_m)
                ...
                if (rounds_left == 0) {
                    ...
                    if (this.details.context.bonus_mechanic.includes(2)) {
                        for (var _i22 in board) {
                            _e_m3.bs_v[_i22] = this.details.context.bs_v_2[_i22].concat([0], this.details.context.bs_v[_i22]);
                            for (var _j19 in board[_i22]) {
                                if (_e_m3.bs_v[_i22][_j19] > 1) {
                                    _e_m3.positions.push([_i22, _j19])
                                }
                            }
                        }
                    } else {
                        _e_m3.bs_v = this.details.context.mult_orig_bs_v
                    }
                    for (var _i23 in board) {
                        for (var _j20 in board[_i23]) {
                            if (_e_m3.bs_v[_i23][_j20]) {
                                _e_m3.positions.push([_i23, _j20])
                            }
                        }
                    }
                    combs.push(_e_m3)
            ...
            if (currentWinLine === -1) {
                combs.forEach(function(element) {
                    return positions = positions.concat(element.positions)
                })
            } else if (combs[currentWinLine]) {
                positions = combs[currentWinLine].positions
            }
            var group = snap.group();
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(2)) {
                var divider = group.rect(0, this.height / 2 - this.cellHeight / 2, this.width - this.cellWidth * 0.7, 2).attr(Drawer.STYLE_DIVIDER)
            ...
            var _loop3 = function _loop3(_i24) {
                var _loop4 = function _loop4(_j21) {
                    if (combs.length == 1 && _this5.details.state !== 'bonus') {
                        currentWinLine = 0
                    }
                    imageUrl = _this5.url(board[_i24][_j21] + '.png');
                    var img = void 0;
                    if (_this5.details.state === 'bonus' && ![].concat(BONUS_SYMBOLS, [BOOST_ID]).includes(board[_i24][_j21])) {
                        imageUrl = _this5.url('0.png')
                    }
                    var extraOffset = (_this5.cellWidth - _this5.cellWidth / widthCoef) / 2;
                    img = group.image(imageUrl, (_this5.cellWidth + _this5.cellOffsetX) * _i24 + extraOffset, (_this5.cellHeight + _this5.cellOffsetY) * _j21, _this5.cellWidth / widthCoef, _this5.cellHeight);
                    if (currentWinLine !== -1) {
                        if (!combs[currentWinLine]) return 'continue';
                        var _positions7 = combs[currentWinLine].positions;
                        img.attr({
                            opacity: 0.5
                        });
                        _positions7.forEach(function(pos) {
                            if (combs[currentWinLine] !== undefined) {
                                if (pos[0] == _i24 && pos[1] == _j21) {
                                    img.attr({
                                        opacity: 1
                                    })
                                }
                    ...
                    var value = void 0;
                    if (_this5.details.context.bs_v != undefined) {
                        value = _this5.details.context.bs_v[_i24][_j21];
                        if (_this5.details.state === 'bonus' && _this5.details.context.bonus_mechanic.includes(2)) {
                            value = scaled_bs_v[_i24][_j21]
                        }
                    } else if (_this5.details.context.bonus_symbols_v != undefined) {
                        value = _this5.details.context.bonus_symbols_v[_i24][_j21]
                    }
                    if (combs[currentWinLine] !== undefined) {
                        if (combs[currentWinLine].trigger == 'mult_win' || combs[currentWinLine].trigger == 'mult_before' || combs[currentWinLine].trigger == 'mult_win_orig') {
                            value = combs[currentWinLine].bs_v[_i24][_j21]
                        }
                        if (combs[currentWinLine].trigger == 'mult_win_2' || combs[currentWinLine].trigger == 'mult_win_2_orig') {
                            value = combs[currentWinLine].bs_v[_i24][_j21]
                        }
                    }
                    if (typeof value === 'number') {
                    ...
                    var mult_value = 1;
                    if (_this5.details.context.multipliers && _this5.details.context.multipliers.length) {
                        if (combs[currentWinLine] && (combs[currentWinLine].trigger == 'mult_win_orig' || combs[currentWinLine].trigger == 'mult_win_2_orig')) {
                            mult_value = combs[currentWinLine].orig_val[_i24][_j21]
                        } else if (combs[currentWinLine] && combs[currentWinLine].trigger == 'mult_win_2') {
                            mult_value = combs[currentWinLine].final_val[_i24][_j21]
                        } else {
                            mult_value = _this5.details.context.multipliers[_i24][_j21]
                        }
                    }
                    if (_this5.details.state === 'bonus' && _this5.details.context.bonus_mechanic.includes(2)) {
                        if (_this5.details.context.bonus_mechanic.includes(3) && (combs[currentWinLine] && (combs[currentWinLine].trigger === 'mult_win' || combs[currentWinLine].trigger === 'mult_before') || !combs[currentWinLine])) {
                            mult_value = scaled_mults[_i24][_j21]
                        }
                    }
                    if (mult_value > 1 && _this5.details.round_win === 0) {
                        var rect_x = (_this5.cellWidth + _this5.cellOffsetX) * _i24 + _this5.cellWidth / 1.77;
                        var rect_y = (_this5.cellHeight + _this5.cellOffsetY) * _j21 - _this5.cellOffsetY + 0.5;
                        var rect_width = _this5.cellWidth / 2.3 + 2 * _this5.cellOffsetX;
                        var text_x_x = rect_x + rect_width / 2;
                        var text_x_y = (_this5.cellHeight + _this5.cellOffsetY) * _j21 + _this5.cellOffsetY + _this5.cellHeight * 0.17;
                        group.rect(rect_x, rect_y, rect_width, _this5.cellHeight / 3).attr({
                            'fill': 'white'
                        });
                        if (_this5.width > 200) {
                            group.text(text_x_x, text_x_y, 'x' + mult_value).attr(Drawer.MULT_VALUE_70)
                    ...
                    if (mult_value > 1 && _this5.details.round_win > 0 && combs[currentWinLine] !== undefined) {
                        if (combs[currentWinLine].trigger != 'mult_after') {
                            var _rect_x4 = (_this5.cellWidth + _this5.cellOffsetX) * _i24 + _this5.cellWidth / 1.8;
                            var _rect_y4 = (_this5.cellHeight + _this5.cellOffsetY) * _j21 - _this5.cellOffsetY + 0.5;
                            var _rect_width4 = _this5.cellWidth / 2.3 + 2 * _this5.cellOffsetX;
                            var _text_x_x2 = _rect_x4 + _rect_width4 / 2;
                            var _text_x_y2 = (_this5.cellHeight + _this5.cellOffsetY) * _j21 + _this5.cellOffsetY + _this5.cellHeight * 0.18;
                            group.rect(_rect_x4, _rect_y4, _rect_width4, _this5.cellHeight / 3).attr({
                                'fill': 'white'
                            });
                            if (_this5.width > 200) {
                                group.text(_text_x_x2, _text_x_y2, 'x' + mult_value).attr(Drawer.MULT_VALUE_70)
                            } else {
                                group.text(_text_x_x2, _text_x_y2, 'x' + mult_value).attr(Drawer.MULT_VALUE_40)
                            }
                        }
                    }
                    if (value != 0 && [10, 11, 12, 13].includes(board[_i24][_j21])) {
                        var _rect_x5 = (_this5.cellWidth + _this5.cellOffsetX) * _i24 - _this5.cellOffsetX;
                        var _rect_y5 = (_this5.cellHeight + _this5.cellOffsetY) * _j21 + _this5.cellHeight / 2.7 - _this5.cellHeight * 0.04;
                        if (_this5.details.state === 'bonus' && _this5.details.context.bonus_mechanic.includes(2)) {
                            _rect_y5 = (_this5.cellHeight + _this5.cellOffsetY) * _j21 + _this5.cellHeight / 2.7 - _this5.cellHeight * 0.04
                        }
                        var _rect_width5 = _this5.cellWidth + 2 * _this5.cellOffsetX;
                        var text_x = _rect_x5 + _rect_width5 / 2;
                        var text_y = (_this5.cellHeight + _this5.cellOffsetY) * _j21 + _this5.cellHeight / 2;
                        group.rect(_rect_x5, _rect_y5, _rect_width5, _this5.cellHeight / 3 + 1).attr({
                            'fill': 'white'
                        });
                        if (value == 'mini') {
                            if (_this5.width > 200) {
                                group.text(text_x, text_y - _this5.cellHeight * 0.01, value).attr(Drawer.BS_MINI_70)
                            } else {
                                group.text(text_x, text_y - _this5.cellHeight * 0.01, value).attr(Drawer.BS_MINI_40)
                            }
                        } else if (value == 'minor') {
                            if (_this5.width > 200) {
                                group.text(text_x, text_y - _this5.cellHeight * 0.01, value).attr(Drawer.BS_MINOR_70)
                            } else {
                                group.text(text_x, text_y - _this5.cellHeight * 0.01, value).attr(Drawer.BS_MINOR_40)
                            }
                        } else if (value == 'major') {
                            if (_this5.width > 200) {
                                group.text(text_x, text_y - _this5.cellHeight * 0.01, value).attr(Drawer.BS_MAJOR_70)
                            } else {
                                group.text(text_x, text_y - _this5.cellHeight * 0.01, value).attr(Drawer.BS_MAJOR_40)
                            }
                        } else {
                            if (value.length <= 8) {
                                if (_this5.width > 200) {
                    ...
                    } else if (board[_i24][_j21] === 14) {
                        var _value2 = 'boost';
                        var _rect_x6 = (_this5.cellWidth + _this5.cellOffsetX) * _i24 - _this5.cellOffsetX;
                        var _rect_y6 = (_this5.cellHeight + _this5.cellOffsetY) * _j21 + _this5.cellHeight / 2.7 - _this5.cellHeight * 0.04;
                        if (_this5.details.state === 'bonus' && _this5.details.context.bonus_mechanic.includes(2)) {
                            _rect_y6 = (_this5.cellHeight + _this5.cellOffsetY) * _j21 + _this5.cellHeight / 2.7 - _this5.cellHeight * 0.04
                        }
                        var _rect_width6 = _this5.cellWidth + 2 * _this5.cellOffsetX;
                        var _text_x2 = _rect_x6 + _rect_width6 / 2;
                        var _text_y2 = (_this5.cellHeight + _this5.cellOffsetY) * _j21 + _this5.cellHeight / 2;
                        group.rect(_rect_x6, _rect_y6, _rect_width6, _this5.cellHeight / 3 + 1).attr({
                            'fill': 'white'
                        });
                        if (_this5.width > 200) {
                            group.text(_text_x2, _text_y2 - _this5.cellHeight * 0.01, _value2).attr(Drawer.BS_BOOST_70)
                        } else {
                            group.text(_text_x2, _text_y2 - _this5.cellHeight * 0.01, _value2).attr(Drawer.BS_BOOST_40)
                        }
                    }
                };
                for (var _j21 in board[_i24]) {
                    var _ret4 = _loop4(_j21);
                    if (_ret4 === 'continue') continue
                }
            };
            for (var _i24 in board) {
                _loop3(_i24)
            }
            return group
        }
        ...
        key: 'is_boost',
        value: function is_boost(details) {
            return Boolean(details.context.boost_pay)
        }
    }, {
        key: 'is_boost_2',
        value: function is_boost_2(details) {
            return Boolean(details.context.boost_pay_2)
        }
    }, {
        key: '_combs',
        value: function _combs(snap, combs, details) {
            ...
            for (var _i25 = 0; _i25 < combs.length; ++_i25) {
                this.drawe_line_patern(group, combs[_i25]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            return group
        }
    }, {
        ...
        value: function _comb(snap, combs, comb, details) {
            ...
            var group = snap.group();
            var labelLeft = this.title(details) + ', Win:' + this.coins(comb.amount);
            if (comb.trigger === 'mult') {
                labelLeft = this.title(details) + ', X2'
            }
            if (comb.trigger === 'add_line') {
                labelLeft = this.title(details) + ', add line'
            }
            if (comb.trigger === 'collect') {
                labelLeft = this.title(details) + ', collector'
            }
            if (comb.trigger === 'mult_win' || comb.trigger === 'mult_win_2') {
                labelLeft = this.title(details) + ', multiplier feature'
            }
            if (comb.trigger === 'mult_win_2_orig' || comb.trigger === 'mult_win_orig') {
                labelLeft = this.title(details) + ', multiplier feature'
            }
            var labelRight = combs.indexOf(comb) + 1 + '/' + combs.length;
            this._panel(group, labelLeft, labelRight);
        ...
        value: function _calculateSizes(cols, rows) {
            ...
            this.progressHeight = 3;
            this.boardHeight = this.height - (this.panelHeight + this.progressHeight + 1);
            this.cellHeight = parseInt(this.boardHeight / rows - 3, 10);
            this.cellWidth = this.cellHeight * 1.02;
            this.cellOffsetX = 0;
            this.cellOffsetY = 0;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10)
        }
    }, {
        key: '_calculateSizesBonus',
        value: function _calculateSizesBonus(cols, rows) {
            this.panelHeight = this.height > 150 ? 20 : 12;
            this.panelFont = this.height > 150 ? '1em' : '0.7em';
            this.progressHeight = 3;
            this.boardHeight = this.height - (this.panelHeight + this.progressHeight + 1);
            this.cellHeight = parseInt(this.boardHeight / rows, 10);
            this.cellWidth = this.cellHeight * 1.02 * 2;
            this.cellOffsetX = 0;
            this.cellOffsetY = 0;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10)
        }
        ...
        key: 'make_boost_snap',
        value: function make_boost_snap(details, combsGroup, margin, snap) {
            var boostWin = details.context.boost_pay + (details.context.boost_pay_2 || 0);
            var group = combsGroup.group();
            var labelLeft = '';
            var labelRight = '';
            this._background(group, details, details.state);
            margin += this.width;
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            progress.animate({
                width: 186
            ...
            labelLeft = 'Boost win!';
            if (this.width > 200) {
                group.text('50%', '50%', 'Boost win').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER_2);
                group.text('50%', '70%', this.coins(boostWin)).attr(Drawer.FREESPINS_STYLE_TEXT_CENTER_2)
            } else {
                group.text('50%', '50%', 'Boost win').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
                group.text('50%', '70%', this.coins(boostWin)).attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
            }
            this._panel(group, labelLeft, labelRight);
            return snap
        }
...
Drawer.FREESPINS_STYLE_TEXT_CENTER_2 = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#0000',
    'font-size': '1.4em'
