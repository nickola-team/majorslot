...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && this.get_pearls_count(details) === 20 || details.state == 'freespins' && details.context.winscatters.length > 0 || details.state == 'bonus' && details.context.last_respin && this.is_extra_respins(details)
        }
    }, {
        key: 'get_arr_sum',
            ...
            if (this.is_boost(details)) {
                ...
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if (game_board[i][j] == 12 || game_board[i][j] == 13) {
        ...
        value: function _comb(snap, combs, comb, details) {
            ...
            var labelLeft = this.title(details) + ', Win:' + this.coins(comb.amount);
            var labelRight = combs.indexOf(comb) + 1 + '/' + combs.length;
            var multLabel = details.state == 'freespins' ? ',x' + this.get_mult(comb.positions) : '';
            this._panel(group, labelLeft + multLabel, labelRight);
            this.drawe_border_win_symbols(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            this.drawe_line_patern(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            return group
        }
        ...
        key: 'get_mult',
        value: function get_mult(positions) {
            var _this3 = this;
            var mult = 1;
            positions.forEach(function(pos) {
                var pos_mult = _this3.details.context.wild_symbols_values[pos[0]][pos[1]];
                if (pos_mult != 0) {
                    mult *= pos_mult
                }
            });
        ...
        value: function board(snap, _board) {
            ...
            if (this.is_boost(this.details)) {
                ...
                for (var i in _board) {
                    for (var j in _board[i]) {
                        if (_board[i][j] === 12 || _board[i][j] === 13) {
                            winboost.positions.push([i, j])
                        }
                    }
                }
            ...
            var _loop = function _loop(_i4) {
                var _loop2 = function _loop2(_j3) {
                    imageUrl = _this4.url(_board[_i4][_j3] + '.png');
                    if (_this4.details.state === 'bonus' && bonus_symbols.indexOf(_board[_i4][_j3]) == -1) {
                        imageUrl = _this4.url('0.png')
                    }
                    if (_this4.details.state === 'freespins' && _board[_i4][_j3] == 10) {
                        var wild_value = _this4.details.context['wild_symbols_values'][_i4][_j3];
                        imageUrl = _this4.url('10' + (wild_value === 1 ? '' : 'x' + wild_value) + '.png')
                    }
                    if (_board[_i4][_j3] == 14) {
                        imageUrl = _this4.url('0.png')
                    }
                    var img = group.image(imageUrl, (_this4.cellWidth + _this4.cellOffsetX) * _i4, (_this4.cellHeight + _this4.cellOffsetY) * _j3, _this4.cellWidth, _this4.cellHeight);
                    if (combs.length == 1) {
                        currentWinLine = 0
                    }
                    var value = void 0;
                    if (_this4.details.context.bs_v != undefined) {
                        value = _this4.details.context.bs_v[_i4][_j3]
                    } else {
                        value = _this4.details.context.bonus_symbols_v[_i4][_j3]
                    }
                    var is_last_respin = _this4.details.context.last_respin;
                    if (typeof value === 'number') {
                        value = (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    }
                    if (value != 0 && _board[_i4][_j3] == 12) {
                        var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i4 - _this4.cellOffsetX;
                        var rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j3 + _this4.cellHeight / 3;
                        var rect_width = _this4.cellWidth + 2 * _this4.cellOffsetX;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = (_this4.cellHeight + _this4.cellOffsetY) * _j3 + _this4.cellHeight / 2;
