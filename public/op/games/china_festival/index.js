...
var DOUBLER_ID = 12;
var COLLECT_ID = 13;
var BOOST_ID_ARRAY = [11, 110];
var DOUBLER_ID_ARRAY = [12, 120];
var COLLECT_ID_ARRAY = [13, 130];
var BONUS_PROGRESS_ID_ARRAY = [11, 12, 13];
var MYSTERY_ID = 15;
var BONUS_ID_ARRAY = [11, 12, 13, 14, 15, 110, 120, 130];
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function make_spins(details) {
            var _this2 = this;
            this._initDynamic();
            var snap = this._make_snap(details);
            this._background(snap);
            var group = snap.group().attr({
                width: this.width,
                height: this.height
            });
            var combs = this._getCombs();
            this._clientCombinations = combs;
            var combsGroup = snap.group();
            var combGroup = void 0;
            var margin = 0;
            this._combs(combsGroup, combs, details);
            ...
            var board = this.board(group);
            board.transform('T' + this.boardMarginLeft + ',' + (this.boardMarginTop + this.panelHeight + 1));
            if (details.last_action === 'bonus_spins_stop') {
                board.remove();
                this._panel(snap, this.get_text_for_action(details), '')
            }
            var counter = 0;
            snap.click(function() {
                if (combsGroup.children().length < 2) return;
                ...
                board.remove();
                board = _this2.board(group, counter - 1);
                board.transform('T' + _this2.boardMarginLeft + ',' + (_this2.boardMarginTop + _this2.panelHeight + 1))
            });
            if (details.last_action === 'bonus_spins_stop') snap.text('50%', '56%', this.get_text_for_action(details)).attr(Drawer.STYLE_TEXT_CENTER_RED);
            return snap.node
        }
    }, {
        key: 'make_bonus',
        value: function make_bonus(details) {
            ...
            this._initDynamic();
            var snap = this._make_snap(details);
            this._clientCombinations = this._get_bonus_combs();
            this._background(snap);
            var group = snap.group().attr({
                width: this.width,
                height: this.height
            });
            var board = this.board(group, counter);
            board.transform('T' + this.boardMarginLeft + ',' + (this.boardMarginTop + this.panelHeight + 1));
            var partSize = this.width / this._clientCombinations.length;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(Drawer.STYLE_PROGRESS);
            var combsGroup = snap.group();
            var combGroup = this._comb(combsGroup, this._clientCombinations, this._clientCombinations[counter], details, true);
            snap.click(function() {
                if (_this3._clientCombinations.length < 2) return;
                counter = counter + 1 > _this3._clientCombinations.length - 1 ? 0 : counter + 1;
                progress.animate({
                    width: partSize * (counter + 1)
                }, 100);
                board.remove();
                board = _this3.board(group, counter);
                board.transform('T' + _this3.boardMarginLeft + ',' + (_this3.boardMarginTop + _this3.panelHeight + 1));
                var currentCombination = _this3._clientCombinations[counter];
                combGroup.remove();
                combGroup = _this3._comb(combsGroup, _this3._clientCombinations, currentCombination, details, true)
            });
            return snap.node
        }
    }, {
        key: '_make_snap',
        value: function _make_snap() {
            var gameBoard = this._dynamicBoard;
            this.cols = gameBoard.length;
            this.rows = gameBoard[0].length;
            this._calculateSizes(this.cols, this.rows);
        ...
        value: function _calculateSizes(cols, rows) {
            ...
            this.panelFont = this.height > 150 ? '1em' : '0.7em';
            this.progressHeight = 3;
            this.boardHeight = this.height - (this.panelHeight + this.progressHeight) - 2;
            var cellWidth = parseInt((this.width - 2) / cols, 10);
            this.cellHeight = cellWidth * 0.921;
            this.cellWidth = cellWidth;
            this.cellOffsetX = 0;
            this.cellOffsetY = 0;
            this.boardMarginLeft = 3;
            this.boardMarginTop = (this.boardHeight - this.cellHeight * rows) / 2
        }
    }, {
        key: '_background',
        value: function _background(snap) {
            if (this._is_bonus_game() || this.details.last_action == 'bonus_spins_stop') {
                snap.rect(0, 0, this.width, this.height).attr({
                    fill: '#04046F'
                })
            } else {
        ...
        key: '_getCombs',
        value: function _getCombs() {
            var combs = [].concat(_toConsumableArray(this.details.context.winlines));
            if (this._is_bac_win()) {
                var beforeWinBonus = {
                    bonusWin: true,
                    positions: [],
                ...
                for (var i in this._dynamicBoard) {
                    for (var j in this._dynamicBoard[i]) {
                        if (BONUS_ID_ARRAY.includes(this._dynamicBoard[i][j])) {
                            beforeWinBonus.positions.push([i, j])
                        }
                    }
                }
                combs.push(beforeWinBonus)
            }
            return combs
        }
    }, {
        key: '_is_bonus_game',
        value: function _is_bonus_game() {
            return this.details.state === 'bonus'
        }
    }, {
        key: '_is_bac_win',
        value: function _is_bac_win() {
            return this.details.context.bac_win || false
        }
    }, {
        key: '_combs',
        value: function _combs(snap, combs, details) {
            var group = snap.group();
            this.bordered_cells = [];
            var totalWin = details.round_win || 0;
            var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin);
            var labelRight = combs.length > 1 ? 'all/' + combs.length : '';
            this._panel(group, labelLeft, labelRight);
            if (combs.length && combs[0].isPopUp) {
                group.text('50%', '56%', this.get_text_for_action(details)).attr(Drawer.STYLE_TEXT_CENTER_RED);
                return group
            } else {
                if (combs.length && !combs[0].positions.length) return group;
                for (var i = 0; i < combs.length; ++i) {
                    if (combs[i].isPopUp) continue;
                    this.drawe_border_win_symbols(group, combs[i]).transform('T' + this.boardMarginLeft + ',' + (this.boardMarginTop + this.panelHeight + 1))
                }
                if (!this._is_bonus_game()) {
                    for (var _i = 0; _i < combs.length; ++_i) {
                        if (combs[_i].isPopUp || combs[_i].bonusWin) continue;
                        this.drawe_line_patern(group, combs[_i]).transform('T' + this.boardMarginLeft + ',' + (this.boardMarginTop + this.panelHeight + 1))
                    }
                }
                return group
            }
        ...
        key: '_comb',
        value: function _comb(snap, combs, comb, details) {
            var isBonus = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
            this.bordered_cells = [];
            var group = snap.group();
            var labelLeft = comb.isGrand ? 'Jackpot!' : this.title(details) + ', Win:' + this.coins(comb.amount);
            var bonusLabel = combs.length > 1 ? (combs.indexOf(comb) ? combs.indexOf(comb) : 'all') + '/' + (combs.length - 1) : '';
            var mainLabel = combs.length > 1 ? combs.indexOf(comb) + 1 + '/' + combs.length : '';
            var labelRight = isBonus ? bonusLabel : mainLabel;
            this._panel(group, labelLeft, labelRight);
            if (comb.isPopUp) {
                return group
            } else {
                this.drawe_border_win_symbols(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.boardMarginTop + this.panelHeight + 1));
                if (!this._is_bonus_game() && !comb.bonusWin) this.drawe_line_patern(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.boardMarginTop + this.panelHeight + 1));
                return group
            }
        }
    }, {
        ...
        value: function get_text_for_action(details) {
            var action = details.last_action;
            if (this.details.state === 'bonus' && this.details.context.bs_count === this.cols * this.rows) {
                return 'Jackpot!'
            }
            switch (action) {
                case 'bonus_init':
        ...
        key: '_isBonusEnd',
        value: function _isBonusEnd() {
            var details = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.details;
            return details.context.rounds_left === 0 && details.state == 'bonus'
        }
    }, {
        key: '_get_bonus_combs',
        value: function _get_bonus_combs() {
            var _this4 = this;
            var combs = [];
            var mysteryData = this._get_mystery_data();
            var boostData = this._get_boost_data();
            var doublerData = this._get_doubler_data();
            var collectData = this._get_collect_data();
            var win = this.details.round_win || 0;
            if (mysteryData.length) {
                mysteryData.forEach(function(data) {
                    combs.push({
                        positions: [data.pos],
                        opened: false,
                        data: data,
            ...
            if (boostData.length) {
                boostData.forEach(function(data) {
                    var skipOpening = mysteryData.length && mysteryData.some(function(d) {
                        return d.pos[0] === data.pos[0] && d.pos[1] === data.pos[1]
                    });
                    if (!skipOpening) combs.push({
                        positions: [data.pos],
                        opened: false,
                        data: data,
                        amount: win
            ...
            if (doublerData.length) {
                doublerData.forEach(function(data) {
                    var skipOpening = mysteryData.length && mysteryData.some(function(d) {
                        return d.pos[0] === data.pos[0] && d.pos[1] === data.pos[1]
                    });
                    if (!skipOpening) combs.push({
            ...
            if (collectData.length) {
                collectData.forEach(function(data) {
                    var skipOpening = mysteryData.length && mysteryData.some(function(d) {
                        return d.pos[0] === data.pos[0] && d.pos[1] === data.pos[1]
                    });
                    if (!skipOpening) combs.push({
            ...
            combs.forEach(function(comb) {
                if (comb.data) {
                    var pos = comb.data.pos;
                    var mysteryOpenedId = comb.data.id;
                    var hasPosition = processedPositions.length && processedPositions.some(function(d) {
                        return d[0] === pos[0] && d[1] === pos[1]
                    });
                    if (!mysteryOpenedId && comb.opened && !DOUBLER_ID_ARRAY.includes(_this4._dynamicBoard[pos[0]][pos[1]]) || comb.doublerOpened) {
                        _this4._dynamicBoard.forEach(function(rows, col) {
                            rows.forEach(function(id, row) {
                                if (BONUS_ID_ARRAY.includes(id) && _this4._dynamicValues[col][row]) comb.positions.push([col, row])
                            })
                        });
                        processedPositions.forEach(function(p) {
                            return comb.positions.push(p)
                        })
                    }
                    if (!hasPosition && (!mysteryOpenedId || BONUS_ID === mysteryOpenedId)) processedPositions.push(pos)
                }
            });
            if (combs.length) combs.unshift({
                positions: [],
                isAllComb: true,
                amount: win
            });
            if (this.details.state === 'bonus' && this.details.context.bs_count === this.cols * this.rows) {
                ...
                if (combs.length === 1) combs.unshift({
                    positions: [],
                    isAllComb: true,
                    amount: win
                })
            }
            if (this.details.last_action === 'bonus_init' || this.details.last_action === 'bonus_spins_stop') {
                combs.unshift({
                    positions: [],
                    isPopUp: true
                })
            ...
            this.details.last_action === 'restore' && combs.splice(1);
            return combs
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            if (details.state !== 'bonus' || details.context.bs_count !== this.cols * this.rows) return;
            ...
            labelLeft = 'Jackpot!';
            if (this.width > 200) {
                group.text('50%', '60%', 'Jackpot !').attr(Drawer.GRAND_STYLE_TEXT_2)
            } else {
                group.text('50%', '60%', 'Jackpot !').attr(Drawer.GRAND_STYLE_TEXT)
            }
            this._panel(group, labelLeft, labelRight);
            return snap
        }
        ...
        key: 'board',
        value: function board(snap) {
            var _this5 = this;
            var currentCombIndex = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : -1;
            var group = snap.group();
            var currentCombination = this._clientCombinations[currentCombIndex];
            var jackpots = this._get_jackpots();
            var textJackpots = ['mini', 'minor', 'major'];
            var isBonusGame = this._is_bonus_game();
            var isAllComb = currentCombination && currentCombination.isAllComb;
            var isNextAfterAllComb = currentCombination && this._clientCombinations[currentCombIndex - 1] && this._clientCombinations[currentCombIndex - 1].isAllComb;
            if (isBonusGame) {
                if (isAllComb) this._initDynamic(true);
                else if (isNextAfterAllComb) this._initDynamic()
            } else if (currentCombIndex === 0) {
                this._initDynamic()
            }
            if (currentCombIndex === 0) this._initDynamic(this._clientCombinations[currentCombIndex].isAllComb);
            if (currentCombination && currentCombination.isPopUp) {
                group.text('50%', '43%', this.get_text_for_action(this.details)).attr(Drawer.STYLE_TEXT_CENTER_RED);
                return group
            }
            if (currentCombination && currentCombination.data) {
                var specialSymbolData = currentCombination.data;
                var pos = specialSymbolData.pos;
                var symbolId = this._dynamicBoard[pos[0]][pos[1]];
                if (currentCombination.opened || currentCombination.doublerOpened) {
                    var collectValue = 0;
                    if (specialSymbolData.id) {
                        this._dynamicBoard[pos[0]][pos[1]] = specialSymbolData.id;
                        if (BONUS_ID === specialSymbolData.id) {
                            if (jackpots.includes[specialSymbolData.bs_v]) {
                                var index = jackpots.indexOf(specialSymbolData.bs_v);
                                this._dynamicValues[pos[0]][pos[1]] = textJackpots[index]
                            } else {
                                this._dynamicValues[pos[0]][pos[1]] = specialSymbolData.bs_v
                            }
                        }
                    } else if (symbolId < 100) {
                        this._dynamicBoard[pos[0]][pos[1]] = symbolId * 10;
                        this._dynamicBoard.forEach(function(rows, i) {
                            rows.forEach(function(r, j) {
                                var symbolValue = _this5._dynamicValues[i][j];
                                if (textJackpots.includes(symbolValue)) {
                                    var _index = textJackpots.indexOf(symbolValue);
                                    symbolValue = jackpots[_index]
                                }
                                var symbolId = _this5._dynamicBoard[i][j];
                                var isAvailableSymbolForUpdate = [14, 110, 120, 130].includes(symbolId);
                                if (DOUBLER_ID_ARRAY.includes(_this5._dynamicBoard[pos[0]][pos[1]]) && isAvailableSymbolForUpdate && currentCombination.doublerOpened) {
                                    _this5._dynamicValues[i][j] = symbolValue * 2
                                }
                                if (BOOST_ID_ARRAY.includes(_this5._dynamicBoard[pos[0]][pos[1]]) && isAvailableSymbolForUpdate && currentCombination.opened) {
                                    _this5._dynamicValues[i][j] = symbolValue + specialSymbolData.bs_v
                                }
                                if (COLLECT_ID_ARRAY.includes(_this5._dynamicBoard[pos[0]][pos[1]]) && isAvailableSymbolForUpdate && currentCombination.opened) {
                                    collectValue = collectValue + symbolValue
                                }
                            })
                        })
                    }
                    if (COLLECT_ID_ARRAY.includes(this._dynamicBoard[pos[0]][pos[1]]) && collectValue) {
                        this._dynamicValues[pos[0]][pos[1]] = collectValue
                    }
                    if (currentCombination.doublerOpened && DOUBLER_ID_ARRAY.includes(this._dynamicBoard[pos[0]][pos[1]])) {
                        this._dynamicValues[pos[0]][pos[1]] = 'x2'
                    }
                    if (currentCombination.opened && DOUBLER_ID_ARRAY.includes(this._dynamicBoard[pos[0]][pos[1]])) {
                        this._dynamicValues[pos[0]][pos[1]] = specialSymbolData.bs_v
                    }
                }
            }
            if (currentCombIndex === -1 || currentCombIndex === this._clientCombinations.length - 1) this._initDynamic(true);
            this._dynamicBoard.forEach(function(rows, i) {
                rows.forEach(function(r, j) {
                    var value = _this5._dynamicValues.length ? _this5._dynamicValues[i][j] : 0;
                    if (value && BONUS_PROGRESS_ID_ARRAY.includes(_this5._dynamicBoard[i][j])) {
                        _this5._dynamicBoard[i][j] = _this5._dynamicBoard[i][j] * 10
                    }
                    var symbolId = _this5._dynamicBoard[i][j];
                    var isBonusSymbol = BONUS_ID_ARRAY.includes(symbolId);
                    var isBonusGame = _this5._is_bonus_game();
                    var imgName = symbolId + '.png';
                    if (isBonusGame && !isBonusSymbol) imgName = '0.png';
                    var img = group.image(_this5.url(imgName), (_this5.cellWidth + _this5.cellOffsetX) * i, (_this5.cellHeight + _this5.cellOffsetY) * j, _this5.cellWidth, _this5.cellHeight);
                    if (isBonusSymbol && value) {
                        if (typeof value === 'number') {
                            var formattedValue = (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                            var rectX = (_this5.cellWidth + _this5.cellOffsetX) * i - _this5.cellOffsetX;
                            var rectY = (_this5.cellHeight + _this5.cellOffsetY) * j + _this5.cellHeight / 3;
                            var rectWidth = _this5.cellWidth + 2 * _this5.cellOffsetX;
                            var textX = rectX + rectWidth / 2;
                            var textY = (_this5.cellHeight + _this5.cellOffsetY) * j + _this5.cellHeight / 2;
                            group.rect(rectX, rectY, rectWidth, _this5.cellHeight / 3).attr({
                                fill: 'white'
                            });
                            if (formattedValue.length > 14) {
                                if (_this5.width > 200) {
                                    group.text(textX, textY, formattedValue).attr(Drawer.PEARL_VALUE_34)
                                } else {
                                    group.text(textX, textY, formattedValue).attr(Drawer.PEARL_VALUE_20)
                                }
                            } else if (formattedValue.length > 10) {
                                if (_this5.width > 200) {
                                    group.text(textX, textY, formattedValue).attr(Drawer.PEARL_VALUE_44)
                                } else {
                                    group.text(textX, textY, formattedValue).attr(Drawer.PEARL_VALUE_25)
                                }
                            } else if (formattedValue.length > 6) {
                                if (_this5.width > 200) {
                                    group.text(textX, textY, formattedValue).attr(Drawer.PEARL_VALUE_60)
                                } else {
                                    group.text(textX, textY, formattedValue).attr(Drawer.PEARL_VALUE_34)
                                }
                            } else {
                                if (_this5.width > 200) {
                                    group.text(textX, textY, formattedValue).attr(Drawer.PEARL_VALUE_80)
                                } else {
                                    group.text(textX, textY, formattedValue).attr(Drawer.PEARL_VALUE_46)
                                }
                            }
                        } else {
                            var _rectX = (_this5.cellWidth + _this5.cellOffsetX) * i - _this5.cellOffsetX;
                            var _rectY = (_this5.cellHeight + _this5.cellOffsetY) * j + _this5.cellHeight / 3;
                            var _rectWidth = _this5.cellWidth + 2 * _this5.cellOffsetX;
                            var _textX = _rectX + _rectWidth / 2;
                            var _textY = (_this5.cellHeight + _this5.cellOffsetY) * j + _this5.cellHeight / 2;
                            group.rect(_rectX, _rectY, _rectWidth, _this5.cellHeight / 3).attr({
                                fill: 'white'
                            });
                            if (value == 'mini') {
                                if (_this5.width > 200) {
                                    group.text(_textX, _textY, value).attr(Drawer.BS_MINI_70)
                                } else {
                                    group.text(_textX, _textY, value).attr(Drawer.BS_MINI_40)
                                }
                            } else if (value == 'minor') {
                                if (_this5.width > 200) {
                                    group.text(_textX, _textY, value).attr(Drawer.BS_MINOR_70)
                                } else {
                                    group.text(_textX, _textY, value).attr(Drawer.BS_MINOR_40)
                                }
                            } else if (value == 'major') {
                                if (_this5.width > 200) {
                                    group.text(_textX, _textY, value).attr(Drawer.BS_MAJOR_70)
                                } else {
                                    group.text(_textX, _textY, value).attr(Drawer.BS_MAJOR_40)
                                }
                            } else if (_this5.width > 200) {
                                group.text(_textX, _textY, value).attr(Drawer.BS_MINI_70)
                            } else {
                                group.text(_textX, _textY, value).attr(Drawer.BS_MINI_40)
                            }
                    ...
                    if ('spins' === _this5.details.state && _this5._clientCombinations.length) {
                        var positions = currentCombination ? currentCombination.positions : _this5._clientCombinations[0].positions;
                        if (!currentCombination && _this5._clientCombinations.length > 1) return;
                        img.attr({
                            opacity: 0.5
                        });
        ...
        key: '_get_boost_data',
        value: function _get_boost_data() {
            return this.details.context.boost_values || []
        }
    }, {
        key: '_get_doubler_data',
        value: function _get_doubler_data() {
            return this.details.context.double_values || []
        }
    }, {
        key: '_get_collect_data',
        value: function _get_collect_data() {
            return this.details.context.collect_values || []
        }
    }, {
        key: '_get_mystery_data',
        value: function _get_mystery_data() {
            return this.details.context.mystery_values || []
        }
    }, {
        key: '_get_special_symbols_data',
        value: function _get_special_symbols_data() {
            var sp_data = [];
            var boostData = this._get_boost_data();
            var doudlerData = this._get_doubler_data();
            var collectData = this._get_collect_data();
            var mysteryData = this._get_mystery_data();
            if (boostData.length) sp_data.push.apply(sp_data, _toConsumableArray(boostData));
            if (doudlerData.length) sp_data.push.apply(sp_data, _toConsumableArray(doudlerData));
            if (collectData.length) sp_data.push.apply(sp_data, _toConsumableArray(collectData));
            if (mysteryData.length) sp_data.push.apply(sp_data, _toConsumableArray(mysteryData));
            return sp_data
        }
    }, {
        key: 'get_origin_bsv',
        value: function get_origin_bsv() {
            return this.details.context.orig_bs_v || []
        }
    }, {
        key: 'get_origin_board',
        value: function get_origin_board() {
            return this.details.context.orig_board || []
        }
    }, {
        key: 'get_board',
        value: function get_board() {
            return this.details.context.board || []
        }
    }, {
        key: 'get_bsv',
        value: function get_bsv() {
            return this.details.context.bs_v || []
        }
    }, {
        key: '_get_jackpots',
        value: function _get_jackpots() {
            return this.details.context.jackpot_values || []
        }
    }, {
        key: '_initDynamic',
        value: function _initDynamic() {
            var initByFinal = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
            var sp_symbols_data = this._get_special_symbols_data();
            var originBsv = this.get_origin_bsv();
            sp_symbols_data.forEach(function(data) {
                originBsv[data.pos[0]][data.pos[1]] = 0
            });
            var finalBsv = this.get_bsv();
            var originBoard = this.get_origin_board();
            var finalBoard = this.get_board();
            this._dynamicBoard = originBoard.length && !initByFinal && this.details.last_action !== 'bonus_init' ? originBoard.map(function(innerArray) {
                return [].concat(_toConsumableArray(innerArray))
            }) : finalBoard.map(function(innerArray) {
                return [].concat(_toConsumableArray(innerArray))
            });
            this._dynamicValues = originBsv.length && !initByFinal ? originBsv.map(function(innerArray) {
                return [].concat(_toConsumableArray(innerArray))
            }) : finalBsv.map(function(innerArray) {
                return [].concat(_toConsumableArray(innerArray))
            })
        }
    }]);
...
Drawer.GRAND_STYLE_TEXT = {
    stroke: '#fff',
    'text-anchor': 'middle',
    fill: '#fff',
    'font-size': '1.2em'
};
Drawer.GRAND_STYLE_TEXT_2 = {
    stroke: '#fff',
    'text-anchor': 'middle',
    fill: '#fff',
    'font-size': '1.4em'
