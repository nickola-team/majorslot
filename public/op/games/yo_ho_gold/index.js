    ...
    _createClass(Drawer, [{
        ...
        value: function make_snap(details) {
            ...
            if (this.details.state === 'bonus') {
                this._calculateSizesBonus(this.cols, this.rows)
            } else {
                this._calculateSizes(this.cols, this.rows)
            }
        ...
        value: function make_bonus(details) {
            ...
            if (this.get_pearls_count(this.details) >= 3 && ['spin', 'freespin'].includes(this.details.last_action)) {
                ...
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if ([10, 16, 17].includes(game_board[i][j])) {
                            winbonus.positions.push([i, j])
                        }
                    }
                }
            ...
            if (new_regulars != undefined || permanents != undefined) {
                for (var _i2 in new_regulars) {
                    var pos = new_regulars[_i2];
                    var win_spec = {
                        'trigger': 'none',
                        'positions': [],
                        'bs_v': []
                    };
                    if (game_board[pos[0]][pos[1]] == add_line_id) {
                        win_spec.trigger = 'add_line';
                        win_spec.positions.push(pos);
                        combs.push(win_spec)
                    }
                    if (game_board[pos[0]][pos[1]] == mult_id) {
                        for (var _j in this.details.context.multipliers) {
                            var reg = this.details.context.multipliers[_j];
                            if (reg.pos[0] === pos[0] && reg.pos[1] === pos[1]) {
                                var win_spec_prev = {
                                    'trigger': 'mult',
                                    'positions': [],
                                    'bs_v': []
                                };
                                win_spec_prev.positions.push(pos);
                                combs.push(win_spec_prev);
                                for (var q in reg.targets) {
                                    var win_mult = {
                                        'trigger': 'mult',
                                        'positions': [reg.targets[q]],
                                        'bs_v': []
                                    };
                                    combs.push(win_mult)
                                }
                    ...
                    if (game_board[pos[0]][pos[1]] == collector_id) {
                        for (var _j2 in this.details.context.collectors) {
                            var _reg = this.details.context.collectors[_j2];
                            if (_reg.pos[0] === pos[0] && _reg.pos[1] === pos[1]) {
                                var _win_spec_prev = {
                                    'trigger': 'collect',
                                    'positions': [],
                                    'bs_v': []
                                };
                                _win_spec_prev.positions.push(pos);
                                combs.push(_win_spec_prev);
                                win_spec.trigger = 'collect';
                                for (var l in this.details.context.bs_pos) {
                                    var p = this.details.context.bs_pos[l];
                                    if (p[0] !== pos[0] || p[1] !== pos[1]) {
                                        win_spec.positions.push(p)
                                    }
                                }
                                combs.push(win_spec);
                                break
                    ...
                    if (game_board[pos[0]][pos[1]] == payer_id) {
                        for (var _j3 in this.details.context.payers) {
                            var _reg2 = this.details.context.payers[_j3];
                            if (_reg2.pos[0] === pos[0] && _reg2.pos[1] === pos[1]) {
                                var _win_spec_prev2 = {
                                    'trigger': 'payer',
                                    'positions': [],
                                    'bs_v': []
                                };
                                _win_spec_prev2.positions.push(pos);
                                _win_spec_prev2.v = _reg2.v;
                                combs.push(_win_spec_prev2);
                                win_spec.trigger = 'payer';
                                win_spec.v = _reg2.v;
                                for (var _l in this.details.context.bs_pos) {
                                    var _p = this.details.context.bs_pos[_l];
                                    if (_p[0] !== pos[0] || _p[1] !== pos[1]) {
                                        win_spec.positions.push(_p)
                                    }
                                }
                                combs.push(win_spec);
                                break
                ...
                for (var _i3 in permanents) {
                    var _pos = permanents[_i3];
                    var _win_spec = {
                        'trigger': 'none',
                        'positions': [],
                        'bs_v': []
                    };
                    if (game_board[_pos[0]][_pos[1]] == perm_collector) {
                        for (var _j4 in this.details.context.permanent_collectors) {
                            var perm = this.details.context.permanent_collectors[_j4];
                            if (perm.pos[0] === _pos[0] && perm.pos[1] === _pos[1]) {
                                var _win_spec_prev3 = {
                                    'trigger': 'collect',
                                    'positions': [],
                                    'bs_v': []
                                };
                                _win_spec_prev3.positions.push(_pos);
                                combs.push(_win_spec_prev3);
                                _win_spec.trigger = 'collect';
                                for (var _l2 in this.details.context.bs_pos) {
                                    var _p2 = this.details.context.bs_pos[_l2];
                                    if (_p2[0] !== _pos[0] || _p2[1] !== _pos[1]) {
                                        _win_spec.positions.push(_p2)
                                    }
                                }
                                combs.push(_win_spec);
                                break
                            }
                        }
                    }
                    if (game_board[_pos[0]][_pos[1]] == perm_payer) {
                        for (var _j5 in this.details.context.permanent_payers) {
                            var _perm = this.details.context.permanent_payers[_j5];
                            if (_perm.pos[0] === _pos[0] && _perm.pos[1] === _pos[1]) {
                                var _win_spec_prev4 = {
                                    'trigger': 'payer',
                                    'positions': [],
                                    'bs_v': []
                                };
                                _win_spec_prev4.positions.push(_pos);
                                _win_spec_prev4.v = _perm.v;
                                combs.push(_win_spec_prev4);
                                _win_spec.trigger = 'payer';
                                _win_spec.v = _perm.v;
                                for (var _l3 in this.details.context.bs_pos) {
                                    var _p3 = this.details.context.bs_pos[_l3];
                                    if (_p3[0] !== _pos[0] || _p3[1] !== _pos[1]) {
                                        _win_spec.positions.push(_p3)
                                    }
                                }
                                combs.push(_win_spec);
                                break
            ...
            if (details.state == 'bonus' && details.context.rounds_left == 0 && details.context.mystery_pos.length) {
                ...
                try {
                    for (var _iterator2 = details.context.mystery_pos[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                        var mys_pos = _step2.value;
                        board_with_mystery[mys_pos[0]][mys_pos[1]] = 13
                    }
                } catch (err) {
        ...
        value: function board(snap, _board) {
            ...
            if (this.get_pearls_count(this.details) >= 3 && ['spin', 'freespin'].includes(this.details.last_action)) {
                ...
                for (var i in _board) {
                    for (var j in _board[i]) {
                        if ([10, 16, 17].includes(_board[i][j])) {
                            winbonus.positions.push([i, j])
                        }
                    }
                }
            ...
            _board = this.details.context.board;
            var new_regulars = this.details.context.new_regulars;
            var permanents = this.details.context.permanents;
            var curr_board = {};
            Object.assign(curr_board, this.details.context.orig_board);
            if (new_regulars != undefined || permanents != undefined) {
                for (var _i5 in new_regulars) {
                    var pos = new_regulars[_i5];
                    var win_spec = {
                        'trigger': 'none',
                        'positions': [],
                        'bs_v': []
                    };
                    if (_board[pos[0]][pos[1]] == add_line_id) {
                        win_spec.positions.push(pos);
                        win_spec.bs_v = curr_board.bs_v;
                        combs.push(win_spec)
                    }
                    if (_board[pos[0]][pos[1]] == mult_id) {
                        for (var _j6 in this.details.context.multipliers) {
                            var reg = this.details.context.multipliers[_j6];
                            if (reg.pos[0] === pos[0] && reg.pos[1] === pos[1]) {
                                var win_spec_prev = {
                                    'trigger': 'mult',
                                    'positions': [],
                                ...
                                win_spec_prev.positions.push(pos);
                                win_spec_prev.bs_v = curr_board.bs_v;
                                combs.push(win_spec_prev);
                                var mult_bs_v = [];
                                for (var l in curr_board.bs_v) {
                                    mult_bs_v.push(curr_board.bs_v[l].slice())
                                }
                                for (var q in reg.targets) {
                                    var p = reg.targets[q];
                                    var win_mult = {
                                    ...
                                    win_mult.positions.push(p);
                                    for (var _l4 in mult_bs_v) {
                                        win_mult.bs_v.push(mult_bs_v[_l4].slice())
                                    }
                                    win_mult.bs_v[p[0]][p[1]] = win_mult.bs_v[p[0]].slice()[p[1]] * 2;
                                    win_mult.positions.push(pos);
                                    combs.push(win_mult);
                                    for (var _l5 in win_mult.bs_v) {
                                        mult_bs_v[_l5] = win_mult.bs_v[_l5].slice()
                                    }
                                }
                                curr_board.bs_v = reg.bs_v;
                                break
                            }
                        }
                    }
                    if (_board[pos[0]][pos[1]] == collector_id) {
                        for (var _j7 in this.details.context.collectors) {
                            var _reg3 = this.details.context.collectors[_j7];
                            if (_reg3.pos[0] === pos[0] && _reg3.pos[1] === pos[1]) {
                                var _win_spec_prev5 = {
                                    'trigger': 'none',
                                    'positions': [],
                                    'bs_v': []
                                };
                                _win_spec_prev5.positions.push(pos);
                                _win_spec_prev5.bs_v = curr_board.bs_v;
                                combs.push(_win_spec_prev5);
                                win_spec.positions = this.details.context.bs_pos;
                                win_spec.bs_v = _reg3.bs_v;
                                combs.push(win_spec);
                                curr_board.bs_v = _reg3.bs_v;
                                break
                            }
                        }
                    }
                    if (_board[pos[0]][pos[1]] == payer_id) {
                        for (var _j8 in this.details.context.payers) {
                            var _reg4 = this.details.context.payers[_j8];
                            if (_reg4.pos[0] === pos[0] && _reg4.pos[1] === pos[1]) {
                                var _win_spec_prev6 = {
                                    'trigger': 'none',
                                    'positions': [],
                                    'bs_v': []
                                };
                                _win_spec_prev6.positions.push(pos);
                                _win_spec_prev6.bs_v = curr_board.bs_v;
                                combs.push(_win_spec_prev6);
                                win_spec.positions = this.details.context.bs_pos;
                                win_spec.bs_v = _reg4.bs_v;
                                combs.push(win_spec);
                                curr_board.bs_v = _reg4.bs_v;
                                break
                            }
                        }
                    }
                }
                for (var _i6 in permanents) {
                    var curr_bs_v = void 0;
                    var _pos2 = permanents[_i6];
                    var _win_spec2 = {
                        'trigger': 'none',
                        'positions': [],
                        'bs_v': []
                    };
                    if (_board[_pos2[0]][_pos2[1]] == perm_collector) {
                        for (var _j9 in this.details.context.permanent_collectors) {
                            var perm = this.details.context.permanent_collectors[_j9];
                            if (perm.pos[0] === _pos2[0] && perm.pos[1] === _pos2[1]) {
                                var _win_spec_prev7 = {
                                    'trigger': 'none',
                                    'positions': [],
                                    'bs_v': []
                                };
                                _win_spec_prev7.positions.push(_pos2);
                                _win_spec_prev7.bs_v = curr_board.bs_v;
                                combs.push(_win_spec_prev7);
                                _win_spec2.positions = this.details.context.bs_pos;
                                _win_spec2.bs_v = perm.bs_v;
                                combs.push(_win_spec2);
                                curr_board.bs_v = perm.bs_v;
                                break
                            }
                        }
                    }
                    if (_board[_pos2[0]][_pos2[1]] == perm_payer) {
                        for (var _j10 in this.details.context.permanent_payers) {
                            var _perm2 = this.details.context.permanent_payers[_j10];
                            if (_perm2.pos[0] === _pos2[0] && _perm2.pos[1] === _pos2[1]) {
                                var _win_spec_prev8 = {
                                    'trigger': 'none',
                                    'positions': [],
                                    'bs_v': []
                                };
                                _win_spec_prev8.positions.push(_pos2);
                                _win_spec_prev8.bs_v = curr_board.bs_v;
                                combs.push(_win_spec_prev8);
                                _win_spec2.positions = this.details.context.bs_pos;
                                _win_spec2.bs_v = _perm2.bs_v;
                                combs.push(_win_spec2);
                                curr_board.bs_v = _perm2.bs_v;
                                break
                            }
                        }
                    }
            ...
            var _loop = function _loop(_i7) {
                var _loop2 = function _loop2(_j11) {
                    if (combs.length == 1 && _this4.details.state !== 'bonus') {
                        currentWinLine = 0
                    }
                    imageUrl = _this4.url(_board[_i7][_j11] + '.png');
                    if (_this4.details.state === 'bonus' && ![10, 11, 12, 13, 14, 15, 16, 17].includes(_board[_i7][_j11])) {
                        imageUrl = _this4.url('0.png')
                    }
                    var extraOffset = (_this4.cellWidth - _this4.cellWidth / widthCoef) / 2;
                    var img = group.image(imageUrl, (_this4.cellWidth + _this4.cellOffsetX) * _i7 + extraOffset, (_this4.cellHeight + _this4.cellOffsetY) * _j11, _this4.cellWidth / widthCoef, _this4.cellHeight);
                    if (currentWinLine !== -1) {
                    ...
                    if (_this4.details.context.bs_v != undefined) {
                        value = _this4.details.context.bs_v[_i7][_j11];
                        if (currentWinLine !== -1 && combs[currentWinLine].bs_v != undefined) {
                            value = combs[currentWinLine].bs_v[_i7][_j11]
                        }
                    } else if (_this4.details.context.bonus_symbols_v != undefined) {
                        value = _this4.details.context.bonus_symbols_v[_i7][_j11]
                    } else {}
                    if (typeof value === 'number') {
                        value = (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    }
                    if (_this4.details.state === 'bonus' && value != 0 && [10, 11, 12, 13, 14, 15, 16, 17].includes(_board[_i7][_j11])) {
                        var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i7 - _this4.cellOffsetX;
                        var rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j11 + _this4.cellHeight / 2;
                        var rect_width = _this4.cellWidth + 2 * _this4.cellOffsetX;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = (_this4.cellHeight + _this4.cellOffsetY) * _j11 + _this4.cellHeight / 1.5;
                ...
                for (var _j11 in _board[_i7]) {
                    var _ret2 = _loop2(_j11);
        ...
        value: function _combs(snap, combs, details) {
            ...
            for (var _i8 = 0; _i8 < combs.length; ++_i8) {
                this.drawe_line_patern(group, combs[_i8]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            return group
        }
    }, {
        ...
        value: function _comb(snap, combs, comb, details) {
            ...
            if (comb.trigger === 'payer') {
                var _value = (comb.v / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
        ...
        value: function _calculateSizes(cols, rows) {
            ...
            this.cellWidth = this.cellHeight * 1.08;
            this.cellOffsetX = 0;
            this.cellOffsetY = 0;
        ...
        value: function _calculateSizesBonus(cols, rows) {
            ...
            this.boardHeight = this.height - (this.panelHeight + this.progressHeight + 1);
            this.cellHeight = parseInt(this.boardHeight / rows, 10);
            this.cellWidth = this.cellHeight * 1.08 * 2;
            this.cellOffsetX = 0;
            this.cellOffsetY = 0;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10)
        }
