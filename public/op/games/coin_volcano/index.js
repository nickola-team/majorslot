...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap, ce) {
        ...
        value: function _calculateSizes(cols, rows) {
            ...
            this.progressHeight = 3;
            this.boardHeight = this.height - (this.panelHeight + this.progressHeight + 1);
            this.cellWidth = parseInt(this.boardHeight / rows, 10) + 12;
            this.cellHeight = this.cellWidth - 12;
            this.cellOffsetX = -1;
            this.cellOffsetY = -1;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10) + 2
        }
        ...
        value: function make_spins(details) {
            ...
            var board = void 0;
            var ce = 1;
            if (extraMultipliersSlide) {
                ce = -1;
                this.add_multipliers_snap(details, combsGroup, margin + this.width * ce, snap, extraMultipliersSlide);
                ce += 1;
                this.add_multipliers_snap(details, combsGroup, margin + this.width * ce, snap);
                ce += 2
            } else {
                var showMainMultipliers = ['spin', 'buy_spin', 'restore'].includes(details.last_action) && this.isValidMultiplier(details);
                board = this.board(group, details.context.board, showMainMultipliers);
                board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            if (this.is_extra_snap(details)) {
                this.make_extra_snap(details, combsGroup, margin, snap, ce)
            }
        ...
        key: 'add_multipliers_snap',
        value: function add_multipliers_snap(details, combsGroup, margin, snap) {
            var extraSlide = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
            var addOffset = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : true;
            var group = combsGroup.group();
            margin += this.width;
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            progress.animate({
            ...
            this.draw_board(group, board, extraSlide, addOffset);
            var showOnBonusRestore = false;
            if (!extraSlide) {
                var combs = [].concat(details.context.winscatters ? details.context.winscatters : []);
                showOnBonusRestore = details.last_action == 'restore' && details.context.rounds_left === 0;
                if (showOnBonusRestore) {
                    var positions = details.context.bs.map(function(bs) {
            ...
            var labelLeft = details.last_action == 'restore' ? 'Restore' : details.state == 'spins' ? 'Spin' : 'Bonus(' + details.context.rounds_left + ')';
            var labelRight = extraSlide ? '1/2' : 'all/2';
            var roundWin = details.round_win;
            if (showOnBonusRestore && roundWin == 0) {
                details.context.bs.forEach(function(bs) {
                    roundWin += bs.value * details.context.mps[bs.reel][bs.position]
                })
            }
            this._panel(group, labelLeft + ', Win:' + this.coins(extraSlide ? 0 : roundWin), labelRight);
            return snap
        }
    }, {
        key: 'board',
        value: function board(snap, _board, showMainMultipliers) {
            var group = snap.group();
            this.draw_board(group, _board, false, false, showMainMultipliers);
            return group
        }
    }, {
        key: 'draw_board',
        value: function draw_board(group, board) {
            ...
            for (var i in board) {
                for (var j in board[i]) {
                    imageUrl = this.url('sym' + board[i][j] + '.png');
                    group.image(imageUrl, (this.cellWidth + this.cellOffsetX) * i + (addOffset ? this.boardMarginLeft : 0), (this.cellHeight + this.cellOffsetY) * j + (addOffset ? this.panelHeight : 0), this.cellWidth, this.cellHeight)
                }
            }
            if (!this.details.context.bs) return group;
            this.draw_symbols_values(group, board, lastExtraSlide, addOffset);
            var showMultipliers = !!(this.details.state === 'bonus' && this.details.context.mps) || showMainMultipliers;
            if (showMultipliers) {
                this.draw_bg_multipliers(group, !lastExtraSlide, addOffset)
            }
        }
    }, {
        ...
        value: function draw_symbols_values(group, board, lastExtraSlide) {
            ...
            try {
                for (var _iterator3 = this.details.context.bs[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                    ...
                    var j = bs.position;
                    var id = board[i][j];
                    if (id < 8 || this.details.last_action !== 'spin' && !(this.details.last_action == 'restore' && this.details.state == 'spins')) {
                        var value = bs.value == this.mini_jp * round_bet && id < 8 && !lastExtraSlide ? 'mini' : bs.value == this.minor_jp * round_bet && id < 8 && !lastExtraSlide ? 'minor' : bs.value == this.major_jp * round_bet && id < 8 && !lastExtraSlide ? 'major' : +bs.value;
                        if (typeof value === 'number') {
                            value = lastExtraSlide && this.isValidMultiplier(this.details) ? (value * this.details.context.mps[i][j] / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                        }
                        var rect_x = (this.cellWidth + this.cellOffsetX) * i - this.cellOffsetX + (addOffset ? this.boardMarginLeft : 0);
                        var rect_y = (this.cellHeight + this.cellOffsetY) * j + this.cellHeight / 3 + (addOffset ? this.panelHeight : 0);
                        var rect_width = this.cellWidth + 2 * this.cellOffsetX;
                        var rect_height = this.cellHeight / 3;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = (this.cellHeight + this.cellOffsetY) * j + this.cellHeight / 2 + (addOffset ? this.panelHeight : 0);
                        ...
                        if (showValues) {
                            ...
                            if (['mini', 'minor', 'major'].includes(value)) {
                                group.text(text_x, text_y, value).attr(this.width > 200 ? Drawer['PEARL_' + value.toUpperCase() + '_65'] : Drawer['PEARL_' + value.toUpperCase() + '_45'])
                            } else {
                                if (value < 100000) {
                                    group.text(text_x, text_y, value).attr(this.width > 200 ? Drawer.PEARL_VALUE_52 : Drawer.PEARL_VALUE_35)
                                } else {
                                    group.text(text_x, text_y, value).attr(this.width > 200 ? Drawer.PEARL_VALUE_37 : Drawer.PEARL_VALUE_23)
                                }
                            }
                        }
                    }
        ...
        value: function draw_bg_multipliers(group, showMultipliers) {
            ...
            var mps = this.details.context.mps;
            for (var i in mps) {
                for (var j in mps[i]) {
                    if (mps[i][j] > 1) {
                        var value_rect_width = this.cellWidth + 2 * this.cellOffsetX;
                        var mltp_rect_x = (this.cellWidth + this.cellOffsetX) * i - this.cellOffsetX + (addOffset ? this.boardMarginLeft : 0) + value_rect_width - value_rect_width / 2.5;
                        var mltp_rect_y = (this.cellHeight + this.cellOffsetY) * j + (addOffset ? this.panelHeight : 0);
                        var mltp_rect_width = value_rect_width / 2.5;
                        var mltp_rect_height = this.cellHeight / 3;
                        var mltp_text_x = mltp_rect_x + mltp_rect_width / 2;
                        var mltp_text_y = (this.cellHeight + this.cellOffsetY) * j + (addOffset ? this.panelHeight : 0) + this.cellHeight / 2 - mltp_rect_height;
                        group.rect(mltp_rect_x, mltp_rect_y, mltp_rect_width, mltp_rect_height).attr({
                            'fill': 'white'
                        });
                        var multiplier = mps[i][j];
