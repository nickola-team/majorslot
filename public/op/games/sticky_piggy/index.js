...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function board(snap, _board) {
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    var postfix = _board[i][j] == _this2.wild_id ? '_' + _this2.details.context.wild_mps[i][j] : '';
                    imageUrl = _this2.url('sym' + _board[i][j] + postfix + '.png');
        ...
        value: function make_spins(details) {
            ...
            var combGroup = void 0;
            var margin = 0;
            var isGambleResult = (details.last_action === 'freespin_gamble' || details.last_action === 'restore' && details.state === 'freespins' && details.context.gamble) && details.context.gamble.history.result != 'loose';
            this._combs(combsGroup, combs, details);
            ...
            } else if ((details.last_action === 'freespin_gamble' || details.last_action === 'restore' && details.state === 'freespins' && details.context.gamble) && details.context.gamble.history.result === 'loose') {
                board.remove();
                this._background(snap, 'freespins');
                this._panel(snap, 'gamble_lose', '');
                snap.text('50%', '56%', 'Lose').attr(Drawer.STYLE_TEXT_CENTER_RED)
            } else if (details.last_action === 'freespin_gamble' || details.last_action === 'restore' && details.state === 'freespins' && details.context.gamble) {
        ...
        value: function make_fs_picks_snap(snap, details) {
            ...
            var imageUrl = void 0,
                backURL = void 0;
            var fromScatters = details.context.winscatters[0] && details.context.winscatters[0].picks;
            for (var i in details.context.board) {
                for (var j in details.context.board[i]) {
                    var pick = fromScatters ? details.context.board[i].includes(this.scatter_id) ? details.context.winscatters[0].picks[i][j] : 0 : details.context.gamble.picks[i][j];
                    var opacity = pick > 0 ? 1 : 0.95;
                    var textScale = this.width / 186 === 1 ? 0 : 0.3 + 0.1 * (this.width / 186).toPrecision(2);
                    var imageX = (this.cellWidth + this.cellOffsetX) * i + this.boardMarginLeft;
                    var imageY = (this.cellHeight + this.cellOffsetY) * j + this.panelHeight + 1;
                    var textX = imageX + this.cellWidth / 2 - 2;
                    var textY = imageY + this.cellHeight / 2 + 3;
                    backURL = fromScatters ? this.url('sym13_open_back.png') : this.url('sym13_open_back_gamble.png');
                    imageUrl = pick > 0 ? this.url('sym13_open.png') : this.url('sym13.png');
                    group.image(backURL, imageX, imageY, this.cellWidth, this.cellHeight);
                    group.image(imageUrl, pick > 0 ? imageX : imageX - 2, imageY, this.cellWidth, this.cellHeight).attr({
                        'opacity': opacity
                    });
                    if (opacity === 1) {
