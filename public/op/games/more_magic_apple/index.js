...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && details.context.bonus_gold_count === 5 || details.state == 'freespins' && details.context.winscatters != undefined && details.context.winscatters.length > 0
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && details.context.bonus_gold_count === 5) {
                labelLeft = 'Grand Jackpot';
                group.text('50%', '56%', 'Grand Jackpot!').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                ...
                group.text('50%', '56%', '+5 FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
            }
            this._panel(group, labelLeft, labelRight);
            return group
        }
        ...
        value: function make_bac_win_snap(details, combsGroup, margin, snap) {
            ...
            group.transform('T' + margin + ',0');
            group.text('40%', '46%', 'BAC Win').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
            return group
        }
    }, {
        key: 'make_x2_snap',
        value: function make_x2_snap(details, combsGroup, margin, snap) {
            var group = snap.group();
            this._background(group, details, details.state);
            margin += this.width;
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
        ...
        value: function make_spins(details) {
            ...
            var game_boards = [];
            var game_bs_values = [];
            if (details.state != 'bonus' && details.context.bac_win != undefined) {
            ...
            if (this.get_pearls_count(details.context.board) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                ...
                if (this.get_pearls_count(details.context.orig_board.board) >= 6) {
                    for (var i in details.context.orig_board.board) {
                        for (var j in details.context.orig_board.board[i]) {
                            if ([12, 13].includes(details.context.orig_board.board[i][j])) {
            ...
            snap.click(function(e) {
                ...
                board.remove();
                if (combs[(counter - 1 + game_boards.length) % game_boards.length] != undefined && combs[(counter - 1 + game_boards.length) % game_boards.length].trigger == 'bac_v2_feature') {
        ...
        value: function make_bonus(details) {
            ...
            if (details.state === 'bonus') {
                for (var reel_index in game_board) {
                    details.context.board[reel_index].unshift(details.context.extra_board[reel_index]);
                    details.context.bs_v[reel_index].unshift(details.context.extra_board_values[reel_index]);
                    if (details.context.orig_board != undefined) {
                        details.context.orig_board.board[reel_index].unshift(details.context.extra_board[reel_index]);
                        details.context.orig_board.bs_v[reel_index].unshift(details.context.extra_board_values[reel_index])
                    }
                    if (details.context.multiplied_bs_v != undefined) {
                        details.context.multiplied_bs_v[reel_index].unshift(details.context.extra_board_values[reel_index])
                    }
                }
            }
            var snap = this.make_snap(details);
            ...
            game_boards.push(details.context.board);
            game_bs_values.push(details.context.bs_v);
            if (details.context.new_collector_pos) {
                console.log(details.context.new_collector_pos);
                game_boards.push(details.context.orig_board.board);
                game_bs_values.push(details.context.orig_board.bs_v);
                details.context.new_collector_pos[1] = details.context.new_collector_pos[1] + 1;
                combs.push({
                    'trigger': 'origin_board',
                    'positions': [details.context.new_collector_pos],
                    'amount': details.round_win
                });
                console.log(details.context.new_collector_pos)
            }
            if (details.state === 'bonus' && details.context.multiplied_bs_v !== undefined) {
                if (!combs.length) {
                    game_boards.push(details.context.orig_board.board);
                    game_bs_values.push(details.context.orig_board.bs_v);
                    combs.push({
                ...
                game_boards.push(details.context.orig_board.board);
                game_bs_values.push(details.context.multiplied_bs_v);
                combs.push({
                    'trigger': 'multiplied_bs_v',
                    'positions': [],
                    'amount': details.round_win
                })
            }
            if (details.state === 'bonus' && details.context.new_bonus_extra !== undefined) {
                if (!combs.length) {
                    game_boards.push(details.context.orig_board.board);
                    game_bs_values.push(details.context.orig_board.bs_v);
                    combs.push({
                ...
                var orig_bs_v = void 0;
                if (details.context.multiplied_bs_v !== undefined) {
                    orig_bs_v = details.context.multiplied_bs_v.map(function(arr) {
                        return arr.slice()
                    })
                } else {
                    orig_bs_v = details.context.orig_board.bs_v.map(function(arr) {
                        return arr.slice()
                    })
                }
                for (var _reel_index in details.context.extra_board) {
                    orig_board_board = JSON.parse(JSON.stringify(orig_board_board));
                    orig_bs_v = JSON.parse(JSON.stringify(orig_bs_v));
                    if (details.context.extra_board[_reel_index] === 0) {
                        continue
                    }
                    var positions = [];
                    positions.push([_reel_index, 0]);
                    if (details.context.extra_board[_reel_index] === 17) {
                        var row_index = details.context.extra_board_values[_reel_index] + 1;
                        if (details.context.orig_board.board[_reel_index][row_index] !== details.context.board[_reel_index][row_index]) {
                            positions.push([_reel_index, row_index])
                        }
                    } else {
                        for (var _row_index = 0; _row_index <= details.context.board[0].length; _row_index++) {
                            if ([12, 13, 14].includes(details.context.board[_reel_index][_row_index])) {
                                positions.push([_reel_index, _row_index])
                            }
                        }
                    }
                    orig_board_board[_reel_index] = details.context.board[_reel_index];
                    orig_bs_v[_reel_index] = details.context.bs_v[_reel_index];
                    game_boards.push(orig_board_board);
                    game_bs_values.push(orig_bs_v);
                    combs.push({
                        'trigger': 'apply_extra_board',
                        'positions': positions,
                        'amount': details.round_win
                    })
                }
            ...
            snap.click(function(e) {
                ...
                if (combs[(counter - 1 + game_boards.length) % game_boards.length] != undefined && combs[(counter - 1 + game_boards.length) % game_boards.length].trigger == 'x2_snap') {
                    board = _this3.make_x2_snap(details, combsGroup, margin, snap)
                } else {
        ...
        value: function board(snap, _board) {
            ...
            if (!combs.length) {
                ...
                if (this.get_pearls_count(_board) >= 6 && ['spin', 'freespin', 'respin'].includes(this.details.last_action)) {
                    ...
                    for (var i in _board) {
                        for (var j in _board[i]) {
                            if ([12, 13, 14, 15, 16, 17].includes(_board[i][j])) {
                                winbonus.positions.push([i, j])
                            }
                        }
                    }
            ...
            var _loop = function _loop(_i2) {
                var _loop2 = function _loop2(_j) {
                    imageUrl = _this4.url(_board[_i2][_j] + '.png');
                    if (_this4.details.state === 'bonus' && ![12, 13, 14, 15, 16, 17].includes(_board[_i2][_j])) {
                    ...
                    var value = void 0;
                    if (_this4.details.state === 'freespins' && _board[_i2][_j] === 10) {
                        value = 'x' + _this4.details.context.wild_mult
                    } else if (_board[_i2][_j] === 17) {
                        value = 0
                        ...
                        value = _this4.details.context.bonus_symbols_v[_i2][_j]
                    } else {
                        return 'continue'
                    }
                    ...
                    if (value !== 0 && [0, 10, 11, 12, 13, 14, 15, 16, 17].includes(_board[_i2][_j])) {
                        var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i2 - _this4.cellOffsetX;
                        var rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j + _this4.cellHeight / 3;
                    ...
                    if ((currentWinLine !== -1 || combs.length && ['bonus', 'freespins'].includes(combs[0].trigger) || ['spins', 'freespins'].includes(_this4.details.state) && combs.length === 1) && positions.length) {
                        img.attr({
                            opacity: 0.5
                        });
