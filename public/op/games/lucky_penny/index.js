...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'freespins' && details.context.winscatters.length) {
        ...
        value: function draw_extra_snap(snap, action_list, currentAction) {
            ...
            var group = snap.group();
            if (action_list.length - 1 !== currentAction) {
                var imageAmount = Object.keys(one_action.winlines).length;
                var imageHeight = this.height * MULT_HEIGHT / 7;
                var startHeight = (this.height * MULT_HEIGHT - imageHeight * imageAmount) / 2;
                ...
                path += 'L' + this.width * (0.17 - width_alpha / 2) + ',' + this.height * 0.87 + ' ';
                group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
                group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER);
                path = 'M' + this.width * (0.38 - width_alpha / 2) + ',' + -1;
                path += 'L' + this.width * (0.38 - width_alpha / 2) + ',' + this.height * 0.87 + ' ';
                group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
                group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER);
                ...
                for (var _i in winlines) {
                    var imageUrl = this.url(winlines[_i].symbol + '.png');
                    counter += 1;
                    group.image(imageUrl, this.width * (width_alpha + 0.09) + (this.colWidth - this.cellWidth) / 2, this.height * (0.06 + height_alpha) + (counter - 0.5) * line_step, this.cellWidth * 0.8, this.cellHeight * 0.8);
                    if (this.height > 200) {
        ...
        key: 'get_mult_values_sum_form_avalanche',
        value: function get_mult_values_sum_form_avalanche(context) {
            var sum = 0;
            if (context.avalanche[context.avalanche.length - 1].mult_values) {
                context.avalanche[context.avalanche.length - 1].mult_values.forEach(function(mults_reel) {
                    mults_reel.forEach(function(mult) {
                        sum += mult
                    })
                })
            }
        ...
        value: function make_spins(details) {
            ...
            if (this.details.last_action === 'respin') {
                totalWin = this.details.round_win
            }
            var total_mults = avalanche.length ? avalanche[avalanche.length - 1].mult_values : details.context.mult_values;
            if (!total_mults) {
                total_mults = []
            }
            var mult_before_spin = this.details.context.total_mult ? this.details.context.total_mult - this.get_mult_values(total_mults) : 0;
            var raw_action_list = [{
                'board': this.details.context.board,
                'winlines': this.details.context.winlines ? this.details.context.winlines : [],
                'round_win': 0,
                'win': this.details.context.win ? this.details.context.win : 0,
                'total_mult': mult_before_spin,
                'mult_values': this.details.context.mult_values
            }];
            ...
            var action_list = [];
            raw_action_list.forEach(function(action) {
                action['action'] = true;
                action['total_mult'] = mult_before_spin;
                action_list = action_list.concat(action);
                var wins = {
                    'action': false,
                    'win': action.win ? action.win : 0,
                    'winlines': action.winlines ? action.winlines : []
                };
                wins['total_mult'] = mult_before_spin;
                wins['mult_values'] = action['mult_values'];
                action_list = action_list.concat(wins)
            });
            for (var i in action_list) {
                if (!action_list[i].winlines) {
                    action_list[i]['winlines'] = []
                }
                if (!action_list[i].total_mult) {
                    action_list[i]['total_mult'] = 0
                }
            }
            ...
            var raw_win = this.get_mult_values(total_mults) ? totalWin / details.context.total_mult : totalWin;
            action_list[action_list.length - 1].win = raw_win;
            if (details.last_action === 'respin') {
                action_list = [action_list[0]];
                action_list[0].win = totalWin
            }
            if (action_list.length > 1 && this.details.context.avalanche && this.get_mult_values_sum_form_avalanche(this.details.context) !== 0) {
                var mult_possitions = [];
                var _board = action_list[action_list.length - 1].board;
                ...
                var tmp_act_winscatters = {};
                Object.assign(tmp_act_winscatters, action_list[action_list.length - 1]);
                tmp_act_winscatters.winlines = [];
                tmp_act_winscatters.win = 0;
                action_list[action_list.length - 1].winscatters = [{
                    'occurrences': mult_possitions.length,
                    'positions': mult_possitions
                }];
                var last_action = action_list[action_list.length - 1];
                action_list = action_list.concat(tmp_act_winscatters);
                action_list = action_list.concat({
                    'action': false,
                    'win': raw_win,
                    'total_mult': mult_before_spin,
                    'mult_values': last_action.mult_values,
                    'winlines': []
                })
            }
            var snap = new Snap(this.width, this.height);
            ...
            if (details.last_action === 'respin') {
                board.remove();
                if (this.height > 200) {
                    group.text('50%', '56%', 'Total win: ' + this.coins(totalWin)).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2)
                } else {
                    group.text('50%', '56%', 'Total win: ' + this.coins(totalWin)).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER)
                }
                var text = '';
                if (details.state == 'freespins') {
                    text = ', x' + details.context.total_mult
                }
            ...
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
                var is_fs_retrigger = !!(details.state === 'freespins' && details.context.winscatters.length);
                counter = (counter + 1) % (action_list.length + is_fs_retrigger);
                combsGroup.transform('T-' + _this2.width * counter);
                progress.animate({
                    width: partSize * (counter + 1)
                }, 100);
                var cur_board = Array.from(_this2.details.context.board);
                board.remove();
                if (is_fs_retrigger && counter == action_list.length) {
                    var _labelLeft = 'Additional FreeSpins!';
                    progress.animate({
                        width: _this2.width
                    }, 100);
                    var roundsGranted = details.context.winscatters.reduce(function(a, _ref2) {
                        var _ref2$freespins = _ref2.freespins,
                            freespins = _ref2$freespins === undefined ? 0 : _ref2$freespins;
                        return a + freespins
                    }, 0);
                    board = group.group().text('50%', '56%', '+' + roundsGranted + ' FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
                    _this2._panel(group, _labelLeft, '')
                } else if (counter === action_list.length - 1 && _this2.details.context.avalanche && _this2.get_mult_values_sum_form_avalanche(_this2.details.context) !== 0) {
                    var total_win = _this2.details.context.avalanche[_this2.details.context.avalanche.length - 1].round_win;
                    var _text = _this2.coins(total_win / _this2.details.context.total_mult) + ' x ' + _this2.details.context.total_mult + ' = ' + _this2.coins(total_win);
        ...
        key: 'get_mult_img',
        value: function get_mult_img(val) {
            if ([2, 3, 4, 5, 6, 8].includes(val)) {
                return '12_1'
            }
            if ([10, 12, 15, 20].includes(val)) {
                return '12_2'
            }
            if ([25, 30, 50].includes(val)) {
                return '12_3'
            }
            if ([100, 250, 500].includes(val)) {
                return '12_4'
            }
            if ([1000].includes(val)) {
                return '12_5'
            }
        }
    }, {
        ...
        value: function board(snap, one_action) {
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    if (board[i][j] == MULT_ID) {
                        var mult_img = _this3.get_mult_img(one_action.mult_values[i][j]);
                        imageUrl = _this3.url(mult_img + '.png')
                    } else {
                        imageUrl = _this3.url(board[i][j] + '.png')
                    }
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * i, (_this3.cellHeight + _this3.cellOffsetY) * j, _this3.cellWidth, _this3.cellHeight);
                    if (one_action.mult_values) {
                        var value = one_action.mult_values[i][j];
                        if (value) {
                            ...
                            if (value < 100000) {
                                if (_this3.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.MULT_VALUE_70)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.MULT_VALUE_40)
                                }
                            }
                        }
                    }
        ...
        value: function _combs(snap, action_list, details) {
            ...
            this.bordered_cells = [];
            var text = '';
            if (action_list[0].mult_values) {
                if (details.state === 'freespins') {
                    text = ',x' + (this.get_mult_values(action_list[0].mult_values) + action_list[0].total_mult || 1)
                } else {
                    text = ',x' + (this.get_mult_values(action_list[0].mult_values) || 1)
                }
            }
            ...
            var delta = details.state == 'freespins' && details.context.prev_mult != details.context.total_mult || details.state == 'spins' && details.context.total_mult > 1 ? 0 : 1;
            var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin) + text;
            var labelRight = '';
            labelRight = action_list.length > 1 ? '1/' + Math.round((action_list.length + delta) / 2) : '';
            this._panel(group, labelLeft, labelRight);
        ...
        value: function _comb(snap, combs, comb, details) {
            ...
            if (comb.mult_values) {
                if (details.state === 'freespins') {
                    if (comb.win) {
                        text = ',x' + (this.get_mult_values(comb.mult_values) + comb.total_mult || 1)
                    } else {
                        text = ',x' + (comb.total_mult || 1)
                    }
                } else {
                    text = ',x' + (this.get_mult_values(comb.mult_values) || 1)
                }
            }
            var labelLeft = this.title(details) + ', Win:' + this.coins(comb.win) + text;
            var labelRight = void 0;
            var delta = details.state == 'freespins' && details.context.prev_mult != details.context.total_mult || details.state == 'spins' && details.context.total_mult > 1 ? 0 : 1;
            if (comb.action) {
                labelRight = Math.round(combs.indexOf(comb) / 2) + 1 + '/' + Math.round((combs.length + delta) / 2);
                this.drawe_border_win_symbols(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            } else if (combs.indexOf(comb) == 1) {
                labelRight = '1/' + Math.round((combs.length + delta) / 2)
            } else {
                labelRight = Math.round((combs.indexOf(comb) + 1) / 2) + '/' + Math.round((combs.length + delta) / 2)
            }
            this._panel(group, labelLeft, labelRight);
            return group
        }
        ...
        value: function get_mult_values(mult_values) {
            var mult = 0;
            mult_values.forEach(function(row) {
