...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function get_text_for_action(details) {
            ...
            switch (action) {
                ...
                case 'freespin_stop':
                    return 'Free Spins End';
                case 'freespin_init':
                    var fsCount = details.context.gamble ? details.context.gamble.freespins : details.context.rounds_left;
                    return 'FS count ' + fsCount;
                default:
                    return '';
            }
        }
        ...
        value: function board(snap, _board) {
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    var id = _board[i][j];
                    var postfix = '';
                    if (_this2.details.context.jwilds && _this2.details.context.jwilds.find(function(pos) {
                            return pos[0] === i && pos[1] === j
                        })) {
                        id = 14;
                        var multi = _this2.details.context.wild_mps[i][j] > 1 ? _this2.details.context.wild_mps[i][j] : 0;
                        var supermulti = _this2.details.context.jwild_mps[i][j] + multi;
                        postfix = '_x' + supermulti
                    } else if (id === _this2.wild_id) {
                        postfix = '_' + _this2.details.context.wild_mps[i][j]
                    }
                    imageUrl = _this2.url('sym' + id + postfix + '.png');
        ...
        value: function make_spins(details) {
            ...
            var combGroup = void 0;
            var margin = 0;
            var isGambleResult = (details.last_action === 'freespin_gamble' || details.last_action === 'restore' && details.state === 'freespins' && details.context.gamble) && details.context.gamble.history && details.context.gamble.history.result != 'loose' || details.context.rounds_granted == 0;
            var isAdditionalFS = details.last_action === 'freespin' && details.context && details.context.level_up;
            var isKeysProgress = false;
            this._combs(combsGroup, combs, details);
            var length = combs ? combs.length : 1;
            if (length > 1) {
                for (var i in combs) {
                    combGroup = this._comb(combsGroup, combs, combs[i], details);
                    margin += this.width;
            ...
            length = combsGroup.children() ? combsGroup.children().length : 1;
            var partSize = this.width / length;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(BaseDrawer.STYLE_PROGRESS);
            var board = this.board(group, details.context.board, false);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            var scatLength = details.context.winscatters ? details.context.winscatters.length : 0;
            var winFSpicks = scatLength > 0 && details.context.winscatters[0].trigger === 'freespins';
            if (winFSpicks) {
                combGroup = this.make_fs_picks_snap(combsGroup, details);
                margin += this.width;
                combGroup.transform('T' + margin + ',0')
            }
            if (details.last_action === 'freespin_init') {
                ...
                this._background(snap, 'freespins');
                this._panel(snap, details.last_action, '');
                var isSuperFreespins = details.achievements && details.achievements.number === 0;
                if (details.context && details.context.fg_type) isSuperFreespins = details.context.fg_type === 'sfs' || details.context.fg_type === 'paid_sfs';
                var freespinsType = isSuperFreespins ? 'Super Free Spins' : 'Free Spins';
                snap.text('50%', '46%', freespinsType).attr(Drawer.STYLE_TEXT_CENTER_RED);
                snap.text('50%', '68%', this.get_text_for_action(details)).attr(Drawer.STYLE_TEXT_CENTER_RED)
            } else if (details.last_action === 'freespin_stop') {
                board.remove();
                this._background(snap, 'freespins');
                this._panel(snap, details.last_action, '');
                var _isSuperFreespins = details.achievements && details.achievements.number === 0;
                if (details.context && details.context.fg_type) _isSuperFreespins = details.context.fg_type === 'sfs' || details.context.fg_type === 'paid_sfs';
                var freespinsEndString = _isSuperFreespins ? 'Super Free Spins End' : 'Free Spins End';
                snap.text('50%', '56%', freespinsEndString).attr(Drawer.STYLE_TEXT_CENTER_RED)
            } else if ((details.last_action === 'freespin_gamble' || details.last_action === 'restore' && details.state === 'freespins' && details.context.gamble && details.context.gamble.history) && details.context.gamble.history.result === 'loose') {
                board.remove();
                this._background(snap, 'freespins');
                this._panel(snap, 'gamble_lose', '');
                snap.text('50%', '56%', 'Lose').attr(Drawer.STYLE_TEXT_CENTER_RED)
            } else if (details.last_action === 'freespin_gamble' || details.last_action === 'restore' && details.state === 'freespins' && details.context.gamble && (details.context.rounds_granted == 0 || details.context.rounds_left == details.context.rounds_granted)) {
                combsGroup = snap.group();
                combGroup = this.make_fs_picks_snap(combsGroup, details);
                margin -= margin;
                combGroup.transform('T' + margin + ',0');
                combGroup = this.make_fs_gamble_snap(combsGroup, details);
                margin += this.width * 1;
                combGroup.transform('T' + margin + ',0')
            }
            if (details.last_action === 'freespin') {
                isKeysProgress = true;
                var keys = details.context.keys || 0;
                var afs_group = combsGroup.group();
                var progressText = 'Progress: ' + keys + '/12';
                afs_group.text('50%', '55%', progressText).attr(Drawer.STYLE_TEXT_CENTER_RED);
                this._panel(afs_group, 'progress', '');
                margin += this.width;
                afs_group.transform('T' + margin + ',0')
            }
            if (isAdditionalFS) {
                var _afs_group = combsGroup.group();
                var roundResultText = 'Additional FS: 2';
                _afs_group.text('50%', '55%', roundResultText).attr(Drawer.STYLE_TEXT_CENTER_RED);
                this._panel(_afs_group, 'additional_freespins', '');
                margin += this.width;
                _afs_group.transform('T' + margin + ',0')
            }
            var counter = 0;
            snap.click(function(e) {
                if (combsGroup.children() && combsGroup.children().length < 2) return;
                counter = (counter + 1) % combsGroup.children().length;
                ...
                if (winFSpicks) {
                    board.attr({
                        'visibility': counter + 1 === combsGroup.children().length ? 'hidden' : 'visible'
                    })
                }
                ...
                var boardPagesSkip = 0;
                if (isGambleResult) boardPagesSkip++;
                if (isAdditionalFS) boardPagesSkip++;
                if (isKeysProgress) boardPagesSkip++;
                if (boardPagesSkip > 0) {
                    board.attr({
                        'visibility': counter + boardPagesSkip >= combsGroup.children().length ? 'hidden' : 'visible'
                    })
                }
            });
            return snap.node
        ...
        key: 'make_fs_gamble_snap',
        value: function make_fs_gamble_snap(snap, details) {
            var group = snap.group();
            var gambleResult = {
                'win': 'gamble_win',
                'loose': 'gamble_lose',
                'refuse': 'gamble_collect',
                'end': 'gamble_end'
            };
            var lebelLeft = void 0;
            if (details.last_action === 'restore') {
                lebelLeft = 'Restore, Gamble'
            } else {
                lebelLeft = details.last_action === 'freespin_gamble' ? details.context.rounds_granted > 0 && details.context.gamble.history.result !== 'refuse' ? gambleResult['end'] : gambleResult[details.context.gamble.history.result] : details.last_action
            }
            if (details.last_action === 'freespin_gamble') {
                var roundResultText = 'Additional FS ' + (details.context.gamble.history.freespins_added || 0);
                var totalText = 'Total FS ' + details.context.gamble.freespins;
                group.text('50%', '40%', roundResultText).attr(Drawer.STYLE_TEXT_CENTER_RED);
                group.text('50%', '70%', totalText).attr(Drawer.STYLE_TEXT_CENTER_RED)
            } else if (details.last_action === 'restore' && details.state === 'freespins' && details.context.gamble) {
                details.context.gamble.history === undefined ? group.text('50%', '56%', 'Total FS ' + details.context.gamble.freespins).attr(Drawer.STYLE_TEXT_CENTER_RED) : group.text('50%', '56%', 'Total FS ' + details.context.gamble.freespins).attr(Drawer.STYLE_TEXT_CENTER_RED)
            }
            this._panel(group, lebelLeft, '');
            return group
        }
    }, {
        key: 'make_fs_picks_snap',
        value: function make_fs_picks_snap(snap, details) {
            var _this4 = this;
            var group = snap.group();
            var imageUrl = void 0,
                backURL = void 0;
            var board = details.context.board;
            var winline = details.context.winscatters[0];
            var picks = winline && winline.picks;
            var _loop3 = function _loop3(i) {
                var _loop4 = function _loop4(j) {
                    var pick = picks && board[i].includes(_this4.scatter_id) ? picks[i][j] : undefined;
                    var opacity = pick >= 0 ? 1 : 0.95;
                    var textScale = _this4.width / 186 === 1 ? 0 : 0.3 + 0.1 * (_this4.width / 186).toPrecision(2);
                    var imageX = (_this4.cellWidth + _this4.cellOffsetX) * i + _this4.boardMarginLeft;
                    var imageY = (_this4.cellHeight + _this4.cellOffsetY) * j + _this4.panelHeight + 1;
                    var textX = imageX + _this4.cellWidth / 2 - 2;
                    var textY = imageY + _this4.cellHeight / 2 + 3;
                    backURL = picks ? _this4.url('sym13_open_back.png') : _this4.url('sym13_open_back_gamble.png');
                    imageUrl = pick >= 0 ? _this4.url('sym13_open.png') : _this4.url('sym13.png');
                    group.image(backURL, imageX, imageY, _this4.cellWidth, _this4.cellHeight);
                    group.image(imageUrl, pick >= 0 ? imageX : imageX - 2, imageY, _this4.cellWidth, _this4.cellHeight).attr({
                        'opacity': opacity
                    });
                    if (opacity === 1) {
                        if (pick === 0) {
                            var cellData = winline.golden_cells && winline.golden_cells.find(function(data) {
                                return data.reel === i && data.position === j
                            });
                            var multi = cellData && cellData.mp ? cellData.mp : 2;
                            var symbolURL = _this4.url('sym12_' + multi + '.png');
                            group.image(symbolURL, imageX + _this4.cellWidth * 0.1, imageY + _this4.cellHeight * 0.2, _this4.cellWidth * 0.6, _this4.cellHeight * 0.6)
                        } else {
                            group.text(textX, textY, '' + pick).attr(Drawer.FREESPINS_PICK_TEXT).attr({
                                'opacity': opacity
                            }).attr({
            ...
            var lebelLeft = details.last_action === 'freespin_gamble' && details.context.gamble.history ? details.context.rounds_granted > 0 && details.context.gamble.history.result !== 'refuse' ? gambleResult['end'] : gambleResult[details.context.gamble.history.result] : 'FS picks:';
            this._panel(group, lebelLeft, '');
            return group
        }
    }, {
        ...
        value: function make_award(details) {
            ...
            if (details.send_transaction) {
                snap.image(this.url('prezent.png'), this.width / 2.5, this.height / 4, this.width / 5, this.height / 4);
                group.text(parseInt(this.width / 2), parseInt(this.height * 0.55), this.coins(details.amount)).attr(BaseDrawer.STYLE_TEXT_AWARD)
            } else {
                snap.image(this.url('prezent.png'), this.width / 2.5, this.height / 4, this.width / 5, this.height / 4);
                group.text(parseInt(this.width / 2), parseInt(this.height * 0.55), 'Manual Mode').attr(BaseDrawer.STYLE_TEXT_AWARD)
            }
            this._calculateSizes(this.cols, this.rows);
            this._panel(snap, 'award', '');
            return snap.node
