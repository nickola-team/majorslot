...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function is_extra_snap(details) {
            return details.state == 'freespins' && details.context.winscatters.length > 0
        }
    }, {
        key: 'make_extra_snap',
        ...
        key: 'draw_extra_snap',
        value: function draw_extra_snap(snap, action_list, currentAction) {
            var one_action = action_list[currentAction];
            var group = snap.group();
            if (one_action.type === 'win') {
                var winlines = one_action.winlines;
                var imageAmount = winlines ? Object.keys(winlines).length : 0;
                var imageHeight = this.height * MULT / 7;
                var startHeight = (this.height * MULT - imageHeight * imageAmount) / 2;
                var counter = 0;
                var width_alpha = 0.082;
                var height_alpha = -0.055;
                var line_start = this.height * 0.08;
                var line_step = this.height * 0.8 / imageAmount;
                if (this.height > 200) {
                    group.text(this.width * (width_alpha - 0.04), this.height * 0.05, 'Count').attr(Drawer.HEADER_TEXT_CENTER_2);
                    group.text(this.width * (width_alpha + 0.15), this.height * 0.05, 'Symbol').attr(Drawer.HEADER_TEXT_CENTER_2);
                    group.text(this.width * (width_alpha + 0.56), this.height * 0.05, 'Win').attr(Drawer.HEADER_TEXT_CENTER_2)
                } else {
                    group.text(this.width * (width_alpha - 0.04), this.height * 0.05, 'Count').attr(Drawer.HEADER_TEXT_CENTER);
                    group.text(this.width * (width_alpha + 0.15), this.height * 0.05, 'Symbol').attr(Drawer.HEADER_TEXT_CENTER);
                    group.text(this.width * (width_alpha + 0.56), this.height * 0.05, 'Win').attr(Drawer.HEADER_TEXT_CENTER)
                }
                var verticalLineHeight = this.height > 200 ? this.height * 0.89 : this.height * 0.87;
                var path = 'M' + this.width * (0.17 - width_alpha / 2) + ',' + -1;
                path += 'L' + this.width * (0.17 - width_alpha / 2) + ',' + verticalLineHeight + ' ';
                group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
                group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER);
                path = 'M' + this.width * (0.38 - width_alpha / 2) + ',' + -1;
                path += 'L' + this.width * (0.38 - width_alpha / 2) + ',' + verticalLineHeight + ' ';
                group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
                group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER);
                path = 'M' + this.width * -0.1 + ',' + this.height * 0.08;
                path += 'L' + this.width + ',' + this.height * 0.08 + ' ';
                group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
                group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER);
                for (var i = 1; i < imageAmount; i++) {
                    path = 'M' + this.width * -0.1 + ',' + (line_start + i * line_step);
                    path += 'L' + this.width + ',' + (line_start + i * line_step) + ' ';
                    group.path(path).attr(BaseDrawer.STYLE_LINE_INNER);
                    group.path(path).attr(BaseDrawer.STYLE_LINE_OUTER)
                }
                for (var _i in winlines) {
                    var imageUrl = this.url('sym' + winlines[_i].symbol + '.png');
                    counter += 1;
                    var cellWidth = this.cellWidth;
                    var cellHeight = this.cellHeight * 1.2;
                    var symbolHeightPadding = this.height > 200 ? 7 : 5;
                    group.image(imageUrl, this.width * (width_alpha + 0.15) - this.cellWidth * 0.5, this.height * (0.06 + height_alpha) + (counter - 0.5) * line_step - symbolHeightPadding, cellWidth, cellHeight);
                    if (this.height > 200) {
                        group.text(this.width * 0.045, line_start - this.height * (0.03 + height_alpha) + (counter - 0.5) * line_step, winlines[_i].occurrences).attr(Drawer.EXTRA_SNAP_TEXT_CENTER_2);
                        group.text(this.width * (width_alpha + 0.56), line_start - this.height * (0.03 + height_alpha) + (counter - 0.5) * line_step, this.coins(winlines[_i].amount)).attr(Drawer.EXTRA_SNAP_TEXT_CENTER_2)
                    } else {
                        group.text(this.width * 0.045, line_start - this.height * (0.03 + height_alpha) + (counter - 0.5) * line_step, winlines[_i].occurrences).attr(Drawer.EXTRA_SNAP_TEXT_CENTER);
                        group.text(this.width * (width_alpha + 0.56), line_start - this.height * (0.03 + height_alpha) + (counter - 0.5) * line_step, this.coins(winlines[_i].amount)).attr(Drawer.EXTRA_SNAP_TEXT_CENTER)
                    }
                }
            } else {
                if (one_action.type === 'win_for_scatters') {
                    var line1 = 'Total win:';
                    var line2 = this.coins(one_action.win) + ' x ' + one_action.multiplier;
                    var line3 = '= ' + this.coins(one_action.win * one_action.multiplier);
                    if (this.height > 200) {
                        group.text('50%', '38%', line1).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2);
                        group.text('50%', '50%', line2).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2);
                        group.text('50%', '62%', line3).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2)
                    } else {
                        group.text('50%', '38%', line1).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER);
                        group.text('50%', '50%', line2).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER);
                        group.text('50%', '62%', line3).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER)
                    }
                } else if (one_action.type === 'additional') {
                    group.text('50%', '55%', '+' + one_action.additional + ' FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                } else if (one_action.type === 'multiplier') {
                    var description = one_action.last_action === 'freespin' ? 'Total Multiplier: x' : 'Multiplier: x';
                    var multiplier = one_action.last_action === 'freespin' ? one_action.mp_progress : one_action.mult;
                    if (this.height > 200) {
                        group.text('50%', '55%', description + multiplier).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2)
                    } else {
                        group.text('50%', '55%', description + multiplier).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER)
                    }
                } else {
                    if (this.height > 200) {
                        group.text('45%', '50%', 'Total win: ' + this.coins(one_action.win)).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2)
                    } else {
                        group.text('45%', '50%', 'Total win: ' + this.coins(one_action.win)).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER)
                    }
                }
            }
            return group
        ...
        value: function make_spins(details) {
            ...
            this.rows = details.context.board[0].length;
            this._calculateSizes(this.cols, this.rows);
            var isRestoreFinished = details.last_action === 'restore' && details.round_win > 0;
            var avalanche = this.details.context.avalanche ? this.details.context.avalanche : [];
            var last_avalanche = avalanche[avalanche.length - 1] || details.context;
            var winscatters = this.details.context.winscatters ? this.details.context.winscatters : [];
            if (avalanche.length && avalanche[avalanche.length - 1].winscatters && avalanche[avalanche.length - 1].winscatters.length) {
                winscatters = avalanche[avalanche.length - 1].winscatters
            }
            var winlines = this.details.context.winlines ? this.details.context.winlines : [];
            var totalWin = avalanche.length ? avalanche[avalanche.length - 1].round_win : 0;
            var mult_values = this.multiplier_values_get(this.details.context);
            var mult_value_final = this.multiplier_values_get(last_avalanche);
            var mult_types = this.multiplier_types_get(this.details.context);
            var mp_progress = this.round_mp_progress_get(this.details.context);
            var mp_acc = this.round_mp_acc_get(avalanche[avalanche.length - 1]);
            var isMultiplier = !!(last_avalanche.mp_count > 0 && avalanche.length);
            var multiplier = this.get_mult_values(mult_values);
            var multiplierFinal = this.get_mult_values(mult_value_final);
            if (details.state === 'freespins' && details.context.mp_progress >= multiplierFinal) multiplier = details.context.mp_progress - multiplierFinal + multiplier;
            var first_action = {
                'board': this.details.context.board,
                winlines: winlines,
                'round_win': 0,
                'win': this.details.context.win ? this.details.context.win : 0,
                isMultiplier: isMultiplier,
                multiplier: multiplier,
                mult: this.details.context.mult ? this.details.context.mult : 0,
                mult_values: mult_values,
                mult_types: mult_types,
                mp_progress: mp_progress
            };
            var raw_action_list = [first_action];
            var action_list = [];
            if (details.last_action === 'restore' && winlines.length === 0 && avalanche.length > 0) {
                first_action.winlines = this.restore_except_winlines_get(details.context)
            }
            if (this.details.context.avalanche) {
                raw_action_list = raw_action_list.concat(this.details.context.avalanche)
            }
            var multiplierAccumulator = details.state === 'freespins' && details.context.mp_progress >= multiplierFinal ? details.context.mp_progress - multiplierFinal : 0;
            raw_action_list.forEach(function(action, index) {
                if (isRestoreFinished && index > 0) return;
                mult_values = _this2.multiplier_values_get(action);
                mult_types = _this2.multiplier_types_get(action);
                var page = {
                    current: index + 1,
                    total: raw_action_list.length
                };
                var multiplier = _this2.get_mult_values(mult_values);
                if (details.state === 'freespins') multiplier = multiplierAccumulator + multiplier;
                action['action'] = true;
                action.page = page;
                action.last_action = _this2.details.last_action;
                if (!action.mult_values) {
                    action.mult_values = mult_values;
                    action.mult_types = mult_types;
                    action.isMultiplier = isMultiplier;
                    action.multiplier = multiplier
                }
                action_list = action_list.concat(action);
                var winlines = action.winlines && action.winlines.length > 0 ? action.winlines : action.winscatters;
                var actionWin = {
                    page: page,
                    action: false,
                    type: 'win',
                    last_action: _this2.details.last_action,
                    win: action.win ? action.win : 0,
                    winlines: winlines || [],
                    isMultiplier: isMultiplier,
                    multiplier: multiplier,
                    mult: action.mult,
                ...
                if (details.last_action === 'restore') return;
                action_list = action_list.concat(actionWin)
            });
            for (var i in action_list) {
                if (!action_list[i].winlines) {
                    action_list[i]['winlines'] = []
                }
                if (this.details.last_action === 'freespins' && !action_list[i].mult) {
                    action_list[i]['mult'] = 1
                }
            }
            if (details.last_action !== 'restore' && action_list.length > 1) {
                action_list.pop()
            }
            if (action_list.length >= 1) {
                action_list[action_list.length - 1].winscatters = winscatters
            }
            if (details.last_action === 'respin') {
                action_list = [action_list[action_list.length - 1]];
                action_list[0].win = totalWin;
                action_list[0].page = undefined
            }
            var isAddMultiGuard = details.last_action !== 'respin';
            if (isAddMultiGuard && (action_list.length > 1 || action_list.length === 1 && action_list[0].win > 0 && winscatters.length > 0)) {
                var last_action = action_list[action_list.length - 1];
                if (last_action && last_avalanche.mp_count >= 1) {
                    var multiplyWinline = {};
                    var mult_possitions = [];
                    var _board = last_action.board;
                    for (var _i2 in _board) {
                        for (var j in _board[_i2]) {
                            if (_board[_i2][j] == MULT_ID) {
                                mult_possitions = mult_possitions.concat([
                                    [_i2, j]
                                ])
                            }
                        }
                    }
                    multiplyWinline.occurrences = mult_possitions.length;
                    multiplyWinline.positions = mult_possitions;
                    var isWinlines = last_action.winscatters && last_action.winscatters.length > 0 || last_action.winlines && last_action.winlines.length > 0;
                    if (isWinlines) {
                        var action = last_action.action,
                            page = last_action.page,
                            _isMultiplier = last_action.isMultiplier,
                            _multiplier = last_action.multiplier,
                            mp_types = last_action.mp_types,
                            mp_values = last_action.mp_values,
                            mps = last_action.mps,
                            _mult_types = last_action.mult_types,
                            _mult_values = last_action.mult_values,
                            win = last_action.win;
                        var screen = {
                            action: action,
                            ...
                            mp_values: mp_values,
                            mps: mps,
                            mult_types: _mult_types,
                            mult_values: _mult_values,
                            win: win,
                            winlines: [multiplyWinline]
                        };
                        action_list.push(screen)
                    } else {
                        last_action.winlines.push(multiplyWinline)
                    }
                }
            }
            if (winscatters.length > 0 && avalanche.length === 0) {
                var _last_action = action_list[action_list.length - 1];
                if (!_last_action) return;
                var _page = _last_action.page,
                    _board2 = _last_action.board,
                    _isMultiplier2 = _last_action.isMultiplier,
                    _multiplier2 = _last_action.multiplier,
                    _mp_types = _last_action.mp_types,
                    _mp_values = _last_action.mp_values,
                    _mps = _last_action.mps,
                    _mult_types2 = _last_action.mult_types,
                    _mult_values2 = _last_action.mult_values,
                    _win = _last_action.win;
                if (_win * _multiplier2 > 0) {
                    var _screen = {
                        action: false,
                        type: 'win_for_scatters',
                        last_action: _last_action.last_action,
                        page: _page,
                        isMultiplier: _isMultiplier2,
                        multiplier: _multiplier2,
                        board: _board2,
                        mp_types: _mp_types,
                        mp_values: _mp_values,
                        mps: _mps,
                        mult_types: _mult_types2,
                        mult_values: _mult_values2,
                        win: _win
                    };
                    action_list.push(_screen)
                }
            }
            if (details.last_action === 'freespin' || details.last_action === 'spin' && avalanche.length && last_avalanche.mp_count > 0) {
                var _last_action2 = action_list[action_list.length - 1];
                action_list = action_list.concat({
                    action: false,
                    last_action: details.last_action,
                    type: 'multiplier',
                    isMultiplier: isMultiplier,
                    win: _last_action2.win,
                    multiplier: _last_action2.multiplier,
                    mult: _last_action2.mp_acc,
                    mult_values: _last_action2.mult_values,
                    mp_progress: details.context.mp_progress || 0,
                    page: _last_action2.page
                })
            }
            if (details.state == 'freespins' && details.last_action !== 'respin' && winscatters.length) {
                var _last_action3 = action_list[action_list.length - 1];
                var roundsGranted = winscatters.reduce(function(a, _ref) {
                ...
                action_list = action_list.concat({
                    ...
                    last_action: details.last_action,
                    type: 'additional',
                    page: _last_action3.page,
                    additional: roundsGranted
                })
            }
            var snap = new Snap(this.width, this.height);
            ...
            var combGroup = void 0;
            var margin = 0;
            this._combs(combsGroup, action_list, details);
            if (action_list.length > 1) {
                for (var _i3 in action_list) {
                    if (_i3 == 0) continue;
                    combGroup = this._comb(combsGroup, action_list, action_list[_i3], details);
                    margin += this.width;
                    combGroup.transform('T' + margin + ',0')
                }
            }
            var partSize = this.width / action_list.length;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(BaseDrawer.STYLE_PROGRESS);
            var board = this.board(group, action_list[0]);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            if (details.last_action === 'respin') {
                board.remove();
                var winValue = last_avalanche.round_win_without_mp;
                var _progress = 0;
                if (details.state === 'spins') _progress = last_avalanche.mp_acc;
                if (details.state === 'freespins') _progress = mp_progress;
                if (winValue > 0 && _progress > 0 && last_avalanche) {
                    var line1 = 'Total win:';
                    var line2 = this.coins(winValue) + ' x ' + _progress;
                    var line3 = '= ' + this.coins(totalWin);
                    if (this.height > 200) {
                        group.text('50%', '38%', line1).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2);
                        group.text('50%', '50%', line2).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2);
                        group.text('50%', '62%', line3).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2)
                    ...
                    if (this.height > 200) {
                        group.text('50%', '50%', 'Total win: ' + this.coins(totalWin)).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2)
                    } else {
                        group.text('50%', '50%', 'Total win: ' + this.coins(totalWin)).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER)
                    }
                }
                var text = '';
                var sum = details.context.mult + mp_progress;
                if (details.state == 'freespins' && mp_progress > 0 && sum > 0) {
                    text = ', x' + sum
                }
                var labelLeft = 'Respin, Win:' + this.coins(totalWin) + text;
                var labelRight = '';
                this._panel(group, labelLeft, labelRight);
                return snap.node
            }
            ...
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
                counter = (counter + 1) % action_list.length;
                combsGroup.transform('T-' + _this2.width * counter);
                progress.animate({
                    width: partSize * (counter + 1)
                }, 100);
                board.remove();
                var action = action_list[counter];
                if (counter === action_list.length - 1 && details.state == 'freespins' && _this2.details.context.mult > 1) {
                    var total_win = _this2.details.context.avalanche[_this2.details.context.avalanche.length - 1].round_win;
                    var _text = _this2.coins(total_win / _this2.details.context.mult) + ' x ' + _this2.details.context.mult + ' = ' + _this2.coins(total_win);
                    if (_this2.height > 200) {
                        if (total_win >= 100000) {
                            board = group.group().text('50%', '56%', _text).attr(Drawer.FS_LAST_EXTRA_SNAP_TEXT_CENTER_2)
                        } else {
                            board = group.group().text('50%', '56%', _text).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2)
                        }
                    } else {
                        if (total_win >= 100000) {
                            board = group.group().text('50%', '56%', _text).attr(Drawer.FS_LAST_EXTRA_SNAP_TEXT_CENTER)
                        } else {
                            board = group.group().text('50%', '56%', _text).attr(Drawer.LAST_EXTRA_SNAP_TEXT_CENTER)
                        }
                    }
                } else if (action_list[counter].action) {
                    board = _this2.board(group, action_list[counter]);
                    board.transform('T' + _this2.boardMarginLeft + ',' + (_this2.panelHeight + 1))
                } else {
                    board = _this2.draw_extra_snap(group, action_list, counter);
                    if (action.type === 'win') {
                        var widthMargin = _this2.width * 0.05;
                        board.transform('T' + widthMargin + ',' + (_this2.panelHeight + 1))
                    }
                }
            });
            if (details.last_action === 'freespin_stop' || details.last_action === 'freespin_init') {
                board.remove();
                this._background(snap, details, 'freespins');
                this._panel(snap, details.last_action)
            }
            snap.text('50%', '56%', this.get_text_for_action(details)).attr(Drawer.STYLE_TEXT_CENTER_WHITE);
            return snap.node
        }
    }, {
        key: 'make_freespins',
        ...
        key: 'board',
        value: function board(snap, one_action) {
            var _this3 = this;
            var board = one_action.board,
                mult_values = one_action.mult_values,
                mult_types = one_action.mult_types;
            var combs = [];
            if (one_action.winscatters) {
                combs = combs.concat(one_action.winscatters)
            }
            if (one_action.winlines) {
                combs = combs.concat(one_action.winlines)
            }
            var group = snap.group();
            var imageUrl = void 0;
            var pos_set = new Set;
            combs.forEach(function(comb) {
                comb.positions.forEach(function(position) {
                    pos_set.add(position)
                })
            });
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    var sym_id = board[i][j];
                    var sym_is_mult = sym_id === MULT_ID;
                    var sym_mult_value = mult_values ? mult_values[i][j] : 0;
                    var sym_type = mult_types ? mult_types[i][j] : 0;
                    var sym_prefix = sym_is_mult && sym_mult_value > 0 && sym_type === 1 ? '_2' : '';
                    imageUrl = _this3.url('sym' + board[i][j] + sym_prefix + '.png');
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * i, (_this3.cellHeight + _this3.cellOffsetY) * j - 2.5, _this3.cellWidth, _this3.cellHeight + 5);
                    if (sym_is_mult && sym_mult_value) {
                        var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * i - _this3.cellOffsetX + _this3.cellWidth * 0.2;
                        var rect_y = (_this3.cellHeight + _this3.cellOffsetY) * j + _this3.cellHeight / 3;
                        var rect_width = _this3.cellWidth * 0.6 + 2 * _this3.cellOffsetX;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = (_this3.cellHeight + _this3.cellOffsetY) * j + _this3.cellHeight / 2;
                        group.rect(rect_x, rect_y, rect_width, _this3.cellHeight / 3).attr({
                            'fill': 'white'
                        });
                        if (sym_mult_value < 100000) {
                            if (_this3.width > 200) {
                                group.text(text_x, text_y, sym_mult_value).attr(Drawer.MULT_VALUE_70)
                            } else {
                                group.text(text_x, text_y, sym_mult_value).attr(Drawer.MULT_VALUE_40)
                            }
                        }
                    }
                    if (!combs.length) return 'continue';
                    img.attr({
                        opacity: 0.5
                    });
                    Array.from(pos_set).forEach(function(pos) {
                        if (pos[0] == i && pos[1] == j) {
                            img.attr({
                                opacity: 1
                            })
        ...
        value: function _calculateSizes(cols, rows) {
            ...
            this.progressHeight = 3;
            this.boardHeight = this.height - (this.panelHeight + this.progressHeight + 1);
            this.cellWidth = parseInt(this.boardHeight / rows, 10) * 1.3;
            this.cellHeight = parseInt(this.boardHeight / rows, 10);
            this.cellWidth = parseInt(this.height * 1.45312 / cols, 10) - 2;
            this.colWidth = parseInt(this.width / cols, 10) - 2;
            this.cellOffsetX = 0;
            this.cellOffsetY = 0;
        ...
        key: '_combs',
        value: function _combs(snap, action_list, details) {
            var group = snap.group();
            this.bordered_cells = [];
            var actionFirst = action_list[0];
            var text = this.get_multiplier_prefix(details, actionFirst);
            var totalWin = details.context.win;
            if (this.details.last_action === 'respin') {
                var avalanche = this.details.context.avalanche ? this.details.context.avalanche : [];
                totalWin = avalanche[avalanche.length - 1].round_win
            }
            if (this.details.last_action === 'restore') {
                totalWin = 0
            }
            var delta = details.state == 'freespins' && this.details.context.mult > 1 ? 0 : 1;
            var labelLeft = this.title(details) + ', Win:' + this.coins(totalWin) + text;
            var labelRight = '';
            labelRight = action_list.length > 1 ? actionFirst.page.current + '/' + actionFirst.page.total : '';
            this._panel(group, labelLeft, labelRight);
            this.drawe_border_win_symbols(group, action_list[0]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            return group
        }
    }, {
        key: '_comb',
        value: function _comb(snap, combs, comb, details) {
            this.bordered_cells = [];
            var group = snap.group();
            var text = this.get_multiplier_prefix(details, comb);
            var win = details.last_action === 'restore' ? 0 : comb.win;
            var labelLeft = this.title(details) + ', Win:' + this.coins(win) + text;
            var labelRight = comb.page.current + '/' + comb.page.total;
            if (comb.action) {
                this.drawe_border_win_symbols(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            } else if (comb.type === 'additional') {
                labelLeft = 'Additional FreeSpins!'
            } else if (comb.type === 'win_for_scatters') {
                labelLeft = this.title(details) + ', Win:' + this.coins(win) + text
            }
            this._panel(group, labelLeft, labelRight);
            return group
        }
    }, {
        key: 'get_multiplier_prefix',
        value: function get_multiplier_prefix(details, action) {
            var text = '';
            var progress = details.context.mp_progress || 0;
            var multi = action.isMultiplier ? action.multiplier : progress;
            if (details.state === 'freespins' && progress > 0 && multi > 0) {
                text = ', x' + multi
            }
            if (details.state === 'spins' && action.isMultiplier) {
                text = ', x' + action.multiplier
            }
            return text
        }
    }, {
        key: 'get_mult_values',
        value: function get_mult_values(mult_values) {
            var mult = 0;
            mult_values && mult_values.forEach(function(row) {
        ...
        key: 'restore_except_winlines_get',
        value: function restore_except_winlines_get(context) {
            var winlines = [];
            var sym_map = {};
            context.board.map(function(column) {
                column.map(function(id) {
                    if (!sym_map[id]) sym_map[id] = 0;
                    sym_map[id] += 1
                })
            });
            var _loop3 = function _loop3(symbol) {
                if (sym_map[symbol] < 8) return 'continue';
                var positions = [];
                context.board.map(function(column, columnIndex) {
                    column.map(function(id, index) {
                        if (id != symbol) return;
                        positions.push([columnIndex, index])
                    })
                });
                var winline = {
                    amount: 0,
                    occurrences: sym_map[symbol],
                    positions: positions,
                    symbol: parseInt(symbol)
                };
                winlines.push(winline)
            };
            for (var symbol in sym_map) {
                var _ret3 = _loop3(symbol);
        ...
        key: 'drawe_border_win_symbols',
        value: function drawe_border_win_symbols(snap, one_action) {
            var group = snap.group().attr(BaseDrawer.STYLE_LINE_GROUP);
            if (this.details.last_action === 'respin') {
                return group
            }
            ...
            for (var i in combs) {
                var line = combs[i];
                for (var _i4 = 0; _i4 < line.positions.length; ++_i4) {
                    var cell = line.positions[_i4];
                    if (!isItemInArray(this.bordered_cells, cell)) {
                        group.rect(cell[0] * (this.cellWidth + this.cellOffsetX), cell[1] * (this.cellHeight + this.cellOffsetY), this.cellWidth + this.cellOffsetX, this.cellHeight + this.cellOffsetY).attr(BaseDrawer.STYLE_LINE_SYMBOL);
                        this.bordered_cells.push(cell)
                    }
        ...
        key: 'multiplier_values_get',
        value: function multiplier_values_get(round) {
            var values = new Array(6).fill(0);
            values = values.map(function() {
                return new Array(5).fill(0)
            });
            if (round.mps) {
                round.mps.map(function(point, index) {
                    var column = point[0];
                    var row = point[1];
                    values[column][row] = round.mp_values[index]
                })
            } else if (round.mult_values) {
                round.mult_values.map(function(column, columnIndex) {
                    column.map(function(value, rowIndex) {
        ...
        key: 'multiplier_types_get',
        value: function multiplier_types_get(round) {
            var values = new Array(6).fill(0);
            values = values.map(function() {
                return new Array(5).fill(0)
            });
            if (round.mps) {
                round.mps.map(function(point, index) {
                    var column = point[0];
                    var row = point[1];
                    values[column][row] = round.mp_types && round.mp_types[index] ? round.mp_types[index] : 0
                })
            }
            return values
        }
    }, {
        key: 'round_mp_progress_get',
        value: function round_mp_progress_get(round) {
            return round && round.mp_progress ? round.mp_progress : 0
        }
    }, {
        key: 'round_mp_acc_get',
        value: function round_mp_acc_get(round) {
            var acc = 0;
            if (round && round.mp_values) {
                round.mp_values.map(function(value) {
                    return acc += value
                })
            }
            return acc
        ...
        key: 'lastwin_get',
        value: function lastwin_get(context) {
            var defaultWin = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
            var win = defaultWin;
            if (context && context.avalanche) {
                var round = context.avalanche[context.avalanche.length - 1];
                win = round ? round.win : defaultWin
            }
            return win
        }
    }, {
        key: 'winlines_totalwin_get',
        value: function winlines_totalwin_get(winlines) {
            if (!winlines || winlines.length === 0) return 0;
            var win_accumulator = 0;
            for (var i in winlines) {
                var winline = winlines[i];
                win_accumulator += winline.amount
            }
...
Drawer.PRETTY_STYLE_TEXT_CENTER = {
    'stroke': '#C71585',
    'text-anchor': 'middle',
    'fill': '#0000',
    'font-size': '1.2em'
};
Drawer.LAST_EXTRA_SNAP_TEXT_CENTER = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#fff',
    'font-size': '1.0em'
};
Drawer.LAST_EXTRA_SNAP_TEXT_CENTER_2 = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#fff',
    'font-size': '1.2em'
};
Drawer.EXTRA_SNAP_TEXT_CENTER = {
    'stroke': '#ffffff',
    'text-anchor': 'middle',
    'fill': '#fff',
    'font-size': '0.65em'
};
Drawer.EXTRA_SNAP_TEXT_CENTER_2 = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#fff',
    'font-size': '1.0em'
};
Drawer.HEADER_TEXT_CENTER = {
    'text-anchor': 'middle',
    'fill': '#fff',
    'font-size': '0.6em',
    'font-weight': 'normal'
};
Drawer.FS_LAST_EXTRA_SNAP_TEXT_CENTER_2 = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#fff',
    'font-size': '0.9em'
};
Drawer.FS_LAST_EXTRA_SNAP_TEXT_CENTER = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#fff',
    'font-size': '0.7em'
};
Drawer.HEADER_TEXT_CENTER_2 = {
    'text-anchor': 'middle',
    'fill': '#fff',
    'font-size': '0.8em',
    'font-weight': 'normal'
};
Drawer.EXTRA_SNAP_TEXT_CENTER_2_COLORED = {
    'stroke': '#474747',
    'text-anchor': 'middle',
    'fill': '#474747',
    'font-size': '0.7em'
};
Drawer.EXTRA_SNAP_TEXT_CENTER_2_COLORED_LEFT = {
    'stroke': '#474747',
    'text-anchor': 'left',
    'fill': '#474747',
    'font-size': '0.7em'
