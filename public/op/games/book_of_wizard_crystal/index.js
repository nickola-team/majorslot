...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function title(details) {
            if (details.last_action == 'restore') {
                return 'Restore'
            } else if (details.last_action == 'freespin_init' || details.last_action == 'freespin_pick' || details.last_action == 'freespin_stop') {
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (this.new_extra) {
                if (this.width > 200) {
                    group.text('50%', '36%', '+' + roundsGranted + ' FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER_BIG)
                } else {
                    group.text('50%', '36%', '+' + roundsGranted + ' FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                }
                group.image(this.url('extra_' + details.context.extra[details.context.extra.length - 1] + '.png'), this.bestMargin + this.bestWidth / 3, this.height / 2 - 3, this.bestWidth / 3, this.height / 2)
            } else {
                if (this.width > 200) {
                    group.text('50%', '50%', '+' + roundsGranted + ' FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER_BIG)
                } else {
                    group.text('50%', '50%', '+' + roundsGranted + ' FreeSpins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                }
            }
            this._panel(group, labelLeft, labelRight);
            return snap
        ...
        value: function get_text_for_action(details) {
            if (details.last_action === 'freespin_stop') {
                ...
                return ''
            } else if (details.last_action === 'freespin_pick') {
                return ''
            }
        }
    }, {
        key: 'addition_winlines',
        value: function addition_winlines() {
            var _this2 = this;
            ...
            var list_indexes = [];
            winscatters.forEach(function(ws, index) {
                if (ws['trigger'] === undefined) {
                    list_indexes.push(ws);
                    var line_number = 0;
                    _this2.lines.forEach(function(nl) {
                        line_number += 1;
                        var line_positions = [];
                        ws['positions'].forEach(function(np) {
                            line_positions.push([np[0], nl[np[0]]])
                        });
                        winlines.push({
                            'line': line_number,
                            'symbol': ws['symbol'],
                            ...
                            'amount': ws['amount'] / 10,
                            'special_sym_id': ws['symbol'],
                            'occurrences': ws['occurrences']
                        })
                    })
                }
            });
            list_indexes.forEach(function(elem) {
                winscatters.splice(winscatters.indexOf(elem, 0), 1)
            });
            if (winlines.length) {
                this.details.context.winlines = winlines
            }
        }
    }, {
        key: 'sort_winlines',
        ...
        value: function board(snap, _board) {
            ...
            var group = snap.group();
            var imageUrl = void 0;
            var extra = details.state === 'freespins' && details.last_action !== 'freespin_init' ? details.context.extra.filter(function(s) {
                return s != _this3.new_extra
            }) : [];
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    if (extra.includes(_board[i][j])) {
                        imageUrl = _this3.url('extra_' + _board[i][j] + '.png')
                    } else {
        ...
        value: function make_spins(details) {
            ...
            snap.click(function(e) {
                ...
                var cur_board = Array.from(_this4.details.context.board);
                board.remove();
            ...
            if (details.last_action === 'freespin_stop') {
                ...
                this._panel(snap, 'freespin_stop')
            }
        ...
        key: 'get_fs_name',
        value: function get_fs_name(fs_num) {
            return {
                '1': 'FREE SPINS',
                '2': 'MAGIC CRYSTAL FREE SPINS'
            }[fs_num]
        }
    }, {
        key: 'make_freespins',
        value: function make_freespins(details) {
            ...
            this.addition_winlines();
            var combs = this.sort_winlines();
            var winscatters = this.details.context.winscatters ? this.details.context.winscatters : [];
            combs = combs.concat(winscatters);
            var combsGroup = snap.group();
            var combGroup = void 0;
            var margin = 0;
            this._combs(combsGroup, combs, details);
            ...
            if (winscatters.length) {
                if (details.context.freegame_mode == 1 && details.context.rounds_granted <= 72 || details.context.freegame_mode == 2 && details.context.rounds_granted <= 52) {
                    this.new_extra = details.context.extra[details.context.extra.length - 1]
                }
            ...
            if (details.last_action === 'freespin_init') {
                ...
                this._background(snap, 'freespins');
                this._panel(snap, details.last_action, '');
                snap.text('50%', '36%', 'Allowed modes:').attr(Drawer.PICKINFO_STYLE_TEXT_CENTER);
                if (details.context.allowed_modes.length == 1) {
                    snap.text('50%', '50%', '1. ' + this.get_fs_name(1)).attr(Drawer.PICKINFO_STYLE_TEXT_CENTER)
                }
                if (details.context.allowed_modes.length == 2) {
                    snap.text('50%', '50%', '1. ' + this.get_fs_name(1)).attr(Drawer.PICKINFO_STYLE_TEXT_CENTER);
                    snap.text('50%', '64%', '2. ' + this.get_fs_name(2)).attr(Drawer.PICKINFO_STYLE_TEXT_CENTER)
                }
            }
            if (details.last_action === 'freespin_pick') {
                board.remove();
                this._background(snap, 'freespins');
                this._panel(snap, details.last_action, '');
                snap.text('50%', '22%', 'Selected mode:').attr(Drawer.PICKINFO_STYLE_TEXT_CENTER);
                snap.text('50%', '34%', '' + this.get_fs_name(details.context.freegame_mode)).attr(Drawer.PICKINFO_STYLE_TEXT_CENTER);
                snap.text('50%', '46%', 'FS count: ' + details.context.rounds_left).attr(Drawer.PICKINFO_STYLE_TEXT_CENTER);
                var num_extras = details.context.extra.length;
                if (num_extras == 1) {
                    snap.image(this.url('extra_' + details.context.extra.pop() + '.png'), this.bestMargin + this.bestWidth / 3, this.height / 2 - 3, this.bestWidth / 3, this.height / 2)
                } else {
                    details.context.extra.forEach(function(extra_elem, index) {
                        snap.image(_this5.url('extra_' + extra_elem + '.png'), _this5.bestMargin + _this5.bestWidth / (num_extras + 1) / 2 + index * _this5.bestWidth / (num_extras + 1), (num_extras - 1) * _this5.height / num_extras - 3, _this5.bestWidth / (num_extras + 1), _this5.height / num_extras)
                    })
                }
            }
            var counter = 0;
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
                counter = (counter + 1) % combsGroup.children().length;
                combsGroup.transform('T-' + _this5.width * counter);
                progress.animate({
                    width: partSize * (counter + 1)
                }, 100);
                board.remove();
                board = _this5.board(group, cur_board, counter - 1);
                board.transform('T' + _this5.boardMarginLeft + ',' + (_this5.panelHeight + 1))
            });
            snap.text('50%', '40%', this.get_text_for_action(details)).attr(Drawer.FREESPINS_STYLE_TEXT_CENTER);
            return snap.node
        }
    }, {
        key: 'drawe_line_patern',
...
Drawer.PICKINFO_STYLE_TEXT_CENTER = {
    'stroke': '#FF0000',
    'stroke-width': 0.68,
    'text-anchor': 'middle',
    'fill': '#FF00FF',
