...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function _calculateSizes(cols, rows) {
            ...
            this.cellOffsetX = 2;
            this.cellOffsetY = -1;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10) - (isBonus ? 14 : 4)
        }
    }, {
        key: '_makeMigrations',
        value: function _makeMigrations(details) {
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && details.context.bs_count == 40 && details.last_action != 'restore' || details.state == 'freespins' && typeof details.context.winscatters !== 'undefined' && details.context.winscatters.length > 0 && (details.context.winscatters[0].trigger != 'bonus' || details.context.winscatters[1] && details.context.winscatters[1].trigger == 'freespins')
        }
    }, {
        key: 'make_extra_snap',
        ...
        value: function make_spins(details) {
            ...
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            var ce = details.state == 'bonus' && details.context.bs_count == 40 ? 1 : 0;
            if (this.is_extra_snap(details)) {
                this.make_extra_snap(details, combsGroup, margin + this.width * ce, snap);
                partSize = this.width / combsGroup.children().length
            }
            if (details.state == 'bonus' && ['respin'].includes(details.last_action)) {
                ce = 0;
                var groupLength = 0;
                if (details.context.multiplied_bs) {
                    board && board.remove();
                    ce = -1;
                    var finalBoard = true;
                    this.make_bonus_board_snap(details, combsGroup, margin + this.width * ce, snap, finalBoard);
                    ce += 1;
                    finalBoard = false;
                    this.make_bonus_board_snap(details, combsGroup, margin + this.width * ce, snap, finalBoard);
                    ce += 1;
                ...
                if (details.context.bs_count !== 40) this.make_unlock_snap(details, combsGroup, margin + this.width * ce, snap);
                partSize = this.width / (groupLength + 1)
            }
            if (details.last_action == 'freespin_init' || details.last_action == 'freespin_stop') {
                board.remove();
                this._background(snap, 'freespins');
            ...
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
                counter = (counter + 1) % combsGroup.children().length;
                var bonusMultFrame = (details.context.multiplied_bs || details.last_action === 'restore' && details.state == 'bonus') && counter === combsGroup.children().length - 1;
                if (bonusMultFrame) {
                    counter = 0
                }
        ...
        value: function make_bonus_board_snap(details, combsGroup, margin, snap, finalBoard) {
            var _this4 = this;
            var currentWinLine = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : -1;
            var group = combsGroup.group();
            margin += this.width;
            ...
            var progress = group.rect(0, this.height - this.progressHeight, 100, 200).attr(Drawer.STYLE_PROGRESS);
            group.transform('T' + margin + ',0');
            var isBonus = details.state === 'bonus';
            var round_bet = details.round_bet_not_restored ? details.round_bet_not_restored : details.round_bet;
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    var bs = details.context.bs ? details.context.bs.filter(function(el) {
                        return el.reel == i && el.position == j
                    })[0] : false;
                    var symType = bs ? bs.type : 'regular';
                    var meteorType = symType == 'regular' ? '1' : '2';
                    var postfix = isBonus ? '_b_' + meteorType : details.context.board[i][j] === 11 ? '_' + meteorType : '';
                    var symName = 'sym' + details.context.board[i][j] + postfix + '.png';
                    imageUrl = _this4.url(symName);
                    var img = group.image(imageUrl, (_this4.cellWidth + _this4.cellOffsetX) * i + _this4.boardMarginLeft, (_this4.cellHeight + _this4.cellOffsetY) * j + (_this4.panelHeight + 1), _this4.cellWidth, _this4.cellHeight);
                    if (isBonus && details.context.board[i][j] === 11) {
                        img.attr({
                            opacity: unlockedRow[j] ? 1 : 0.4
                        })
                    }
                    if (combs.length == 1) {
                        currentWinLine = 0
                ...
                for (var j in details.context.board[i]) {
                    var _ret2 = _loop2(j);
                    if (_ret2 === 'continue') continue
                }
            };
            for (var i in details.context.board) {
                _loop(i)
            }
            var filled_row = details.context.filled_row;
            if (isBonus && filled_row) {
                for (var i = 0; i < 4; i++) {
                    var multipliers = [2, 3, 5, 10];
                    var imgX = group.image(this.url('x' + multipliers[i] + '.png'), (this.cellWidth + this.cellOffsetX) * 5 + this.boardMarginLeft, (this.cellHeight + this.cellOffsetY) * (3 - i) + (this.panelHeight + 1), this.cellWidth * 0.725, this.cellHeight);
                    imgX.attr({
                        opacity: unlockedRow[3 - i] && filled_row[i] === 1 ? 1 : 0.4
                    })
                }
            }
            if (typeof details.context.bs === 'undefined') return snap;
            var multiplied_bs = details.context.multiplied_bs;
            ...
            try {
                var _loop3 = function _loop3() {
                    ...
                    var i = bs.reel;
                    var j = bs.position;
                    if (details.context.board[i][j] !== 11) return 'continue';
                    var value = void 0;
                    if (bs.type !== 'regular') {
                        value = bs.type
                    } else {
                        value = isBonus && finalBoard && multiplied_bs && multiplied_bs.some(function(mbs) {
                            return mbs.reel === i && mbs.position === j
                        }) ? multiplied_bs.filter(function(mbs) {
                            return mbs.reel === i && mbs.position === j
                        })[0].value : bs.value;
                        value = (+value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    }
                    var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * i - _this4.cellOffsetX + 2 + _this4.boardMarginLeft;
                    var rect_y = (_this4.cellHeight + _this4.cellOffsetY) * j + _this4.cellHeight / 3 + (_this4.panelHeight + 1);
                    var rect_width = _this4.cellWidth + 2 * _this4.cellOffsetX - 3;
                    var text_x = rect_x + rect_width / 2;
                    var text_y = rect_y + (_this4.cellHeight / 2 - _this4.cellHeight / 3);
                    var opacity = !unlockedRow ? 1 : unlockedRow[j] ? 1 : 0.5;
                    var attr = void 0;
                    group.rect(rect_x, rect_y, rect_width, _this4.cellHeight / 3).attr({
                        'fill': 'white',
                        'opacity': opacity
                    });
                    if (['mini', 'midi', 'minor', 'major', 'grand'].includes(value)) {
                        attr = Object.assign({}, _this4.width > 200 ? Drawer['BS_' + value.toUpperCase() + '_55'] : Drawer['BS_' + value.toUpperCase() + '_40'], {
                            'opacity': opacity
                        });
                        ...
                        if (numValue < 999999) {
                            attr = Object.assign({}, _this4.width > 200 ? Drawer.BS_VALUE_52 : Drawer.BS_VALUE_28, {
                                'opacity': opacity
                            });
                            group.text(text_x, text_y, value).attr(attr)
                        } else {
                            attr = Object.assign({}, _this4.width > 200 ? Drawer.BS_VALUE_34 : Drawer.BS_VALUE_22, {
                                'opacity': opacity
                            });
                            group.text(text_x, text_y, value).attr(attr)
                        }
                ...
                for (var _iterator = details.context.bs[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    var _ret3 = _loop3();
            ...
            var addOffset = true;
            var showMultiplier = !finalBoard;
            if (isBonus && filled_row) {
                this.draw_bg_multipliers(group, addOffset, showMultiplier)
            }
            var labelLeft = details.last_action == 'restore' ? 'Restore' : 'Bonus(' + details.context.rounds_left + ')';
            var labelRight = finalBoard ? 'all/2' : '';
            this._panel(group, labelLeft + ', Win:' + this.coins(details.round_win), labelRight);
            if (!finalBoard) {
                var _combs2 = details.context.multiplied_bs.map(function(bs) {
                    return [bs.reel, bs.position]
                });
                this.drawe_border_win_symbols(group, null, _combs2).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            return snap
        }
    }, {
        ...
        value: function board(snap, _board) {
            ...
            var _loop4 = function _loop4(i) {
                var _loop5 = function _loop5(_j) {
                    var bs = _this5.details.context.bs ? _this5.details.context.bs.filter(function(el) {
                        return el.reel == i && el.position == _j
                    })[0] : false;
                    var symType = bs ? bs.type : 'regular';
                    var meteorType = symType == 'regular' ? '1' : '2';
                    var postfix = isBonus ? '_b_' + meteorType : _board[i][_j] === 11 ? '_' + meteorType : '';
                    var symName = 'sym' + _board[i][_j] + postfix + '.png';
                    imageUrl = _this5.url(symName);
                    var img = group.image(imageUrl, (_this5.cellWidth + _this5.cellOffsetX) * i, (_this5.cellHeight + _this5.cellOffsetY) * _j, _this5.cellWidth, _this5.cellHeight);
                    if (isBonus && _board[i][_j] === 11) {
                        img.attr({
                            opacity: unlockedRow[_j] ? 1 : 0.4
                        })
                    }
                    if (combs.length == 1) {
                        currentWinLine = 0
            ...
            var filled_row = this.details.context.filled_row;
            if (isBonus && filled_row) {
                for (var i = 0; i < 4; i++) {
                    var multipliers = [2, 3, 5, 10];
                    var imgX = group.image(this.url('x' + multipliers[i] + '.png'), (this.cellWidth + this.cellOffsetX) * 5, (this.cellHeight + this.cellOffsetY) * (3 - i), this.cellWidth * 0.725, this.cellHeight);
                    imgX.attr({
                        opacity: unlockedRow[3 - i] && filled_row[i] === 1 ? 1 : 0.4
                    })
                }
            }
            if (typeof this.details.context.bs === 'undefined') return group;
            var multiplied_bs = this.details.context.multiplied_bs;
            ...
            try {
                var _loop6 = function _loop6() {
                    ...
                    var i = bs.reel;
                    var j = bs.position;
                    if (_this5.details.context.board[i][j] !== 11) return 'continue';
                    var value = void 0;
                    if (bs.type !== 'regular') {
                        value = bs.type
                    } else {
                        value = isBonus && multiplied_bs && multiplied_bs.some(function(mbs) {
                            return mbs.reel === i && mbs.position === j
                        }) ? multiplied_bs.filter(function(mbs) {
                            return mbs.reel === i && mbs.position === j
                        })[0].value : bs.value;
                    ...
                    var rect_x = (_this5.cellWidth + _this5.cellOffsetX) * i - _this5.cellOffsetX + 2;
                    var rect_y = (_this5.cellHeight + _this5.cellOffsetY) * j + _this5.cellHeight / 3;
                    var rect_width = _this5.cellWidth + 2 * _this5.cellOffsetX - 3;
                    var text_x = rect_x + rect_width / 2;
                    var text_y = (_this5.cellHeight + _this5.cellOffsetY) * j + _this5.cellHeight / 2;
                    var opacity = !unlockedRow ? 1 : unlockedRow[j] ? 1 : 0.5;
                    var attr = void 0;
                    ...
                    if (['mini', 'midi', 'minor', 'major', 'grand'].includes(value)) {
                        attr = Object.assign({}, _this5.width > 200 ? Drawer['BS_' + value.toUpperCase() + '_55'] : Drawer['BS_' + value.toUpperCase() + '_40'], {
                            'opacity': opacity
                        });
                        group.text(text_x, text_y, value).attr(attr)
                    } else {
                        var numValue = parseInt(value.replace(/\,/g, ''), 10);
                        if (numValue < 999999) {
                            attr = Object.assign({}, _this5.width > 200 ? Drawer.BS_VALUE_52 : Drawer.BS_VALUE_28, {
                                'opacity': opacity
                            });
                            group.text(text_x, text_y, value).attr(attr)
                        } else {
                            attr = Object.assign({}, _this5.width > 200 ? Drawer.BS_VALUE_34 : Drawer.BS_VALUE_22, {
                                'opacity': opacity
                            });
                            group.text(text_x, text_y, value).attr(attr)
                        }
            ...
            if (isBonus && filled_row) {
                this.draw_bg_multipliers(group)
            }
            return group
        }
    }, {
        key: 'draw_bg_multipliers',
        value: function draw_bg_multipliers(group) {
            var _this6 = this;
            var addOffset = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
            var showMultiplier = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined;
            var threshold_num = this.details.context.threshold_num;
            var filled_row = this.details.context.filled_row;
            var filled_positions = filled_row.map(function(el, i) {
                return el === 1 ? 3 - i : -1
            }).filter(function(el) {
                return el >= 0
            });
            ...
            try {
                var _loop7 = function _loop7() {
                    var bs = _step3.value;
                    var show = showMultiplier && multiplied_bs.some(function(mbs) {
                        return mbs.reel === bs.reel && mbs.position === bs.position
                    }) || !showMultiplier && bs.type !== 'regular';
                    if (show && filled_positions.includes(bs.position) && bs.position >= 4 - threshold_num) {
                        var i = bs.reel;
                        var _j2 = bs.position;
                        var multiplier = [2, 3, 5, 10][3 - _j2];
                        var value_rect_width = _this6.cellWidth + 2 * _this6.cellOffsetX;
                        var mltp_rect_x = -5 + (_this6.cellWidth + _this6.cellOffsetX) * i - _this6.cellOffsetX + (addOffset ? _this6.boardMarginLeft : 0) + value_rect_width - value_rect_width / 2.5;
                        var mltp_rect_y = -0 + (_this6.cellHeight + _this6.cellOffsetY) * _j2 + (addOffset ? _this6.panelHeight + 1 : 0);
                        var mltp_rect_width = value_rect_width / 2;
                        var mltp_rect_height = _this6.cellHeight / 3;
                        var mltp_text_x = mltp_rect_x + mltp_rect_width / 2;
                        var mltp_text_y = -0.5 + (_this6.cellHeight + _this6.cellOffsetY) * _j2 + (addOffset ? _this6.panelHeight + 1 : 0) + _this6.cellHeight / 2 - mltp_rect_height;
                        group.rect(mltp_rect_x, mltp_rect_y, mltp_rect_width, mltp_rect_height).attr({
                            'fill': 'white'
                        });
                        group.text(mltp_text_x, mltp_text_y, 'x' + multiplier).attr(Drawer.MULTIPLIER_VALUE_47)
                    }
                };
        ...
        key: 'drawe_border_win_symbols',
        value: function drawe_border_win_symbols(snap, line, bonusPositions) {
            var group = snap.group().attr(BaseDrawer.STYLE_LINE_GROUP);
            var combs = bonusPositions ? bonusPositions : line.positions;
            for (var i = 0; i < combs.length; ++i) {
                var cell = combs[i];
                if (!isItemInArray(this.bordered_cells, cell)) {
                    group.rect(cell[0] * (this.cellWidth + this.cellOffsetX), cell[1] * (this.cellHeight + this.cellOffsetY), this.cellWidth + this.cellOffsetX, this.cellHeight + this.cellOffsetY).attr(BaseDrawer.STYLE_LINE_SYMBOL);
