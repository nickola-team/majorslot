...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        key: '_background',
        value: function _background(snap, state) {
            if (state === 'freespins' || this.details.last_action == 'freespin_init' || this.details.last_action == 'freespin_stop') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#4682B4'
                })
            } else if (state === 'spins') {
            ...
            } else if (this.details.context.bs_count === 15 || this.details.context.bs_count === 15) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#6B8E23'
                })
            } else {
        ...
        value: function is_extra_snap(details) {
            return details.state == 'freespins' && (details.context.bs_count === 15 || details.context.bs_count === 15) || details.state == 'freespins' && details.context.winscatters != undefined && details.context.winscatters.length > 0
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'freespins' && (details.context.bs_count === 15 || details.context.bs_count === 15)) {
                labelLeft = 'Jackpot!';
        ...
        value: function make_spins(details) {
            ...
            if (details.last_action === 'freespin_init' || details.last_action === 'freespin_stop') {
                board.remove();
                this._background(snap, 'freespin');
                this._panel(snap, details.last_action, '')
            }
            var counter = 0;
            snap.click(function(e) {
        ...
        value: function board(snap, _board) {
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    if (_this3.details.state === 'freespins' && [10, 12].includes(_board[i][j])) {
                        imageUrl = _this3.url(_board[i][j] + '_fs.png')
                    } else {
                    ...
                    if (_this3.details.state === 'freespins' && ![10, 11, 12].includes(_board[i][j])) {
                        img.attr({
                            opacity: 0.5
                        })
                    }
                    if (combs.length === 1 && _this3.details.state !== 'freespins') {
                        currentWinLine = 0
                    }
                    var value = void 0;
