...
var COLLECT_ID_ARRAY = [12, 120];
var MULTI_ID_ARRAY = [13, 130];
var BONUS_PROGRESS_ID_ARRAY = [11, 12, 13];
var MYSTERY_ID = 14;
var BONUS_ID_ARRAY = [10, 11, 12, 13, 14, 110, 120, 130];
var MIN_COUNT_BN_TRIGGER = 6;
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function _getCombs() {
            ...
            if (this._is_bonus_trigger(this.details)) {
                var winBonus = {
                    bonusWin: true,
                    positions: [],
                    amount: 0
            ...
            if (this._is_bac_win()) {
                ...
                for (var _i in this._dynamicBoard) {
                    for (var _j in this._dynamicBoard[_i]) {
                        if (BONUS_ID_ARRAY.includes(this._dynamicBoard[_i][_j])) {
                            beforeWinBonus.positions.push([_i, _j])
                        }
                    }
                }
                ...
                for (var _i2 in board) {
                    for (var _j2 in board[_i2]) {
                        if (BONUS_ID_ARRAY.includes(board[_i2][_j2])) {
                            _winBonus.positions.push([_i2, _j2])
                        }
                    }
                }
                combs.push(_winBonus)
            }
            return combs
        }
    }, {
        ...
        key: '_is_bonus_trigger',
        value: function _is_bonus_trigger(details) {
            var count = 0;
            var board = this._dynamicBoard;
            for (var col in board) {
                for (var row in board[col]) {
                    if (BONUS_ID_ARRAY.includes(board[col][row])) {
                        count++
                    }
                }
            }
            return count >= MIN_COUNT_BN_TRIGGER && details.last_action === 'spin'
        }
    }, {
        key: '_is_bac_win',
            ...
            if (combs.length && combs[0].isPopUp) {
                ...
                if (!this._is_bonus_game()) {
                    for (var _i3 = 0; _i3 < combs.length; ++_i3) {
                        if (combs[_i3].isPopUp || combs[_i3].bonusWin) continue;
                        this.drawe_line_patern(group, combs[_i3]).transform('T' + this.boardMarginLeft + ',' + (this.boardMarginTop + this.panelHeight + 1))
                    }
                }
                return group
            }
        ...
        value: function _get_bonus_combs() {
            ...
            var multiData = this._get_multi_data();
            ...
            if (mysteryData.length) {
                mysteryData.forEach(function(data) {
                    ...
                    if (!BONUS_PROGRESS_ID_ARRAY.includes(data.id)) combs.push({
                        positions: [data.pos],
                        opened: true,
                        data: data,
                        amount: win
            ...
            combs.forEach(function(comb) {
                if (comb.data) {
                    ...
                    if (!mysteryOpenedId && comb.opened && !MULTI_ID_ARRAY.includes(_this4._dynamicBoard[pos[0]][pos[1]]) || comb.multiplierOpened) {
                    ...
                    if (!hasPosition && BONUS_ID === mysteryOpenedId) processedPositions.push(pos);
                    if (!hasPosition && comb.opened) processedPositions.push(pos)
                }
            });
        ...
        value: function board(snap) {
            ...
            if (currentCombination && currentCombination.data) {
                ...
                if (currentCombination.id) {
                    this._dynamicBoard[pos[0]][pos[1]] = currentCombination.id
                } else if (currentCombination.opened || currentCombination.multiplierOpened) {
                    var collectValue = 0;
                    if (BONUS_ID === specialSymbolData.id) {
                        ...
                        this._dynamicBoard.forEach(function(rows, i) {
                            rows.forEach(function(r, j) {
                                ...
                                var isAvailableSymbolForUpdate = [10, 110, 120, 130].includes(symbolId);
                                if (MULTI_ID_ARRAY.includes(_this5._dynamicBoard[pos[0]][pos[1]]) && isAvailableSymbolForUpdate && currentCombination.multiplierOpened) {
                                    _this5._dynamicValues[i][j] = symbolValue * specialSymbolData.mult_value
                                }
                    ...
                    if (currentCombination.multiplierOpened && MULTI_ID_ARRAY.includes(this._dynamicBoard[pos[0]][pos[1]])) {
                        this._dynamicValues[pos[0]][pos[1]] = 'x' + specialSymbolData.mult_value
                    }
                    if (currentCombination.opened && MULTI_ID_ARRAY.includes(this._dynamicBoard[pos[0]][pos[1]])) {
                        this._dynamicValues[pos[0]][pos[1]] = specialSymbolData.bs_v
                    }
                }
            }
            ...
            this._dynamicBoard.forEach(function(rows, i) {
                rows.forEach(function(r, j) {
                    var value = _this5._dynamicValues[i][j];
                    ...
                    if (MYSTERY_ID === _this5._dynamicBoard[i][j]) value = '';
                    group.image(_this5.url(imgName), (_this5.cellWidth + _this5.cellOffsetX) * i, (_this5.cellHeight + _this5.cellOffsetY) * j, _this5.cellWidth, _this5.cellHeight);
        ...
        key: '_get_multi_data',
        value: function _get_multi_data() {
            return this.details.context.multi_values || []
        }
    }, {
        ...
        value: function get_origin_bsv() {
            return this.details.context.origin_bs_v || []
        }
    }, {
        key: 'get_origin_board',
        value: function get_origin_board() {
            return this.details.context.origin_board || []
        }
    }, {
        key: 'get_board',
        ...
        value: function _initDynamic() {
            ...
            this._dynamicBoard = originBoard.length && !initByFinal ? originBoard.map(function(innerArray) {
