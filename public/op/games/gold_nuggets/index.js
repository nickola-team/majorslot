...
var BONUS_ID = 1;
var COLLECCTOR_ID = 5;
var BONUS_ID_ARRAY = [1, 2, 3, 4, 5, 6, 7];
var MIN_COUNT_BN_TRIGGER = 3;
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            if (details.state !== 'bonus' || details.context.bs_count !== 9) return;
        ...
        value: function make_spins(details) {
            ...
            var combsGroup = snap.group();
            var combGroup = void 0;
            if (this.is_bonus_trigger(this.details) && 'spin' === this.details.last_action) {
                var winbonus = {
                    trigger: 'bonus',
                    positions: [],
                    amount: 0
        ...
        key: 'is_bonus_trigger',
        value: function is_bonus_trigger(details) {
            var count = 0;
            var board = details.context.board;
            for (var i in board) {
                if (BONUS_ID_ARRAY.includes(board[i][1])) {
                    count = count + 1
                }
            }
            return count >= MIN_COUNT_BN_TRIGGER
        }
    }, {
        key: 'make_bonus',
        value: function make_bonus(details) {
            ...
            var combs = [];
            var isCollectorSymbol = details.context.collectors;
            var isMystery = details.context.rounds_left === 0 && details.context.mystery_pos.length;
            var mysteryPos = [];
            if (isMystery) mysteryPos = details.context.mystery_pos.sort(function(a, b) {
                return a[1] - b[1]
            });
            var isBonusInit = details.last_action === 'bonus_init';
            if (isBonusInit && isCollectorSymbol) combs.push({
                positions: [],
                amount: details.round_win,
                isPopUp: true
            });
            if (isCollectorSymbol && !isMystery) {
                combs = combs.concat([{
                    positions: [],
                    amount: details.round_win
                }])
            }
            if (mysteryPos.length) {
                mysteryPos.forEach(function(p, i) {
                    var isCollect = _this3.details.context.board[p[0]][p[1]] === COLLECCTOR_ID;
                    var mysteryIndex = i;
                    if (isCollect) {
                        combs = combs.concat([{
                            positions: [],
                            ...
                            isCollectorInMystery: mysteryIndex
                        }])
                    }
                    combs = combs.concat([{
                        positions: [],
            ...
            var partSize = this.width / combsGroup.children().length;
            var progress = group.rect(0, this.height - this.progressHeight, partSize, 3).attr(Drawer.STYLE_PROGRESS);
            var board = this.board(group, game_board, -1, false);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            if (details.last_action === 'bonus_init') {
                board.remove();
                if (!isCollectorSymbol) this._background(snap, 'bonus');
                if (!isCollectorSymbol) this._panel(snap, details.last_action, '')
            }
            var counter = 0;
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
                ...
                var mysteryIndex = counter && combs.length ? combs[counter - 1].mysteryIndex : -1;
                var isCollectorInMystery = counter && combs.length ? combs[counter - 1].isCollectorInMystery : -1;
                var cur_board = Array.from(game_board);
                board.remove();
                if (isBonusInit && counter) board = _this3.board(group, cur_board, counter - 1, counter - 1, mysteryIndex, isCollectorInMystery);
                if (!isBonusInit) board = _this3.board(group, cur_board, counter, counter, mysteryIndex, isCollectorInMystery);
                board.transform('T' + _this3.boardMarginLeft + ',' + (_this3.panelHeight + 1))
            });
            if (!isCollectorSymbol) snap.text('50%', '56%', this.get_text_for_action(details)).attr(Drawer.STYLE_TEXT_CENTER_RED);
            return snap.node
        }
    }, {
        key: 'board',
        value: function board(snap, _board) {
            ...
            var newBonusBoard = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
            var _this4 = this;
            var mysteryIndex = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : -1;
            var isCollectorInMystery = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : -1;
            var combs = [].concat(this.details.context.winlines, this.details.context.winscatters);
            var isBonusGame = this.details.state === 'bonus';
            if (mysteryIndex >= 0) {
                _board = this.details.context.orig_board.map(function(row) {
                    return row.slice()
                });
                var finalBoard = this.details.context.board;
                var mysteryPos = this.details.context.mystery_pos.sort(function(a, b) {
                    return a[1] - b[1]
                });
                mysteryPos.forEach(function(p, i) {
                    if (i <= mysteryIndex) _board[p[0]][p[1]] = finalBoard[p[0]][p[1]]
                })
            } else if (this.details.context.orig_board && !newBonusBoard && isBonusGame && mysteryIndex == -1) {
                _board = this.details.context.orig_board
            }
            if (this.is_bonus_trigger(this.details) && 'spin' == this.details.last_action) {
                var winbonus = {
                    trigger: 'bonus',
            ...
            var _loop = function _loop(_i) {
                var _loop2 = function _loop2(_j) {
                    var isCollectorWin = _this4.details.context.collectors;
                    var isCollectorSymbol = _board[_i][_j] === COLLECCTOR_ID;
                    var isBonusSymbol = _board[_i][_j] === BONUS_ID;
                    var mult_value = 1;
                    imageUrl = _this4.url(_board[_i][_j] + '.png');
                    if (_this4.details.state === 'bonus' && !BONUS_ID_ARRAY.includes(_board[_i][_j])) {
                        imageUrl = _this4.url('0.png')
                    }
                    var img = group.image(imageUrl, (_this4.cellWidth + _this4.cellOffsetX) * _i, (_this4.cellHeight + _this4.cellOffsetY) * _j, _this4.cellWidth, _this4.cellHeight);
                    var value = void 0;
                    if (_this4.details.context.bs_v != undefined) {
                        value = _this4.details.context.bs_v[_i][_j]
                    } else {
                        value = _this4.details.context.bs_v[_i][_j]
                    }
                    if (isBonusGame && isCollectorWin && isCollectorSymbol && !newBonusBoard || isBonusGame && isCollectorWin && isCollectorSymbol && isCollectorInMystery >= 0) {
                        var collector = null;
                        var index = Number(_i) + Number(_j) * _board.length;
                        if (isCollectorInMystery >= 0) {
                            var _mysteryPos = _this4.details.context.mystery_pos.sort(function(a, b) {
                                return a[1] - b[1]
                            })[mysteryIndex];
                            var currentPos = _mysteryPos[0] + _mysteryPos[1] * _board.length;
                            var currentCollector = _this4.details.context.collectors.find(function(c) {
                                return c.pos === currentPos
                            });
                            collector = currentCollector;
                            if (collector && collector.pos !== index) collector = null
                        } else {
                            collector = _this4.details.context.collectors.find(function(c) {
                                return c.pos === index
                            })
                        }
                        if (collector) mult_value = collector.mult;
                        value = _this4.details.context.bs_v[_i][_j] / mult_value
                    }
                    if (isBonusSymbol || isCollectorSymbol) {
                        if (typeof value === 'number') {
                            value = (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                            if (value != 0) {
                                var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i - _this4.cellOffsetX;
                            ...
                            if (mult_value > 1) {
                                var _rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i - _this4.cellOffsetX + _this4.cellWidth * 0.25;
                                var _rect_y = (_this4.cellHeight + _this4.cellOffsetY) * _j + _this4.cellHeight * 0.04;
                                var _rect_width = _this4.cellWidth * 0.5;
                                var _text_x = (_this4.cellWidth + _this4.cellOffsetX) * _i - _this4.cellOffsetX + _this4.cellWidth * 0.5;
                                var _text_y = (_this4.cellHeight + _this4.cellOffsetY) * _j + _this4.cellHeight * 0.2;
                                group.rect(_rect_x, _rect_y, _rect_width, _this4.cellHeight / 3).attr({
                                    fill: 'white'
                                });
                                if (_this4.width > 200) {
                                    group.text(_text_x, _text_y, 'x' + mult_value).attr(Drawer.MULT_VALUE_50)
                                } else {
                                    group.text(_text_x, _text_y, 'x' + mult_value).attr(Drawer.MULT_VALUE_35)
                                }
                            }
                        }
                    }
                ...
                for (var _j in _board[_i]) {
                    var _ret2 = _loop2(_j);
                    if (_ret2 === 'continue') continue
                }
            };
            for (var _i in _board) {
                _loop(_i)
            }
            return group
        }
        ...
        value: function _combs(snap, combs, details) {
            if (combs.length && combs[0].isPopUp) {
                ...
                _group.text('50%', '56%', this.get_text_for_action(details)).attr(Drawer.STYLE_TEXT_CENTER_RED);
                return _group
            }
            var group = snap.group();
            this.bordered_cells = [];
        ...
        value: function _comb(snap, combs, comb, details) {
            if (comb.isPopUp) {
                var _group2 = snap.group();
                var _labelLeft2 = this.title(details) + ', Win:' + this.coins(comb.amount);
                var _labelRight2 = combs.indexOf(comb) + 1 + '/' + combs.length;
                this._panel(_group2, _labelLeft2, _labelRight2);
                return _group2
            }
            this.bordered_cells = [];
            var group = snap.group();
        ...
        key: 'newBonusSymbolsToCollect',
        value: function newBonusSymbolsToCollect() {
            var game_board = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.details.context.board;
            return game_board.some(function(col) {
                return col.includes(BONUS_ID) || col.includes(JACKPOT_ID)
            })
        }
    }, {
        key: '_calculateSizes',
        value: function _calculateSizes(cols, rows) {
            ...
            this.cellHeight = parseInt(this.boardHeight / cols, 10);
            this.cellWidth = this.cellHeight * 1.25;
            this.cellOffsetX = 0;
            this.cellOffsetY = 0;
