...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function _background(snap, state) {
            ...
            } else if (this.details.context.winscatters && this.details.context.winscatters.length > 0) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#4682B4'
                })
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && this.get_pearls_count(details) === 15 || details.state == 'freespins' && details.context.winscatters && details.context.winscatters.length > 0 || details.state == 'bonus' && details.context.last_respin && this.is_extra_respins(details)
        }
    }, {
        key: 'get_arr_sum',
        ...
        value: function is_extra_respins(details) {
            ...
            if (this.details.context.bs_v != undefined) {
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            } else if (details.state == 'freespins' && details.context.winscatters && details.context.winscatters.length) {
        ...
        value: function make_spins(details) {
            ...
            var combs = [];
            if (details.context.winlines) {
                combs = combs.concat(details.context.winlines)
            }
            if (details.context.winscatters) {
                    ...
                    if (value != 0) {
                        ...
                        if (value == 'mini') {
                            if (_this3.width > 200) {
                                ...
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MINI_JACKPOT_70)
                            } else {
                                group.rect(rect_x, rect_y, rect_width, _this3.cellHeight / 3).attr({
                                    'fill': 'white'
                                });
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MINI_JACKPOT_40)
                            }
                        } else if (value == 'minor') {
                            if (_this3.width > 200) {
                                group.rect(rect_x, rect_y, rect_width, _this3.cellHeight / 3).attr({
                                ...
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MINOR_JACKPOT_70)
                            } else {
                                group.rect(rect_x, rect_y, rect_width, _this3.cellHeight / 3).attr({
                                    'fill': 'white'
                                });
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_MINOR_JACKPOT_40)
                            }
                        } else {
                            if (value == 'extra_respins') {
                                if (is_last_respin) {
                                    imageUrl = _this3.url('11.png');
                                    img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * i, (_this3.cellHeight + _this3.cellOffsetY) * j, _this3.cellWidth, _this3.cellHeight);
                                    value = '+' + _this3.details.context.extra_respins[i][j]
                                } else {
                                    value = ''
                                }
                                if (value != '') {
