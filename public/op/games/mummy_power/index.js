...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function board(snap, _board) {
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    var w = _this2.cellWidth;
                    var h = _this2.cellHeight;
                    var x = +i;
                    var y = +j;
                    var isBigSymbol = false;
                    if (_this2.bigSymbols && [0, 1].includes(x - _this2.bigSymbols.x) && [0, 1].includes(y - _this2.bigSymbols.y)) {
                        w *= 2;
                        h *= 2;
                        x = _this2.bigSymbols.x;
                        y = _this2.bigSymbols.y;
                        isBigSymbol = true
                    }
                    imageUrl = _this2.url(_board[i][j] + '.png');
                    var img = void 0;
                    if (isBigSymbol) {
                        if (!_this2.bigSymbols.img) {
                            _this2.bigSymbols.img = group.image(imageUrl, (_this2.cellWidth + _this2.cellOffsetX) * x, (_this2.cellHeight + _this2.cellOffsetY) * y, w, h)
                        }
                        img = _this2.bigSymbols.img;
                        group.add(_this2.bigSymbols.img)
                    } else {
                        img = group.image(imageUrl, (_this2.cellWidth + _this2.cellOffsetX) * i, (_this2.cellHeight + _this2.cellOffsetY) * j, w, h)
                    }
                    ...
                    if (currentWinLine !== -1) {
                        ...
                        if (combs[currentWinLine]) {
                            ...
                            positions.forEach(function(pos) {
                                if (pos[0] == i && pos[1] == j || _this2.bigSymbols && [0, 1].includes(i - _this2.bigSymbols.x) && [0, 1].includes(j - _this2.bigSymbols.y) && _this2.bigSymbols && [0, 1].includes(pos[0] - _this2.bigSymbols.x) && [0, 1].includes(pos[1] - _this2.bigSymbols.y)) {
                                    img.attr({
                                        opacity: 1
                                    })
                                }
        ...
        key: '_calculateBigSymbols',
        value: function _calculateBigSymbols(board_type, block_type) {
            this.bigSymbols = {};
            if (!block_type) {
                return
            }
        ...
        value: function make_spins(details) {
            ...
            this.cols = details.context.board.length;
            this.rows = details.context.board[0].length;
            this._calculateBigSymbols(details.context.board_type, details.context.block_type);
            this._calculateSizes(this.cols, this.rows);
            var snap = new Snap(this.width, this.height);
        ...
        value: function make_freespins(details) {
            ...
            if (details.context.winscatters != undefined && details.context.winscatters.length) {
                var roundsGranted = details.context.rounds_granted_for_last_spin;
            ...
            if (line) {
                for (var i = 0; i < line.positions.length; ++i) {
                    ...
                    var _cell = _slicedToArray(cell, 2),
                        _x2 = _cell[0],
                        _y = _cell[1];
                    var _w = this.cellWidth + this.cellOffsetX;
                    var _h = this.cellHeight + this.cellOffsetY;
                    if (this.bigSymbols && [0, 1].includes(_x2 - this.bigSymbols.x) && [0, 1].includes(_y - this.bigSymbols.y)) {
                        _w *= 2;
                        _h *= 2;
                        _x2 = this.bigSymbols.x;
                        _y = this.bigSymbols.y
                    }
                    if (!isItemInArray(this.bordered_cells, cell)) {
                        group.rect(_x2 * (this.cellWidth + this.cellOffsetX), _y * (this.cellHeight + this.cellOffsetY), _w, _h).attr(Drawer.STYLE_LINE_SYMBOL);
                        this.bordered_cells.push(cell)
                    }
                }
            }
