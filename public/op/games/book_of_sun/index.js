...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function board(snap, _board) {
            ...
            if (combs[currentWinLine] && combs[currentWinLine].special_sym_id !== undefined) {
                (function() {
                    _board = Array.from(_board);
                    var specialSymbolId = details.context.extra;
                    var extraSymbolId = 'extra_' + details.context.extra;
                    for (var row in _board) {
                        if (_board[row].includes(specialSymbolId)) {
                            _board[row] = _board[row].map(function() {
                                return extraSymbolId
                            })
                        }
                    }
                })()
            ...
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    if (details.state === 'freespins' && _board[i][j] == details.context['extra']) {
        ...
        value: function make_freespins(details) {
            ...
            if (details.last_action === 'freespin_init') {
                board.remove();
                snap.image(this.url('extra_' + details.context['extra'] + '.png'), this.width / 2.5, this.height / 2, this.width / 5, this.height / 4)
            }
