...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function title(details) {
            if (details.last_action == 'restore') {
                return 'Restore'
            } else if (details.last_action == 'freespin_init' || details.last_action == 'freespin_stop') {
        ...
        key: '_background',
        value: function _background(snap, state) {
            if (this.details.state == 'freespins' || this.details.last_action == 'freespin_stop') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#0099B1'
                })
            } else if (this.details.state == 'spins') {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#120A8F'
                })
        ...
        value: function _makeMigrations(details) {
            ...
            if (details.last_action == 'restore') {
                details.context.winlines = [];
                details.context.winscatters = [];
                details.context.diver_feature = undefined;
                details.context.feature_guarantee = undefined;
                details.round_win = 0;
                details.round_bet = 0
            }
            return details
        ...
        key: 'is_grand_jackpot',
        value: function is_grand_jackpot(details) {
            var grand_flag = false;
            if (details.context.diver_feature && Boolean(details.context.diver_feature.mysteries)) {
                var _iteratorNormalCompletion = true;
                var _didIteratorError = false;
                var _iteratorError = undefined;
                try {
                    for (var _iterator = details.context.diver_feature.mysteries[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        var element = _step.value;
                        if (element.value === this.grand_jackpot_value) {
                            grand_flag = true
                        }
                    }
                } catch (err) {
        ...
        key: 'is_diver_feature_win',
        value: function is_diver_feature_win(details) {
            return details.context.diver_feature && details.context.diver_feature.total_win > 0
        }
    }, {
        key: 'is_addFS_win',
        value: function is_addFS_win(details) {
            return details.state == 'freespins' && typeof details.context.winscatters !== 'undefined' && details.context.winscatters.reduce(function(a, _ref) {
        ...
        key: 'is_pre_feature_guarantee',
        value: function is_pre_feature_guarantee(details) {
            return details.state == 'freespins' && typeof details.context.feature_guarantee !== 'undefined' && typeof details.context.feature_guarantee.next_gold_reels !== 'undefined' && Boolean(details.context.feature_guarantee.next_gold_reels)
        }
    }, {
        key: 'is_feature_guarantee',
        value: function is_feature_guarantee(details) {
            return typeof details.context.feature_guarantee !== 'undefined' && typeof details.context.feature_guarantee.gold_reels !== 'undefined' && Boolean(details.context.feature_guarantee.gold_reels)
        }
    }, {
        key: 'is_extra_snap',
        value: function is_extra_snap(details) {
            return this.is_diver_feature_win(details) || this.is_addFS_win(details) || this.is_pre_feature_guarantee(details) || this.is_feature_guarantee(details)
        }
    }, {
        key: '_comb',
        value: function _comb(snap, combs, comb, details) {
            ...
            this._panel(group, labelLeft, labelRight);
            this.drawe_border_win_symbols(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            this.drawe_line_patern(group, comb).transform('T' + (this.boardMarginLeft + this.left_shift) + ',' + (this.counters_panelHeight + this.panelHeight + 1));
            return group
        }
    }, {
        key: '_combs',
        value: function _combs(snap, combs, details) {
            ...
            for (var _i = 0; _i < combs.length; ++_i) {
                this.drawe_line_patern(group, combs[_i]).transform('T' + (this.boardMarginLeft + this.left_shift) + ',' + (this.counters_panelHeight + this.panelHeight + 1))
            }
            return group
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap, labelL, main_text) {
            ...
            labelLeft = labelL;
            if (this.width > 200) {
                group.text('50%', '56%', main_text).attr(Drawer.FEATURE_STYLE_TEXT_CENTER_BIG)
            } else {
                group.text('50%', '56%', main_text).attr(Drawer.FEATURE_STYLE_TEXT_CENTER)
            }
            this._panel(group, labelLeft, labelRight);
            return snap
        }
        ...
        value: function make_snap(details) {
            ...
            this.rows = details.context.board[0].length;
            this.scat_id = 14;
            this.diver_feature_ids = [14, 6, 7, 8, 9, 15];
            this.grand_jackpot_value = 1000;
            this._calculateSizes(this.cols, this.rows);
            if (details.state == 'freespins') {
                this.counters_panelHeight = parseInt(this.boardHeight / this.rows / 2.6, 10);
                this.cellWidth = parseInt(this.boardHeight / this.rows, 10) - parseInt(this.counters_panelHeight / this.rows, 10);
                this.cellHeight = this.cellWidth;
                this.left_shift = 0;
                this.boardMarginLeft = parseInt((this.width - this.cellWidth * this.cols) / 2, 10) + 1
            } else {
                this.counters_panelHeight = 0;
                this.cellWidth = parseInt(this.boardHeight / this.rows, 10) - 2;
                this.cellHeight = this.cellWidth;
                this.counters_panelHeight = 0;
                this.left_shift = 0
            }
        ...
        value: function make_spins(details) {
            ...
            if (this.is_diver_feature_win(details)) {
                if (typeof details.context.winscatters === 'undefined') {
                    details.context.winscatters = []
                }
                details.context.winscatters.unshift({
                    'positions': [],
                    'trigger': 'diver_feature',
                    'amount': details.context.diver_feature.total_win
                });
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if (this.diver_feature_ids.includes(game_board[i][j])) {
                            details.context.winscatters[0].positions.push([+i, +j])
                        }
                    }
                }
            }
            ...
            combs = combs.concat(details.context.winscatters ? details.context.winscatters : []);
            var combsGroup = snap.group();
            var combGroup = void 0;
            var margin = 0;
            this._combs(combsGroup, combs, details);
            ...
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            var ce = 0;
            if (this.is_feature_guarantee(details)) {
                var fg_gold_reels = details.context.feature_guarantee.gold_reels;
                for (var k = 0; k < fg_gold_reels.length; k++) {
                    fg_gold_reels[k] += 1
                }
                this.make_extra_snap(details, combsGroup, margin + this.width * ce, snap, 'Diver Guarantee Feature!', 'Guaranteed Reels: ' + fg_gold_reels);
                ce += 1
            }
            if (this.is_grand_jackpot(details)) {
                this.make_extra_snap(details, combsGroup, margin + this.width * ce, snap, 'Jackpot!', 'Jackpot!');
                ce += 1
            }
            if (this.is_diver_feature_win(details)) {
                this.make_extra_snap(details, combsGroup, margin + this.width * ce, snap, 'Diver Feature!', 'Diver Feature: ' + details.context.diver_feature.total_win / 100);
                ce += 1
            }
            if (this.is_addFS_win(details)) {
                ...
                this.make_extra_snap(details, combsGroup, margin + this.width * ce, snap, 'Additional freespins', '+' + roundsGranted + ' FreeSpins');
                ce += 1
            }
            if (this.is_pre_feature_guarantee(details)) {
                var fg_roundsGranted = parseInt(details.context.feature_guarantee.freespins_won, 10);
                this.make_extra_snap(details, combsGroup, margin + this.width * ce, snap, 'Diver Feature Guarantee won!', '+' + fg_roundsGranted + ' FreeSpins');
                ce += 1
            }
            if (details.last_action == 'freespin_init' || details.last_action == 'freespin_stop') {
                board.remove();
        ...
        value: function board(snap, _board) {
            ...
            var combs_line_len = combs.length;
            combs = combs.concat(this.details.context.winscatters ? this.details.context.winscatters : []);
            var group = snap.group();
            var imageUrl = void 0;
            if (this.details.state == 'freespins') {
                for (var k = 0; k < this.cols; k++) {
                    var img = group.image(this.url(String(this.details.context.pearl_collection[k]) + '_pb.png'), this.left_shift + (this.cellWidth + this.cellOffsetX) * k, 0, this.cellWidth, this.counters_panelHeight - 1)
                }
            }
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(j) {
                    if (ar_wilds.includes(_board[i][j]) && _this3.details.state == 'freespins') {
                        imageUrl = _this3.url(_board[i][j] + '-f.png')
                    } else if (ar_pearls.includes(_board[i][j]) && _this3.details.last_action == 'freespin') {
                        var _iteratorNormalCompletion2 = true;
                        var _didIteratorError2 = false;
                        var _iteratorError2 = undefined;
                        try {
                            for (var _iterator2 = _this3.details.context.diver_feature.pearls[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                                var pearl = _step2.value;
                                if (pearl.positions[0] == i && pearl.positions[1] == j) {
                                    if (pearl.special) {
                                        imageUrl = _this3.url(_board[i][j] + '-fg.png')
                                    } else {
                                        imageUrl = _this3.url(_board[i][j] + '-f.png')
                                    }
                                }
                    ...
                    } else if (ar_pearls.includes(_board[i][j]) && _this3.details.last_action == 'restore' && _this3.details.state == 'freespins') {
                        imageUrl = _this3.url(_board[i][j] + '-f.png')
                    } else if (ar_mysteries.includes(_board[i][j])) {
                        imageUrl = _this3.url(_board[i][j] + '.png');
                        if (currentWinLine !== -1) {
                            if (combs_line_len <= currentWinLine) {
                                if (!combs[currentWinLine]) return 'continue';
                                else if (combs[currentWinLine]['trigger'] == 'diver_feature') {
                                    imageUrl = _this3.url(_board[i][j] + '-2.png')
                                }
                            }
                        }
                    } else {
                    ...
                    var img = group.image(imageUrl, _this3.left_shift + (_this3.cellWidth + _this3.cellOffsetX) * i, _this3.counters_panelHeight + (_this3.cellHeight + _this3.cellOffsetY) * j, _this3.cellWidth, _this3.cellHeight);
                    if (combs.length == 1) {
                        currentWinLine = 0
                    }
                    if (currentWinLine !== -1) {
                        if (combs_line_len <= currentWinLine) {
                            if (!combs[currentWinLine]) return 'continue';
                                ...
                                if (typeof _this3.details.context.diver_feature.pearls !== 'undefined') {
                                    _this3.drawe_pearls_mystery_values(group, _this3.details.context.diver_feature.pearls)
                                }
                                if (typeof _this3.details.context.diver_feature.mysteries !== 'undefined') {
                                    _this3.drawe_pearls_mystery_values(group, _this3.details.context.diver_feature.mysteries)
                                }
                            }
                        }
        ...
        key: 'drawe_pearls_mystery_values',
        value: function drawe_pearls_mystery_values(group, values_array) {
            var round_bet = this.details.context.lines * this.details.context.bet_per_line;
            if (typeof values_array !== 'undefined') {
                var _iteratorNormalCompletion3 = true;
                var _didIteratorError3 = false;
                var _iteratorError3 = undefined;
                try {
                    for (var _iterator3 = values_array[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                        var element = _step3.value;
                        var value = (+element.value * round_bet / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                        var i = element.positions[0];
                        var j = element.positions[1];
                        var rect_x = (this.cellWidth + this.cellOffsetX) * i - this.cellOffsetX;
                        var rect_y = this.counters_panelHeight + (this.cellHeight + this.cellOffsetY) * j + this.cellHeight / 4.5;
                        var rect_width = this.cellWidth + 2 * this.cellOffsetX;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = this.counters_panelHeight + (this.cellHeight + this.cellOffsetY) * j + this.cellHeight / 3;
                        group.rect(rect_x, rect_y, rect_width, this.cellHeight / 4.5).attr({
                            'fill': 'white'
                        });
                        if (value.length < 9) {
                            if (this.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_68)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_38)
                            }
                        } else {
                            if (this.width > 200) {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_34)
                            } else {
                                group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_18)
                            }
                        }
                    }
                } catch (err) {
        ...
        value: function drawe_border_win_symbols(snap, line) {
            ...
            for (var i = 0; i < line.positions.length; ++i) {
                var cell = line.positions[i];
                if (!isItemInArray(this.bordered_cells, cell)) {
                    group.rect(cell[0] * (this.cellWidth + this.cellOffsetX), this.counters_panelHeight + cell[1] * (this.cellHeight + this.cellOffsetY), this.cellWidth + this.cellOffsetX, this.cellHeight + this.cellOffsetY).attr(BaseDrawer.STYLE_LINE_SYMBOL);
                    this.bordered_cells.push(cell)
                }
            }
            return group
...
Drawer.FREESPINS_STYLE_TEXT_CENTER_BIG = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#0000',
    'font-size': '1.4em'
};
Drawer.FEATURE_STYLE_TEXT_CENTER = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#0000',
    'font-size': '0.90em'
};
Drawer.FEATURE_STYLE_TEXT_CENTER_BIG = {
    'stroke': '#fff',
    'text-anchor': 'middle',
    'fill': '#0000',
    'font-size': '1.04em'
