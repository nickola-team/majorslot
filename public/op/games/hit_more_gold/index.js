...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function is_extra_snap(details) {
            return details.context.boost_win || details.state == 'bonus' && details.context.bs_count == 20 && details.last_action != 'restore' || details.state == 'freespins' && typeof details.context.winscatters !== 'undefined' && details.context.winscatters.length > 0 && details.context.winscatters[0].trigger != 'bonus'
        }
    }, {
        key: 'make_extra_snap',
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.context.boost_win) {
                labelLeft = 'Boost win!';
                group.text('50%', '56%', 'Boost win').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
            } else if (details.state == 'bonus' && details.context.bs_count == 20) {
                labelLeft = 'Jackpot !';
                ...
                if (details.context.winscatters[0].symbol == this.scat_id) {
        ...
        value: function _calculateSizes(cols, rows) {
            ...
            this.cellWidth = parseInt(this.boardHeight / rows, 10) - 2;
            this.cellHeight = this.cellWidth;
            this.cellWidth = parseInt(this.boardHeight / rows, 10) * 1.1 - 2;
            this.cellOffsetX = -1;
            this.cellOffsetY = -1;
            this.boardMarginLeft = parseInt((this.width - this.cellWidth * cols) / 2, 10) + 2
        }
            ...
            if (details.context.boost_win) {
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if (game_board[i][j] == this.boost_id) {
        ...
        key: 'set_mysteries',
        value: function set_mysteries(details, board) {
            var mysteries = details.context.mystery_jackpots;
            var boost_win = this.details.context.boost_win_not_restored ? this.details.context.boost_win_not_restored : this.details.context.boost_win;
            if (typeof mysteries !== 'undefined' && details.state == 'bonus' && !boost_win && details.context.rounds_left > 0) {
                var _iteratorNormalCompletion = true;
                var _didIteratorError = false;
                var _iteratorError = undefined;
                try {
                    for (var _iterator = mysteries[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        var mystery = _step.value;
                        board[mystery[0]][mystery[1]] = this.mystery_id
                    }
                } catch (err) {
        ...
        value: function board(snap, _board) {
            ...
            var bonus_symbols = [11, 12, 13, 14, 15, 16];
            ...
            _board = this.set_mysteries(this.details, _board);
            var group = snap.group();
            var imageUrl = void 0;
            var _loop = function _loop(i) {
                var _loop2 = function _loop2(_j) {
                    if (_board[i][_j] === _this3.boost_id && _this3.details.context.boost_win) {
                        imageUrl = _this3.url(_board[i][_j] + '.png')
                    } else {
                        imageUrl = _this3.url(_board[i][_j] + '.png')
                    }
                    if (_this3.details.state === 'bonus' && bonus_symbols.indexOf(_board[i][_j]) === -1) {
                        imageUrl = _this3.url('0.png')
                    }
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * i, (_this3.cellHeight + _this3.cellOffsetY) * _j, _this3.cellWidth, _this3.cellHeight);
                    if (combs.length == 1) {
                        currentWinLine = 0
                    }
                    if (currentWinLine !== -1) {
                ...
                try {
                    for (var _iterator2 = this.details.context.bs[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                        ...
                        if (_board[i][j] != this.mystery_id) {
                            ...
                            var rect_y = (this.cellHeight + this.cellOffsetY) * j + this.cellHeight / 3;
                            var rect_width = this.cellWidth + 2 * this.cellOffsetX;
                            var text_x = rect_x + rect_width / 2;
                            var text_y = (this.cellHeight + this.cellOffsetY) * j + this.cellHeight / 2;
