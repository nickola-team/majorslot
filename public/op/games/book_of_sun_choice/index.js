...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function board(snap, _board) {
            ...
            var imageUrl = void 0;
            var extra = [];
            if (details.context.extra) {
                extra = details.state === 'freespins' ? details.context.extra.filter(function(s) {
                    return s != extraSym
                }) : []
            }
        ...
        value: function make_freespins(details) {
            ...
            if (details.last_action === 'freespin_pick') {
                ...
                if (details.context['allowed_extras'] && details.context['allowed_extras'].length > 0) {
                    if (roundsGranted > 0) {
                        snap.text('50%', '32%', '+' + roundsGranted + ' freespins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                    } else {
                        snap.text('50%', '32%', 'freespins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                    }
                    snap.image(this.url('extra_' + extra[extra.length - 1] + '.png'), this.width / 2 - this.width / (3 * 1.5) / 2, this.height / 3 + 8, this.width / (3 * 1.5), this.height / (2 * 1.5))
                } else {
                    if (roundsGranted > 0) {
                        snap.text('50%', '47%', '+' + roundsGranted + ' freespins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                    } else {
                        snap.text('50%', '47%', 'freespins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                    }
                }
            } else {
                var group = snap.group().attr({
