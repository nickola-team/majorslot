...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function title(details) {
            ...
            } else if (details.state == 'bonus' && details.context.extra_respin) {
                return 'Bonus extra respins(' + details.context.rounds_left + ')'
            } else if (details.state == 'bonus' && !details.context.extra_respin) {
                return 'Bonus(' + details.context.rounds_left + ')'
            } else {
                return 'Spin'
        ...
        value: function _background(snap, state) {
            ...
            } else if (this.get_pearls_count(this.details) === 20) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#6B8E23'
                })
            } else {
        ...
        key: '_contextCutMigration',
        value: function _contextCutMigration(details) {
            details = _get(Drawer.prototype.__proto__ || Object.getPrototypeOf(Drawer.prototype), '_contextCutMigration', this).call(this, details);
            if (!details.mystery_values) {
                details.mystery_values = []
            }
            if (!details.mystery_pos) {
                details.mystery_pos = []
            }
            if (!details.bonus_types) {
                details.bonus_types = []
            }
            if (!details.triger_board) {
                details.triger_board = []
            }
            return details
        }
    }, {
        ...
        value: function is_extra_snap(details) {
            return details.state == 'bonus' && this.get_pearls_count(details, [11, 12]) === 20 || details.state == 'freespins' && details.context.winscatters.length > 0 || details.state == 'bonus' && details.context.last_respin && this.is_extra_respins(details)
        }
    }, {
        key: 'get_arr_sum',
        value: function get_arr_sum(arr) {
            var sum = 0;
            for (var i in arr) {
                for (var j in arr[i]) {
                    sum = sum + arr[i][j]
        ...
        value: function get_pearls_count(details) {
            var bonus_symbols = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [11, 12, 13, 14];
            var count = 0;
        ...
        key: 'is_extra_respins',
        value: function is_extra_respins(details) {
            var bonus_symbols_v = details.context.bs_v;
            for (var i in bonus_symbols_v) {
                for (var j in bonus_symbols_v[i]) {
                    var value = bonus_symbols_v[i][j];
                    if (value == 'extra_respins') {
                        return true
                    }
                }
            }
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && this.get_pearls_count(details) === 20) {
                labelLeft = 'Jackpot!';
                if (this.width > 200) {
        ...
        value: function make_spins(details) {
            ...
            this._background(snap, details.state);
            var open_mystery = this.details.state === 'bonus' && this.details.context.rounds_left === 0 && this.details.context.mystery_pos !== undefined && this.details.context.mystery_pos.length > 0;
            var winSuperBonusGame = this.details.state === 'bonus' && this.details.context.is_sb_trigger;
            var group = snap.group().attr({
                width: this.width,
                height: this.height
            });
            var combs = details.context.orig_board && details.context.orig_board.board && details.context.orig_board.board.length ? [{
                positions: []
            }] : [];
            combs = combs.concat(details.context.winlines, details.context.winscatters);
            if (this.get_pearls_count(this.details) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                var winbonus = {
                    'trigger': 'bonus',
                    'positions': [],
                    'amount': 0
                ...
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if ([11, 12, 13, 14].includes(game_board[i][j])) {
                ...
                for (var _i in game_board) {
                    for (var _j in game_board[_i]) {
                        if ([11, 12, 14].includes(game_board[_i][_j])) {
                            winboost.positions.push([_i, _j])
                        }
                    }
                }
                combs.push(winboost)
            } else if (open_mystery) {
                var _winbonus = {
                    'trigger': 'mystery_open',
                    'positions': [],
                    'amount': 0
                };
                combs.push(_winbonus)
            } else if (winSuperBonusGame) {
                var superGameTrigger = {
                    'trigger': 'super_bonus',
                    'positions': [],
                    'amount': 0
                };
                combs.push(superGameTrigger)
            }
            if (!combs) {
                combs = []
            }
            ...
            var margin = 0;
            this._combs(combsGroup, combs, details);
            if (combs.length > 1 || this.details.context.is_sb_trigger || open_mystery || winSuperBonusGame) {
            ...
            var board = this.board(group, game_board);
            board.transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
            ...
            snap.click(function(e) {
                if (combsGroup.children().length < 2) return;
                if (counter === 0 && details.context.orig_board && details.context.orig_board.board && details.context.orig_board.board.length) {
                    game_board = details.context.orig_board.board
                } else {
                    game_board = details.context.board
                }
                counter = (counter + 1) % combsGroup.children().length;
        ...
        value: function _combs(snap, combs, details) {
            ...
            this._panel(group, labelLeft, labelRight);
            for (var i = 0; i < combs.length; ++i) {
                if (combs[i].positions.length) {
                    this.drawe_border_win_symbols(group, combs[i]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
                }
            }
            for (var _i3 = 0; _i3 < combs.length; ++_i3) {
                if (combs[_i3].positions.length) {
        ...
        value: function board(snap, _board) {
            ...
            var bonus_symbols = [11, 12, 13, 14];
            var combs = this.details.context.orig_board && this.details.context.orig_board.board && this.details.context.orig_board.board.length ? [{
                positions: []
            }] : [];
            combs = combs.concat(this.details.context.winlines).concat(this.details.context.winscatters);
            if (this.get_pearls_count(this.details, bonus_symbols) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                var winbonus = {
                    'trigger': 'bonus',
                    'positions': [],
                    'amount': 0
                ...
                for (var i in _board) {
                    for (var j in _board[i]) {
                        if (bonus_symbols.includes(_board[i][j])) {
                            winbonus.positions.push([i, j])
                        }
                    }
                }
            ...
            if (this.is_boost(this.details)) {
                ...
                for (var _i4 in _board) {
                    for (var _j2 in _board[_i4]) {
                        if ([11, 12, 14].includes(_board[_i4][_j2])) {
                            winboost.positions.push([_i4, _j2])
                        }
                    }
                }
                combs.push(winboost)
            }
            if (this.details.context.is_sb_trigger) {
                var winSuperBonus = {
                    'trigger': 'super_bonus',
                    'positions': [],
                    'amount': 0
                ...
                for (var _i5 in _board) {
                    for (var _j3 in _board[_i5]) {
                        if ([11, 12, 13, 14].includes(_board[_i5][_j3])) {
                            winSuperBonus.positions.push([_i5, _j3])
                        }
                    }
                }
                combs.push(winSuperBonus)
            }
            if (!combs) {
                combs = []
            }
            var group = snap.group();
            var imageUrl = void 0;
            var sb_first_snap = this.details.context.is_sb_trigger && currentWinLine === -1;
            var is_bac_win = this.is_bac_win(this.details);
            if (is_bac_win) sb_first_snap = this.details.context.is_sb_trigger && currentWinLine !== -1;
            var bs_v = sb_first_snap ? this.details.context.orig_bs_v : this.details.context.bs_v;
            var boost_mult_value = this.details.context.boost_mult === 1 ? 0 : this.details.context.boost_mult;
            var is_show_mystery_case = this.details.state === 'bonus' && this.details.context.rounds_left === 0 && currentWinLine === -1;
            var _loop = function _loop(_i6) {
                var _loop2 = function _loop2(_j4) {
                    var show_mystery = false;
                    if (is_show_mystery_case) {
                        _this3.details.context.mystery_pos.forEach(function(pos) {
                            if (_i6 == pos[0] && _j4 == pos[1]) {
                                show_mystery = true
                            }
                        })
                    }
                    imageUrl = _this3.url(_board[_i6][_j4] + '.png');
                    if (_this3.details.state === 'bonus' && bonus_symbols.indexOf(_board[_i6][_j4]) == -1) {
                        imageUrl = _this3.url('0.png')
                    }
                    if (!_this3.details.context.is_super_game && !sb_first_snap && _this3.details.context.is_sb_trigger && _board[_i6][_j4] == 11) {
                        imageUrl = _this3.url('12.png')
                    }
                    if (_this3.details.context.is_super_game && _board[_i6][_j4] == 11 && !sb_first_snap) {
                        imageUrl = _this3.url('12.png')
                    }
                    if (show_mystery) {
                        imageUrl = _this3.url('13.png')
                    }
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * _i6, (_this3.cellHeight + _this3.cellOffsetY) * _j4, _this3.cellWidth, _this3.cellHeight);
                    if (combs.length === 1) currentWinLine = 0;
                    var value = void 0;
                    value = bs_v[_i6][_j4];
                    if (typeof value === 'number') {
                        value = (value / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                    }
                    if (value != 0 && !show_mystery && (_board[_i6][_j4] == 11 || _board[_i6][_j4] == 12 && !sb_first_snap)) {
                        var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i6 - _this3.cellOffsetX;
                        var rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j4 + _this3.cellHeight / 3;
                        var rect_width = _this3.cellWidth + 2 * _this3.cellOffsetX;
                        var text_x = rect_x + rect_width / 2;
                        var text_y = (_this3.cellHeight + _this3.cellOffsetY) * _j4 + _this3.cellHeight / 2;
                        ...
                        if (value == 'mini') {
                            ...
                            if (value.length <= 8) {
                                if (_this3.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_65)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_35)
                                }
                            } else if (value.length <= 10) {
                                if (_this3.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_48)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_27)
                                }
                            } else if (value.length <= 12) {
                                if (_this3.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_40)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_24)
                                }
                            } else if (value.length <= 14) {
                                if (_this3.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_35)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_21)
                                }
                            } else {
                                if (_this3.width > 200) {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_27)
                                } else {
                                    group.text(text_x, text_y, value).attr(Drawer.PEARL_VALUE_17)
                                }
                            }
                        }
                    }
                    if (_board[_i6][_j4] == 14 && boost_mult_value && !show_mystery) {
                        var _rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i6 + _this3.cellWidth / 1.8;
                        var _rect_y = (_this3.cellHeight + _this3.cellOffsetY) * _j4 - _this3.cellOffsetY + 0.5;
                        var _rect_width = _this3.cellWidth / 2.3 + 2 * _this3.cellOffsetX;
                        var text_x_x = _rect_x + _rect_width / 2;
                        var text_x_y = (_this3.cellHeight + _this3.cellOffsetY) * _j4 + _this3.cellOffsetY + _this3.cellHeight * 0.18;
                        group.rect(_rect_x, _rect_y, _rect_width, _this3.cellHeight / 3).attr({
                            'fill': 'white'
                        });
                        if (_this3.width > 200) {
                            group.text(text_x_x, text_x_y, 'x' + boost_mult_value).attr(Drawer.MULT_VALUE_70)
                        } else {
                            group.text(text_x_x, text_x_y, 'x' + boost_mult_value).attr(Drawer.MULT_VALUE_40)
                        }
                    }
                    if (currentWinLine !== -1) {
                        if (!combs[currentWinLine] || combs[currentWinLine].trigger === 'sb_state') return 'continue';
                        ...
                        if (!!_this3.details.context.boost_pay) {
                            var _positions = combs[0].positions;
                            img.attr({
                                opacity: 0.5
                            });
        ...
        value: function make_boost_snap(details, combsGroup, margin, snap) {
            ...
            var boost_mult_value = this.details.context.boost_mult === 1 ? 0 : this.details.context.boost_mult;
            if (boost_mult_value) {
                labelRight = this.coins(details.context.boost_pay / boost_mult_value) + ' * ' + boost_mult_value
            }
            labelLeft = 'Boost win!';
            if (this.width > 200) {
        ...
        key: 'isLastRespin',
        value: function isLastRespin(details) {
