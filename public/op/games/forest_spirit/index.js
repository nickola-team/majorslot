...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function _background(snap, state) {
            ...
            } else if (this.details.context.bs_count === 15) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#6B8E23'
                })
            } else if (state === 'bonus' && this.details.context.bonus_mechanic.includes(2) && this.details.context.bs_count_2 === 15) {
                snap.rect(0, 0, this.width, this.height).attr({
                    'fill': '#6B8E23'
                })
            } else {
        ...
        value: function is_extra_snap(details) {
            if (details.state == 'bonus') {
                if (this.details.context.bonus_mechanic.includes(2)) {
                    if (details.context.bs_count_2 === 15) {
                        return true
                    }
                }
                if (details.context.bs_count === 15) {
                    return true
                }
            }
            return false
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && this.details.context.bs_count === 15) {
                labelLeft = 'Jackpot!';
                if (this.width > 200) {
            ...
            if (details.state == 'bonus' && this.details.context.bonus_mechanic.includes(2) && this.details.context.bs_count_2 === 15) {
                labelLeft = 'Jackpot!';
                if (this.width > 200) {
                    group.text('50%', '60%', 'Jackpot !').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER_2)
                } else {
        ...
        value: function get_pearls_count(details) {
            ...
            for (var i in board) {
                for (var j in board[i]) {
                    if ([10, 11, 12, 13, 14, 15, 16, 17].includes(board[i][j])) {
                        count = count + 1
                    }
                }
            }
        ...
        value: function make_spins(details) {
            ...
            this._background(snap, details.state);
            if (this.details.state === 'freespins' && details.context.winscatters != undefined && details.context.new_wilds != undefined) {
                if (details.context.winscatters.length > 0) {
                    if (details.context.wilds === undefined) {
                        details.context.wilds = []
                    }
                    if (details.context.new_wilds === undefined) {
                        details.context.new_wilds = []
                    }
                    details.context.winscatters[0].positions = [].concat(details.context.wilds, details.context.new_wilds)
                }
            }
            var group = snap.group().attr({
                width: this.width,
            ...
            var combs = [].concat(details.context.winlines, details.context.winscatters);
            if (this.get_pearls_count(this.details) >= 1 && ['spin', 'freespin'].includes(this.details.last_action) && this.details.context.bac_win != undefined) {
                var winbonus = {
                    'trigger': 'bonus',
                    'positions': []
                };
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if ([10, 11, 12, 13, 14, 15, 16, 17].includes(game_board[i][j])) {
            ...
            var game_boards = [];
            if (details.state == 'bonus' && details.context.rounds_left == 0 && details.context.mystery_pos.length) {
                var board_with_mystery = details.context.board.map(function(arr) {
                    return arr.slice()
                });
                ...
                try {
                    for (var _iterator = details.context.mystery_pos[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        var mys_pos = _step.value;
                        board_with_mystery[mys_pos[0]][mys_pos[1]] = 13
                    }
                } catch (err) {
                ...
                game_boards.push(board_with_mystery);
                var mysterys = {
                    'trigger': 'mystery',
                    'positions': [],
        ...
        value: function make_bonus(details) {
            ...
            var combs = [].concat(details.context.winlines, details.context.winscatters);
            if (this.get_pearls_count(this.details) >= 3 && ['spin', 'freespin'].includes(this.details.last_action)) {
                var winbonus = {
                    'trigger': 'bonus',
                    'positions': []
                };
                for (var _i2 in game_board) {
                    for (var j in game_board[_i2]) {
                        if ([10, 11, 12, 13].includes(game_board[_i2][j])) {
                            winbonus.positions.push([_i2, j])
                        }
                    }
                }
                combs.push(winbonus)
            ...
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(3)) {
                if (this.details.context.bonus_mechanic.includes(2)) {
                    if (this.details.context.us_win_2 != undefined) {
                        var us_win_2 = {
                            'trigger': 'none',
                            'positions': [],
                            'bs_v': [],
                        ...
                        us_win_2.trigger = 'us_win_2';
                        us_win_2.val = this.details.context.us_win_2.v;
                        us_win_2.positions = this.details.context.us_win_2.pos;
                        combs.push(us_win_2)
                    }
                    if (this.details.context.us_win != undefined) {
                        var us_win = {
                            'trigger': 'none',
                            'positions': [],
                            'bs_v': [],
                        ...
                        us_win.trigger = 'us_win';
                        us_win.val = this.details.context.us_win.v;
                        for (var _i3 in this.details.context.us_win.pos) {
                            var pos = this.details.context.us_win.pos[_i3];
                            pos[1] += 4;
                            us_win.positions.push(pos)
                        }
                        combs.push(us_win)
                    }
                } else {
                    if (this.details.context.us_win != undefined) {
                        ...
                        _us_win.trigger = 'us_win';
                        _us_win.val = this.details.context.us_win.v;
                        _us_win.positions = this.details.context.us_win.pos;
                        combs.push(_us_win)
                    }
                }
            }
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(1)) {
        ...
        value: function board(snap, _board) {
            ...
            if (this.get_pearls_count(this.details) >= 1 && ['spin', 'freespin'].includes(this.details.last_action) && this.details.context.bac_win != undefined) {
                ...
                for (var i in _board) {
                    for (var j in _board[i]) {
                        if ([10, 11, 12, 13].includes(_board[i][j])) {
                            winbonus.positions.push([i, j])
                        }
                    }
                }
            ...
            if (this.details.state === 'bonus' && this.details.context.bonus_mechanic.includes(2)) {
                for (var _i5 in _board) {
                    scaled_bs_v[_i5] = this.details.context.bs_v_2[_i5].concat([0], this.details.context.bs_v[_i5])
                }
                if (this.details.context.bonus_mechanic.includes(1)) {
                    for (var _i6 in _board) {
                        scaled_mults[_i6] = this.details.context.multipliers_2[_i6].concat([0], this.details.context.multipliers[_i6])
                    }
                }
            }
            var new_regulars = this.details.context.new_regulars;
            var permanents = this.details.context.permanents;
            var curr_board = {};
                ...
                if (this.details.context.bonus_mechanic.includes(2)) {
                    if (this.details.context.us_win_2 != undefined) {
                        ...
                        for (var _j in _board) {
                            if (this.details.context.us_win != undefined) {
                                us_win_2.bs_v.push(this.details.context.us_orig_bs_v_2[_j].concat([0], this.details.context.us_orig_bs_v[_j]))
                            } else {
                                us_win_2.bs_v.push(this.details.context.us_orig_bs_v_2[_j].concat([0], this.details.context.bs_v[_j].slice(-3)))
                            }
                        }
                        combs.push(us_win_2)
                    }
                    if (this.details.context.us_win != undefined) {
                        ...
                        us_win.trigger = 'us_win';
                        us_win.val = this.details.context.us_win.v;
                        for (var _i7 in this.details.context.us_win.pos) {
                            var pos = this.details.context.us_win.pos[_i7];
                            us_win.positions.push(pos)
                        }
                        for (var _j2 in _board) {
                            if (this.details.context.us_win_2 != undefined) {
                                us_win.bs_v.push(this.details.context.us_orig_bs_v_2[_j2].concat([0], this.details.context.us_orig_bs_v[_j2]))
                            } else {
                                var part_1 = this.details.context.bs_v[_j2].slice(0, 3);
                                us_win.bs_v.push(part_1.concat([0], this.details.context.us_orig_bs_v[_j2]))
                            }
                        }
                        combs.push(us_win)
                    }
                    ...
                    if (this.details.context.us_win != undefined) {
                        ...
                        _us_win2.trigger = 'us_win';
                        _us_win2.val = this.details.context.us_win.v;
                        _us_win2.positions = this.details.context.us_win.pos;
                        _us_win2.bs_v = this.details.context.us_orig_bs_v;
                        combs.push(_us_win2)
                    }
                }
            }
                ...
                if (rounds_left == 0) {
                    ...
                    if (this.details.context.bonus_mechanic.includes(2)) {
                        for (var _i8 in _board) {
                            e_m.bs_v[_i8] = this.details.context.mult_orig_bs_v_2[_i8].concat([0], this.details.context.mult_orig_bs_v[_i8]);
                            for (var _j3 in _board[_i8]) {
                                if (e_m.bs_v[_i8][_j3] > 1) {
                                    e_m.positions.push([_i8, _j3])
                                }
                            }
                        }
                    } else {
                    ...
                    for (var _i9 in _board) {
                        for (var _j4 in _board[_i9]) {
                            if (e_m.bs_v[_i9][_j4] > 1) {
                                e_m.positions.push([_i9, _j4])
                            }
                        }
                    }
                    combs.push(e_m)
                ...
                if (rounds_left == 0) {
                    ...
                    if (this.details.context.bonus_mechanic.includes(2)) {
                        for (var _i10 in _board) {
                            _e_m.bs_v[_i10] = this.details.context.bs_v_2[_i10].concat([0], this.details.context.bs_v[_i10]);
                            for (var _j5 in _board[_i10]) {
                                if (_e_m.bs_v[_i10][_j5] > 1) {
                                    _e_m.positions.push([_i10, _j5])
                                }
                            }
                        }
                    } else {
                        _e_m.bs_v = this.details.context.mult_orig_bs_v
                    }
                    for (var _i11 in _board) {
                        for (var _j6 in _board[_i11]) {
                            if (_e_m.bs_v[_i11][_j6] > 1) {
                                _e_m.positions.push([_i11, _j6])
                            }
                        }
                    }
                    combs.push(_e_m)
            ...
            var _loop = function _loop(_i12) {
                var _loop2 = function _loop2(_j7) {
                    ...
                    imageUrl = _this4.url(_board[_i12][_j7] + '.png');
                    var img = void 0;
                    if (_this4.details.state === 'bonus' && ![10, 11, 12, 13].includes(_board[_i12][_j7])) {
                        imageUrl = _this4.url('0.png');
                        if (_this4.details.context.bonus_mechanic.includes(1)) {
                            var m_v = _this4.details.context.multipliers[_i12][_j7];
                            if (_this4.details.state === 'bonus' && _this4.details.context.bonus_mechanic.includes(2)) {
                                if (_this4.details.context.bonus_mechanic.includes(1)) {
                                    m_v = scaled_mults[_i12][_j7]
                                }
                            }
                        ...
                        img = group.image(imageUrl, _this4.cellWidth * _i12, (_this4.cellHeight + _this4.cellOffsetY) * _j7, _this4.cellWidth, _this4.cellHeight)
                    } else {
                        var _extraOffset = (_this4.cellWidth - _this4.cellWidth / widthCoef) / 2;
                        img = group.image(imageUrl, (_this4.cellWidth + _this4.cellOffsetX) * _i12 + _extraOffset, (_this4.cellHeight + _this4.cellOffsetY) * _j7, _this4.cellWidth / widthCoef, _this4.cellHeight)
                    }
                    if (currentWinLine !== -1) {
                        if (!combs[currentWinLine]) return 'continue';
                        var _positions = combs[currentWinLine].positions;
                        img.attr({
                            opacity: 0.5
                        });
                    ...
                    if (_this4.details.context.bs_v != undefined) {
                        value = _this4.details.context.bs_v[_i12][_j7];
                        if (_this4.details.state === 'bonus' && _this4.details.context.bonus_mechanic.includes(2)) {
                            value = scaled_bs_v[_i12][_j7]
                        }
                        if (currentWinLine !== -1 && combs[currentWinLine].bs_v != undefined) {
                            ...
                            if (isUS == true) {
                                value = combs[currentWinLine].bs_v[_i12][_j7]
                            }
                        }
                    } else if (_this4.details.context.bonus_symbols_v != undefined) {
                        value = _this4.details.context.bonus_symbols_v[_i12][_j7]
                    }
                    if (combs[currentWinLine] !== undefined) {
                    ...
                    var mult_value = 1;
                    if (_this4.details.context.multipliers === undefined) {
                        mult_value = 1
                    } else {
                        mult_value = _this4.details.context.multipliers[_i12][_j7]
                    }
                    if (_this4.details.state === 'bonus' && _this4.details.context.bonus_mechanic.includes(2)) {
                        if (_this4.details.context.bonus_mechanic.includes(1)) {
                            mult_value = scaled_mults[_i12][_j7]
                        }
                    }
                    if (mult_value > 1 && _this4.details.round_win === 0) {
                        var rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i12 + _this4.cellWidth / 1.8;
                    ...
                    if (mult_value > 1 && _this4.details.round_win > 0 && combs[currentWinLine] !== undefined) {
                        if (combs[currentWinLine].trigger != 'mult_after') {
                            var _rect_x = (_this4.cellWidth + _this4.cellOffsetX) * _i12 + _this4.cellWidth / 1.8;
                    ...
                    if (value != 0 && [10, 11, 12, 13].includes(_board[_i12][_j7])) {
                        var _rect_x2 = (_this4.cellWidth + _this4.cellOffsetX) * _i12 - _this4.cellOffsetX;
                        ...
                        group.rect(_rect_x2, _rect_y2, _rect_width2, _this4.cellHeight / 3).attr({
                            'fill': 'white'
                        });
                        if (value == 'mini') {
                            if (_this4.width > 200) {
        ...
        value: function _combs(snap, combs, details) {
            ...
            for (var _i13 = 0; _i13 < combs.length; ++_i13) {
                this.drawe_line_patern(group, combs[_i13]).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1))
            }
            return group
        }
    }, {
        ...
        value: function _comb(snap, combs, comb, details) {
            ...
            if (comb.trigger === 'us_win' || comb.trigger === 'us_win_2') {
                var _value = (comb.val / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                labelLeft = this.title(details) + ', +' + _value
            }
            if (comb.trigger === 'mult_before' || comb.trigger === 'mult_after') {
                labelLeft = this.title(details) + ', extra mult'
            }
            var labelRight = combs.indexOf(comb) + 1 + '/' + combs.length;
            this._panel(group, labelLeft, labelRight);
            this.drawe_border_win_symbols(group, comb).transform('T' + this.boardMarginLeft + ',' + (this.panelHeight + 1));
