...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function get_pearls_count(details) {
            var bonus_symbols = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [11, 12];
            var count = 0;
        ...
        value: function make_extra_snap(details, combsGroup, margin, snap) {
            ...
            if (details.state == 'bonus' && details.context.last_respin) {
                var extra_respins_count = this.get_arr_sum(details.context.extra_respins);
                if (extra_respins_count > 0 && this.is_extra_respins(details)) {
                    labelLeft = 'Extra Respins!';
                    group.text('50%', '60%', '+' + extra_respins_count + ' Extra Respins').attr(Drawer.FREESPINS_STYLE_TEXT_CENTER)
                }
            }
            this._panel(group, labelLeft, labelRight);
            return snap
            ...
            if (this.get_pearls_count(this.details) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                ...
                for (var i in game_board) {
                    for (var j in game_board[i]) {
                        if ([11, 12].includes(game_board[i][j])) {
                ...
                for (var _i in game_board) {
                    for (var _j in game_board[_i]) {
                        if ([11, 12].includes(game_board[_i][_j])) {
        ...
        value: function board(snap, _board) {
            ...
            if (this.is_boost(this.details)) {
                ...
                for (var _i4 in _board) {
                    for (var _j2 in _board[_i4]) {
                        if ([11, 12].includes(_board[_i4][_j2])) {
            ...
            var _loop = function _loop(_i5) {
                var _loop2 = function _loop2(_j3) {
                    ...
                    if (_this3.details.state === 'freespins' && _board[_i5][_j3] === 9) {
                        var walkingWildPath = _this3.details.context.wild_path || [];
                        var walkingWildPos = walkingWildPath.length ? walkingWildPath[walkingWildPath.length - 1] : _this3.details.context.new_w_wild_pos;
                        var wildsOnTheBoard = 0;
                        _board.forEach(function(reel, i) {
                            reel.forEach(function(symId) {
                                if (symId === 9) wildsOnTheBoard++
                            })
                        });
                        if (walkingWildPos[0] == _i5 && walkingWildPos[1] == _j3 || wildsOnTheBoard === 1) {
                            imageUrl = _this3.url('9_ww.png')
                        } else {
                            imageUrl = _this3.url('9_fs.png')
                        }
                    }
                    ...
                    if (value != 0 && (_board[_i5][_j3] == 11 || _board[_i5][_j3] == 12)) {
                    ...
                    if (currentWinLine !== -1) {
                        var _walkingWildPath = _this3.details.context.wild_path;
                        var _wildsOnTheBoard = 0;
                        _board.forEach(function(reel, i) {
                            reel.forEach(function(symId) {
                                if (symId === 9) _wildsOnTheBoard++
                            })
                        });
                        if (!combs[currentWinLine] || _this3.details.state === 'freespins' && _walkingWildPath && _walkingWildPath.length && (_wildsOnTheBoard === 1 || !_this3.details.round_win)) return 'continue';
