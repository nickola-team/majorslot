...
var Drawer = function(_BaseDrawer) {
    ...
    _createClass(Drawer, [{
        ...
        value: function get_pearls_count(details) {
            ...
            for (var i in board) {
                for (var j in board[i]) {
                    if ([11, 12, 13, 14].includes(board[i][j])) {
                        count = count + 1
                    }
                }
            }
        ...
        value: function make_spins(details) {
            ...
            if (this.get_pearls_count(this.details) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                ...
                for (var _i in game_board) {
                    for (var _j in game_board[_i]) {
                        if ([11, 14].includes(game_board[_i][_j])) {
            ...
            var margin = 0;
            this._combs(combsGroup, combs, details);
            if (combs.length > 1 || this.details.context.is_sb_trigger || open_mystery) {
        ...
        value: function board(snap, _board) {
            ...
            if (this.get_pearls_count(this.details) >= 6 && ['spin', 'freespin'].includes(this.details.last_action)) {
                ...
                combs.push(winbonus)
            } else if (this.is_boost(this.details)) {
                var winboost = {
                    'trigger': 'boost',
                    'positions': [],
                ...
                for (var _i3 in _board) {
                    for (var _j2 in _board[_i3]) {
                        if ([11, 14].includes(_board[_i3][_j2])) {
                ...
                var _loop2 = function _loop2(_j3) {
                    ...
                    if (is_show_mystery_case) {
                        _this3.details.context.mystery_pos.forEach(function(pos) {
                            if (_i4 == pos[0] && _j3 == pos[1]) {
                                show_mystery = true
                            }
                        })
                    }
                    imageUrl = _this3.url(_board[_i4][_j3] + '.png');
                    if (show_mystery) {
                        imageUrl = _this3.url('13.png')
                    } else if (_this3.details.state === 'bonus' && bonus_symbols.indexOf(_board[_i4][_j3]) == -1) {
                        imageUrl = _this3.url('0.png')
                    } else if (_board[_i4][_j3] === 11 && _this3.details.context.is_super_state && !sb_first_snap) {
                        imageUrl = _this3.url('12.png')
                    }
                    var img = group.image(imageUrl, (_this3.cellWidth + _this3.cellOffsetX) * _i4, (_this3.cellHeight + _this3.cellOffsetY) * _j3, _this3.cellWidth, _this3.cellHeight);
                    if (combs.length === 1 && combs[0].trigger !== 'sb_state') {
                        currentWinLine = 0
                    }
                    var value = void 0;
                    value = bs_v[_i4][_j3];
                    if (_board[_i4][_j3] === 12) {
                        if (_this3.details.context.is_sb_trigger && currentWinLine === -1 || !_this3.details.context.is_super_state) {
                            value = 0
                        }
                    }
                    ...
                    if (value != 0 && !show_mystery && (_board[_i4][_j3] == 11 || _board[_i4][_j3] == 12)) {
                        var rect_x = (_this3.cellWidth + _this3.cellOffsetX) * _i4 - _this3.cellOffsetX;
