...
function FormLoginCheck(f) {
	if (!Web.checkPID(f,'id')) {
		alert('아이디는 4자 이상 12자 이하 영문, 숫자, - 그리고 _ 만 입력 가능합니다.');
		f.id.focus();
		return false;
	}
...
function LoadSWF(id,src,w,h,bg,isplay,wmode) {
	...
	document.write("<object classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" codebase=\"../../fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,0,0\" width=\"" + w + "\" height=\"" + h + "\" id=\"" + id + "\">\n");
...
function GoGame1(StrPage,Selectname,SelectBox,GameType,ViewType,League) {

		StrURL = StrPage + '?' + Selectname + '=' + SelectBox.value;
	
			StrURL += '&gametype='+GameType+'&viewtype='+ViewType+'&league='+League;
	
		location.href = StrURL;
	
}

function GoGame2(StrPage,Selectname,SelectBox,GameType,ViewType,Sports) {


		StrURL = StrPage + '?' + Selectname + '=' + SelectBox.value;
		
			StrURL += '&gametype='+GameType+'&viewtype='+ViewType+'&sports='+Sports;
