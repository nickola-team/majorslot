...
function withdrawRealtime() {
    ...
    for (i=0;i<5;i++)
    {
        ...
        html += `<li class="items ng-scope">
                    <div class="col-xs-5 text-left"><span class="ng-binding">${fakedata['date']}</span></div>
                    <div class="col-xs-12 goldTxt text-center"><span class="ng-binding">${fakedata['amount']}원</span></div>
                    <div class="col-xs-4 text-left"><span class="ng-binding">${fakedata['id']}</span></div>
            </li>`
  ...
  function depositRealtime() {
        ...
        for (i=0;i<5;i++)
        {
            ...
            var amount = Math.floor(Math.random() * 500) * 10000;
            var id = Math.random().toString(36).substr(2,3) + "***";
            var sec_delta = Math.floor(Math.random() * 10);            
            date.setMinutes(date.getMinutes() - delta);
            date.setSeconds(date.getSeconds() - sec_delta);
...
function setMsgNotice(name, elem,pagename,nextelem) {
    $("." + pagename).addClass('ng-hide');
    $("." + name).removeClass('ng-hide');
    $("#" + elem).addClass('active');
