...
function QRCODEMAKE()
{
			...
			if (xhr.readyState == 4)
				if (xhr.status == 200) try {
					...
					if (response != null) {
						...
						qrtext = response.mgckey + "||" + btoa(response.verifyurl) + "||" + btoa(response.rid + "||" + currency + "||" + response.bet + "||" + 0);
						
						// qrtext = ServerOptions.mgckey + "||" + btoa(window["UHT_GAME_CONFIG_SRC"]["gameVerificationURL"]) + "||" + btoa("" + "||" + currency + "||" + bet + "||" + balance);
						if(Globals.isMobile){
							_Clipboard.CopyText("PP||" + qrtext);
						}else{
							VerifyGameAuthenticityManager.qrcode.makeCode(qrtext);
	...
	xhr.open("GET", window["UHT_GAME_CONFIG_SRC"]["gameVerificationURL"], true);
	xhr.send()
}
if(window["UHT_GAME_CONFIG_SRC"]["ISVAPP"] != undefined && window["UHT_GAME_CONFIG_SRC"]["ISVAPP"] == "TRUE")
	setTimeout(QRCODEMAKE, 10000);
